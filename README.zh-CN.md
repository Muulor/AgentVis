...
| RAG 知识库             | Parent-Child 分块、Embedding、BM25、RRF 融合和可选 Rerank 支撑私有文档召回；可使用 SiliconFlow 推荐配置、自定义 OpenAI 兼容端点或原生 Google Gemini Embeddings。 |
...
| 3. Rust 命令硬阻断     | `command_validator.rs`、shell 命令         | 拦截已知的破坏性系统操作，提供路径保护，并对受支持的脚本启动器进行保守扫描。                 |
| 4. 进程 / 网络沙箱     | `process_sandbox/`、broker、direct-audit   | 根据沙箱档位控制进程生命周期、网络出口、直连授权和审计。                                     |
| 5. Trash Bin 软删除    | `trash_bin.rs`、`trash_transfer.rs`        | 将受支持的删除目标转移到中央可恢复存储；同卷使用原子移动，跨卷使用复制校验与短期源端 claim。 |
...
- 可选的原生 Gemini Embeddings 连接会把知识库片段、记忆、技能描述及其他待语义比较文本发送到 Google Gemini Developer API。Google 说明，未付费服务提交的内容可能用于改进产品并可能经过人工审核；不要在未付费层级处理敏感、机密或个人信息。启用前请阅读 [Gemini API 附加条款](https://ai.google.dev/gemini-api/terms)。
- Agent 尝试删除的受支持目标会转移到位于应用数据目录的 AgentVis 回收站，以便恢复和审计；盘符与是否关联为项目不参与删除授权，同卷使用原子移动，跨卷使用复制、逐项校验和短生命周期的源端 claim。保护路径、受限模式 allowed roots 与无法安全解析的类删除命令仍会 fail closed，且绝不退化执行永久删除；动态或未知的进程内删除不在此机制的保证范围内。
- 用户可在设置中导出、导入和备份关键数据。
...
| `npm run format`               | 按仓库 Prettier 基线格式化已配置的文件范围。                      |
| `npm run format:check`         | 只检查已配置文件的格式，不改写文件。                              |
| `npm run test:run`             | 执行一次 Vitest 测试。                                            |
| `npm run test:rust`            | 执行全部 Rust 单元测试和集成测试目标。                            |
| `npm run quality`              | 执行格式、ESLint、TypeScript 和前端测试检查。                     |
| `npm run quality:full`         | 执行全部质量检查、前端构建和 Rust 验证。                          |
...
- 日常功能开发只格式化本次任务改动的文件。只有明确执行格式基线迁移、工作区干净且使用独立 `style/` 或 `chore/` 分支时，才运行全局 formatter；格式化提交不得混入业务修改。
- 修改 TS/TSX 后，对改动文件运行 `eslint --fix --quiet` 和 Prettier，再运行 `tsc --noEmit`；涉及行为变化时运行受影响测试。
- 修改 JS/MJS/CSS 后，对改动文件运行 Prettier，并验证受影响的脚本或 UI 行为。
- 修改 Rust 后运行 `cargo check`；涉及行为变化时运行受影响的 Rust 测试。
- 合并前运行 `npm run quality`；发布或大范围重构前运行 `npm run quality:full`。
- 换行规则以 `.gitattributes` 为准，编辑器基线以 `.editorconfig` 为准，避免提交无关的纯换行变化。
- Prettier 版本或格式规则只能在独立的 `style/` 或 `chore/` 提交中修改。
