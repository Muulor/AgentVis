...
**Focus on:**
- **Typography**: Choose beautiful, distinctive, and interesting fonts. Avoid mediocre fonts such as Arial and Inter; instead, use distinctive choices that elevate frontend aesthetics, namely fonts that feel unexpected and highly individual. It is recommended to pair highly recognizable display fonts with refined body fonts.
- **Color & Theme**: Maintain a highly unified aesthetic style. Use CSS variables to ensure visual consistency. Use a "dominant color + vivid accent color" scheme, which is far better than conservative, bland palettes with evenly distributed colors.
- **Motion**: Apply animation to visual effects and micro-interactions. For pure HTML structures, prioritize pure CSS solutions; if using React, use the Motion library as much as possible. Focus energy on the moments with the strongest visual impact: for example, a carefully choreographed page load with staggered reveals (based on `animation-delay`) often creates more surprise than scattered micro-interactions. Flexibly use scroll-triggered and hover motion that can bring unexpected delight.
- **Spatial Composition**: Use unexpected layouts. Boldly use asymmetry, overlapping elements, diagonal momentum, and elements that break grid constraints. Use large areas of whitespace (negative space) **or** precisely controlled high-density typography.
- **Backgrounds & Visual Details**: Create atmosphere and spatial depth; reject default solid-color backgrounds. Add contextual effects and textures that fit the overall aesthetic. Boldly use creative expressive forms such as mesh gradients, noise textures, geometric patterns, layered transparency, high-tension shadows, decorative borders, custom mouse cursors, and grain filter overlays.

**Strictly forbidden**: generic aesthetics with an "AI-generated feel", such as overused font families (Inter, Roboto, Arial, or system default fonts), stale color schemes (especially pure white backgrounds with purple gradients), uninspired layouts and component patterns, and assembly-line designs that lack specific contextual personality (cookie-cutter design).

**Strictly forbidden**: using Emoji as icons. Use the Lucide icon library, or SVG vector graphics with an independent design aesthetic as icons. When designing visualized interactive charts, prioritize visually elegant Plotly rendering. Treat Figma's design standard as the industry benchmark and strive to surpass its visual philosophy.

Please interpret creatively and make unexpected design choices that truly fit the current scenario. Never be uniform. Vary flexibly between light and dark themes, different fonts, and multiple aesthetic styles. **Absolutely avoid** always falling back to certain conventional choices across repeated generation iterations (for example, always using the Space Grotesk font). **Try more often to embed images, avg text boxes, and other elements into text, creating surprising effects through text wrapping; pretext can do this!**

**Important note**: implementation complexity must match the aesthetic vision. Maximalist design requires writing complex code to implement abundant motion and visual effects. Minimalist or refined elegant design requires restraint and precision, with extreme attention to spacing, typography, and subtle detail handling. True elegance comes from perfect execution of the design vision.

**Remember**: you have the ability to complete extraordinary creative work. Do not limit yourself; fully show what astonishing work you can create when you think outside the box and commit wholeheartedly to a unique visual vision.
