...
```
python scripts/make.py check   # verify all deps
python scripts/make.py fix     # auto-install missing deps
python scripts/make.py demo    # build a sample PDF
```
...
> **中文支持**：`palette.py` 会自动检测系统中文字体（等线 → 微软雅黑 → Noto Sans SC → 宋体），无需手动配置。
