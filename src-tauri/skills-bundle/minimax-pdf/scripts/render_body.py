...
# CJK 字体注册后的 ReportLab 内部名称常量
_CJK_BODY_NAME      = "CJKBody"
_CJK_BODY_BOLD_NAME = "CJKBodyBold"


def register_fonts(tokens: dict) -> dict:
    """
    注册 tokens["font_paths"] 中的 TTF/TTC 字体文件。

    对于 .ttc 字体集合文件，自动尝试按 subfontIndex=0 加载
    （含盘微软雅黑、宋体等主流 TTC）。

    Returns:
        成功注册的字体名称字典。核心键：
            "body"      — body/paragraph 字体名
            "body_bold" — bold 字体名
        若注册失败，返回空字典（调用方将回退内置字体）。
    """
    font_paths: dict = tokens.get("font_paths", {})
    regular_path = font_paths.get("CJKBody", "")
    bold_path    = font_paths.get("CJKBodyBold", "")

    if not regular_path or not os.path.exists(regular_path):
        return {}

    def _register_one(name: str, path: str) -> bool:
        """注册单个字体，自动处理 TTC 子字体索引。"""
    ...
    regular_ok = _register_one(_CJK_BODY_NAME, regular_path)
    if not regular_ok:
        return {}

    # 粗体路径与常规相同时（Noto Sans SC 可变字体场景），拉高 weight 轴来模拟粗体
    bold_ok = False
    if bold_path and os.path.exists(bold_path):
        bold_ok = _register_one(_CJK_BODY_BOLD_NAME, bold_path)

    if not bold_ok:
        # 内部备用：系射粗体名到常规体，避免未注册字体导致崩溃
        is_ttc = regular_path.lower().endswith(".ttc")
        if is_ttc:
            pdfmetrics.registerFont(
                TTFont(_CJK_BODY_BOLD_NAME, regular_path, subfontIndex=0))
        else:
            pdfmetrics.registerFont(TTFont(_CJK_BODY_BOLD_NAME, regular_path))

    return {"body": _CJK_BODY_NAME, "body_bold": _CJK_BODY_BOLD_NAME}
...
class BeautifulDoc(BaseDocTemplate):
    def __init__(self, path: str, tokens: dict, cjk_fonts: dict, **kw):
        self._t   = tokens
        # cjk_fonts: {"body": name, "body_bold": name}，空字典表示未注册任何 CJK 字体
        self._cjk = cjk_fonts
    ...
    def _decorate(self, canv, doc):
        ...
        # 页眉页脚需要能显示中文，优先使用已注册的 CJK 字体
        meta_font = self._cjk.get("body", t["font_body_rl"])
        ...
        canv.setFont(meta_font, t["size_meta"])
...
def make_styles(t: dict, cjk_fonts: dict) -> dict:
    # 若 CJK 字体已注册，用其替换内置的 Helvetica/Times-Bold，从而支持中文输出
    hf  = cjk_fonts.get("body_bold", t["font_display_rl"])
    bf  = cjk_fonts.get("body",      t["font_body_rl"])
    bfb = cjk_fonts.get("body_bold", t["font_body_b_rl"])
    dk  = t["body_text"]
    d   = t["dark"]
    mu  = t["muted"]
...
def build(tokens: dict, content: list, out_path: str) -> dict:
    cjk_fonts = register_fonts(tokens)
    styles    = make_styles(tokens, cjk_fonts)

    doc = BeautifulDoc(
        out_path, tokens, cjk_fonts,
        pagesize=A4,
