...
# ── 中文字体自动检测 ────────────────────────────────────────────────────────────
# 按优先级扫描系统中常见的中文字体，Win10/11 默认均自带。
# 返回格式：{"CJKBody": <regular_path>, "CJKBodyBold": <bold_path>}
# 若找不到任何中文字体，返回空字典（render_body.py 将回退到内置字体）。
_CJK_CANDIDATES: list[tuple[str, str, str]] = [
    # (字体名称标签, 常规体相对路径, 粗体相对路径)  — 优先级从高到低
    # Noto Sans SC（可变字体，单文件兼常规与粗体）
    ("NotoSansSC",       "NotoSansSC-VF.ttf", "NotoSansSC-VF.ttf"),
    # 微软雅黑 / 微软雅黑 UI（msyh.ttc 内含两个子字体，索引0即雅黑）
...
# Windows 字体目录搜索顺序（用户字体目录优先，系统字体次之）
_FONT_DIRS: list[str] = [
...
def _find_chinese_font() -> dict[str, str]:
    """扫描系统字体目录，按优先级返回第一个可用的中文字体路径字典。"""
    # 构建一个 filename -> full_path 的快查映射（忽略大小写）
    ...
    for font_dir in _FONT_DIRS:
        ...
        try:
            for fname in os.listdir(font_dir):
                font_map[fname.lower()] = os.path.join(font_dir, fname)
        except PermissionError:
            continue

    for _label, regular_fname, bold_fname in _CJK_CANDIDATES:
        regular_path = font_map.get(regular_fname.lower())
        bold_path    = font_map.get(bold_fname.lower())
        if regular_path and os.path.isfile(regular_path):
            # 粗体找不到时退化使用常规体（ReportLab 会模拟粗体）
            result_bold = bold_path if (bold_path and os.path.isfile(bold_path)) else regular_path
            return {"CJKBody": regular_path, "CJKBodyBold": result_bold}
...
def build_tokens(
    ...
    tokens = {
        ...
        # 中文字体路径（由系统自动检测；空字典表示未找到，render_body.py 将回退内置字体）
        "font_paths":    _find_chinese_font(),
