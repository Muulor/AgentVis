...
// 重新导出常用类型
pub use models::{Hub, Agent, Message, Memory, FileInfo, MessageRole, MemoryLayer};
pub use models::{HubUpdate, AgentUpdate};
pub use models::{DiffRecord, DiffRecordStatus, DiffRecordCreateRequest};
...
pub use vector_repo::{VectorRepository, ChunkEmbedding, VectorSearchResult, IndexStats};
...
impl Database {
    ...
    pub async fn new(db_path: &Path) -> AppResult<Self> {
        ...
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| AppError::FileSystem(format!("Unable to create database directory: {}", e)))?;
        }
    ...
    pub async fn in_memory() -> AppResult<Self> {
        let pool = create_pool("sqlite::memory:").await
            .map_err(|e| AppError::Database(format!("Unable to create in-memory database: {}", e)))?;
