...
impl MemoryTriggerRepository {
    ...
    ) -> Result<MemoryTriggerState, sqlx::Error> {
        // 确保存在
        let current = self.get_or_create(agent_id).await?;
        let now = Utc::now().timestamp_millis();

        // 合并更新
        let new_turns = update
            .turns_since_last_extract
            .unwrap_or(current.turns_since_last_extract);
        let new_score = update
            .candidate_signal_score
            .unwrap_or(current.candidate_signal_score);
        let new_last_turn = update
            .last_extract_turn
            .unwrap_or(current.last_extract_turn);
    ...
    pub async fn update_last_processed_message(
        &self,
        agent_id: &str,
        message_id: &str,
    ) -> Result<(), sqlx::Error> {
    ...
    pub async fn reset_after_extract(
        &self,
        agent_id: &str,
        current_turn: i64,
    ) -> Result<MemoryTriggerState, sqlx::Error> {
