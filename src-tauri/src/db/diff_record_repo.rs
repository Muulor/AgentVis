...
pub async fn get_by_id(pool: &Pool<Sqlite>, id: &str) -> AppResult<Option<DiffRecord>> {
    sqlx::query_as::<_, DiffRecord>(
        "SELECT * FROM diff_records WHERE id = ?"
    )
    .bind(id)
    .fetch_optional(pool)
    .await
...
pub async fn get_pending_by_context(pool: &Pool<Sqlite>, context_id: &str) -> AppResult<Vec<DiffRecord>> {
    sqlx::query_as::<_, DiffRecord>(
...
/// 更新 DiffRecord 状态
pub async fn update_status(pool: &Pool<Sqlite>, id: &str, status: DiffRecordStatus) -> AppResult<()> {
    let now = Utc::now().timestamp_millis();
    sqlx::query(
        "UPDATE diff_records SET status = ?, updated_at = ? WHERE id = ?"
    )
    .bind(status.as_str())
    .bind(now)
    .bind(id)
...
pub async fn update_modified_content(pool: &Pool<Sqlite>, id: &str, modified_content: &str, status: DiffRecordStatus) -> AppResult<()> {
    let now = Utc::now().timestamp_millis();
    sqlx::query(
...
pub async fn revert_by_message(pool: &Pool<Sqlite>, message_id: &str) -> AppResult<Vec<DiffRecord>> {
    // 先获取要回滚的记录
    ...
    let now = Utc::now().timestamp_millis();
    sqlx::query(
        "UPDATE diff_records SET status = 'reverted', updated_at = ? WHERE message_id = ?"
    )
    .bind(now)
    .bind(message_id)
    .execute(pool)
...
pub async fn update_modification_statuses(pool: &Pool<Sqlite>, context_id: &str, document_id: &str, statuses_json: &str) -> AppResult<()> {
    let now = Utc::now().timestamp_millis();
    sqlx::query(
...
pub async fn update_active_snapshot(pool: &Pool<Sqlite>, context_id: &str, snapshot_id: Option<&str>) -> AppResult<()> {
    let now = Utc::now().timestamp_millis();
    sqlx::query(
...
pub async fn update_message_id(pool: &Pool<Sqlite>, context_id: &str, old_message_id: &str, new_message_id: &str) -> AppResult<u64> {
    let now = Utc::now().timestamp_millis();
