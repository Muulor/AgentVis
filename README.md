...
| RAG knowledge base                | Parent-child chunking, embeddings, BM25, RRF fusion, and optional reranking support private document retrieval, using the recommended SiliconFlow setup, a custom OpenAI-compatible endpoint, or native Google Gemini Embeddings. |
...
| 3. Rust command blocking         | `command_validator.rs`, shell commands     | Block recognized destructive system operations, protect paths, and conservatively scan supported script launchers.                                                          |
...
| 5. Trash Bin soft delete         | `trash_bin.rs`, `trash_transfer.rs`        | Transfer supported delete targets into central recoverable storage, using atomic moves on the same volume and verified copy plus a short-lived source claim across volumes. |
...
- The optional native Gemini Embeddings connection sends Knowledge Base chunks, memory, skill descriptions, and other semantic-comparison text to the Google Gemini Developer API. Google states that content submitted through unpaid services may be used to improve its products and may be reviewed by humans; do not use the unpaid tier for sensitive, confidential, or personal content. Review the [Gemini API Additional Terms](https://ai.google.dev/gemini-api/terms) before enabling it.
- Supported Agent deletion targets are transferred into the AgentVis Trash Bin under app data for recovery and auditing. Drive letters and project attachment do not grant or deny deletion: same-volume targets use an atomic move, while cross-volume targets use copy, full verification, and a short-lived source-side claim. Protected paths, restricted-mode allowed roots, and recognized delete-like commands whose semantics cannot be resolved still fail closed without falling back to permanent deletion; dynamic or unknown in-process deletion remains outside this guarantee.
