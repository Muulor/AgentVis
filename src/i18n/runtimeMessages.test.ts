...
describe('runtimeMessages', () => {
  it('translates current English runtime phases', () => {
    expect(translateRuntimeProgressPhase('Checking Python installation', translate)).toBe(
      'runtime.progress.checkPython'
    );
  });

  it('translates current English runtime batch phases', () => {
    expect(
      translateRuntimeProgressPhase('Installing base packages 1-3/10: pip, wheel', translate)
    ).toBe(
      'runtime.progress.installBaseBatch:{"range":"1-3","total":"10","packages":"pip, wheel"}'
    );
  });
  ...
  it('translates dependency failures with affected skill and packages', () => {
    expect(
      ...
      'settings.skills.depInstallFailedForSkillPackages:{"name":"desktop-control","packages":"pyautogui, pygetwindow"}'
    );
  });
});
