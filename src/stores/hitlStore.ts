...
export const useHitlStore = create<HitlState & HitlActions>()((set, get) => ({
  ...
  resume: (contextId: string, message?: string) => {
    ...
    if (resolver) {
      ...
      resolver(message);
      logger.debug(
        `[hitlStore] ▶ 恢复执行: ${contextId}${message ? ` (含介入消息: ${message.slice(0, 50)}...)` : ' (无消息)'}`
      );
    } else {
      ...
      const shouldUpdate =
        !preResolvedMap.has(contextId) || // 尚无预解决值
      ...
      if (shouldUpdate) {
        ...
        logger.debug(
          `[hitlStore] ▶ 恢复（预解决）: ${contextId}，消息已暂存等待 waitForResume 消费`
        );
      } else {
        // 旧指令存在，本次空内容静默忽略，保护用户已发送的指令
        logger.debug(
          `[hitlStore] ▶ 恢复（预解决）忽略空内容: ${contextId}，保留已暂存指令 "${existingMessage.slice(0, 30)}..."`
        );
      }
  ...
  waitForResume: (contextId: string, signal: AbortSignal): Promise<string | undefined> => {
    ...
    if (preResolvedMap.has(contextId)) {
      ...
      logger.debug(
        `[hitlStore] ⚡ waitForResume 消费预解决值: ${contextId}${preResolved ? ` (${preResolved.slice(0, 50)}...)` : ' (无消息)'}`
      );
