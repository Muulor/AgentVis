...
vi.hoisted(() => {
    ...
    globalThis.localStorage = {
        ...
        removeItem: (key: string) => { Reflect.deleteProperty(memoryStorage, key); },
        clear: () => { Object.keys(memoryStorage).forEach(k => Reflect.deleteProperty(memoryStorage, k)); },
...
describe('RuntimeStore', () => {
    ...
    describe('待安装依赖管理', () => {
        ...
        it('removePendingDependencies 应移除指定技能依赖', () => {
            const { addPendingDependencies, removePendingDependencies } =
        ...
        it('clearPendingDependencies 应清空所有依赖', () => {
            const { addPendingDependencies, clearPendingDependencies } =
                useRuntimeStore.getState();
