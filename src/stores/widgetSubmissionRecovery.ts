...
import {
  ...
  parseWidgetLanguage,
  resolveWidgetType,
} from '@components/widgets/widgetParsing';
...
const WIDGET_TITLE_SUFFIX_PATTERN =
  /\s*(?:\((?:multi[- ]?select|single[- ]?select|required|optional|multiple)\)|\uff08(?:\u53ef\u591a\u9009|\u5355\u9009|\u5fc5\u9009)\uff09)\s*/gi;
...
function parseStructuredSelections(
  metadata: Record<string, unknown>
): WidgetSelectionSnapshot[] | undefined {
...
function parseSelectionLine(line: string): ParsedSelectionRow | null {
  ...
  const rawLabels = trimmed
    .slice(separatorIndex + (trimmed[separatorIndex] === '：' ? 1 : 2))
    .trim();
  if (!title || !rawLabels) return null;
  if (title === '补充说明' || title.toLowerCase() === 'note') return null;

  const labels = rawLabels.includes('、') ? rawLabels.split('、') : rawLabels.split(/,\s+/);
...
export function collectWidgetBubbleSubmissions(
  ...
  for (const message of messages) {
    ...
    const textFallback =
      structuredSelections === undefined && structuredExtraText === undefined
