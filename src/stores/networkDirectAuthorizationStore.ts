...
interface NetworkDirectAuthorizationState {
    ...
    requestAuthorization: (request: NetworkDirectAuthorizationRequest) => Promise<NetworkDirectAllowance[] | null>;
    approvePending: (scope: NetworkDirectAllowanceScope) => void;
    denyPending: () => void;
    activeAllowancesForSubject: (subjectType: NetworkDirectSubjectType, subjectId?: string) => NetworkDirectAllowance[];
}
...
function buildAllowances(
    ...
    const effectiveScope = scope === 'session' && hasPrivateNetworkDirectTarget(request.targets)
        ? 'currentExecution'
        : scope;
...
export const useNetworkDirectAuthorizationStore = create<NetworkDirectAuthorizationState>((set, get) => ({
    pending: null,
    sessionAllowances: [],
    ...
    activeAllowancesForSubject: (subjectType, subjectId) => {
        const active = get().sessionAllowances.filter((allowance) =>
...
export function activeNetworkDirectAllowancesForSubject(
    ...
    return useNetworkDirectAuthorizationStore.getState().activeAllowancesForSubject(subjectType, subjectId);
}
