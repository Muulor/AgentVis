    ...
    persist(
        (set, get) => ({
            ...
            addBotConfig: (config) => {
                ...
                const platformBotCount = botConfigs.filter(bot => bot.platform === config.platform).length;
            ...
            updateBotConfig: (botId, updates) => {
                set((state) => ({
                    botConfigs: state.botConfigs.map((c) =>
                        c.botId === botId ? { ...c, ...updates } : c,
                    ),
                }));
            ...
            setConnected: (connected) => {
                ...
                if (firstBotId) {
                    ...
                    logger.warn('setConnected 调用失败：botConfigs 为空，状态未更新。请通过 setBotConnected(botId, ...) 调用。');
                }
            },
