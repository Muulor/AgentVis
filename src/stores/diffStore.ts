...
function inferModificationStatus(
    ...
    return modifications.map((mod, index) => {
        ...
        if (searchContent.trim().length < MIN_MATCH_LENGTH) {
            logger.trace(`[inferStatus] #${index + 1}: search 内容太短 (${searchContent.trim().length} < ${MIN_MATCH_LENGTH})，保持原状态:`, mod.status);
            return mod;
        }
        ...
        if (mod.modification.operation === 'DELETE') {
            ...
            logger.trace(`[inferStatus] #${index + 1} DELETE: inCurrent=${searchExistsInCurrent}, inOriginal=${searchExistsInOriginal} → ${newStatus}`);
            return { ...mod, status: newStatus };
        }

        // REPLACE/INSERT 操作
        const replaceExists = replaceContent.trim().length >= MIN_MATCH_LENGTH && contentContainsSearch(currentContent, replaceContent);
        const searchExists = contentContainsSearch(currentContent, searchContent);

        let inferredStatus: 'pending' | 'applied' | 'failed' = mod.status as 'pending' | 'applied' | 'failed';
        ...
        logger.trace(`[inferStatus] #${index + 1} ${mod.modification.operation}: searchExists=${searchExists}, replaceExists=${replaceExists} → ${inferredStatus}`);
...
async function rebuildContentByDiff(
    ...
    const allMatchable = allMods.every(
        m => m.matchResult.success && m.matchResult.startLine > 0
    );
...
function rebuildByMatchResult(
    ...
    logger.trace('[diffStore] rebuildByMatchResult: 原始行数:', originalLines.length,
        ', 有效修改数:', validMods.length,
        ', 结果行数:', resultLines.length);
...
async function rebuildByMyersDiff(
    ...
    while (i < editOps.length) {
        const op = editOps[i];
        if (!op || op.type === 'context') { i++; continue; }

        const block: ChangeBlock = {
            ops: [], startOpIndex: i,
            origStartLine: Infinity, origEndLine: 0,
        };
        while (i < editOps.length) {
            const cur = editOps[i];
    ...
    logger.trace('[diffStore] rebuildByMyersDiff: 原始行数:', originalLines.length,
        ', 变更块数:', changeBlocks.length,
        ', 结果行数:', resultLines.length);
 ...
 */
function buildModificationStatusMap(modifications: ModificationApplyResult[]): Record<string, string> {
...
interface DiffActions {
     ...
     */
    loadModifications: (contextId: string, documentId: string, content: string, xml: string, messageId: string, fileName?: string, isRestoring?: boolean, currentContentForInference?: string, preAppliedContent?: string, persistedModStatuses?: string) => Promise<void>;
    /** 接受单个修改 */
    ...
    getFileList: (contextId: string) => Array<{ documentId: string; fileName: string; pendingCount: number }>;
...
export const useDiffStore = create<DiffState & DiffActions>((set, get) => ({
    ...
    setMode: (contextId, mode) => {
        set((state) => {
            ...
            if (mode === 'diff') {
                ...
                const currentFilePending = ctx.pendingModifications.filter(m => m.status === 'pending').length > 0;
                const otherFilePendingEntry = Array.from(ctx.fileEntries.values()).find(
                    e => e.documentId !== ctx.activeFileId &&
                ...
                if (!currentFilePending && otherFilePendingEntry) {
                    ...
                    newMap.set(contextId, applyFileDiffEntry({ ...ctx, mode: 'diff' }, otherFilePendingEntry));
            ...
            for (const [docId, record] of latestByDoc) {
                ...
                if (record.activeSnapshotId) {
                    try {
                        ...
                        const snapshot = await invoke<SnapshotData | null>('snapshot_get', { id: record.activeSnapshotId });
                        if (snapshot) {
                ...
                if (currentContent === record.originalContent) {
                    try {
                        ...
                        const fileContent = await invoke<string>('file_read_content', { filePath: record.documentId });
                ...
                if (record.documentId.includes(':')) {
                    try {
                        // 参数名应为 filePath 而非 path
                        const diskContent = await invoke<string>('file_read_content', { filePath: record.documentId });
                        ...
                        logger.warn('[diffStore]  无法读取文件，使用 DB 存储的 modifiedContent 作为 preAppliedContent');
                    }
                }
                // 重启恢复时优先使用 activeSnapshot 的修改块状态（精确还原用户最后所在版本的 diff 面板）
                // 兜底：若快照无状态数据（旧版快照），使用 diff_record 级别的状态
                const effectiveModStatuses = activeSnapshotStatusesJson ?? record.modificationStatuses ?? undefined;
                logger.trace(`[diffStore]  重启恢复使用状态来源: ${activeSnapshotStatusesJson ? 'activeSnapshot' : (record.modificationStatuses ? 'diffRecord' : 'none')}`);
                    ...
                    try {
                        ...
                        if (targetSnap && baseSnap) {
                            ...
                            if (targetSnap.content === baseSnap.content) {
                                ...
                                const { generateWholeFileReplaceXml } = await import(
                                    '../services/fast-apply/DiffToXmlConverter'
                                );
                                const dynamicXml = generateWholeFileReplaceXml(baseSnap.content, targetSnap.content);
                                ...
                                logger.trace(`[diffStore] 重启恢复：动态 preview 刷新完成，${refreshedMods.length} 个修改`);
                            }
                            ...
                            set((state) => {
                                ...
                                if (restoredCtx) {
                                    newMap.set(contextId, { ...restoredCtx, activeSnapshotId: record.activeSnapshotId });
                                }
                                return { diffByContext: newMap };
                            });
                        }
                        ...
                        logger.warn('[diffStore] 重启恢复后 preview 刷新失败，降级为仅恢复 activeSnapshotId:', refreshError);
                        set((state) => {
                            const newMap = new Map(state.diffByContext);
                            const restoredCtx = newMap.get(contextId);
                            if (restoredCtx) {
    ...
    loadModifications: async (contextId, documentId, content, xml, messageId, fileName = 'document', isRestoring = false, currentContentForInference, preAppliedContent, persistedModStatuses) => {
        logger.trace(`[diffStore] loadModifications: contextId=${contextId}, doc=${documentId}, contentLen=${content.length}, xmlLen=${xml.length}, preAppliedLen=${preAppliedContent?.length ?? 0}, messageId=${messageId}`);
        ...
        try {
            ...
            if (loadModificationsGeneration.get(generationKey) !== currentGen) {
                logger.trace(`[diffStore] loadModifications 跳过过期的 preview()（gen=${currentGen}，当前=${loadModificationsGeneration.get(generationKey) ?? 'unknown'}）`);
                return;
            }
            ...
            if (!isRestoring && documentId.includes(':') && (!preAppliedContent || preAppliedContent === content)) {
                try {
                    const { invoke: tauriInvoke } = await import('@tauri-apps/api/core');
                    const diskContent = await tauriInvoke<string>('file_read_content', { filePath: documentId });
                    if (diskContent.length !== content.length && diskContent !== content) {
                        // 磁盘内容与传入基准不一致，说明用户手动编辑过
                        // 使用磁盘内容作为新的基准，确保 search 能正确匹配
                        logger.warn(`[diffStore] 5.3: 基准内容与磁盘不同步 (传入=${content.length} 字符, 磁盘=${diskContent.length} 字符)，使用磁盘内容更新基准`);
                        content = diskContent;
                    }
                } catch (diskError) {
                    // 读取失败时静默降级，不影响主流程
            ...
            if (loadModificationsGeneration.get(generationKey) !== currentGen) {
                logger.trace(`[diffStore] loadModifications 结果已过期（gen=${currentGen}，当前=${loadModificationsGeneration.get(generationKey) ?? 'unknown'}），丢弃`);
                return;
            }

            logger.trace(`[diffStore] preview 结果: ${batchResult.results.length} 个修改, pending=${batchResult.pendingCount}, failed=${batchResult.failedCount} (${previewElapsed}ms)`);
            ...
            if (hasMatchFailure && !isRestoring && preAppliedContent && preAppliedContent !== content) {
                ...
                try {
                    const { generateWholeFileReplaceXml } = await import('../services/fast-apply/DiffToXmlConverter');
            ...
            if (isRestoring) {
                if (persistedModStatuses) {
                    ...
                    try {
                        ...
                        finalModifications = inferModificationStatus(contentForInference, batchResult.results, content);
                    }
                } else {
                    ...
                    const isFreshUnapproved =
                        preAppliedContent !== undefined &&
                        contentForInference === preAppliedContent;
            ...
            if (isRestoring && persistedModStatuses && preAppliedContent) {
                ...
                if (hasRejected) {
                    rebuiltContent = await rebuildContentByDiff(content, preAppliedContent, finalModifications);
                ...
                if (currentCtx) {
                    ...
                    if (staleDocIds.size > 0) {
                        try {
                            ...
                            interface StaleRecord { id: string; documentId: string; }
                            const pendingRecords = await tauriInvoke2<StaleRecord[]>('diff_record_get_pending', { contextId });
                            for (const rec of pendingRecords) {
                                if (staleDocIds.has(rec.documentId)) {
                                    await tauriInvoke2('diff_record_update_status', {
                                        id: rec.id,
                                        status: 'reverted',
                                    }).catch((e: unknown) => logger.warn('[diffStore] 标记 stale diff_record reverted 失败:', e));
                                }
                            }
                        } catch (cleanupError) {
            ...
            logger.error(`[diffStore] loadModifications 失败: contextId=${contextId}, doc=${documentId}, xmlLen=${xml.length}`, error);
            set((state) => {
                const newMap = new Map(state.diffByContext);
                const ctx = newMap.get(contextId) ?? createEmptyContextState();
                newMap.set(contextId, {
    ...
    acceptModification: async (contextId, id) => {
        ...
        try {
            ...
            // 推入 Undo 栈
            const description = modResult.modification.description ??
            ...
            if (ctx.documentId?.includes(':') === true) {
                try {
                    ...
                    if (needsWrite) {
                        ...
                        logger.trace('[diffStore]  acceptModification: 磁盘内容已是最新，跳过写入（SA 模式已预写）:', ctx.documentId);
                    }
                    ...
                    try {
                        ...
                        const latestSnapshot = await invoke<SnapshotData | null>('snapshot_get_latest', { documentId: ctx.documentId });
                        if (latestSnapshot) {
            ...
            if (allReviewed) {
                try {
                    ...
                    const diffRecords = await invoke<PersistedDiffRecord[]>('diff_record_get_pending', { contextId });
                    const matchingRecords = diffRecords.filter((r: PersistedDiffRecord) => r.documentId === ctx.documentId);
                    await Promise.all(matchingRecords.map((record: PersistedDiffRecord) =>
                        invoke('diff_record_update_status', { id: record.id, status: 'applied' })
                    ));
                    if (matchingRecords.length > 0) {
                        logger.trace(`[diffStore] ✅ 所有修改已审批完成, ${matchingRecords.length} 条 diff_record 已更新为 applied, documentId:`, ctx.documentId);
                    }
                } catch (statusError) {
                ...
                set((state) => {
                    ...
                    const nextPendingEntry = Array.from(newFileEntries.values()).find(
                        e => e.documentId !== current.documentId &&
                            e.pendingModifications.some(m => m.status === 'pending')
                    );

                    if (nextPendingEntry) {
                        // 还有其他文件待审批，切换到它
                        newMap.set(contextId, applyFileDiffEntry(
                            { ...current, fileEntries: newFileEntries },
                            nextPendingEntry
                        ));
                    } else {
    ...
    rejectModification: async (contextId, id) => {
        ...
        newContent = await rebuildContentByDiff(ctx.originalContent, ctx.preAppliedContent, updatedModifications);
        ...
        if (allReviewed) {
            try {
                ...
                if (currentFileRecord) {
                    ...
                    logger.trace('[diffStore] ✅ diff_record 状态已更新为 applied, documentId:', ctx.documentId);
                }
            } catch (statusError) {
                logger.warn('[diffStore] ⚠ 更新 diff_record 状态失败:', statusError);
            }
    ...
    acceptAll: async (contextId) => {
        ...
        for (const mod of pendingMods) {
            const modIndex = updatedModifications.findIndex(m => m.modificationId === mod.modificationId);
            updatedModifications[modIndex] = {
                ...mod,
                status: 'applied',
            };
        ...
        if (ctx.documentId?.includes(':') === true) {
            try {
                ...
                try {
                    ...
                    logger.warn('[diffStore] acceptAll: 无法读取磁盘内容（file_read_content 失败），将直接写入 ctx.content');
                }
                ...
                if (needsWrite) {
                    ...
                    logger.trace('[diffStore]  acceptAll: 磁盘内容已是最新，跳过写入（SA 模式已预写）:', ctx.documentId);
                }
                ...
                if (matchingRecords.length > 0) {
                    logger.trace(`[diffStore] ✅ acceptAll: ${matchingRecords.length} 条 diff_record 已更新为 applied, documentId:`, ctx.documentId);
                }
    ...
    rejectAll: async (contextId) => {
        ...
        try {
            ...
            if (currentFileRecord) {
                ...
                logger.trace('[diffStore] ✅ rejectAll: diff_record 状态已更新为 applied, documentId:', ctx.documentId);
            }
        } catch (statusError) {
            logger.warn('[diffStore] ⚠ 更新 diff_record 状态失败:', statusError);
        }
    ...
    getFileList: (contextId) => {
        ...
        for (const [docId, entry] of ctx.fileEntries) {
            ...
            const mods = docId === ctx.activeFileId
                ? ctx.pendingModifications
                : entry.pendingModifications;
    ...
    rollback: async (contextId, snapshotId) => {
        ...
        if (ctx.snapshots.length > 0 && targetSnapshot) {
            ...
            logger.trace('[diffStore] 回滚：快照列表为空或未找到目标快照，使用旧行为（ctx.originalContent + ctx.originalXml）');
        }
            ...
            try {
                ...
                if (targetSnapshot?.modificationStatuses) {
                    ...
                    refreshedModifications = batchResult.results.map((newMod, idx) => {
                        const savedStatus = savedStatuses[String(idx)] as ModificationApplyResult['status'] | undefined;
                        return {
                    ...
                    refreshedModifications = batchResult.results.map((newMod, idx) => {
                        ...
                        const inheritedStatus = oldMod?.status === 'applied' || oldMod?.status === 'rejected'
                            ? oldMod.status
                            : newMod.status;
                ...
                logger.trace(`[diffStore] 回滚后重新 preview 完成：${refreshedModifications.length} 个修改`);
            } catch (previewError) {
                logger.warn('[diffStore] 回滚后重新 preview 失败，使用状态推断兜底:', previewError);
                refreshedModifications = inferModificationStatus(content, ctx.pendingModifications, ctx.originalContent);
            }
        } else {
            // 无 XML 时回退到推断
            refreshedModifications = inferModificationStatus(content, ctx.pendingModifications, ctx.originalContent);
...
export const selectApprovalProgressByContext = (contextId: string) => (state: DiffState) => {
    ...
    const approved = ctx.pendingModifications.filter(m => m.status === 'applied' || m.status === 'rejected').length;
