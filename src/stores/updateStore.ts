...
import { checkForUpdates, getConfiguredReleaseChannel, getConfiguredManifestUrl, getCurrentAppVersion } from '@services/update';
    ...
    persist(
        (set, get) => ({
            ...
            autoCheckIfNeeded: async () => {
                const state = get();
                if (!state.autoCheckEnabled || state.status === 'checking' || !shouldAutoCheck(state.lastCheckedAt)) {
                    return;
                }
