...
export const useChatStore = create<ChatState & ChatActions>((set, get) => ({
    ...
    prependMessages: (agentId, olderMessages) => set((state) => {
        const newMap = new Map(state.messagesByAgent);
    ...
    setHasMore: (agentId, hasMore) => set((state) => {
    ...
    // ========== Hub 消息操作 ==========
    addHubMessage: (hubId, message) => set((state) => {
    ...
    prependHubMessages: (hubId, olderMessages) => set((state) => {
        const newMap = new Map(state.messagesByHub);
    ...
    setHubHasMore: (hubId, hasMore) => set((state) => {
    ...
    setModeFor: (contextId, mode) => set((state) => {
    ...
    // ========== Hub 窗口引用操作 ==========
    addQuote: (hubId, quote) => set((state) => {
    ...
    removeQuote: (hubId, messageId) => set((state) => {
        const newMap = new Map(state.pendingQuotesByHub);
        const quotes = newMap.get(hubId) ?? [];
        newMap.set(hubId, quotes.filter(q => q.messageId !== messageId));
        return { pendingQuotesByHub: newMap };
    }),

    clearQuotes: (hubId) => set((state) => {
        const newMap = new Map(state.pendingQuotesByHub);
        newMap.delete(hubId);
        return { pendingQuotesByHub: newMap };
    }),
    ...
    // ========== Agent 窗口引用操作（隔离存储）==========
    addQuoteToAgent: (agentId, quote) => set((state) => {
    ...
    removeQuoteFromAgent: (agentId, messageId) => set((state) => {
        const newMap = new Map(state.pendingQuotesByAgent);
        const quotes = newMap.get(agentId) ?? [];
        newMap.set(agentId, quotes.filter(q => q.messageId !== messageId));
        return { pendingQuotesByAgent: newMap };
    }),

    clearAgentQuotes: (agentId) => set((state) => {
        const newMap = new Map(state.pendingQuotesByAgent);
        newMap.delete(agentId);
        return { pendingQuotesByAgent: newMap };
    }),
    ...
    appendStreamingContent: (contextId: string, chunk: string) => set((state) => {
        const newMap = new Map(state.streamingByContext);
        const current = newMap.get(contextId) ?? { content: '', isStreaming: true, agentName: undefined };
        // 保留 agentName 不变
    ...
    setStreamingContent: (contextId: string, content: string) => set((state) => {
        const newMap = new Map(state.streamingByContext);
        const current = newMap.get(contextId) ?? { content: '', isStreaming: true, agentName: undefined };
    ...
    // 设置中断控制器
    setAbortController: (contextId: string, controller: AbortController) => set((state) => {
    ...
    // 设置流式请求 session ID（用于后端取消）
    setSessionId: (contextId: string, sessionId: string) => set((state) => {
    ...
    closeSearch: (contextId) => set((state) => {
        const newMap = new Map(state.searchByContext);
        newMap.delete(contextId);
        return { searchByContext: newMap };
    }),

    setSearchQuery: (contextId, query) => set((state) => {
        const newMap = new Map(state.searchByContext);
        const current = newMap.get(contextId) ?? { isOpen: true, query: '', results: [], activeIndex: -1 };
        newMap.set(contextId, { ...current, query, activeIndex: -1 });
        return { searchByContext: newMap };
    }),

    setSearchResults: (contextId, results) => set((state) => {
        const newMap = new Map(state.searchByContext);
        const current = newMap.get(contextId) ?? { isOpen: true, query: '', results: [], activeIndex: -1 };
    ...
    setSearchActiveIndex: (contextId, index) => set((state) => {
        const newMap = new Map(state.searchByContext);
        const current = newMap.get(contextId);
        if (current) {
    ...
    // ========== 多选操作（按 contextId 隔离）==========
    enterMultiSelect: (contextId, initialId) => set((state) => {
    ...
    exitMultiSelect: (contextId) => set((state) => {
        const newMap = new Map(state.multiSelectByContext);
        newMap.delete(contextId);
        return { multiSelectByContext: newMap };
    }),

    toggleMessageSelect: (contextId, messageId) => set((state) => {
        const newMap = new Map(state.multiSelectByContext);
        const current = newMap.get(contextId);
    ...
    selectAllMessages: (contextId, messageIds) => set((state) => {
        const newMap = new Map(state.multiSelectByContext);
        const current = newMap.get(contextId);
        if (!current) return state;
    ...
    clearSelection: (contextId) => set((state) => {
        const newMap = new Map(state.multiSelectByContext);
        const current = newMap.get(contextId);
        if (!current) return state;
