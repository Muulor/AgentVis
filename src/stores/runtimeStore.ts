  ...
  persist(
    (set, get) => ({
      ...
      initSkillPreferences: async () => {
        ...
        try {
          const { loadSkillPreferences } =
            await import('@services/planning/skills/external/skillPreferencesService');
      ...
      toggleSkillEnabled: (skillName) => {
        ...
        // 更新内存中的开关 Map
        const newOverrides: SkillEnabledOverrides = {
          ...skillEnabledOverrides,
          [skillName]: newEnabled,
        };

        // 同步更新 installedSkills 中对应技能的 enabled 状态
        const updatedSkills = installedSkills.map((skill) =>
          skill.name === skillName ? { ...skill, enabled: newEnabled } : skill
        );

        logger.debug(`[RuntimeStore] 技能 "${skillName}" ${newEnabled ? '已启用' : '已禁用'}`);
        set({
      ...
      addPendingDependencies: (dep) => {
        ...
        // 去重：同名技能只保留最新
        const filtered = pendingDependencies.filter((d) => d.skillName !== dep.skillName);
      ...
      removePendingDependencies: (skillName) => {
        const { pendingDependencies } = get();
        set({
          pendingDependencies: pendingDependencies.filter((d) => d.skillName !== skillName),
        });
      },
      ...
      setSkillAuditResult: (result) => {
        ...
        logger.debug(`[RuntimeStore] 审查结果: ${result.auditResult} → 状态: ${auditStatus}`);
        set({
      ...
      reconcileWithPhysical: (runtimeUsable) => {
        ...
        if (isStaleIntermediateStatus) {
          ...
          if (_isActiveInstall) {
            ...
            logger.debug(`[RuntimeStore] 中间状态 "${envStatus}" 有活跃安装进程，跳过重置`);
            return;
          }
          ...
          if (runtimeUsable) {
            ...
            const isBaseEnvReady = envStatus === 'installing_extra' || envStatus === 'error';
            ...
            if (envStatus === 'error' && errorMessage) {
              logger.warn('[RuntimeStore] runtime 不可用且当前会话已有错误详情，保持 error 状态');
          ...
          logger.warn('[RuntimeStore] runtime 不可用但状态为 ready，重置为 not_created');
          set({
            envStatus: 'not_created',
            pythonVersion: null,
            installProgress: null,
...
/** 环境是否就绪 */
export const useRuntimeReady = () => useRuntimeStore((state) => state.envStatus === 'ready');
...
/** 已安装技能列表 */
export const useInstalledSkills = () => useRuntimeStore((state) => state.installedSkills);
...
export const useNeedsSetup = () =>
  useRuntimeStore(
    (state) => state.envStatus === 'not_created' || state.envStatus === 'not_checked'
  );
