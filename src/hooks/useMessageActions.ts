...
export function useMessageActions(options: UseMessageActionsOptions): UseMessageActionsReturn {
  ...
  // 回滚确认弹窗状态（仅 Agent 模式使用）
  const [revertDialogState, setRevertDialogState] =
    useState<RevertDialogState>(initialRevertDialogState);
  ...
  // 删除确认弹窗状态
  const [deleteDialogState, setDeleteDialogState] = useState<ActionConfirmDialogState>(
    initialActionConfirmDialogState
  );

  // 撤回确认弹窗状态（在 FileRevertDialog 之前的第一道确认门）
  const [revokeConfirmDialogState, setRevokeConfirmDialogState] =
    useState<ActionConfirmDialogState>(initialActionConfirmDialogState);
    ...
    async (
      ...
      switch (action) {
        ...
        case 'quote': {
          ...
          if (contextType === 'agent') {
            ...
            if (msg.metadata?.agentName) {
              ...
              const agentFromStore = useAgentStore
                .getState()
        ...
        case 'delete': {
          ...
          const isPlanningAssistant =
            msg?.role === 'assistant' && msg.metadata?.mode === 'planning';
          ...
          const executeDelete = async () => {
            if (contextType === 'agent') {
              ...
              if (deletedMsg?.role === 'assistant') {
                ...
                if (prevMsg?.role === 'user' && prevMsg.metadata) {
                  ...
                  if (meta.source === 'widget' || meta.source === 'cron') {
                    ...
                    logger.trace(
                      `[useMessageActions] 联动删除隐藏的 ${meta.source} user 消息:`,
                      prevMsg.id
                    );
                  }
                }
              ...
              let deletedIds = idsToDelete.filter(
                (_, index) => deleteResults[index]?.status === 'fulfilled'
              );
              if (failedDeletes.length > 0) {
                logger.error(
                  '[useMessageActions] Delete message failed:',
                  failedDeletes.map((result) => String(result.reason))
                );
                ...
                if (deletedIds.length > 0) {
                  try {
                    ...
                    logger.warn(
                      '[useMessageActions] Failed to clean partially deleted messages from memory buffer:',
                      e
                    );
                  }
                }
        ...
        case 'revoke': {
          ...
          const hasPlanningContent = followingMessages.some((m) => m.metadata?.mode === 'planning');
          ...
          const executeRevoke = async () => {
            ...
            if (contextType === 'agent') {
              ...
              if (idx !== -1) {
                ...
                const failedRetracts = retractResults.filter(
                  (result): result is PromiseRejectedResult => result.status === 'rejected'
                );
                if (failedRetracts.length > 0) {
                  logger.error(
                    '[useMessageActions] Hub revoke failed:',
                    failedRetracts.map((result) => String(result.reason))
                  );
