 ...
 * 构建多模态图片数据
 */
function buildImageData(imageAttachments: AttachmentInfo[]): Array<{ mime_type: string; data: string }> {
 ...
 */
function extractGeneratedImages(content: string): { cleanedContent: string; images: ExtractedImage[] } {
...
export function useChatSender(options: UseChatSenderOptions): UseChatSenderReturn {
    ...
    const sendMessage = useCallback(async (
        ...
        try {
            ...
            const quotedFromMeta = quotes.length > 0
                ? { quotedFrom: serializeQuotesForMessage(quotes) }
                : undefined;

            const userMetadata = (autoMetadata || userMessageMeta || quotedFromMeta)
            ...
            const streamingAgentName = contextType === 'agent'
                ? agentConfig?.name
                : mentionedAgent?.name;
            ...
            if (chatRules) {
                ...
                const rulesPathsToRead = [...new Set([mbRulesPath, saRulesPath].filter(Boolean) as string[])];
            ...
            if (enableMemory && contextType === 'agent') {
                try {
                    ...
                    // 身份/偏好注入到身份层
                    const bindingFactsPrompt = memoryContextProvider.buildBindingFactsPrompt(memoryContext.facts);
                    ...
                    contextFactsPrompt = memoryContextProvider.buildContextFactsPrompt(memoryContext.facts) ?? undefined;
                    summariesPrompt = memoryContextProvider.buildSummariesPrompt(memoryContext.summaries) ?? undefined;
                } catch (error) {
            ...
            const currentMessages = contextType === 'agent'
            ...
            for (const msg of chatHistory) {
                ...
                if (pending && pending.length > 0) {
                    if (!supportsVisionInput) {
                        resolvedHistory.push({ role: msg.role, content: msg.content, createdAt: msg.createdAt });
                        continue;
                    }
                    // 先添加原始 assistant 文本
                    resolvedHistory.push({ role: msg.role, content: msg.content, createdAt: msg.createdAt });
                    ...
                    try {
                        ...
                        if (loadedImages.length > 0) {
                            resolvedHistory.push({
                                role: 'user',
                                content: '[Historical reference: image generated by the previous assistant message]',
                                images: loadedImages,
                            });
                            logger.trace('[useChatSender] 🖼️ 恢复历史 assistant 生成图片:', loadedImages.length, '张');
                        }
                    } catch (error) {
            ...
            const { contextWindowManager, HISTORY_CONFIG } = await import('@services/planning/ContextWindowManager');
            ...
            logger.trace('[useChatSender] 预算报告:', JSON.stringify(preparedContext.budgetReport, null, 2));
            ...
            if (preparedContext.wasTruncated) {
                if (preparedContext.conversationHistory && preparedContext.conversationHistory !== NO_CONVERSATION_HISTORY) {
                    messages.push({
                        role: 'system',
                ...
                for (const m of chatHistory) {
                    const timePrefix = (m.role === 'user' && m.createdAt)
            ...
            const unlisten = await listen<{
                ...
                if (event.payload.done) {
                    ...
                    const outputTokens = getPositiveTokenCount(event.payload.outputTokens) ?? estimatedOutput;
            ...
            try {
                ...
                useStatusStore.getState().setContextPressure(contextId, estimatedInputTokens, contextWindow);
            } catch {
                // 上下文压力设置失败不影响主流程
            }
            ...
            let imageModelConfig: { response_modalities?: string[]; image_config?: { aspect_ratio?: string; image_size: string } } | undefined;
            ...
            if (cancellationState.cancelled || abortController.signal.aborted) {
                ...
                if (contextType === 'agent') {
                    ...
                    const agentResponse: Message = {
                        ...
                        metadata: accumulatedReasoning ? { reasoningContent: accumulatedReasoning } : undefined,
                    };
