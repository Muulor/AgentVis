...
import {
  ...
  getActiveImTask,
  markImTaskStarted,
} from '@services/im-channel/ImTaskBridge';
...
function getHistoricalMessageAttachments(
  metadata: Message['metadata']
): HistoricalAttachmentContextItem[] {
  if (!metadata) return [];
...
export function buildHistoricalAttachmentContext(
  ...
  const maxMessageChars =
    options.maxMessageChars ?? PLANNING_CONSTANTS.MASTER_BRAIN_MAX_MESSAGE_CHARS;
  ...
  const items = validAttachments
    .map((attachment) =>
      translateText('chat.historicalAttachmentContextItem', {
        fileName: attachment.fileName,
        type: attachment.type,
        extension: attachment.fileExtension,
  ...
  const contentBlocks = validAttachments
    ...
    .map((attachment) =>
      translateText('chat.historicalAttachmentContentBlock', {
        fileName: attachment.fileName,
        content: attachment.parsedContent ?? '',
      })
    );

  const rawContext = [header, ...contentBlocks].join('\n\n');
...
export function getPlanningHistoryEffectiveContent(
  ...
  let effectiveContent =
    message.role === 'assistant' && message.metadata
      ? (((message.metadata as Record<string, unknown>).persistContent as string | undefined) ??
        message.content)
      : message.content;
...
function buildPlanningCheckpointMbSection(
  ...
  const buildSection = (items: string[]): string => [header, items.join('\n\n')].join('\n');

  const rawSection = buildSection(phases);
  if (rawSection.length <= maxChars) return rawSection;

  const overhead = header.length + 1 + Math.max(0, phases.length - 1) * 2;
...
function formatPlanningCheckpointObservationEvent(
  ...
  if (event.toolAction) {
    const status =
      event.toolAction.success === undefined
...
function buildPlanningCheckpointSaSection(
  ...
  return [translateText('chat.planningCheckpointSaProgressHeader'), observationLines].join('\n');
}
...
export function buildPlanningCheckpointProgressText(
  ...
  const observationLimit =
    options.observationLimit ?? PLANNING_CHECKPOINT_PERSIST_OBSERVATION_LIMIT;
...
export function isPlanningCheckpointMessage(message: Pick<Message, 'role' | 'metadata'>): boolean {
...
export function isRecoverablePlanningCheckpointMessage(
  ...
  const sourceUserMessageId =
    typeof metadata.createdUserMessageId === 'string' ? metadata.createdUserMessageId : undefined;
...
export function usePlanningMode(options: UsePlanningModeOptions): UsePlanningModeReturn {
  const { contextType, contextId, agentConfig } = options;
   ...
   * 执行 Planning 任务
   */
  const executePlanningTask = useCallback(
    async (content: string, executeOptions?: ExecutePlanningOptions): Promise<void> => {
      ...
      const messageAgentId = contextType === 'agent' ? contextId : mentionedAgent?.id;
      ...
      const { startStreaming, finishStreaming, setStreamingContent } = useChatStore.getState();
      ...
      const checkpointFlushTimerRef: { current: ReturnType<typeof setTimeout> | null } = {
        current: null,
      };
      ...
      const buildPlanningCheckpointObservationText = (): string => {
        return buildPlanningCheckpointProgressText(thinkingChainData, subAgentObservationsData, t);
      };
      ...
      const buildPlanningCheckpointMetadata = (
        ...
        error: errorMessage,
        reasoningTrace: reasoningTraceData.content.trim() ? { ...reasoningTraceData } : undefined,
        thinkingChain: thinkingChainData,
        thinkingSteps: fsmVisualizationActions
        ...
        subAgentObservations:
          subAgentObservationsData.length > 0
      ...
      const deletePlanningCheckpoint = async (reason: string): Promise<void> => {
        ...
        try {
          ...
          try {
            ...
            logger.debug(
              '[usePlanningMode] Planning checkpoint abandoned 标记跳过或失败:',
              abandonError
            );
          }
        } finally {
      ...
      try {
        ...
        try {
          ...
          logger.debug(
            '[usePlanningMode] Planning checkpoint 已创建:',
            planningCheckpointMessage.id
          );
        ...
        const mbAgentRules =
          getNonEmptyRulesText(effectiveAgentConfig.mbRules) ??
          (effectiveAgentConfig.mbRulesFilePath
            ? await readRulesFile(effectiveAgentConfig.mbRulesFilePath)
            : undefined);
        const saAgentRules =
          getNonEmptyRulesText(effectiveAgentConfig.saRules) ??
        ...
        try {
          ...
          subAgentSafetyFooterText =
            currentAgent?.subAgentSafetyFooterText ?? subAgentSafetyFooterText;
        } catch (avatarError) {
        ...
        if (attachmentsToSend.length > 0) {
          ...
          try {
            ...
            let builtContent =
              attachmentService.buildAttachmentContext(attachmentsToSend, { mode: 'planning' }) ||
              '';

            // 图片附件：提取 base64 数据用于多模态传递，同时注入文件名提示到文本上下文
            if (supportsVisionInput && imageAttachments.length > 0) {
              // 构建 base64 图片数据供后端多模态处理
              imageDataForLLM = imageAttachments.flatMap((img) => {
              ...
              const imageDescriptions = imageAttachments
                .map(
                  (img) =>
                    `- Image: ${img.fileName} (${img.fileExtension}, ${Math.round((img.size || 0) / 1024)}KB)`
                )
                .join('\n');
        ...
        storedMessages.forEach((message, index) => {
          ...
          const hasLaterNonCurrentMessage = storedMessages
            .slice(index + 1)
            .some(
              (candidate) => candidate.id !== userMessageResult.id && candidate.role !== 'system'
            );
        ...
        const historyMessages = storedMessages
          .filter((m) => {
            ...
            if (isPlanningCheckpointMessage(m)) {
              return (
                isRecoverablePlanningCheckpointMessage(m, storedMessages) &&
          ...
          .map((m) => {
            ...
            if (m.role === 'user' && m.metadata) {
              interface QuotedFromItem {
                content: string;
                agentName?: string;
              }
              const quotedFrom = (m.metadata as Record<string, unknown>).quotedFrom as
                | QuotedFromItem[]
                | undefined;
            ...
            if (supportsVisionInput && m.role === 'user' && m.metadata) {
              ...
              if (attachmentsList) {
                ...
                if (imgAttachments.length > 0) {
                  ...
                  logger.trace(
                    '[usePlanningMode] 🖼️ 恢复历史 user 消息图片:',
                    base.images.length,
                    '张'
                  );
                }
            ...
            try {
              ...
              const results = await Promise.allSettled(
                pending.map(async (imgPath) => {
                  ...
                  const base64 = await tauriInvoke<string>('file_read_as_base64', {
                    path: imgPath,
                  });
              ...
              if (loadedImages.length > 0) {
                ...
                logger.trace(
                  '[usePlanningMode] 🖼️ 恢复历史 assistant 生成图片:',
                  loadedImages.length,
                  '张'
                );
              }
        ...
        const shouldScheduleVisualEnhancement =
          result.terminationReason !== 'cancelled' && visualEnhancementEnabled;
        ...
        const messageMetadata = {
          ...
          subAgentObservations:
            subAgentObservationsData.length > 0 ? subAgentObservationsData : undefined,
          // 可视化增强标记（调试/统计用途）
        ...
        if (!isCreatedUserMessageStillPresent()) {
          logger.debug(
            '[usePlanningMode] 用户消息已撤回，跳过迟到的 Planning assistant 持久化:',
            createdUserMessageId
          );
          await deletePlanningCheckpoint(
            'source user message disappeared before final assistant persistence'
          );
          return;
        }
        ...
        if (shouldScheduleVisualEnhancement) {
          void (async () => {
            try {
              const { enhance, shouldEnhance, visualEnhancementJobManager } =
              ...
              visualEnhancementJobManager.enqueue({
                ...
                execute: async (signal) => {
                  ...
                  const currentMetadata =
                    currentMessage.metadata && typeof currentMessage.metadata === 'object'
                  ...
                  if (contextType === 'agent') {
                    useChatStore
                      .getState()
                      .updateMessage(contextId, assistantMessageResult.id, messageUpdates);
                  } else {
                    useChatStore
                      .getState()
                      .updateHubMessage(contextId, assistantMessageResult.id, messageUpdates);
                  }
                  logger.debug(
                    '[usePlanningMode] ✨ 后台可视化增强成功:',
                    assistantMessageResult.id
                  );
                },
              });
        ...
        if (tempMessageIdForDiff) {
          void (async () => {
            try {
              ...
              await useDiffStore
                .getState()
                .updateMessageId(contextId, tempMessageIdForDiff, assistantMessageResult.id);
              logger.trace(
                '[usePlanningMode]  messageId 已更新:',
                tempMessageIdForDiff,
                '->',
                assistantMessageResult.id
              );
        ...
        if (errorMsg.includes('FOREIGN KEY') || errorMsg.includes('foreign key')) {
          logger.warn(
            '[usePlanningMode] Planning 计划消息持久化失败（Agent 已不在 DB）:',
            errorMsg
          );
        } else {
        ...
        if (!isCreatedUserMessageStillPresent()) {
          logger.debug(
            '[usePlanningMode] 用户消息已撤回，跳过迟到的 Planning 错误消息:',
            createdUserMessageId
          );
          await deletePlanningCheckpoint(
            'source user message disappeared before error assistant persistence'
          );
          return;
        }
  ...
  const stopPlanningTask = useCallback(() => {
    ...
    if (contextId) {
      ...
      if (service) {
        ...
        logger.warn(
          '[usePlanningMode] 未找到可取消的 Planning 服务，仅清理 UI 状态, contextId:',
          contextId
        );
      }
