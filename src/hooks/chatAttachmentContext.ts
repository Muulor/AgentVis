 ...
 * 因此提示语必须强调只能基于内联摘录回答，并在需要完整附件时建议切换 Task 模式。
 */
...
type ChatAttachmentContextTranslate = (key: TranslationKey, params?: TranslationParams) => string;
...
export function buildChatHistoricalAttachmentContext(
  ...
  const maxContextChars = Math.floor(
    maxTokens * CHAT_HISTORICAL_ATTACHMENT_CONTEXT_CHARS_PER_TOKEN
  );
  const maxMessageChars =
    options.maxMessageChars ?? CHAT_HISTORICAL_ATTACHMENT_CONTEXT_MAX_MESSAGE_CHARS;
  ...
  const contentBlocks = validAttachments
    ...
    .map((attachment) =>
      translateText('chat.chatHistoricalAttachmentContentBlock', {
        fileName: attachment.fileName,
  ...
  const notice = translateText('chat.chatHistoricalAttachmentContextTruncatedNotice', {
    maxTokens,
  });
