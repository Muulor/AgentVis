...
function getAttachmentIdentity(attachment: AttachmentInfo): string {
  return (
    normalizeAttachmentPath(attachment.localPath) ??
...
function isSameAttachmentPath(filePath: string, attachment: AttachmentInfo): boolean {
  ...
  return (
    normalizedPath === normalizeAttachmentPath(attachment.localPath) ||
...
export function useAttachmentManager(
  ...
  useEffect(() => {
    ...
    logger.trace(
      '[useAttachmentManager] 上下文切换，加载附件:',
      contextId,
  ...
  const addAttachments = useCallback(
    async (filePaths: string[], addOptions: AddAttachmentsOptions = {}) => {
      if (!contextId || filePaths.length === 0) return;

      for (const filePath of filePaths) {
        if (
          pendingAttachmentsRef.current.some((attachment) =>
            isSameAttachmentPath(filePath, attachment)
          )
        ) {
        ...
        try {
          ...
          useAttachmentViewerStore
            .getState()
            .setContextAttachments(contextId, pendingAttachmentsRef.current);

          // 自动预览：仅第一个文档类型附件自动预览，图片不自动预览
          // 检查是否为第一个文档（之前没有任何附件）
          const isFirstDocument =
            attachment.type === 'document' && pendingAttachmentsRef.current.length === 1;
          if (isFirstDocument) {
    ...
    (error: unknown) => {
      if (error instanceof ImageCompressionError) {
        ...
        toast({
          ...
          description: t('chat.supportedFormats', {
            formats: t('chat.attachmentSupportedFormats'),
          }),
          duration: 5000,
        });
      } else {
