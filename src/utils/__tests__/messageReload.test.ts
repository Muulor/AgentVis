...
vi.hoisted(() => {
    ...
    globalThis.localStorage = {
        ...
        clear: () => { Object.keys(memoryStorage).forEach((key) => Reflect.deleteProperty(memoryStorage, key)); },
...
describe('messageReload', () => {
    ...
    it('restores widget submitted state from reloaded hidden widget messages', () => {
        ...
        expect(state.bubbleSelections.get('bubble-1')?.get('choices:bubble-1:Scope')).toEqual(['Uploaded only']);
