...
describe('classifyNetworkDirectTarget', () => {
    it('classifies public targets as public', () => {
        expect(classifyNetworkDirectTarget({ protocol: 'ssh', host: 'example.com', port: 22 }))
            .toBe('public');
    });

    it('classifies localhost, private, link-local, and CGNAT targets as private', () => {
        for (const host of ['localhost', '127.0.0.1', '192.168.1.10', '169.254.1.1', '100.64.1.10', '::1']) {
    ...
    it('classifies cloud metadata targets as metadata', () => {
        for (const host of ['169.254.169.254', '169.254.170.2', '100.100.100.200', 'metadata.google.internal']) {
    ...
    it('prefers backend resolved DNS risk when present', () => {
        expect(classifyNetworkDirectTarget({
            protocol: 'ssh',
            host: 'db.example.com',
            port: 22,
