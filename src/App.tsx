...
import { useAttachmentViewerStore, selectLightboxImage, selectLightboxImages, selectLightboxIndex } from '@stores/attachmentViewerStore';
...
function App() {
    ...
    useEffect(() => {
        import('@/config/modelRegistry').then(({ loadUserModels }) => {
    ...
    useEffect(() => {
        const bootstrapSkills = async () => {
            try {
                const result = await invoke<{ deployed: boolean; packages_dir: string; skill_count: number }>(
                    'bootstrap_skills_if_needed'
                );
                if (result.deployed) {
                    logger.trace(`[App] 内置 skill 包已部署: ${result.skill_count} 个 → ${result.packages_dir}`);
                }
            } catch (err) {
        ...
        const autoConnectImChannels = async () => {
            try {
                ...
                if (!autoConnect || botConfigs.length === 0) {
                    ...
                    const { defaultAgentId } = useImChannelStore.getState() as { defaultAgentId: string | null };
                ...
                for (const botConfig of enabledBots) {
                    ...
                    try {
                        // 读取 per-bot 凭据
                        const creds = await invoke<{ appId: string; appSecret: string; botToken?: string; appToken?: string }>(
                            'im_get_bot_credentials',
                            { platform: botConfig.platform, botId: botConfig.botId },
                        );
    ...
    useEffect(() => {
        ...
        return () => {
            void unlisten.then(fn => fn()).catch((err: unknown) => {
