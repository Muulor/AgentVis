...
const BUILTIN_PROVIDERS: ProviderDefinition[] = [
    { id: 'openai', name: 'OpenAI', protocol: 'openai', apiKeyPlaceholder: 'sk-...', apiKeyUrl: 'https://platform.openai.com/api-keys' },
    { id: 'anthropic', name: 'Anthropic', protocol: 'anthropic', apiKeyPlaceholder: 'sk-ant-...', apiKeyUrl: 'https://console.anthropic.com/settings/keys' },
    { id: 'gemini', name: 'Google AI', protocol: 'gemini', apiKeyPlaceholder: 'AIza...', apiKeyUrl: 'https://aistudio.google.com/api-keys' },
    { id: 'zhipu', name: 'ZhipuAI', protocol: 'openai', apiKeyPlaceholder: 'API Key...', apiKeyUrl: 'https://bigmodel.cn/coding-plan/personal/overview' },
    { id: 'deepseek', name: 'DeepSeek', protocol: 'openai', apiKeyPlaceholder: 'sk-...', apiKeyUrl: 'https://platform.deepseek.com/api_keys' },
    { id: 'agnes', name: 'Agnes AI', protocol: 'openai', apiKeyPlaceholder: 'API Key...', apiKeyUrl: 'https://platform.agnes-ai.com/settings/profile' },
    { id: 'stepfun', name: 'StepFun (Step Plan)', protocol: 'openai', apiKeyPlaceholder: 'API Key...', apiKeyUrl: 'https://platform.stepfun.com/interface-key' },
    { id: 'xiaomi-mimo', name: 'Xiaomi(Token Plan)', protocol: 'anthropic', apiKeyPlaceholder: 'API Key...', apiKeyUrl: 'https://platform.xiaomimimo.com/console/plan-manage' },
    { id: 'zhipu-coding', name: 'ZhipuAI (Coding Plan)', protocol: 'openai', apiKeyPlaceholder: 'API Key...', apiKeyUrl: 'https://bigmodel.cn/coding-plan/personal/overview' },
    { id: 'minimax', name: 'MiniMax(Token Plan)', protocol: 'anthropic', apiKeyPlaceholder: 'API Key...', apiKeyUrl: 'https://platform.minimaxi.com/user-center/payment/token-plan' },
    { id: 'volcengine', name: 'Volcengine (Coding Plan)', protocol: 'openai', apiKeyPlaceholder: 'sk-sp-...', apiKeyUrl: 'https://console.volcengine.com/ark/region:ark+cn-beijing/apikey' },
    { id: 'openrouter', name: 'OpenRouter', protocol: 'openai', apiKeyPlaceholder: 'sk-or-...', apiKeyUrl: 'https://openrouter.ai/workspaces/default/keys' },
...
function getMergedModels(): ModelDefinition[] {
    ...
    const userModelKeys = new Set(
        userModels.map(m => `${m.id}::${m.providerId}`)
    );
...
function sameModelKey(a: { id: string; providerId: string }, b: { id: string; providerId: string }): boolean {
 ...
 */
export function findRegisteredModel(modelId: string, providerId: string): ModelDefinition | undefined {
...
export async function updateUserModel(
    ...
    const duplicate = getMergedModels().some(existing =>
        sameModelKey(existing, normalized) && !sameModelKey(existing, originalKey)
    );
    if (duplicate) {
        throw new ModelConfigError('Model already exists');
    }
...
export async function importModelsFromJson(jsonContent: string): Promise<number> {
    ...
    if (!config.version || !Array.isArray(config.models)) {
        throw new ModelConfigError('Invalid configuration file format: missing version or models field');
    }
