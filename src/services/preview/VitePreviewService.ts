...
function sanitizeCssForTailwindV3(css: string): string {
  ...
  //    保留内部的 CSS 变量声明
  result = result.replace(/@theme\s*\{([^}]*)\}/gs, ':root {$1}');
...
class VitePreviewService {
  ...
  async startProject(
    ...
    try {
      ...
      const templatePath = await templateManager.ensureTemplateReady(templateId, (message) =>
        logger.trace(`[VitePreviewService] 模板进度: ${message}`)
      );
  ...
  private async initProjectDirectory(
    ...
    const hasExtraDeps =
      Object.keys(extraDeps.dependencies).length > 0 ||
    ...
    const agentConfigRoots = new Set(
      files.filter((f) => !f.path.includes('/')).map((f) => f.path.replace(/\.[^.]+$/, ''))
    );
   ...
   */
  private async linkAssetDirectories(projectDir: string, deliverableDir: string): Promise<void> {
    const { join } = await import('@tauri-apps/api/path');
      ...
      for (const entry of parentEntries) {
        ...
        if (!(await exists(targetInProject))) {
          try {
            ...
            logger.warn(
              `[VitePreviewService] 父目录图片 hardlink 到项目失败 ${entry.name}:`,
              linkError
            );
          }
        }
        ...
        if (!(await exists(targetInDeliverable))) {
          try {
            ...
            logger.warn(
              `[VitePreviewService] 父目录图片 hardlink 到交付物失败 ${entry.name}:`,
              linkError
            );
          }
        }
  ...
  private async waitForViteReady(url: string): Promise<void> {
    ...
    while (Date.now() - startTime < VITE_START_TIMEOUT_MS) {
      ...
      if (this.state.pid === null) {
        throw new Error('[VitePreviewService] Vite process exited before startup completed');
      }
  ...
  private assertNotPreempted(expectedGeneration: number): void {
    if (this.startGeneration !== expectedGeneration) {
      throw new Error(
        '[VitePreviewService] Preview startup was cancelled because a newer start/stop request was detected'
      );
    }
  }
      ...
      try {
        ...
        if (response.ok || response.status === 404) {
          ...
          try {
            ...
            const pids = netstatResult.stdout
              .trim()
              .split('\n')
