...
async function readAsDataUrl(absolutePath: string, relPath: string): Promise<string | null> {
    try {
        ...
        logger.debug('[htmlResourceInliner] 读取成功:', relPath, `(${(dataUrl.length / 1024).toFixed(0)} KB data URL)`);
        return dataUrl;
    } catch (error) {
...
async function inlineExternalCss(html: string, baseDir: string): Promise<string> {
    ...
    for (const { fullMatch, relPath } of matches) {
        ...
        if (cssContent) {
            ...
            result = result.replace(fullMatch, `<style>/* inlined from ${relPath} */\n${cssContent}\n</style>`);
...
async function inlineExternalJs(html: string, baseDir: string): Promise<string> {
    ...
    for (const { fullMatch, relPath } of matches) {
        ...
        if (jsContent) {
            result = result.replace(fullMatch, `<script>/* inlined from ${relPath} */\n${jsContent}\n</script>`);
...
export async function inlineHtmlResources(
    ...
    logger.debug('[htmlResourceInliner] 检测到', resourcePaths.length, '个二进制资源:', resourcePaths);
    ...
    logger.debug('[htmlResourceInliner] 完成, 内嵌', resourceMap.size, '/', resourcePaths.length, '个资源');
