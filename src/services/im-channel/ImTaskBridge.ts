...
async function finalizeCancelledTask(
    botId: string,
    taskId: string,
    error: string,
): Promise<void> {
...
function ensureCompletionListenerInitialized(): void {
    ...
    // 监听任务失败事件
    listen<{ taskId: string; error: string; botId?: string }>(
        'im:task_failed',
        (event) => {
...
async function handleTaskCompleted(
    ...
    if (isTerminalTaskStatus(task.status)) {
        logger.trace(`忽略已终态 IM 任务的完成事件: taskId=${taskId}, botId=${resolvedBotId ?? 'legacy'}, status=${task.status}`);
        return;
    }
...
async function handleTaskFailed(
    ...
    if (task.status === 'completed' || task.status === 'failed') {
        logger.trace(`忽略已终态 IM 任务的失败事件: taskId=${taskId}, botId=${resolvedBotId ?? 'legacy'}, status=${task.status}`);
        return;
    }

    if (task.status === 'cancelled') {
        logger.trace(`IM 任务取消已被 AgentLoop 确认: taskId=${taskId}, botId=${resolvedBotId ?? 'legacy'}`);
        task.error = task.error ?? error;
        if (resolvedBotId) {
...
async function handleIncomingMessage(
    ...
    logger.info(`收到 IM 消息: botId=${botId} [${msg.platform}] ${msg.senderName || msg.senderId}: ${msg.content.slice(0, 100)}`);
    ...
    if (activeTask) {
        await channel.sendCard(
            msg.chatId,
            buildBusyCard(translate(channel.platform === 'slack' || channel.platform === 'feishu'
                ? 'im.bridge.busySlack'
                : 'im.bridge.busy')),
        );
        return;
...
async function triggerPlanningExecution(
    ...
    if (targetHubId && targetHubId !== currentHubId) {
        logger.trace(`IM 任务跨 Hub 导航: ${currentHubId ?? 'none'} → ${targetHubId}, agentId=${task.agentId}`);
    ...
    setTimeout(() => {
        ...
        if (currentTask?.id === taskId && currentTask.status === 'pending') {
            ...
            const tracker = activeTrackers.get(botId);
            if (tracker) {
                tracker.sendErrorCard(translate('im.bridge.timeoutCardError'))
...
async function handleAbortTask(
    ...
    activeTask.feishuCardUpdateToken = getFeishuCardUpdateToken(value) ?? activeTask.feishuCardUpdateToken;
...
async function buildEnhancedPrompt(channel: ImChannel, msg: ImIncomingMessage): Promise<string> {
    ...
    const attachmentSection = [
        ...
        ...savedPaths.map((p, i) => translate('im.bridge.attachmentItem', {
            index: i + 1,
            path: p,
        })),
    ].join('\n');

    const baseContent = msg.content || translate('im.bridge.attachmentFileFallback', {
        platform: getPlatformDisplayName(msg.platform),
    });
