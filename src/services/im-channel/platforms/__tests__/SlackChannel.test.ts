...
describe('stripSlackMention', () => {
    it('removes the configured bot mention and decodes Slack text entities', () => {
        expect(stripSlackMention('<@U123BOT> please read A &amp; B &lt; C', 'U123BOT'))
            .toBe('please read A & B < C');
    });

    it('removes generic Slack mentions when bot user id is unavailable', () => {
        expect(stripSlackMention('<@U999|agent> summarize this'))
            .toBe('summarize this');
    });

    it('removes mentions for both Slack bot user id and bot id', () => {
        expect(stripSlackMention('<@B123BOT> chat with me', ['U123USER', 'B123BOT']))
            .toBe('chat with me');
    });
});
...
describe('slack mention detection', () => {
    it('detects bot mentions with Slack display labels', () => {
        expect(slackTextMentionsUser('<@U123BOT|agentvis> summarize this', 'U123BOT'))
            .toBe(true);
    });

    it('detects bot id mentions from channel message text', () => {
        expect(slackTextMentionsUser('<@B123BOT> chat with me', ['U123USER', 'B123BOT']))
            .toBe(true);
    });
