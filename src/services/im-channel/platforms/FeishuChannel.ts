...
export class FeishuChannel implements ImChannel {
    ...
    async sendCard(chatId: string, card: ImCardContent): Promise<string> {
        ...
        try {
            ...
            if (isFeishuCardParseError(primaryError)) {
                logger.warn('发送完整卡片失败（解析错误），降级为纯文本卡片重试', { error: String(primaryError) });
    ...
    async updateCard(messageId: string, card: ImCardContent, context?: ImCardUpdateContext): Promise<void> {
        ...
        if (updateToken) {
            try {
                ...
                if (isFeishuCardParseError(primaryError)) {
                    logger.warn('飞书 delayed update 完整卡片解析失败，降级为纯文本卡片重试', { error: String(primaryError) });
                    const fallbackJson = buildFeishuCardTextOnly(card);
                    await this.updateCardByInteractionToken(updateToken, fallbackJson);
                    return;
                }

                logger.warn('飞书 delayed update 失败，回退到普通消息更新', { error: String(primaryError) });
            }
        }
        ...
        try {
            ...
            // —— 避免手机端永久卡在"正在执行任务..."状态
            if (isFeishuCardParseError(primaryError)) {
                logger.warn('更新完整卡片失败（解析错误），降级为纯文本卡片重试', { error: String(primaryError) });
                const fallbackJson = buildFeishuCardTextOnly(card);
                await invoke('feishu_update_message', {
                    token,
                    messageId,
    ...
    private async updateCardByInteractionToken(updateToken: string, cardJson: Record<string, unknown>): Promise<void> {
        const token = await this.ensureToken();
        const result = await invoke<{ status: number; headers: Record<string, string>; body: string }>('feishu_http_proxy', {
            request: {
        ...
        try {
            ...
            throw new Error(`Feishu delayed update returned non-JSON response: ${result.body.slice(0, 200)}`);
        }
    ...
    async sendFile(chatId: string, fileBase64: string, fileName: string, fileType: string): Promise<string> {
        const token = await this.ensureToken();
    ...
    private handleMessageEvent(event: FeishuMessageEvent): void {
        try {
            ...
            if (this.connectInitiatedAt > 0 && messageTimestamp > 0 && messageTimestamp < this.connectInitiatedAt) {
    ...
    private injectAxiosProxy(): void {
        ...
        if (typeof (window as unknown as Record<string, unknown>).Buffer === 'undefined') {
            const BufferPolyfill = {
                from(data: string | Uint8Array | number[], encoding?: string): Uint8Array & { toString(enc?: string): string } {
                ...
                concat(list: Uint8Array[]): Uint8Array & { toString(enc?: string): string } {
                    ...
                    result.toString = (enc?: string): string =>
                        BufferPolyfill.from(result).toString(enc);
                    return result;
                },
        ...
        const ProxiedXHR = function () {
            const wrapper: FeishuProxyXhr = {
                ...
                send(body?: Document | XMLHttpRequestBodyInit | null) {
                    if (wrapper._useProxy) {
                        // 飞书请求：通过 Rust 后端代理
                        invoke<{ status: number; headers: Record<string, string>; body: string }>('feishu_http_proxy', {
                            request: {
                                url: wrapper._url,
                ...
                removeEventListener(type: string, listener: EventListenerOrEventListenerObject) {
                    ...
                    if (listeners) {
                        wrapper._eventListeners[type] = listeners.filter((l: EventListenerOrEventListenerObject) => l !== listener);
                    }
                },
