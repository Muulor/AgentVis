...
export function describeCronExpression(expression: string, language: CronDescriptionLanguage = 'zh-CN'): string {
    ...
    if (minField !== '*' && hourField === '*' && domField === '*' && monField === '*' && dowField === '*') {
        return isEnglish ? `At minute ${minField} every hour` : `\u6bcf\u5c0f\u65f6\u7684\u7b2c ${minField} \u5206\u949f`;
    }
    ...
    if (dowField !== '*' && domField === '*' && monField === '*') {
        const dayNames = isEnglish ? ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] : ['\u65e5', '\u4e00', '\u4e8c', '\u4e09', '\u56db', '\u4e94', '\u516d'];
    ...
    if (monField !== '*' && domField !== '*' && minField !== '*' && hourField !== '*' && dowField === '*') {
        const minutes = Array.from(parsed.minutes).sort((a, b) => a - b);
        const hours = Array.from(parsed.hours).sort((a, b) => a - b);
        const timeStr = hours.map(h =>
            minutes.map(m => `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`).join(', ')
        ).join(', ');
        return isEnglish ? `On ${monField}/${domField} at ${timeStr}` : `${monField}\u6708${domField}\u65e5 ${timeStr}`;
    }

    // 每月某日 + 时间（如 "0 9 15 * *" → "每月15日 09:00"）
    if (domField !== '*' && monField === '*' && minField !== '*' && hourField !== '*' && dowField === '*') {
        const minutes = Array.from(parsed.minutes).sort((a, b) => a - b);
        const hours = Array.from(parsed.hours).sort((a, b) => a - b);
        const timeStr = hours.map(h =>
            minutes.map(m => `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`).join(', ')
        ).join(', ');
        return isEnglish ? `Every month on day ${domField} at ${timeStr}` : `\u6bcf\u6708${domField}\u65e5 ${timeStr}`;
    }
...
export function parseScheduleConfig(expression: string): ScheduleConfig | null {
    ...
    if (intervalMatch && hourField === '*' && domField === '*' && monField === '*' && dowField === '*') {
        return {
    ...
    // 每小时: N * * * *
    if (!isNaN(min) && hourField === '*' && domField === '*' && monField === '*' && dowField === '*') {
