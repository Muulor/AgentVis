...
async function executePlanningMode(job: CronJob): Promise<CronExecutionResult> {
    ...
    if (cachedAgent?.hubId) {
        ...
        try {
            ...
            logger.error(`Cron 任务: 查询 agent_get 失败, agentId=${job.agentId}`, { error: String(err) });
        }
    }

    // 第二步：若目标 Hub 与当前 Hub 不同，先切换并等待 agents 加载完成
    if (targetHubId && targetHubId !== currentHubId) {
        logger.info(`Cron 任务跨 Hub 导航: ${currentHubId ?? 'none'} → ${targetHubId}, agentId=${job.agentId}`);
...
function resolveCronImBotId(agentId: string): string | undefined {
    ...
    const matchedBots = botConfigs.filter((bot) =>
        bot.enabled
        && bot.agentId === agentId
        && bot.hasCredentials
    );
