...
export function upsertSubAgentObservationEvent(
  ...
  if (event.reasoningTrace) {
    ...
    if (existingReasoningIndex >= 0) {
      ...
      if (previous) {
        ...
        const isStreaming =
          event.reasoningTrace.isStreaming ?? previous.reasoningTrace?.isStreaming;
  ...
  const mergedToolAction =
    previous.toolAction && event.toolAction
      ? {
          ...previous.toolAction,
          ...event.toolAction,
