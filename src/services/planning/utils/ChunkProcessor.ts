...
const EXTENSION_CATEGORIES: Record<string, FileCategory> = {
    // 文档类型
    md: 'document', txt: 'document', rst: 'document', adoc: 'document', tex: 'document',
    // 代码类型
...
export class ChunkProcessor {
    ...
    splitIntoChunks(
        ...
        logger.trace(`[ChunkProcessor] 分割完成: ${chunks.length} 个 chunk, ` +
    ...
    shouldSkipChunk(chunk: FileChunk, filePath: string): boolean {
        ...
        if (nonEmptyLines.length < MIN_MEANINGFUL_LINES) {
            logger.trace(`[ChunkProcessor] 跳过稀疏 chunk (行 ${chunk.startLine}-${chunk.endLine}): ` +
    ...
    async processChunksInBatches<T>(
        ...
        logger.trace(`[ChunkProcessor] 开始批量处理 ${chunks.length} 个 chunk, ` +
        ...
        logger.trace(`[ChunkProcessor] 处理完成: 成功 ${successCount}, ` +
    ...
    private async processWithRetry<T>(
        ...
        try {
            ...
            logger.trace(`[ChunkProcessor] 处理 chunk ${index + 1}: ` +
            ...
            if (retryCount < maxRetries && this.isRetryableError(errorMessage)) {
                ...
                logger.warn(`[ChunkProcessor] chunk ${index + 1} 处理失败，` +
                    `${delay}ms 后重试: ${errorMessage}`);

                await new Promise(resolve => setTimeout(resolve, delay));

                return this.processWithRetry(
                    chunk, index, processor,
                    maxRetries, retryDelayBaseMs, retryCount + 1
                );
            }
