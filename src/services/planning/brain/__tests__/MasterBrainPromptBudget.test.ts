...
function generateLargeToolCatalog(count: number): ToolCatalogEntry[] {
    ...
    for (let i = 0; i < count; i++) {
        tools.push({
            ...
            whenNotToUse: [
                `When operation type ${i} is not needed`,
...
function generateLargeMemorySnapshot(factCount: number): MemorySnapshot {
    ...
    snapshot.summaries.push({
        ...
        openQuestions: [
            { question: 'What database to use?', scope: 'technical', reason: 'Need to finalize data layer architecture' },
        ],
...
describe('MasterBrainPrompt 预算管理', () => {
    ...
    describe('对话历史格式化', () => {
        ...
        it('短消息不应被截断', () => {
            ...
            // 不应出现截断标记（针对该消息）
            const historySection = prompt.split('[CONVERSATION_HISTORY]')[1]?.split('[RAG_EVIDENCE]')[0] ?? '';
    ...
    describe('极端场景', () => {
        ...
        it('未知 modelId 应使用 default 上下文窗口', () => {
            ...
            const expectedBudget = Math.floor(
                PLANNING_CONSTANTS.DEFAULT_CONTEXT_WINDOW * PLANNING_CONSTANTS.MASTER_BRAIN_PROMPT_BUDGET_RATIO
            );
