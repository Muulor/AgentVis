...
describe('MasterBrainPrompt', () => {
  ...
  describe('Prompt 结构', () => {
    ...
    it('should prefer a pre-resolved language hint from the original request', () => {
      const input = createTestInput({
        ...
        outputLanguageHint: resolveOutputLanguage('Please provide the final answer in French.', {
  ...
  describe('RAG 证据格式化', () => {
    it('应该正确格式化 RAG 证据（正 relevance 渲染相关度）', () => {
      const input = createTestInput({
        ragEvidence: [{ source: 'attachment', content: 'User uploaded content', relevance: 1.0 }],
      });
      const prompt = builder.build(input);
    ...
    it('relevance 为负值时不应渲染相关度标签', () => {
      const input = createTestInput({
        ragEvidence: [{ source: 'knowledge_base', content: 'RAG chunk content', relevance: -1 }],
      });
      const prompt = builder.build(input);
        ...
        toolCatalog: [
          {
            name: 'file_write',
            description: '创建或编辑文件',
            whenNotToUse: ['纯粹读取文件内容（应使用 read）', '操作二进制文件'],
            riskLevel: 'high',
          },
        ],
      });
  ...
  describe('External Guides 注入', () => {
    ...
    it('混合技能应全部注入全文', () => {
      const input = createTestInput({
        externalGuideSkills: [
          createGuideSkill('kb-guide', '知识库指南', '# 知识库详细内容'),
  ...
  describe('Checkpoint 评估 Prompt', () => {
    ...
    it('TERMINATE_SUB_AGENT 条件应包含环境/工具缺失判断', () => {
      const prompt = builder.buildCheckpointEvaluationPrompt(createMockReport(), createMockSpec());
  ...
  describe('输出格式 Footer', () => {
    ...
    it('Footer 应在 Prompt 最末尾（在所有可变内容之后）', () => {
      const input = createTestInput({
        toolCatalog: [{ name: 'exec', description: 'Shell execution' }],
        externalGuideSkills: [
          {
            name: 'test-guide',
