...
const APPROXIMATE_NOVEL_PROGRESS_MIN = 0.2;
const APPROXIMATE_SIMILARITY_MIN = 0.9;
...
export class MbEstimatedTokenCounter {
  ...
  get estimatedTokens(): number {
    return Math.ceil(this.cjkCharacterCount / 1.5) + Math.ceil(this.otherCharacterCount / 4);
  }
}
...
function findExactPeriodicSuffix(content: string): ExactCycleMatch | null {
  ...
  for (
    let repeatedChars = reversed.length;
    repeatedChars >= EXACT_MIN_REPEATED_CHARS;
    repeatedChars--
  ) {
...
export class MasterBrainReasoningGuard {
  ...
  constructor(private readonly config: MbReasoningGuardConfig) {
    this.preview = new BoundedReasoningPreview(config.previewHeadChars, config.previewTailChars);
  }
  ...
  appendReasoning(content: string, nowMs: number): MbReasoningGuardResult {
    ...
    if (this.startedAt !== null && metrics.elapsedMs >= this.config.hardDurationMs) {
      return { action: 'abort', reason: 'hard_time_fuse', metrics };
    }

    if (content && this.totalChars - this.lastExactCheckChars >= this.config.exactCheckStepChars) {
    ...
    if (
      content &&
      this.totalChars - this.lastApproximateCheckChars >= this.config.approximateCheckStepChars
    ) {
