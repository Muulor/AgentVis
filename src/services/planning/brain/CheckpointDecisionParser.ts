...
export function parseCheckpointDecision(llmOutput: string): CheckpointDecision {
  ...
  throw new Error(
    'Failed to parse checkpoint decision: no valid JSON or recognizable text signals found'
  );
}
...
function inferDecisionFromText(text: string): CheckpointDecision | null {
  ...
  if (failureSignals.some((signal) => lowerText.includes(signal))) {
    ...
    return {
      type: 'TERMINATE_SUB_AGENT',
      reason:
        'Inferred from text content: detected missing environment dependencies or execution failure, terminating Sub-Agent',
    };
  }
  ...
  if (text.length > 500) {
    ...
    return {
      type: 'TERMINATE_SUB_AGENT',
      reason:
        'Inferred from text content: the LLM returned a long analytical response, so terminate the Sub-Agent and wait for the user decision',
    };
  }
