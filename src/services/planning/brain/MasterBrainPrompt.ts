...
export class MasterBrainPrompt {
    ...
    build(input: MasterBrainInput): string {
        ...
        const characterBlock = this.buildCharacterGrounding(input.agentName, input.hasAvatar, input.workdir);
        ...
        if (input.modelId) {
            ...
            logger.trace(`[MasterBrainPrompt] 预算: total=${totalBudget}, fixed=${fixedTokens}, variable=${variableBudget}`);

            // 按比例上限截断各可变部分
            const C = PLANNING_CONSTANTS;
            toolCatalogBlock = this.truncateToolCatalog(
                input.toolCatalog, Math.floor(variableBudget * C.MASTER_BRAIN_TOOL_CATALOG_MAX_RATIO)
            );
            historyBlock = this.truncateToTokenBudget(
                historyBlock, Math.floor(variableBudget * C.MASTER_BRAIN_HISTORY_MAX_RATIO)
            );
            memoryBlock = this.truncateMemory(
                input.memory, Math.floor(variableBudget * C.MASTER_BRAIN_MEMORY_MAX_RATIO)
            );
            ragBlock = this.truncateRAG(
                input.ragEvidence, Math.floor(variableBudget * C.MASTER_BRAIN_RAG_MAX_RATIO)
            );
            artifactBlock = this.truncateToTokenBudget(
                artifactBlock, Math.floor(variableBudget * C.MASTER_BRAIN_TASK_ARTIFACT_MAX_RATIO)
            );
        }

        // ═══ MB 决策历史区块（不参与预算截断，放在 CONVERSATION_HISTORY 之后） ═══
        // 有 mbDecisionLog 时展示多轮历史，否则充底 lastMBDecision 单条向后兼容
        const mbDecisionHistoryBlock = input.mbDecisionLog && input.mbDecisionLog.length > 0
...
`;
    ...
    private buildSandboxAwarenessBlock(sandboxMode?: MasterBrainInput['sandboxMode']): string {
        if (sandboxMode === 'ControlledNetwork') {
            return `[MB_SANDBOX_AWARENESS]\n${translate('planning.masterBrain.sandboxAwareness.ControlledNetwork', {
                mode: sandboxMode,
            })}`;
        }

        if (sandboxMode === 'OfflineIsolated') {
            return `[MB_SANDBOX_AWARENESS]\n${translate('planning.masterBrain.sandboxAwareness.OfflineIsolated', {
                mode: sandboxMode,
            })}`;
        }
...
`;
     ...
     */
    private buildCharacterGrounding(agentName?: string, hasAvatar?: boolean, workdir?: string): string {
...
${workdirBlock}${workdirSnapshotBlock}${projectContextBlock}`;
    ...
    private formatUserIntent(
        ...
        if (userIntent.sentAt) {
            ...
            const formatted = `${sentDate.getFullYear()}-${pad(sentDate.getMonth() + 1)}-${pad(sentDate.getDate())} ` +
            ...
            const relativeTime = diffMin > 0
                ? `${diffMin}m ${diffSec}s ago`
                : `${diffSec}s ago`;
     ...
     */
    private formatMbDecisionHistory(
        log: import('./types').MbDecisionLogEntry[]
    ): string {
     ...
     */
    private formatLastMBDecision(
        lastMBDecision?: { rationale: string; task: string }
    ): string {
...
`;
    ...
    private normalizeFactsByCategory(memory: MemorySnapshot): MemorySnapshot['factsByCategory'] {
        ...
        const knownFactIds = new Set(
            Object.values(factsByCategory).flat().map(fact => fact.id)
        );
    ...
    private formatMemory(memory: MemorySnapshot): string {
        ...
        this.appendMemoryFactSection(
            ...
            [
                ...factsByCategory.identity_role,
                ...factsByCategory.preference_style,
            ],
        ...
        const otherFacts = memory.facts.filter(f =>
            f.category !== 'task_experience' && !renderedFactIds.has(f.id)
        );
        this.appendMemoryFactSection(
            lines,
            '**Other Facts:**',
     ...
     */
    private formatInstalledScriptSkillCatalog(
        catalog?: ExternalScriptSkillCatalogEntry[]
    ): string {
        if (!catalog || catalog.length === 0) {
            return '';
        }
        ...
        for (const skill of catalog) {
            ...
            lines.push(`- **${skill.name}**: ${skill.description}${permissions ? ` (${permissions})` : ''}`);
        }
    ...
    private formatTaskArtifacts(
        ...
        if (hasArtifacts) {
            ...
            if (interventions.length > 0) {
                lines.push(`> 🧑 **The user adjusted strategy through Human-in-the-Loop intervention (${interventions.length} time(s))**`);
                lines.push(`> The intervention information is embedded in the prior SA reasoning-process summary below. Use the timeline context to understand the user's intent.`);
                lines.push(`> ⚠️ **Important: when dispatching a new SA, include the user's adjustment instruction in the task description so the new SA follows the user's latest intent.**`);
                lines.push('');
            }
        }
        ...
        lines.push('');
        lines.push('> ⚠️ When dispatching SA again, instruct the new SA in nextStep.task to use the existing results and not repeat the operations above.');
     ...
     * 避免盲目重新派遣已完成的任务阶段。
     */
    private formatWorkdirSnapshot(
        snapshot?: import('./types').WorkdirSnapshot
    ): string {
        ...
        // 按文件数量降序排列扩展名统计
        const sortedExts = Object.entries(snapshot.byExtension)
            .sort(([, a], [, b]) => b - a);
        if (sortedExts.length > 0) {
            const extSummary = sortedExts
        ...
        if (snapshot.scanTruncated) {
            lines.push('> Scan note: this is a partial snapshot; large dependency/cache/build directories may have been skipped or the scan budget may have stopped traversal early.');
        }
        lines.push('> System snapshot: use this to judge SA execution progress and avoid redispatching completed task stages.');
...
`;
    ...
    private formatConversationHistory(
        ...
        for (const msg of history) {
            ...
            const timeLabel = (msg.role === 'user' && msg.timestamp)
                ? `[${formatTimestamp(msg.timestamp)}] `
                : '';
            // 单条消息超长时截断
            const content = msg.content.length > maxChars
                ? msg.content.slice(0, maxChars) + `... (truncated, original ${msg.content.length} characters)`
                : msg.content;
    ...
    private calculateTotalBudget(modelId: string): number {
        const totalTokens = (MODEL_CONTEXT_WINDOWS[modelId]
        ...
        for (let keepCount = sortedFacts.length; keepCount >= 0; keepCount--) {
            ...
            if (this.estimateTokens(text) <= maxTokens) {
                if (keepCount < memory.facts.length) {
                    logger.trace(`[MasterBrainPrompt] memory Level 3: 保留 ${keepCount}/${memory.facts.length} facts`);
                }
                return text;
            }
        }
        ...
        for (let count = sorted.length; count >= 1; count--) {
            ...
            if (this.estimateTokens(text) <= maxTokens) {
                if (count < evidence.length) {
                    logger.trace(`[MasterBrainPrompt] RAG: 保留 ${count}/${evidence.length} 条 (按 relevance)`);
                }
                return text;
            }
        }
    ...
    buildCheckpointEvaluationPrompt(
        ...
        const preExecutionWarning = report.checkpointTrigger === 'high_risk_pre_execution'
            ? `
...
`
        ...
        const budgetNearExhaustionWarning = report.checkpointTrigger === 'budget_near_exhaustion'
            ? `
