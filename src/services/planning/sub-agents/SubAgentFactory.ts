...
export class SubAgentFactory {
  ...
  constructor(promptBuilder?: SubAgentPromptBuilder, riskGuard?: ToolRiskGuard) {
  ...
  create(
    ...
    if (toolValidation.unknownTools.length > 0) {
      // 未注册工具仅输出警告日志，不阻止创建
      logger.warn(`[SubAgentFactory] 未注册工具: ${toolValidation.unknownTools.join(', ')}`);
    }
    if (toolValidation.highRiskTools.length > 0) {
      logger.trace(`[SubAgentFactory] 高风险工具: ${toolValidation.highRiskTools.join(', ')}`);
    }
  ...
  private resolveVenvPythonPath(): string | undefined {
    try {
      ...
      const pythonExe = isWindows ? `${venvPath}\\Scripts\\python.exe` : `${venvPath}/bin/python`;
