...
function buildObservationToolTarget(value: string): ObservationToolTarget {
    ...
    return fullTarget && fullTarget !== target
        ? { target, fullTarget }
        : { target };
}
...
function formatLocalSearchTarget(args: Record<string, unknown>, compact = true): string {
    ...
    switch (mode) {
        ...
        default: {
            const fallback = getFirstToolArg(args, 'query', 'pattern', 'path', 'searchPath', 'symbolName');
...
function createSyntheticToolCallId(toolName: string, runSequence: number, step: number, index: number): string {
...
export interface LLMResponse {
    ...
    rawToolCalls?: Array<{ name: string; args: Record<string, unknown>; id?: string; thoughtSignature?: string }>;
    /** 是否需要用户交互（如 Diff 确认） */
 ...
 */
function extractToolTarget(toolName: string, args: Record<string, unknown>, defaultWorkdir?: string): ObservationToolTarget {
    switch (toolName) {
        case 'read':
        case 'file_write':
        ...
        case 'external_skill_execute':
            return buildObservationToolTarget(getFirstToolArg(args, 'skillName', 'skill_name') || toolName);
        default:
...
export class SubAgentRunner {
     ...
     */
    private compactExecutedFileWriteToolCallArgs(messages: LoopMessage[], toolCallId: string | undefined): void {
        ...
        for (let i = messages.length - 1; i >= 0; i--) {
            ...
            logger.info(
                '[SubAgentRunner] Compacted large file_write tool args after execution',
                {
                    toolCallId,
    ...
    private emitObservation(
        ...
        const eventWithRunId: SubAgentObservationEvent = this.currentObservationRunId && !event.runId
        ...
        if (shouldCollect) {
            ...
            const trimmedThinking = cleanThinking.length > MAX_THINKING_CHARS
            ...
            if (toolCallId) {
                const existingIndex = this.collectedObservationEvents.findIndex(
                    item => item.toolAction?.toolCallId === toolCallId && item.runId === collectedEvent.runId
                );
                if (existingIndex >= 0) {
                    ...
                    if (previous) {
                        this.collectedObservationEvents[existingIndex] = {
                            ...
                            toolAction: previous.toolAction && collectedEvent.toolAction ? {
                                ...previous.toolAction,
    ...
    async runWithDynamicLoop(
         ...
         */
        pairedHistoryMessages?: Array<{ role: 'user' | 'assistant'; content: string; images?: Array<{ mimeType: string; data: string }> }>
    ...
    private async runAtomicEventLoop(
        ...
        if (imageAttachments?.length) {
            logger.trace(`[SubAgentRunner] 📷 已注入 ${imageAttachments.length} 张图片到 SA 首条 user 消息`);
        }
        ...
        if (pairedHistoryMessages?.length) {
            logger.trace(`[SubAgentRunner] 🔗 已插入 ${pairedHistoryMessages.length} 条配对历史消息到 SA messages[]（initialUserMessage 之前）`);
            logger.trace('[SubAgentRunner] SA messages[] history prefix:', pairedHistoryMessages.map((message, index) => ({
                index,
                role: message.role,
        ...
        while (!terminated && toolCallSteps < maxSteps && totalToolCalls < PLANNING_CONSTANTS.TOOL_CALLS_HARD_LIMIT) {
            ...
            if (this.contextId && useHitlStore.getState().isPaused(this.contextId)) {
                logger.debug(`[SubAgentRunner] ⏸ 检测到 HITL 暂停请求 (contextId: ${this.contextId})，等待用户介入...`);
                try {
                    const userMessage = await useHitlStore.getState().waitForResume(
                        this.contextId,
                    ...
                    if (userMessage) {
                        ...
                        logger.debug(`[SubAgentRunner] 📌 用户介入消息已永久追加到 messages[] (第 ${stepCount} 步)`);
                        ...
                        if (this.artifactStore) {
                            ...
                            logger.debug(`[SubAgentRunner] 📝 用户介入消息已写入 Artifact Store (Step ${stepCount})`);
                        }
            ...
            const shouldTriggerBudgetCheckpoint =
                toolCallSteps > 0 &&
                remainingStepsBeforeDecision <= PLANNING_CONSTANTS.SUB_AGENT_BUDGET_CHECKPOINT_REMAINING_STEPS &&
            ...
            if (shouldTriggerBudgetCheckpoint) {
                ...
                try {
                    const report = this.buildProgressReportFromMessages(
                        spec.role, messages, totalToolCalls, config, stepCount,
                        undefined, 'budget_near_exhaustion',
                        undefined, toolCallSteps, maxSteps
                    );
            ...
            logger.debug(`[SubAgentRunner] 📍 Step ${stepCount}: 调用 LLM (消息数: ${messages.length}, 预估tokens: ${preCallTokens})`);

            // 预算阈值信号注入（85% 和 95%）
            // 使用离散信号而非连续数值
            // 直接用步数比例计算——步数即主预算，无需折算
            const budgetRatio = maxSteps > 0 ? toolCallSteps / maxSteps : 0;
            if (budgetRatio >= PLANNING_CONSTANTS.SUB_AGENT_BUDGET_CRITICAL_RATIO && !budgetCriticalInjected) {
            ...
            } else if (budgetRatio >= PLANNING_CONSTANTS.SUB_AGENT_BUDGET_WARNING_RATIO && !budgetWarningInjected) {
            ...
            if (!contextResetInProgress) {
                ...
                const resetThreshold = this.getContextWindowSize()
                    * PLANNING_CONSTANTS.SUB_AGENT_CONTEXT_RESET_RATIO;
                const remainingSteps = maxSteps - toolCallSteps;

                if (rawTokens > resetThreshold
            ...
            const { messages: managedMessages } =
                this.compressHistoricalToolOutputs(messages, compressor, stepCount);
            ...
            try {
                ...
                const tokenContextId = this.contextId
                ...
                if (tokenContextId) {
                    ...
                    statusState.setContextPressure(
                        tokenContextId,
                        inputTokens,
            ...
            const responseType = response.error ? 'error' : (response.rawToolCalls?.length ? 'tool_use' : 'text');
            const responseTokens = this.estimateMessageTokens([{ role: 'assistant', content: response.content || '' }]);
            ...
            logger.debug(`[SubAgentRunner] 📊 Token 用量 - 本轮发送: ${preCallTokens}, 响应: ${responseTokens}, 累计: ${postCallTokens} (Step ${stepCount}, 工具步: ${toolCallSteps}/${maxSteps})`);
            ...
            if (response.error) {
                ...
                if (retryClassification.shouldRetry) {
                    ...
                    if (consecutiveApiErrors <= maxRetries) {
                        const waitMs = getLlmRetryDelayMs(
                            consecutiveApiErrors,
                            SUB_AGENT_LLM_RETRY_DELAYS_MS
                        );
                        logger.debug(
            ...
            if (isEmptyTextDecision) {
                ...
                if (consecutiveEmptyResponses <= PLANNING_CONSTANTS.SUB_AGENT_EMPTY_RESPONSE_RETRY_LIMIT) {
                    const emptyResponseRetryInstruction = translate('chat.subAgentEmptyDecisionRetryInstruction');
                    additionalInstructions = additionalInstructions
            ...
            if (response.rawToolCalls && response.rawToolCalls.length > 0) {
                // 有工具调用：执行工具
                logger.debug(`[SubAgentRunner] 🔧 工具调用: ${response.rawToolCalls.map(t => t.name).join(', ')}`);
                consecutiveTextOnlyResponses = 0;
                ...
                const toolCallsThisStep = response.rawToolCalls.length > maxToolsPerStep
                ...
                if (response.rawToolCalls.length > maxToolsPerStep) {
                    logger.debug(
                        ...
                        `(丢弃: ${response.rawToolCalls.slice(maxToolsPerStep).map(tc => tc.name).join(', ')})`
                    );
                }
                ...
                const hasHighRiskTool = response.rawToolCalls.some(
                    tc => {
                ...
                const allApproved = pendingToolNamesForCheck.every(name => approvedHighRiskTools.has(name));
                if (hasHighRiskTool && totalToolCalls > 0 && !allApproved) {
                    ...
                    try {
                        ...
                        const pendingActionDetails = response.rawToolCalls
                            .map(tc => {
                                const argsPreview = tc.name === 'exec'
                        ...
                        const report = this.buildProgressReportFromMessages(
                            spec.role, messages, totalToolCalls, config, stepCount,
                            pendingToolNames, 'high_risk_pre_execution', pendingActionDetails,
                        ...
                        if (decision.type === 'TERMINATE_SUB_AGENT') {
                            ...
                            const isTaskComplete = reasonLower.includes('complete') ||
                                reasonLower.includes('fulfill') ||
                                reasonLower.includes('success') ||
                                reasonLower.includes('accomplish');
                            terminationReason = isTaskComplete ? 'task_completed_by_master' : 'high_risk_rejected';
                            logger.debug(`[SubAgentRunner] 🛑 Master Brain 终止: ${terminationReason} - ${decision.reason}`);
                            break;
                        }
                        ...
                        logger.debug(`[SubAgentRunner] ✅ Master Brain 批准高风险操作，继续执行 (已批准工具: ${Array.from(approvedHighRiskTools).join(', ')})`);
                    } catch (e) {
                ...
                for (let tcIndex = 0; tcIndex < toolCallsWithIds.length; tcIndex++) {
                    ...
                    if (tc.name === 'file_write' && isMissingFullFileWriteContent(tc.args)) {
                        const path = getFirstToolArg(tc.args, 'path', 'file_path', 'filePath') || '(missing path)';
                        ...
                        const missingContent = translate('chat.subAgentFileWriteMissingContentCorrection', { path });
                        messages.push({
                            role: 'tool',
                    ...
                    if (result.data && typeof result.data.type === 'string' &&
                        ...
                        logger.trace(`[SubAgentRunner]   📄 收集到 Diff 数据: ${result.data.type} → ${String(result.data.filePath)}`);
                    ...
                    if (result.images && result.images.length > 0) {
                        logger.trace('[SubAgentRunner] 📷 images 透传到消息:', result.images.length, '张, 首张 MIME:', result.images[0]?.mimeType, 'data 长度:', result.images[0]?.data.length);
                    }
                    ...
                    this.storeToolResultAsArtifact(
                        tc.name, tc.args, toolResultContent, spec.role
                    );
                    ...
                    if (signal?.aborted) {
                        logger.debug('[SubAgentRunner] ⛔ 工具执行完毕后检测到中断信号，已保留工具结果并停止执行');
                        terminated = true;
                        terminationReason = 'cancelled';
                        break;
                    }
                    ...
                    if (result.success) {
                        ...
                        if (tc.name === 'file_write') {
                            if (result.content.includes('File content is unchanged')) {
                                ...
                                logger.debug(`[SubAgentRunner]   ⚠️ file_write 无变化 (${consecutiveNoChangeWrites}/${MAX_CONSECUTIVE_NO_CHANGE})`);
                            } else {
                        ...
                        if (tc.name === 'exec') {
                            ...
                            if (execSignature && execSignature === lastExecSignature) {
                                ...
                                logger.debug(`[SubAgentRunner]   ⚠️ exec 相同命令 (${consecutiveIdenticalExecs}/${MAX_CONSECUTIVE_IDENTICAL_EXECS})`);
                                if (consecutiveIdenticalExecs >= MAX_CONSECUTIVE_IDENTICAL_EXECS) {
                                    logger.debug(`[SubAgentRunner] ⚠️ 连续 ${consecutiveIdenticalExecs} 次相同 exec 命令，强制终止`);
                                    terminated = true;
                ...
                if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                    ...
                    try {
                        ...
                        const report = this.buildProgressReportFromMessages(
                            spec.role, messages, totalToolCalls, config, stepCount,
                            pendingToolNames, 'consecutive_failures',
                            undefined, toolCallSteps, maxSteps
                        );
                        const decision = await onCheckpoint(report, spec);
                        ...
                        if (decision.type === 'ADJUST_STRATEGY') {
                            ...
                            logger.debug(`[SubAgentRunner] 🎯 策略调整（含进度摘要）: ${decision.refinedInstructions.slice(0, 80)}...`);
                ...
                if (consecutiveNoChangeWrites >= MAX_CONSECUTIVE_NO_CHANGE && !terminated) {
                    logger.debug(`[SubAgentRunner] ⚠️ 连续 ${consecutiveNoChangeWrites} 次 file_write 无变化，强制终止`);
                    terminated = true;
                    ...
                    try {
                        ...
                        switch (decision.type) {
                            case 'TERMINATE_SUB_AGENT':
                                ...
                                logger.debug(`[SubAgentRunner] 🛑 Master Brain 决策：终止执行 - ${decision.reason}`);
                                break;
                            ...
                            case 'ADJUST_STRATEGY': {
                                ...
                                logger.debug(`[SubAgentRunner] 🎯 Master Brain 决策：调整策略（含进度摘要） - ${decision.refinedInstructions.slice(0, 50)}...`);
                ...
                if (contextResetInProgress
                    && response.content.includes(CONTEXT_SUMMARY_MARKER)) {
                    // 提取摘要内容（标记之后的全部文字）
                    const summaryContent = response.content
                        .split(CONTEXT_SUMMARY_MARKER)[1]?.trim() ?? response.content;
        ...
        logger.debug(`[SubAgentRunner] 🏁 原子循环结束: ${terminationReason ?? 'unknown'} (步骤: ${stepCount}, 工具步: ${toolCallSteps}/${maxSteps}, 工具调用总计: ${totalToolCalls}, 最终消息Tokens: ${finalTokens})`);
        ...
        return this.buildAtomicLoopOutput(
            lastContent, allToolCallNames, terminationReason, requiresInteraction,
            collectedDiffData, messages, this.collectedObservationEvents
        );
    }
    ...
    private compressHistoricalToolOutputs(
        ...
        const pressureThreshold = this.getContextWindowSize() * PLANNING_CONSTANTS.SUB_AGENT_TOKEN_PRESSURE_RATIO;
        ...
        const compressedMessages: LoopMessage[] = messages.map((msg, index) => {
            ...
            if (msg.role === 'assistant' && msg.toolCalls && msg.toolCalls.length > 0) {
                const mappedToolCalls = msg.toolCalls.map(tc => {
                    ...
                    if (!isUnderPressure && contentTokens < FILE_WRITE_ARGS_COMPRESS_TOKEN_THRESHOLD) return { toolCall: tc, compressed: false };

                    // 确认后续 tool 结果为成功（已执行的 file_write）
                    const followingToolMsg = messages.slice(index + 1)
                        .find(m => m.role === 'tool' && m.toolName === 'file_write'
                ...
                if (argsCompressed) {
                    ...
                    const originalTcTokens = msg.toolCalls.reduce((sum, tc) =>
                        sum + this.estimateTextTokens(JSON.stringify(tc.args)), 0);
                    const newTcTokens = compressedToolCalls.reduce((sum, tc) =>
                        sum + this.estimateTextTokens(JSON.stringify(tc.args)), 0);
                    totalCompressed++;
            ...
            if (isUnderPressure
                ...
                const nextAssistant = nearbyMessages
                ...
                if (hasAnalysis) {
                    ...
                    const savedTokens = originalTokens - this.estimateMessageTokens([{ ...msg, content: newContent }]);
                    if (savedTokens > 0) {
                        totalCompressed++;
    ...
    private extractSourceFromToolContent(content: string, toolName: string): string {
        ...
        if (toolName === 'read' || toolName === 'file_read' || toolName === 'file_write') {
            ...
            const pathMatch = content.match(/(?:文件路径|path|file)[:：]\s*(.+)/i)
                ?? content.match(/(?:\/|\\|[A-Z]:).+\.\w+/);
    ...
    private buildProgressReportFromMessages(
        ...
        if (recentToolMessages.length > 0) {
            ...
            const recentDetails = recentToolMessages.map(m => {
        ...
        if (lastAssistant?.content) {
            ...
            const reasoning = lastAssistant.content.length > maxReasoningChars
        ...
        if (checkpointTrigger === 'high_risk_pre_execution' && pendingToolNames?.length) {
            ...
            observations += '⛔ The task objective CANNOT be considered complete until this operation is actually executed and verified.';
        ...
        let confidenceLevel = toolMessages.length > 0
        ...
        const needsMoreIterations = checkpointTrigger === 'high_risk_pre_execution'
        ...
        return {
            ...
            requestedAdditionalBudget: needsMoreIterations
                ...
                : undefined,
            checkpointTrigger,
            pendingHighRiskAction: checkpointTrigger === 'high_risk_pre_execution'
    ...
    private buildAtomicLoopOutput(
        ...
        const observations = content
            || (messages ? this.buildExecutionSummary(messages, terminationReason) : translate('chat.subAgentTaskExecutionCompleted'));
    ...
    private buildExecutionSummary(messages: LoopMessage[], terminationReason?: string): string {
        ...
        lines.push(translate('chat.subAgentTerminationReasonLine', {
            reason: this.formatTerminationReason(terminationReason),
        }));
        lines.push(translate('chat.subAgentToolCallCountLine', {
        ...
        if (assistantAnalyses.length > 0) {
            ...
            for (const analysis of recentAnalyses) {
                const truncated = analysis.length > 800
    ...
    private extractToolResultForSummary(
        toolName: string, content: string, isSuccess: boolean
    ): string {
        ...
        if (toolName === 'web_search' && isSuccess) {
            ...
            return cleanContent.length > 1500
                ? cleanContent.slice(0, 1500) + '...'
                : cleanContent;
        }

        if ((toolName === 'read' || toolName === 'file_read') && isSuccess) {
            // 文件内容保留路径和关键片段
            return cleanContent.length > 800
    ...
    private storeToolResultAsArtifact(
        ...
        // 截断内容到配置的最大长度
        const content = resultContent.length > config.maxChars
    ...
    private formatTerminationReason(reason?: string): string {
        ...
        return reasonMap[reason ?? ''] ?? reason ?? translate('chat.subAgentTerminationCompletedNormally');
    }

    private checkTerminationSignal(
        content: string,
        config: SubAgentLoopConfig
    ): boolean {
        return config.terminationPatterns.some((pattern) =>
    ...
    private executeContextReset(
        ...
        const sourceLabel = source === 'sa_summary'
    ...
    private buildMechanicalSummary(messages: LoopMessage[]): string {
        ...
        if (assistantAnalyses.length > 0) {
            ...
            for (const analysis of recentAnalyses) {
                // 每条限制 500 字符，避免摘要过长
                const truncated = analysis.length > 500
