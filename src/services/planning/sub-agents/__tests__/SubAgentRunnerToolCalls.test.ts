...
function generateLargeText(marker: string, lineCount = 700): string {
    return Array.from({ length: lineCount }, (_, index) =>
        `path: /src/large-result.ts line ${index}: ${marker} ${'x'.repeat(100)}`
    ).join('\n');
}
...
describe('SubAgentRunner - Tool Calls Summary', () => {
    it('retries an empty text decision with stricter guidance', async () => {
        ...
        const mockLLMCaller: LLMCaller = {
            callWithContext: vi.fn().mockImplementation(async (
    ...
    it('warns after consecutive text-only decisions and terminates if the next decision still has no tool call', async () => {
        ...
        expect(output.observedEffects).toContain(translate('chat.subAgentObservedEffects', {
            count: 0,
            tools: '',
        }));
    });
    ...
    it('stops immediately when identical exec commands repeat in the same workdir', async () => {
        ...
        expect(output.observedEffects).toBe(translate('chat.subAgentObservedEffects', {
            count: 3,
            ...
            callWithContext: vi.fn().mockImplementation(async (
                ...
                if (callCount === 1) {
                    return {
                        ...
                        rawToolCalls: [{
                            name: 'file_write',
                            args: { path: '/test.md', content: '# Very Long Content That Should Be Visible' }
                        }],
...
describe('SubAgentRunner - Live Tool Observations', () => {
    ...
    it('persists a completed tool result before honoring an abort signal', async () => {
        ...
        expect(mockToolExecutor).toHaveBeenCalledTimes(1);
        expect(artifactStore.getAll()[0]?.content).toContain('The command returned after user cancellation');
        expect(output.observationEvents?.[0]?.toolAction?.success).toBe(false);
    ...
    it('keeps a stable synthetic tool call id when the provider omits one', async () => {
        ...
        const secondCallContext = capturedContexts[1];
        const assistantMessage = secondCallContext?.find(message =>
            message.role === 'assistant' &&
            message.toolCalls?.some(toolCall => toolCall.name === 'generate_image')
        );
        const toolMessage = secondCallContext?.find(message =>
            message.role === 'tool' &&
    ...
    it('emits explicit exec timeout seconds in live tool observations', async () => {
        ...
        const runner = new SubAgentRunner(mockLLMCaller);
        runner.setToolExecutor(vi.fn().mockResolvedValue({
            success: true,
            content: 'ok',
        }));
                        ...
                        rawToolCalls: [
                            {
                                id: 'grep-call',
                                name: 'local_search',
                                args: { mode: 'grep', query: 'function handleSubmit', searchPath: 'src/components' },
                            },
                            {
                                id: 'find-call',
                                name: 'local_search',
    ...
    it('should compress old tool results and large file_write args before the next LLM call', async () => {
        ...
        const mockLLMCaller: LLMCaller = {
            callWithContext: vi.fn().mockImplementation(async (
                ...
                if (callCount === 2) {
                    return {
                        content: 'I analyzed the large read output and captured the relevant conclusion for later steps.',
                        output: {},
                        rawToolCalls: [
                            {
                                name: 'read',
        ...
        const compressedAssistant = thirdCallContext.find(message =>
            message.role === 'assistant' &&
        ...
        const fileWriteCall = compressedAssistant.toolCalls.find(toolCall => toolCall.name === 'file_write');
        ...
        const compressedReadResult = thirdCallContext.find(message =>
            message.role === 'tool' &&
