...
function generateReadOutput(lines: number): string {
  return Array.from(
    { length: lines },
    (_, i) => `${i + 1}: const variable${i} = "value${i}";`
  ).join('\n');
}
...
describe('ToolOutputCompressor Level 3 (元信息摘要)', () => {
  ...
  it('read 工具元信息包含文件路径和行数', () => {
    ...
    expect(result.content).toContain(translate('chat.subAgentToolOutputReadStatusSucceeded'));
  });
  ...
  it('通用工具元信息格式正确', () => {
    ...
    expect(result.content).toContain(translate('chat.subAgentToolOutputExecutionSucceeded'));
  });
  ...
  it('失败的工具输出标记为 ❌', () => {
    ...
    expect(result.content).toContain('❌');
    // 失败输出包含执行失败标记
    expect(result.content).toContain(translate('chat.subAgentToolOutputReadStatusFailed'));
  });
});
