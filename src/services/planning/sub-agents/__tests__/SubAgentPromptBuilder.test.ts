...
describe('SubAgentPromptBuilder', () => {
  ...
  describe('build', () => {
    ...
    it('should preserve the language hint resolved from the original user request', () => {
      const spec = createMockSpec({
        ...
        outputLanguageHint: resolveOutputLanguage('Please provide the deliverable in French.', {
    ...
    it('ControlledNetwork sandbox mode should inject sandbox runtime context', () => {
      ...
      const prompt = builder.build(
        spec,
        createMockContext({ sandboxMode: 'ControlledNetwork' }),
        []
      );
    ...
    it('应排除 artifactSnapshot 避免 JSON 二次序列化', () => {
      ...
      const context = createMockContext({
        data: {
          artifactSnapshot: {
            index: [
              { key: 'read_0', toolName: 'read', sourceHint: 'test.txt', estimatedTokens: 10 },
            ],
            artifacts: [
              {
                key: 'read_0',
                content: '不应出现在JSON上下文中的内容',
                toolName: 'read',
                sourceHint: 'test.txt',
              },
            ],
  ...
  describe('External Guide Section', () => {
    ...
    it('共享技能应注入全文到 Sub-Agent', () => {
      const spec = createMockSpec();
      const guides = [
        createGuideSkill('shared-guide', '共享指南', '# Shared Guide\n\nAgent 和 MB 共享的内容。'),
      ];
    ...
    it('所有技能都应注入 Sub-Agent（不再过滤）', () => {
      const spec = createMockSpec();
      const guides = [createGuideSkill('master-only', '仅 MB 使用')];
    ...
    it('should include uninjected guide and script skills in the installed external skill catalog', () => {
      ...
      const prompt = builder.build(spec, createMockContext(), [], {}, guides, scriptSkills, [
        'selected-guide',
        'guide-only',
        'script-only',
        'selected-script',
  ...
  describe('External Script Skill Section', () => {
    it('应注入 Script Skill compact contract card', () => {
      ...
      const prompt = builder.build(spec, createMockContext(), [], {}, undefined, scriptSkills);
  ...
  describe('Venv 环境约束注入', () => {
    it('提供 venvPythonPath 时应注入 Python 环境约束段', () => {
      ...
      const prompt = builder.build(spec, createMockContext(), [], { venvPythonPath: venvPath });
    ...
    it('venvPythonPath 为 undefined 时不应注入约束段', () => {
      ...
      const prompt = builder.build(spec, createMockContext(), [], { venvPythonPath: undefined });
  ...
  describe('历史任务经验注入', () => {
    ...
    it('taskExperiences 不应出现在 sanitizedContext JSON 块中', () => {
      ...
      const context = createMockContext({
        data: {
          taskExperiences: [{ content: '不应出现在JSON中的经验' }],
          normalData: '正常数据',
        },
      });
