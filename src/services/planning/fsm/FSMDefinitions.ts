 ...
 * 解析 YAML 状态定义
 */
const parseState = <S extends string>(yamlState: YAMLState): StateConfig<S, FSMEvent> => {
...
export const parseFSMDefinition = <S extends string>(
  ...
  const states: Record<S, StateConfig<S, FSMEvent>> = {} as Record<S, StateConfig<S, FSMEvent>>;
 ...
 * 硬编码版本，不依赖外部 YAML 文件
 */
export const createAgentServiceFSMDefinition = (): FSMDefinition<AgentServiceState, FSMEvent> => {
 ...
 */
export const createSubAgentFSMDefinition = (): FSMDefinition<SubAgentState, FSMEvent> => ({
