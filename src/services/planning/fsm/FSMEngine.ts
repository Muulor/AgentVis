...
import type { FSMDefinition, FSMContext, FSMTransition, FSMTraceEntry, IFSMEngine } from './types';
 ...
 * @template E - 事件类型
 */
export class FSMEngine<S extends string, E extends { type: string }> implements IFSMEngine<S, E> {
  /** 当前状态 */
  private state: S;
  ...
  async step(): Promise<void> {
    ...
    if (!transition) {
      // 无匹配转移，记录警告但不报错
      logger.warn(`[FSM] 无匹配转移: 状态=${this.state}, 事件=${event.type}`);
      return;
    }
   ...
   * 查找匹配的转移
   */
  private findTransition(state: S, event: E): FSMTransition<S, E> | undefined {
