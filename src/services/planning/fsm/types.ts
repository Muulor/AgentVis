 ...
 */
export type MasterBrainDecisionType = 'SPAWN_SUB_AGENT' | 'REQUEST_MORE_INPUT' | 'RESPOND_TO_USER';
 ...
 * 用于条件判断，必须是纯函数（无副作用）
 */
export type GuardFn<E extends { type: string }> = (context: FSMContext, event: E) => boolean;
...
export interface FSMTraceEntry<
  ...
  E extends { type: string } = { type: string },
> {
  /** 时间戳 */
  timestamp: Date;
  /** 迭代次数 */
