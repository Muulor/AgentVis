...
describe('VisualEnhancerPrompt invariants', () => {
  ...
  it('preserves required visual fence names', () => {
    ...
    const requiredFences = ['echarts', 'mermaid', 'widget-choices', 'widget-chart', 'widget-tree'];
  ...
  it('forbids adding unsupported data during enhancement', () => {
    ...
    const userPrompt = buildVisualEnhancerUserPrompt(
      'The report mentions a market map but includes no numeric data.'
    );

    expect(systemPrompt).toContain(
      'Do not introduce facts, entities, rankings, dates, metrics, percentages, estimates, or numeric values'
    );
    expect(systemPrompt).toContain(
  ...
  it('requires a single presentation for visually represented facts', () => {
    ...
    expect(prompt).toContain(
      'Use exactly one primary presentation for each fact, metric, or dataset.'
    );
    expect(prompt).toContain(
      'do not repeat that same label-value pair in another chart, table, list, or adjacent prose.'
    );
