...
const EXISTING_VISUAL_PATTERNS = [/```widget(?:-[\w]+)?/, /```echarts/, /```mermaid/];
...
const QUANTIFIED_VALUE_PATTERN =
  /\d+(?:\.\d+)?\s*(?:[\u4e07\u4ebf\u5343\u767e]|k|m|b|bn|thousand|million|billion|trillion|users?|records?|items?|rows?|columns?|steps?|tasks?|points?|metrics?)/i;
...
export async function enhance(
  ...
  try {
    ...
    if (enhancedContent.length < response.length * MIN_ENHANCED_LENGTH_RATIO) {
      logger.warn('[VisualEnhancer] 增强结果过短，降级使用原始内容', {
    ...
    logger.trace('[VisualEnhancer] ✨ 增强成功', {
      original: response.length,
      enhanced: enhancedContent.length,
    });
...
async function collectStreamResponse(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const scheduleStreamDelta = (content: string) => {
      ...
      streamFlushTimer = setTimeout(flushStreamDelta, VISUAL_ENHANCER_STREAM_UI_FLUSH_INTERVAL_MS);
    };
