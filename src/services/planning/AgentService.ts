...
export class AgentService {
    ...
    async processMessage(
        ...
        try {
            ...
            const callbacks: AgentLoopCallbacks = {
                ...
                onThought: options.onThought ? (step) => {
                    options.onThought?.({
                        phase: step.phase,
            ...
            // 保存引用（用于取消）
            if (this.activeRunId !== runId) {
                return this.createCancelledResult('Processing cancelled before agent loop ownership was assigned');
            }
                ...
                try {
                    ...
                    for (const imgPath of generatedImagePaths.slice(-1)) {
                        ...
                        try {
                            ...
                            logger.trace('[AgentService] 生成图历史注入已压缩:',
                                fileName,
                            ...
                            logger.warn('[AgentService] 生成图历史注入压缩失败，回退原图 base64:', compressionError);
                        }
                    }
                    ...
                    for (let i = sessionMessages.length - 1; i >= 0; i--) {
                        ...
                        if (msg?.role === 'assistant') {
                            ...
                            logger.trace('[AgentService] 🖼️ 已将', imageDataList.length,
    ...
    private async loadRuntimeContext(
        ...
        try {
            ...
            const isEmbeddingRelated = errorMsg.includes('Embedding API') ||
        ...
        if (this.config.enableRag !== false) {
            ...
            try {
                ...
                const ragResults = await ragService.retrieveAndFormat(
                    this.config.agentId,
                    userQuery,
                    { topK: 5 }
                );
    ...
    if (cached) {
        ...
        if (configChanged) {
            ...
            logger.trace('[AgentService]  检测到配置变化，重建服务:', {
                cacheKey,
                old: { provider: cachedConfig.providerId, model: cachedConfig.modelId, hasMbRules: !!cachedConfig.mbAgentRules, hasSaRules: !!cachedConfig.saAgentRules },
                new: { provider: config.providerId, model: config.modelId, hasMbRules: !!config.mbAgentRules, hasSaRules: !!config.saAgentRules },
            });
            // 销毁旧服务
            cached.resetSession();
