...
export class MasterBrainInputBuilder {
  ...
  async build(
    ...
    if (this.dependencies.getExternalGuideSkills) {
      try {
        externalGuideSkills = await this.dependencies.getExternalGuideSkills(userIntent.explicit);
    ...
    if (this.dependencies.getExternalScriptSkills) {
      try {
        externalScriptSkills = await this.dependencies.getExternalScriptSkills(userIntent.explicit);
    ...
    const taskArtifactIndex =
      this.artifactStore && !this.artifactStore.isEmpty()
        ? this.artifactStore.getIndex()
        : undefined;

    // 已安装技能目录（静态全量，仅 name + description）
    const installedSkillCatalog = this.dependencies.getInstalledSkillCatalog?.() ?? undefined;
    const installedScriptSkillCatalog =
      this.dependencies.getInstalledScriptSkillCatalog?.() ?? undefined;
    ...
    const taskArtifactObservations =
      this.saObservationsSummaries.length > 0 ? this.saObservationsSummaries : undefined;
    ...
    if (hasExecutedSA) {
      logger.trace(
        '[MasterBrainInputBuilder] 🔄 检测到 SA 已完成，[USER_INTENT] 将在 Round 2+ 脱敏渲染'
      );
    }
