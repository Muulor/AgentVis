...
function asFlexibleDecision(
  decision: MasterBrainDecision
): MasterBrainDecision & FlexibleDecisionFields {
...
export class SubAgentSpecBuilder {
  ...
  buildFromNextStep(
    ...
    // 提取任务描述
    const task = asString(nextStep.task) ?? asString(nextStep.description) ?? decision.rationale;
   ...
   */
  private ensureGuideSkillTools(tools: string[], guideSkills?: ExternalGuideSkillInfo[]): void {
   ...
   */
  private ensureScriptSkillTool(tools: string[], scriptSkills?: ExternalScriptSkillInfo[]): void {
