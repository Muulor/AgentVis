...
export function classifyAgentLoopError(error: unknown): AgentLoopErrorClassification {
  ...
  if (
    errorName === 'mbemptydecisioncontenterror' ||
...
export function getAgentLoopErrorKindForTerminationReason(
  reason?: string
): AgentLoopErrorKind | null {
  switch (reason) {
    case 'cancelled':
      return 'manual_stop';
    case 'api_error':
...
export function formatAgentLoopFailureMessage(
  ...
  const details =
    options.details ?? (options.includeRawDetail === false ? '' : classification.rawMessage);
