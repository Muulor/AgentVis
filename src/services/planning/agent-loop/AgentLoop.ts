...
interface AgentLoopLLMMessage { role: string; content: string; images?: unknown }
...
export class AgentLoop {
    ...
    private initializeFSM(): void {
        ...
        const dependencies: FSMIntegrationDependencies = {
            ...
            getInstalledScriptSkillCatalog: (): ExternalScriptSkillCatalogEntry[] => {
                ...
                return this.getEnabledExternalScriptSkills()
                    .map(s => ({
                        name: s.name,
                        description: s.description,
    ...
    private async ensureSkillRetriever(): Promise<SkillRetriever | null> {
        ...
        this.skillRetrieverInitPromise ??= (async () => {
                try {
                    const { createSkillRetriever } = await import(
                        '../skills/external/SkillRetriever'
                    );
                    ...
                    if (retriever.isEmbeddingDegraded()) {
                        ...
                        logger.warn('[AgentLoop] SkillRetriever 处于 embedding 降级状态（仅 L1 关键词可用），下次请求将重试 embedding 初始化');
                ...
                try {
                    ...
                    if (options?.skipSessionMessages) {
                        ...
                        const response = await callMasterBrainWithRetry(
                            ...
                            () => invoke<LLMResponseWithTools>('llm_chat_with_tools', {
                                request: {
                                    ...
                                    providerId: this.config.providerId,
                                    baseUrl: this.config.baseUrl,
                                    supportsVision: modelSupportsVision(this.config.modelId ?? '', this.config.providerId),
                        ...
                        if (response.type === 'text' && response.content) {
                            logger.trace('[MasterBrain] Checkpoint LLM 响应:', response.content.substring(0, 200));
                    ...
                    const convertedMessages = sessionMessages
                        ...
                        .flatMap((msg, index) => {
                            ...
                            if (msg.role === 'tool') {
                                ...
                                const truncatedContent = msg.content.length > MAX_TOOL_OUTPUT
                            ...
                            if (mbSupportsVisionInput && msg.role === 'user' && msg.images && msg.images.length > 0) {
                            ...
                            if (mbSupportsVisionInput && msg.role === 'assistant' && msg.images && msg.images.length > 0) {
                                const camelCaseImages = msg.images.map(img => ({
                                    mimeType: img.mime_type,
                                    data: img.data,
                                }));
                    ...
                    logger.trace(`[AgentLoop] 📊 messages 数组: ${convertedMessages.length} 条（Session 总量 ${sessionMessages.length}, 历史跳过 ${historyBoundary}）`);
                    ...
                    if (this.config.imageAttachments?.length) {
                        ...
                        if (mbSupportsVisionInput) {
                            ...
                            if (currentUserMsg) {
                                const existing = (currentUserMsg as unknown as Record<string, unknown>).images as typeof camelCaseImages | undefined;
                                ...
                                logger.trace('[AgentLoop] 📷 已注入', camelCaseImages.length, '张图片到当轮 user 消息');
                            }
                        } else {
                        ...
                        for (const msg of sessionMessages.slice(historyWindowStart)) {
                            if (msg.role === 'user' && msg.images && msg.images.length > 0) {
                                ...
                                pairedMessages.push({
                                    role: 'user',
                                    content: msg.content || translate('planning.masterBrain.historicalUserMessagePlaceholder'),
                    ...
                    if (imageMessageCount > 0 && !mbSupportsVisionInput) {
                        ...
                        logger.trace('[AgentLoop] 沿用本次请求已确认的视觉 fallback 策略：移除历史 images，保留当前轮 images', {
                            providerId: this.config.providerId,
                            modelId: this.config.modelId,
                            strippedImageCount: stripped.imageCount,
                        });
                    ...
                    if (options?.onStreamDelta) {
                        ...
                        try {
                            ...
                            if (alreadyStrippedImages && appliedFallbackMode === 'strip-unmarked' && this.hasImages(messages) && this.isVisionUnsupportedError(streamError)) {
                            ...
                            if (!alreadyStrippedImages && this.hasImages(messages) && this.isVisionUnsupportedError(streamError)) {
                                const partial = this.stripImagesForVisionFallback(messages, {
                                    preserveMessages: currentTurnImageMessages,
                                });
                                const stripped = partial.imageCount > 0
                                    ? partial
                                    : this.stripImagesForVisionFallback(messages);
                                logger.warn('[AgentLoop] MB 流式调用返回视觉输入不支持，优先移除历史 images 后重试一次:', {
                                    providerId: this.config.providerId,
                                    modelId: this.config.modelId,
                                    strippedImageCount: stripped.imageCount,
                    ...
                    try {
                        ...
                        if (alreadyStrippedImages && appliedFallbackMode === 'strip-unmarked' && this.hasImages(messages) && this.isVisionUnsupportedError(invokeError)) {
                            const allStripped = this.stripImagesForVisionFallback(messages);
                            logger.warn('[AgentLoop] 沿用保留当前图片策略仍失败，移除全部 images 后重试一次:', {
                                providerId: this.config.providerId,
                                modelId: this.config.modelId,
                            ...
                            this.setVisionFallbackMode('strip-all');
                        } else if (!alreadyStrippedImages && this.hasImages(messages) && this.isVisionUnsupportedError(invokeError)) {
                            const partial = this.stripImagesForVisionFallback(messages, {
                                preserveMessages: currentTurnImageMessages,
                            });
                            const stripped = partial.imageCount > 0
                                ...
                                : this.stripImagesForVisionFallback(messages);
                            alreadyStrippedImages = true;
                            logger.warn('[AgentLoop] MB 调用返回视觉输入不支持，优先移除历史 images 后重试一次:', {
                                providerId: this.config.providerId,
                                modelId: this.config.modelId,
                                strippedImageCount: stripped.imageCount,
                                preservedCurrentImageMessages: partial.imageCount > 0,
                    ...
                    if (
                        ...
                        const allStripped = this.stripImagesForVisionFallback(messages);
                        logger.warn('[AgentLoop] 沿用保留当前图片策略仍返回视觉错误，移除全部 images 后重试一次:', {
                            providerId: this.config.providerId,
                            modelId: this.config.modelId,
                            strippedImageCount: allStripped.imageCount,
                            error: ((response.error ?? response.content) ?? '').slice(0, 240),
    ...
    private isMbEmptyDecisionContentError(error: unknown): boolean {
        return error instanceof MbEmptyDecisionContentError ||
    ...
    private isMbAnomalousDecisionContentError(error: unknown): boolean {
        return error instanceof MbAnomalousDecisionContentError ||
    ...
    private getDeepSeekV4MbFinalRetryReason(rawContent: string, normalizedContent: string): string | null {
    ...
    private hasPathologicalRepetition(content: string): boolean {
        const segments = content
            ...
            .filter(segment =>
                segment.length >= REPETITION_SEGMENT_MIN_CHARS &&
                segment.length <= 500
            );
    ...
    private buildHistoricalUserImageMessageContent(
        ...
        return translate('planning.masterBrain.historicalUserImageMessage', {
            timestamp: createdAt ? formatTimestamp(createdAt) : translate('planning.masterBrain.unknownTimestamp'),
            imageCount,
            content: originalContent.trim() || translate('planning.masterBrain.historicalUserMessagePlaceholder'),
        });
    }
    ...
    private async collectMBStreamResponse(
        ...
        const streamSessionId = this.currentSessionId
            ?? `planning-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
            ...
            listen<{
                ...
                const request: Record<string, unknown> = {
                    ...
                    messages: sanitizedMessages.map(m => ({
                        ...
                        ...(m.images ? {
                            images: (m.images as Array<{ mimeType?: string; mime_type?: string; data: string }>).map(img => ({
     ...
     */
    private buildAvatarIdentityMessage(): Array<{ role: string; content: string; images?: Array<{ mimeType: string; data: string }> }> {
        ...
        return [{
            ...
            content: '[Agent Identity Visual Context] The following image is your visual appearance. Interpret and use it together with the "Appearance Awareness" guidance in the system instructions.',
            images: [{
    ...
    private async persistAvatarToWorkdir(): Promise<void> {
        ...
        try {
            ...
            const originalPath = await join(appData, 'avatars', `${this.config.agentId ?? 'unknown'}.webp`);
    ...
    private extractSectionItems(fullContent: string, sectionKeywords: string[]): string[] {
        ...
        for (const line of lines) {
            ...
            if (trimmed.startsWith('#') && sectionKeywords.some(kw =>
                trimmed.includes(kw) || trimmed.toLowerCase().includes(kw.toLowerCase())
            )) {
                inSection = true;
                continue;
            }
    ...
    private extractRAGEvidenceFromSession(): import('../brain/types').RAGEvidence[] {
        ...
        if (ragBlock?.content) {
            ...
            logger.trace(`[AgentLoop] 📚 提取 RAG 证据: ${ragEvidence.length} 条，共 ${ragBlock.tokenEstimate} tokens`);
        }
    ...
    private createToolAdapter(): FSMIntegrationDependencies['executeTool'] {
        return async (toolCall: ToolCallInfo, options?: ToolExecuteOptions): Promise<ToolExecutionResult> => {
            ...
            if (result.images && result.images.length > 0) {
                logger.trace('[AgentLoop] 📷 createToolAdapter 透传 images:', result.images.length, '张, data长度:', result.images[0]?.data.length);
            }
            return {
    ...
    private async runWithFSM(userMessage: string): Promise<AgentLoopResult> {
        ...
        try {
            ...
            logger.trace(`[AgentLoop] 📊 历史消息分界线: ${this.historyMessageCount} 条（当轮消息将从此索引开始）`);
    ...
    private getEnabledExternalScriptSkills(): SkillDefinition[] {
        const { skillEnabledOverrides } = useRuntimeStore.getState();
        return skillLoader.getExternalScriptSkills()
    ...
    private matchExternalScriptSkillsByName(
        ...
        return scriptSkills.filter(skill =>
            normalizedQuery.includes(skill.name.toLowerCase())
        );
    }
    ...
    private async getAppManagedSandboxRoots(): Promise<string[]> {
        this.appManagedSandboxRootsPromise ??= (async () => {
            try {
                ...
                return [
                    await join(appData, 'runtime'),
                    await join(appData, 'skills'),
                ];
            } catch (error) {
    ...
    private async executeTool(toolCall: ToolCall, options?: ToolExecuteOptions): Promise<ToolResult> {
        ...
        const effectiveToolWorkdir = options?.workdirOverride ?? this.config.projectPath ?? this.config.workdir;
        const baseSandboxRoots = options?.sandboxRoots ?? [
            effectiveToolWorkdir,
        ...
        try {
            ...
            this.callbacks.onToolCallEnd?.(toolCall, result);
            logger.trace(`[AgentLoop] 工具完成: ${toolCall.name} (${result.success ? 'success' : 'failed'})`);
            this.setState('running');
            return result;
    ...
    private buildResult(reason: TerminationReason, error?: string): AgentLoopResult {
        ...
        return {
            ...
            success: !!uiContent.trim() || reason === 'text_response' || reason === 'awaiting_interaction' || reason === 'cancelled',
            content: uiContent,
