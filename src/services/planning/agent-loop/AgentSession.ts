...
export class AgentSession {
  ...
  async prepareContext(runtimeContext: RuntimeContext = {}): Promise<PreparedContext> {
    ...
    if (runtimeContext.facts && runtimeContext.facts.length > 0) {
      const bindingFacts = runtimeContext.facts.filter(
        (f) => f.category !== undefined && BINDING_FACT_CATEGORIES.has(f.category)
      );
    ...
    if (runtimeContext.facts && runtimeContext.facts.length > 0) {
      const contextFacts = runtimeContext.facts.filter(
        (f) => f.category === undefined || !CHAT_CONTEXT_EXCLUDED_FACT_CATEGORIES.has(f.category)
      );
