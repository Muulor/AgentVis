...
import { SubAgentLLMCallerFactory, type ToolCallInfo, type ToolExecutionResult, type VisionFallbackMode } from '../callers/SubAgentLLMCaller';
...
export class SubAgentDispatcher {
    ...
    async dispatch(
        ...
        return this.dispatchWithDynamicLoop(spec, onCheckpoint, signal, imageAttachments, savedAttachmentPaths);
    }
    ...
    private buildDispatchResult(
        ...
        if (output.status === 'completed') {
            ...
            if (output.diffDataList && output.diffDataList.length > 0) {
                logger.debug(`[SubAgentDispatcher] 📦 SA 完成，共产生 ${output.diffDataList.length} 个 Diff 记录（已实时发射）`);
            }
            ...
            if (this.config.workdir && this.config.agentId) {
                ...
                import('../../utils/DeliverableIndexer').then(({ indexUnindexedDeliverables }) => {
            ...
            // [Diff 数据已通过 diffDataCallback 实时发射，无需批量发射]
            if (output.diffDataList && output.diffDataList.length > 0) {
                logger.debug(`[SubAgentDispatcher] 📦 (失败但有已完成的修改) 共 ${output.diffDataList.length} 个 Diff 记录（已实时发射）`);
            }

            // 将失败 SA 的中间成果写入 session，确保 MB 重新派遣时能传递前序成果
            // observations 已包含增强版摘要（web_search 1500字符、read 800字符）
            const toolCallSummary = output.toolCalls && output.toolCalls.length > 0
            ...
            const recoveryHint = output.toolCalls && output.toolCalls.length > 0
    ...
    private async buildTaskContext(): Promise<TaskContext> {
        ...
        const totalTokens = (MODEL_CONTEXT_WINDOWS[modelKey]
        ...
        if (this.artifactStore && !this.artifactStore.isEmpty()) {
            ...
            if (interventions.length > 0) {
                hitlOverride = interventions
                    .map(a => a.content)
                    .join('\n---\n');
                logger.debug(
                    `[SubAgentDispatcher] 🚨 提取 HITL 介入约束: ${interventions.length} 条`
                );
            }
        }
    ...
    private setupRealtimeDiffCallback(
        ...
        this.runner.setDiffDataCallback((record) => {
            ...
            if (existing === undefined) {
                ...
                firstOriginalContentByPath.set(
                    record.filePath,
                ...
                const originalContent = (rawFirstOrig.length === 0 && recordOrig.length > 0)
                    ? recordOrig
                    : rawFirstOrig;

                const newContent = record.newContent ?? '';
                logger.trace(`[SubAgentDispatcher] 增量合并输入: firstOrig=${originalContent.length}字符/${originalContent.split('\n').length}行, latest=${newContent.length}字符/${newContent.split('\n').length}行, recordOrig=${recordOrig.length}字符`);
                try {
                    const { generateWholeFileReplaceXml } = await import(
                        '../../../fast-apply/DiffToXmlConverter'
                    );
                    ...
                    logger.trace(`[SubAgentDispatcher] 增量合并结果: wholeFileReplace, xmlLen=${xml.length}字符`);
                    onDiffData({
                        filePath: record.filePath,
                        originalContent,
                        newContent,
                    ...
                    logger.trace(`[SubAgentDispatcher] 📄 实时发射 Diff: ${record.filePath} (增量合并后更新)`);
                } catch (mergeError) {
     ...
     */
    async scanWorkdirFiles(): Promise<{ files: WorkdirFileInfo[]; totalFileCount: number; scanTruncated?: boolean }> {
                    ...
                    if (entry.isDirectory) {
                        ...
                        try {
                            ...
                            if (fileStat.mtime != null) {
                                const mtime = fileStat.mtime instanceof Date
                                    ? fileStat.mtime
