...
import type { AgentServiceState, FSMEvent, FSMContext, UserRequestPayload } from '../fsm/types';
...
export class AgentLoopFSMIntegration {
  ...
  constructor(config: FSMIntegrationConfig) {
    ...
    const effectiveMbBudget =
      config.mbDecisionBudget ?? PLANNING_CONSTANTS.LOOP_GOVERNOR_INITIAL_BUDGET;
   ...
   */
  setDependencies(dependencies: FSMIntegrationDependencies, skills: SkillDefinition[]): void {
    ...
    (dependencies as unknown as Record<string, unknown>).getWorkdirFiles = async () => {
    ...
    logger.trace(
      '[AgentLoopFSMIntegration] 委托模块初始化完成（含 Task Artifact Store + WORKDIR 扫描）'
    );
  }
  ...
  private initializeMasterBrain(llmService: LLMServiceInterface): void {
    ...
    this.masterBrain = new MasterBrain(promptBuilder, decisionParser, llmService);
  ...
  async run(userMessage: string): Promise<TerminationReason> {
    ...
    try {
      ...
      this.config.callbacks.onError?.(error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  private injectRationaleBeforeReturn(finalReason: string): void {
    ...
    if (
      ...
      if (this.lastSAObservations) {
        ...
        latestSaObservations =
          this.lastSAObservations.length > PLANNING_CONSTANTS.SA_OBSERVATIONS_MAX_CHARS
  ...
  setPairedHistoryMessages(
    ...
    logger.trace(
      `[AgentLoopFSMIntegration] 📷 已设置配对历史消息，待 DISPATCH 透传给 SA (${messages?.length ?? 0} 条消息)`
    );
  }
  ...
  private createStateHandlers(): StateHandlerMap {
    ...
    // 创建适配器：将模块化处理器包装为旧签名
    const createAdapter = (handlerFn: typeof modularHandlers.PREPARE_CONTEXT): StateHandler => {
      if (!handlerFn) {
  ...
  private createHandlerContext(): HandlerContext {
    ...
    const dependencies: HandlerDependencies = {
      ...
      createCheckpointHandler: (spec: SubAgentSpec) => this.createCheckpointHandler(spec),
  ...
  private syncSharedState(sharedState: HandlerSharedState): void {
    ...
    this.masterBrainInputBuilder?.setSaObservationsSummaries(this.saObservationsSummaries);
    // 同步上一轮 MB 决策摘要到 MasterBrainInputBuilder，
    // 下一轮 MB 决策的 build() 将自动包含 [LAST_MB_DECISION] 区块
    this.masterBrainInputBuilder?.setLastMBDecision(this.lastMBRationale, this.lastMBTask);
  ...
  private createCheckpointHandler(spec: SubAgentSpec): CheckpointCallback {
    return async (report: ProgressReport, _spec: SubAgentSpec): Promise<CheckpointDecision> => {
      ...
      logger.trace(
        '[Checkpoint] 待评估高风险操作:',
        report.pendingHighRiskAction?.substring(0, 80) ?? 'none'
      );
      ...
      const promptBuilder = new MasterBrainPrompt();
      const artifactIndexOnly = this.artifactStore.isEmpty()
        ? undefined
        : (() => {
      ...
      const prompt = promptBuilder.buildCheckpointEvaluationPrompt(report, spec, artifactIndexOnly);
      ...
      const estimatedCheckpointTokens =
        Math.ceil(chineseChars / 1.5) + Math.ceil((fullContext.length - chineseChars) / 4);
      logger.trace('[Checkpoint] 📋 Checkpoint LLM 完整上下文:', {
      ...
      logger.debug(
        '[Checkpoint] Master Brain 决策:',
        decision.type,
        decision.type === 'ADJUST_STRATEGY'
          ? `(+${(decision as { additionalIterations?: number }).additionalIterations ?? 0} 轮)`
          : ''
      );
  ...
  private createSubAgentToolExecutor(
    ...
    return async (
      ...
      const sandboxRoots =
        ...
        [effectiveWorkdir, this.config.projectPath ? this.config.workdir : undefined].filter(
