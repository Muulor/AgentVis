...
function extractRequestMoreInputContent(decision: unknown): string {
  ...
  return formatQuestionContent(
    nextStep?.questionsForUser ?? decisionRecord?.questionsForUser ?? decisionRecord?.response ?? ''
  );
}
  ...
  map(
    ...
    switch (decision.decision) {
      ...
      case 'SPAWN_SUB_AGENT':
        return this.mapSpawnSubAgent(decision, guideSkills, scriptSkills, outputLanguageHint);
