 ...
 */
function createMockDecision(decision: string, overrides?: Partial<MasterBrainDecision>): MasterBrainDecision {
    return {
...
describe('StateHandlers', () => {
    ...
    describe('handlePrepareContext', () => {
        ...
        it('返回的 payload 应该包含空的上下文数据', async () => {
            ...
            const payload = (result as unknown as Record<string, unknown>).payload as Record<string, unknown>;
    ...
    describe('handleMasterDecision', () => {
        ...
        it('MasterBrain 决策失败时应该返回 DECISION_INVALID', async () => {
            ...
            (mockBrain!.decide as ReturnType<typeof vi.fn>).mockRejectedValue(
                new Error('LLM 调用失败')
            );
    ...
    describe('handleDispatch', () => {
        ...
        it('should pass enabled guide and script skill names to the Sub-Agent catalog', async () => {
            ...
            expect(handlerContext.dependencies.subAgentDispatcher.setAllInstalledSkillNames)
        ...
        it('should not pass paired image history when includeHistory is false', async () => {
            ...
            expect(handlerContext.dependencies.subAgentDispatcher.setPairedHistoryMessages)
        ...
        it('没有任务时应该返回 ACTION_COMPLETED', async () => {
            ...
            expect(((result as unknown as Record<string, unknown>).result as Record<string, unknown>).noAction).toBe(true);
        });
        ...
        it('SA 被用户手动终止（cancelled）且有 observationEvents 时，lastSAObservations 应使用步骤摘要（标题为"被中止"）', async () => {
            ...
            handlerContext.dependencies.subAgentDispatcher.dispatch = vi.fn().mockResolvedValue({
                ...
                output: {
                    ...
                    observationEvents: [
                        ...
                        {
                            thinking: '获取竞争对手数据',
                            toolAction: { tool: 'web_search', target: 'IKEA financial performance 2024', success: true },
                            step: 2,
                        },
                    ],
                },
            ...
            // 不应包含原始的单行标题文字
            expect(handlerContext.sharedState.lastSAObservations).not.toBe('**步骤2：获取竞争对手数据（优衣库 UNIQLO）**');
