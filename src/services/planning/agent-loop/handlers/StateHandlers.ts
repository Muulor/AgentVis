...
import type { HandlerContext, StateHandlerMap } from './types';
 ...
 */
function applyGuideSkillExec(spec: SubAgentSpec, guideSkills: ExternalGuideSkillInfo[]): void {
  ...
  for (const skill of guideSkills) {
    ...
    if (hasScriptFiles || hasScriptSteps) {
      ...
      logger.trace(`[StateHandlers] 🔧 DISPATCH 兜底补全 exec（来自 Guide 技能 "${skill.name}"）`, {
        hasScriptFiles,
        hasScriptSteps,
      });
      return;
...
export async function handleMasterDecision(
  ...
  const { masterBrain, decisionMapper, masterBrainInputBuilder, loopGovernor, callbacks } =
    dependencies;
  ...
  try {
    ...
    if (
      isParserFallback &&
      decision.decision === 'RESPOND_TO_USER' &&
    ...
    const taskContent =
      (decision.nextStep as { task?: string } | undefined)?.task ??
    ...
    if (sharedState.pendingSubAgentSpec && dependencies.getExternalGuideSkills) {
      ...
      try {
        ...
        if (secondarySkills.length > 0) {
          ...
          const existingNames = new Set((sharedState.externalGuideSkills ?? []).map((s) => s.name));
    ...
    if (sharedState.pendingSubAgentSpec && dependencies.getExternalScriptSkills) {
      const spec = sharedState.pendingSubAgentSpec;
      const taskText = [spec.role, spec.contextSummary, spec.terminationCondition]
        .filter(Boolean)
        .join(' ');
    ...
    sharedState.lastLLMContent =
      formatAgentLoopFailureMessage(error) + rationaleContext + saContext;
...
export async function handleDispatch(
  ...
  if (sharedState.pendingSubAgentSpec) {
    ...
    if (sharedState.spawnCount > PLANNING_CONSTANTS.MAX_SPAWN_RETRIES) {
      logger.debug(
        `[StateHandlers] ❗ spawnCount(${sharedState.spawnCount}) > MAX_SPAWN_RETRIES(${PLANNING_CONSTANTS.MAX_SPAWN_RETRIES})，系统强制终止`
      );
      ...
      const systemMessage =
        formatAgentLoopFailureMessage('sub_agent_failure_circuit_breaker', {
    ...
    const installedGuideSkillNames =
      dependencies.getInstalledSkillCatalog?.().map((s) => s.name) ?? [];
    const installedScriptSkillNames =
      dependencies.getInstalledScriptSkillCatalog?.().map((s) => s.name) ?? [];
    const installedSkillNames = Array.from(
      new Set([...installedGuideSkillNames, ...installedScriptSkillNames])
    );
    ...
    if (dispatchResult.output?.observations) {
      ...
      if (events && events.length > 0) {
        ...
        for (const evt of events) {
          ...
          if (evt.thinking) {
            ...
            const thinking =
              evt.thinking.length > MAX_THINKING_CHARS_PER_STEP
          ...
          if (evt.toolAction) {
            const status =
              evt.toolAction.success === undefined ? '⏳' : evt.toolAction.success ? '✅' : '❌';
        ...
        summary =
          obs.length > MAX_OBS_SUMMARY_CHARS ? obs.slice(0, MAX_OBS_SUMMARY_CHARS) + '...' : obs;
      }
      ...
      if (events && events.length > 0) {
        // 有步骤记录——按终止原因生成不同标题
        const isApiError =
          dispatchResult.output.executionStatus === 'failure' &&
    ...
    if (sharedState.lastMBRationale) {
      ...
      sharedState.mbDecisionLog = [...sharedState.mbDecisionLog.slice(-(maxRounds - 1)), entry];
      logger.debug(
  ...
  if (decision?.decision === 'REQUEST_MORE_INPUT') {
    ...
    logger.debug(
      '[StateHandlers] DISPATCH → REQUEST_MORE_INPUT, 消息:',
    ...
    return {
      ...
      result: { response: sharedState.lastLLMContent, requiresInteraction: true },
    };
  }
...
export function handleEvaluate(
  ...
  const observation: Observation = {
    ...
    riskDelta: 0,
    toolCalled:
      sharedState.currentToolCalls.length > 0 ? sharedState.currentToolCalls[0]?.name : undefined,
  ...
  const snapshot = loopGovernor.getSnapshot();
  callbacks.onBudgetUpdate?.(snapshot.budgetRemaining, snapshot.initialBudget);
