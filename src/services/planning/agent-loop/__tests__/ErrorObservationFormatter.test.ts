...
describe('ErrorObservationFormatter', () => {
    ...
    it('classifies checkpoint and Sub-Agent circuit-breaker errors', () => {
        ...
        expect(classifyAgentLoopError('Sub-Agent failed 4 consecutive times').kind)
            .toBe('sub_agent_failure_circuit_breaker');
    });
    ...
    it('maps known Sub-Agent termination reasons to categorized error kinds', () => {
        ...
        expect(getAgentLoopErrorKindForTerminationReason('high_risk_checkpoint_failed')).toBe('checkpoint_failure');
        expect(getAgentLoopErrorKindForTerminationReason('consecutive_failures'))
            .toBe('sub_agent_failure_circuit_breaker');
    });
