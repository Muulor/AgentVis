...
function createMockSession(): AgentSession {
  return {
    id: 'test-session-id',
    addMessage: vi.fn(),
    getMessages: vi.fn().mockReturnValue([{ role: 'user', content: '请帮我完成任务' }]),
...
describe('AgentLoopFSMIntegration', () => {
  ...
  describe('run (Native MasterBrain 模式)', () => {
    it('没有设置依赖时应该抛出错误', async () => {
      await expect(integration.run('test message')).rejects.toThrow('Dependencies not set');
    });
    ...
    it('RESPOND_TO_USER 决策应该返回响应内容', async () => {
      ...
      const mockLLMService = createMockLLMService(createRespondToUserDecision(responseContent));
    ...
    it('REQUEST_MORE_INPUT 决策应该返回 awaiting_interaction', async () => {
      ...
      const mockLLMService = createMockLLMService(createRequestMoreInputDecision(questions));
