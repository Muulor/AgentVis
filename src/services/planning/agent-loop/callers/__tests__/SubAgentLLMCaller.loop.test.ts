...
describe('SubAgentLLMCaller Loop', () => {
    ...
    describe('callWithContext', () => {
        ...
        it('工具结果正确追加到消息历史', async () => {
            ...
            const accumulatedContext: AccumulatedMessage[] = [
                ...
                { role: 'tool', content: '[file_write] 写入成功', toolName: 'file_write', timestamp: Date.now() },
            ];
        ...
        it('模型注册表标记不支持视觉时，应在首次调用前移除 images', async () => {
            ...
            expect(request.request.messages.some(message => message.images && message.images.length > 0)).toBe(false);
        ...
        it('API 返回视觉不支持错误时，应移除 images 并重试一次', async () => {
            vi.mocked(invoke)
                .mockResolvedValueOnce({
                    type: 'error',
                    error: '(404 Not Found): {"error":{"message":"No endpoints found that support image input"}}',
                })
                .mockResolvedValueOnce({
                    type: 'text',
                    content: 'retry ok',
            ...
            expect(firstRequest.request.messages.some(message => message.images && message.images.length > 0)).toBe(true);
            expect(secondRequest.request.messages.some(message => message.images && message.images.length > 0)).toBe(false);
        ...
        it('reuses the successful partial image fallback on later SA steps', async () => {
            ...
            const secondStepResponse = await caller.callWithContext('System prompt', ['read'], accumulatedContext);
    ...
    describe('buildMessagesWithContext', () => {
        ...
        it('有 additionalInstructions 时包含策略调整', async () => {
            ...
            expect(messageContent).toContain(translate('chat.subAgentSystemNote', {
        ...
        it('默认不追加 Sub-Agent Safety Footer', async () => {
            ...
            expect(request.request.messages.some(message => message.content.includes('Correction Reminder'))).toBe(false);
        });
        ...
        it('启用后追加自定义 Sub-Agent Safety Footer', async () => {
            ...
            const finalUserMessage = [...request.request.messages].reverse().find(message => message.role === 'user');
