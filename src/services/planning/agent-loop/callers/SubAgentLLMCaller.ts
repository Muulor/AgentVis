...
export class SubAgentLLMCallerFactory {
    ...
    private buildMessagesWithContext(
        ...
        if (additionalInstructions) {
            finalSections.push(translate('chat.subAgentSystemNote', {
        ...
        if (persistedIntervention) {
            ...
            const stepsAgoLabel = stepsSinceIntervention === 0
            ...
            finalSections.push(translate('chat.subAgentForcedUserInstruction', {
    ...
    private async invokeWithMessages(
        ...
        const abortHandler = () => {
            ...
            logger.debug(`[SubAgentLLMCaller] ⛔ AbortSignal 触发，已发送取消信号 (sessionId: ${sessionId})`);
        };
        ...
        if (msgsWithImages.length > 0 && !modelSupportsVision(modelId, providerId)) {
            ...
            initialMessages = stripped.messages;
            initialStrippedImageCount = stripped.imageCount;
            logger.trace('[SubAgentLLMCaller] 沿用本次 SA 已确认的视觉 fallback 策略：移除历史/未标记 images，保留当前任务 images', {
                providerId,
                modelId,
        ...
        if (
            ...
            const allStripped = this.stripImagesForVisionFallback(messages);
            logger.warn('[SubAgentLLMCaller] 沿用保留当前图片策略仍返回视觉错误，移除全部 images 后重试一次:', {
                providerId,
                modelId,
                strippedImageCount: allStripped.imageCount,
                error: ((response.error ?? response.content) ?? '').slice(0, 240),
    ...
    private hasMarkedImages(messages: Message[]): boolean {
        return messages.some(message =>
            message.preserveImagesOnVisionFallback && (message.images?.length ?? 0) > 0
        );
    }
