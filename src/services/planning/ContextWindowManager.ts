    ...
    prepareContext(
        ...
        if (maxRounds !== undefined && maxRounds > 0) {
            ...
            if (chatHistory.length < originalCount) {
                logger.trace(`[ContextWindowManager] 轮次截断: 原始 ${originalCount} 条 → ${chatHistory.length} 条 (maxRounds=${maxRounds})`);
            }
        }
        ...
        logger.trace(`[ContextWindowManager] 模型: ${modelId ?? 'default'}, 总预算: ${budget.totalTokens} tokens`);
        logger.trace(`[ContextWindowManager] 三层预算: Identity=${budget.identityBudget}, Context=${budget.contextBudget}, History=${budget.historyBudget}`);
        ...
        if (identityOverflow > 0) {
            logger.trace(`[ContextWindowManager]  Identity 超出预算 ${identityOverflow} tokens，从 Context 借用`);
        }
        ...
        for (const { key, type } of layerMapping) {
            ...
            if (contentTokens <= remainingBudget) {
                ...
                logger.trace(`[ContextWindowManager]  ${type}: 截断 ${contentTokens} → ${truncatedTokens} tokens`);
            } else {
                // 完全丢弃
                wasTruncated = true;
                truncatedItems.push({
        ...
        logger.trace(`[ContextWindowManager] 对话历史: ${chatHistory.length} 条, ${historyTokens} tokens`);
        ...
        if (wasTruncated) {
            ...
            truncationNotice = {
                reason: truncatedItems.some(i => i.type === 'history') ? 'HISTORY_OVERFLOW' : 'CONTEXT_OVERFLOW',
                truncatedItems,
        ...
        logger.trace(`[ContextWindowManager] 预算使用: ${totalUsed}/${budget.totalTokens} (${(totalUsed / budget.totalTokens * 100).toFixed(1)}%)`);
    ...
    private truncateHistory(
        ...
        for (let i = messages.length - 1; i >= 0; i--) {
            ...
            if (content.length > HISTORY_CONFIG.MAX_MESSAGE_CHARS) {
                content = content.substring(0, HISTORY_CONFIG.MAX_MESSAGE_CHARS) + '\n... [Message truncated]';
            }

            // 仅给 user 消息注入时间标签：assistant 消息如果也带时间戳，
            // LLM 会通过 in-context learning 模仿该格式在输出中附带时间戳
            const timeLabel = (msg.role === 'user' && msg.createdAt) ? `[${formatTimestamp(msg.createdAt)}] ` : '';
        ...
        if (usedCount < messages.length) {
            ...
            result.unshift(`> ${droppedCount} earlier conversation messages were omitted; their semantics are handled by the Memory system.\n`);
        }
