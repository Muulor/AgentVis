...
export class DecisionLogger {
  ...
  getSessionLog(sessionId: string): PersistentDecisionLogEntry[] {
    return Array.from(this.entries.values()).filter((entry) => entry.sessionId === sessionId);
  }
  ...
  getStatistics(sessionId: string): DecisionStatistics {
    ...
    for (const log of sessionLogs) {
      decisionTypeCount[log.decisionType] = (decisionTypeCount[log.decisionType] ?? 0) + 1;
    }

    // 成功率计算（仅计算有执行结果的记录）
    const logsWithResult = sessionLogs.filter((log) => log.executionResult);
    const successCount = logsWithResult.filter((log) => log.executionResult?.success).length;
    const successRate = logsWithResult.length > 0 ? successCount / logsWithResult.length : 0;
