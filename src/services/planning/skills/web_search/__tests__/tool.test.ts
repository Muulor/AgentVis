...
describe('webSearchTool sandbox mode', () => {
    ...
    it('blocks web search in OfflineIsolated mode before invoking backend network', async () => {
        ...
        expect(result.content).toContain('WEB_SEARCH_ERROR kind=sandbox_blocked retryable=false status=none');
    ...
    it('returns concise retryable metadata for Tavily rate limit failures', async () => {
        mockInvoke.mockRejectedValue('LLM API call failed: Tavily API returned error 429: too many requests');
        ...
        expect(result.success).toBe(false);
        expect(result.content).toContain('WEB_SEARCH_ERROR kind=rate_limited retryable=true status=429');
        expect(result.data?.error).toMatchObject({
            kind: 'rate_limited',
            retryable: true,
            status: 429,
    ...
    it('returns concise retryable metadata for DNS failures', async () => {
        mockInvoke.mockRejectedValue('LLM API call failed: Network broker DNS lookup failed: no such host');
    ...
    it('returns concise retryable metadata for timeout failures', async () => {
        mockInvoke.mockRejectedValue(new Error('LLM API call failed: Network broker request failed: operation timed out'));
