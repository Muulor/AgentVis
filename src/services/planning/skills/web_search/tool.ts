...
const SCHEMA: ToolSchema = {
    name: 'web_search',
    description: 'Search the web for information. Returns summaries, page snippets, and links. Supports basic and advanced search depth.',
    parameters: {
        type: 'object',
        properties: {
            query: {
...
function buildFailureContent(query: string, errorMessage: string, meta: WebSearchErrorMeta): string {
...
function classifyWebSearchError(errorMessage: string): WebSearchErrorMeta {
    ...
    if (normalized.includes('network broker dns lookup failed')
    ...
    if (normalized.includes('network broker request failed')
...
class WebSearchToolImpl implements Tool {
    ...
    async execute(
        ...
        try {
            // 报告进度（含模式提示，方便调试）
            const modeLabel = searchDepth === 'advanced'
            ...
            context.onProgress?.(translate('tools.webSearch.searching', {
                mode: modeLabel,
