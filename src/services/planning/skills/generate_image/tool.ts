...
const SCHEMA: ToolSchema = {
    name: 'generate_image',
    description: 'Generate an image from a text prompt using an AI image model. The generated image is saved automatically to the deliverables directory.',
    parameters: {
        type: 'object',
        properties: {
            prompt: {
                type: 'string',
                description: 'Image generation prompt. Describe the scene narratively instead of listing keywords; include lighting, composition, style, and other useful details.',
            },
            provider: {
                type: 'string',
                description: 'Optional image provider override. Usually leave empty so the tool can use the internal image generation pipeline automatically.',
            },
            ref_image_path: {
                type: 'string',
                description: 'Optional single reference image file path for image editing, style transfer, or reference-based generation. Use ref_image_paths for multiple images.',
            },
            ref_image_paths: {
                type: 'array',
                items: { type: 'string', description: 'Reference image file path.' },
                description: 'Optional array of reference image file paths, up to 14 images, for multi-image composition, style blending, and similar tasks.',
            },
            aspect_ratio: {
                type: 'string',
                description: 'Optional output aspect ratio. Supported: 1:1 (default), 1:4, 1:8, 4:1, 8:1, 16:9, 9:16, 3:2, 2:3, 4:3, 3:4, 4:5, 5:4, 21:9.',
            },
            image_size: {
                type: 'string',
            ...
            quality: {
                ...
                description: 'Optional gpt-image-2 rendering quality. Defaults to auto; use low for drafts/thumbnails and medium or high for final assets.',
            },
            output_format: {
                type: 'string',
                enum: ['png', 'jpeg', 'webp'],
                description: 'Optional gpt-image-2 output format. Defaults to png; jpeg and webp are also available.',
            },
            output_compression: {
                type: 'number',
                description: 'Optional jpeg/webp compression level for gpt-image-2, from 0 to 100. Only applies when output_format is jpeg or webp.',
            },
            background: {
                type: 'string',
                enum: ['auto', 'opaque'],
                description: 'Optional background mode. gpt-image-2 does not support transparent here; use auto or opaque.',
            },
            custom_name: {
                type: 'string',
                description: 'Optional custom file name without extension, useful for meaningful generated asset names such as "hero_banner" or "logo_dark". Only letters, numbers, underscores, and hyphens are allowed.',
            },
        },
        required: ['prompt'],
    },
...
function notifyImageGenerationError(message: string): void {
    try {
        window.dispatchEvent(new CustomEvent('agentvis:image-generation-error', {
            detail: { message },
        }));
    } catch {
...
class GenerateImageToolImpl implements Tool {
    ...
    async execute(
        ...
        try {
            ...
            if (provider === 'auto') {
                ...
                logger.warn('[GenerateImageTool] gpt-image-2 path failed, falling back to Gemini:', gptResult.content);
            ...
            if (isCancelled(context.signal)) return cancelledResult();
            const errorMessage = error instanceof Error ? error.message : String(error);
            return { success: false, content: translate('tools.generateImage.failed', { error: errorMessage }) };
        }
    }
    ...
    private async runGemini(
        ...
        for (const candidate of IMAGE_MODEL_CANDIDATES) {
            if (isCancelled(context.signal)) return cancelledResult();
            context.onProgress?.(translate('tools.generateImage.generatingWith', {
            ...
            if (result.error) {
                ...
                logger.warn(`[GenerateImageTool] ${candidate.provider}/${candidate.model} 失败:`, result.error);
                continue;
            }
