...
function pyrightOutput(diagnostics: Array<{
    file: string;
    line: number;
    column: number;
    message: string;
    ...
    it('错误数超 5 条时只返回前 5 条', async () => {
        ...
        mockInvoke.mockImplementation(async () => {
            ...
            const manyErrors = Array.from({ length: 10 }, (_, i) =>
                `src/foo.ts(${i + 1},1): error TS${2000 + i}: err${i}.`
            ).join('\n');
...
describe('Python (.py)', () => {
    ...
    it('py_compile 当前文件有语法错误时不继续运行项目检查', async () => {
        ...
        mockInvoke.mockImplementation(async (command, args) => {
            ...
            return shellFail('', '  File "/project/script.py", line 8\n    return x +\nSyntaxError: invalid syntax');
        });
    ...
    it('有 Pyright 配置和本地 pyright 时解析 current/related 诊断', async () => {
        mockInvoke.mockImplementation(async (command, args) => {
            ...
            return shellFail(pyrightOutput([
                ...
                { file: '/project/src/warn.py', line: 1, column: 1, message: 'Warning only', severity: 'warning' },
            ]), '');
        });
...
describe('JavaScript (.js / .mjs)', () => {
    it('有本地 ESLint 时优先运行 eslint --quiet --format json', async () => {
        mockInvoke.mockImplementation(async (command, args) => {
            ...
            return shellFail(JSON.stringify([
                {
                    filePath: '/project/src/bad.js',
                    messages: [
                        { severity: 1, line: 1, column: 1, ruleId: 'no-console', message: 'Unexpected console statement.' },
                        { severity: 2, line: 5, column: 3, ruleId: 'no-undef', message: "'x' is not defined." },
                    ],
                },
            ]), '');
        });
    ...
    it('.jsx 无 ESLint 时不运行 node --check，直接括号兜底', async () => {
        ...
        mockInvoke.mockImplementation(async (command, args) => {
            ...
            if (command === 'file_read_content') {
                const filePath = normalizeTestPath(getInvokeStringArg(invokeArgs, 'filePath'));
                if (filePath.endsWith('/project/src/App.jsx')) return 'export function App() { return <div />; }';
                throw new Error('not found');
            }
