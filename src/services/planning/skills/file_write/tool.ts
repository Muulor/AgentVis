...
} from '@services/rag/KnowledgeFileFilter';
...
const SCHEMA: ToolSchema = {
  name: 'file_write',
  description:
    'Write text file content. Never call with empty arguments. Full mode requires path and content. Patch mode requires path, mode="patch", and patches with search/replace pairs.',
  parameters: {
    type: 'object',
    description:
      'Non-empty argument object. For full writes, pass {"path": "...", "mode": "full", "content": "..."}. For patches, pass {"path": "...", "mode": "patch", "patches": [{"search": "...", "replace": "..."}]}.',
    properties: {
      path: {
        type: 'string',
      ...
      mode: {
        ...
        enum: ['full', 'patch'],
        description:
          'Write mode. Use "full" for complete content writes. Use "patch" only when patches is provided.',
      },
      patches: {
        type: 'array',
        description:
          'Required when mode is "patch". Non-empty list of search/replace patch objects.',
        items: {
          type: 'object',
          ...
          properties: {
            search: {
              type: 'string',
              description:
                'Original text fragment to find. It should be specific enough to match uniquely.',
            },
            replace: {
              type: 'string',
...
function mergeOutputMatchesIntendedContent(mergeOutput: string, intendedContent: string): boolean {
...
class FileWriteToolImpl implements Tool {
  ...
  async execute(
    ...
    if (typeof rawPatches === 'string') {
      ...
      if (result.success && Array.isArray(result.data)) {
        patches = result.data;
        logger.trace(
          `[FileWriteTool] patches 从字符串解析成功: ${patches.length} 个补丁 (策略: ${result.strategy ?? 'unknown'}, 质量: ${result.quality ?? 'unknown'})`
        );
      } else {
    ...
    try {
      ...
      context.onProgress?.(
        translate('tools.fileWrite.progressExisting', {
          mode: decision.mode,
  ...
  private async executeStagedFullWrite(
    ...
    context.onProgress?.(
      translate('tools.fileWrite.progressCreate', {
    ...
    try {
      ...
      if (context.agentId) {
        if (await this.shouldAutoIndex(context.agentId)) {
          try {
            const indexedContent = await invoke<string>('file_read_content', {
  ...
  private async decideMode(
    ...
    const changedLines = diff.hunks.reduce(
    ...
    logger.trace(
      `[FileWriteTool] 差异分析: ${(changeRatio * 100).toFixed(1)}% 变化 (${changedLines}/${totalLines} 行)`
    );
  ...
  private async executePatch(
    ...
    for (const [i, patch] of patches.entries()) {
      ...
      const searchPreviewLog =
        patch.search.length > 80 ? `${patch.search.substring(0, 80)}...` : patch.search;
      const replacePreviewLog =
        patch.replace.length > 80 ? `${patch.replace.substring(0, 80)}...` : patch.replace;
      ...
      logger.trace(
        `[FileWriteTool] patch[${i}] match: ${matchResult.success ? matchResult.matchLevel : 'FAILED'} (${matchElapsed}ms, searchLen=${patch.search.length})`
      );
      ...
      if (matchResult.success) {
        ...
        const searchPreview =
          patch.search.length > 50 ? `${patch.search.substring(0, 50)}...` : patch.search;
        ...
        if (best) {
          const candidatePreview =
            best.content.length > 80 ? `${best.content.substring(0, 80)}...` : best.content;
    ...
    logger.trace(
      `[FileWriteTool] 全部 patch 匹配完成: ${locatedPatches.length}/${patches.length} 成功 (总耗时 ${Date.now() - patchTimingStart}ms)`
    );
    ...
    locatedPatches.sort((a, b) => {
      const offsetA =
        a.matchResult.startOffset ?? originalContent.indexOf(a.matchResult.matchedContent);
      const offsetB =
        b.matchResult.startOffset ?? originalContent.indexOf(b.matchResult.matchedContent);
    ...
    for (const { patch, matchResult } of locatedPatches) {
      ...
      if (idx !== -1 && idx + matchLen <= newContent.length) {
        newContent =
          newContent.substring(0, idx) + patch.replace + newContent.substring(idx + matchLen);
      }
    }
    ...
    logger.trace(
      `[FileWriteTool] file_write_to_path 完成 (${Date.now() - writeStart}ms, ${newContent.length} 字符)`
    );
    if (isCancelled(context)) return cancelledResult(path);
    ...
    logger.trace(
      `[FileWriteTool] diff + xml 生成完成 (${Date.now() - diffStart}ms, hunks=${diff.hunks.length}, xmlLen=${xml.length})`
    );
    ...
    const detailedResults = patchResults.map((r, i) => {
      if (r.success) {
        ...
        if (located && context.isSubAgentContext) {
          const snippet = this.getModifiedSnippet(newContentLines, located.patch.replace, 3);
          if (snippet) {
  ...
  private async executeOverwrite(
    ...
    // 写入文件
    context.onProgress?.(
      translate('tools.fileWrite.progressOverwrite', {
        fileName: this.getFileName(path),
      })
    );
    await invoke('file_write_to_path', {
    ...
    try {
      ...
      if (currentAgent) {
        ...
        try {
          ...
          /* knowledgePaths 解析失败时视为空 */
        }

        if (!currentPaths.includes(filePath)) {
          logger.trace('[FileWriteTool] 索引前发现路径已从知识库移除，跳过索引:', filePath);
          return;
        }
      }

      const chunkCount = await ragService.indexDocument(agentId, documentId, content, {
        fileName,
        filePath,
        documentType,
      });
  ...
  private getModifiedSnippet(
    ...
    const matchIndex = newContentLines.findIndex((line) => line.trim().includes(replaceFirstLine));
    ...
    const endIndex = Math.min(newContentLines.length, matchIndex + replaceLineCount + contextLines);
    ...
    // 格式化为带行号的片段
    return snippetLines.map((line, i) => `     | ${startIndex + i + 1}: ${line}`).join('\n');
  }
  ...
  private formatSyntaxErrorLines(errors: PostWriteSyntaxError[], includeFile: boolean): string {
    return errors
      .map((e) => {
  ...
  private formatSyntaxLocation(error: PostWriteSyntaxError, includeFile: boolean): string {
    const compactPath =
      includeFile && error.filePath ? this.compactDiagnosticPath(error.filePath) : '';
