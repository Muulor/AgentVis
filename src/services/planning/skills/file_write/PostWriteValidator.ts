...
async function findTsconfigDir(filePath: string): Promise<string | undefined> {
  ...
  logger.trace(
    `[PostWriteValidator] 未找到 tsconfig.json（从 ${getDirPath(filePath)} 向上搜索 ${MAX_TSCONFIG_SEARCH_DEPTH} 层）`
  );
  return undefined;
}
...
async function findCargoTomlDir(filePath: string): Promise<string | undefined> {
  ...
  logger.trace(
    `[PostWriteValidator] 未找到 Cargo.toml（从 ${getDirPath(filePath)} 向上搜索 ${MAX_PROJECT_CONFIG_SEARCH_DEPTH} 层）`
  );
  return undefined;
}
...
async function findEslintConfigDir(filePath: string): Promise<string | undefined> {
  ...
  logger.trace(
    `[PostWriteValidator] 未找到 ESLint 配置（从 ${getDirPath(filePath)} 向上搜索 ${MAX_PROJECT_CONFIG_SEARCH_DEPTH} 层）`
  );
  return undefined;
}
...
async function findPyrightConfigDir(filePath: string): Promise<string | undefined> {
  ...
  logger.trace(
    `[PostWriteValidator] 未找到 Pyright 配置（从 ${getDirPath(filePath)} 向上搜索 ${MAX_PROJECT_CONFIG_SEARCH_DEPTH} 层）`
  );
  return undefined;
}
...
async function findMypyConfigDir(filePath: string): Promise<string | undefined> {
  ...
  logger.trace(
    `[PostWriteValidator] 未找到 Mypy 配置（从 ${getDirPath(filePath)} 向上搜索 ${MAX_PROJECT_CONFIG_SEARCH_DEPTH} 层）`
  );
  return undefined;
}
...
async function findGoModuleDir(filePath: string): Promise<string | undefined> {
  ...
  logger.trace(
    `[PostWriteValidator] 未找到 go.mod/go.work（从 ${getDirPath(filePath)} 向上搜索 ${MAX_PROJECT_CONFIG_SEARCH_DEPTH} 层）`
  );
  return undefined;
}
...
async function findLocalNodeBinary(
  startDir: string,
  binaryNames: string[],
  label: string
...
function selectRelatedProjectErrors(
  ...
  return projectErrors
    ...
    .sort(
      (a, b) =>
        scoreRelatedProjectError(a, targetFilePath) - scoreRelatedProjectError(b, targetFilePath)
    )
...
function parseCargoCheckJsonOutput(output: string): SyntaxError[] {
  ...
  for (const lineText of lines) {
    ...
    const spanRecord = isRecord(primarySpan) ? primarySpan : spans.find(isRecord);
...
function parseGoTestOutput(stdoutAndStderr: string): SyntaxError[] {
  ...
  while ((match = pattern.exec(stdoutAndStderr)) !== null) {
    ...
    if (message.length === 0 || message.startsWith('exit status ') || message.startsWith('FAIL')) {
      continue;
    }
...
function parseTomlDecodeOutput(stderr: string): SyntaxError[] {
  ...
  const message = messageMatch ? capture(messageMatch, 1).trim() : 'TOML parse error';
 ...
 */
async function runTscCheck(filePath: string, context: ValidatorContext): Promise<ValidationResult> {
  ...
  if (tsconfigDir) {
    ...
    try {
      ...
      const relatedErrors = selectRelatedProjectErrors(projectErrors, errors, filePath);
      logger.trace(
        `[PostWriteValidator] tsc 项目模式完成: exitCode=${result.exitCode}, 当前文件错误=${errors.length}`
      );
      return {
        checked: true,
        tool: 'tsc',
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] tsc 单文件模式完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
...
async function runPyCompileCheck(
  ...
  const pythonExe = context.venvPythonPath ? `"${context.venvPythonPath}"` : 'python';
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] py_compile 完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
    const pyCompileResult: ValidationResult = {
      checked: true,
      tool: 'py_compile',
      errors,
...
async function runPyrightCheckIfAvailable(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] Pyright 完成: exitCode=${result.exitCode}, errors=${projectErrors.length}`
    );
...
async function runMypyCheckIfAvailable(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] Mypy 完成: exitCode=${result.exitCode}, errors=${projectErrors.length}`
    );
...
async function runEslintCheckIfAvailable(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] ESLint 完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
    return {
      checked: true,
      tool: 'eslint',
...
async function runNodeCheck(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] node --check 完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
...
async function runJsonToolCheck(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] json.tool 完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
...
async function runJsonParseFallback(filePath: string): Promise<ValidationResult> {
  try {
    ...
    const message = parseErr instanceof SyntaxError ? parseErr.message : String(parseErr);
    return {
      checked: true,
      tool: 'json_tool',
...
async function runTomlParseCheck(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] TOML 检查完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
...
async function runRustCheck(
  ...
  try {
    ...
    const relatedErrors = selectRelatedProjectErrors(projectErrors, errors, filePath);
    logger.trace(
      `[PostWriteValidator] cargo check 完成: exitCode=${result.exitCode}, 当前文件错误=${errors.length}, related=${relatedErrors.length}`
    );
    return {
      checked: true,
      tool: 'cargo_check',
...
async function runRustcCheck(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] rustc 完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
...
async function runGoCheck(filePath: string, context: ValidatorContext): Promise<ValidationResult> {
  ...
  logger.trace(
    `[PostWriteValidator] go test package 编译: workdir=${goModuleDir}, package=${packagePattern}`
  );
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] go test 完成: exitCode=${result.exitCode}, errors=${projectErrors.length}`
    );
...
async function runGofmtCheck(
  ...
  try {
    ...
    logger.trace(
      `[PostWriteValidator] gofmt 完成: exitCode=${result.exitCode}, errors=${errors.length}`
    );
 ...
 */
async function runBracketFallback(filePath: string, failedTool: string): Promise<ValidationResult> {
