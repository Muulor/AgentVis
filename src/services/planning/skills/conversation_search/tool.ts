...
const SCHEMA: ToolSchema = {
  ...
  description:
    "Search, browse by timeline, or fetch this Agent's own saved conversation history. Search mode returns lightweight snippets; timeline mode lists lightweight chronological messages; get mode returns full selected messages. Scope is always the current Agent only.",
  parameters: {
    type: 'object',
    properties: {
      mode: {
        type: 'string',
        description:
          'Operation mode: "search" for keyword snippets, "timeline" for chronological browsing without a keyword, or "get" to fetch full messages by messageId/messageIds. Defaults to "search".',
        enum: ['search', 'timeline', 'get'],
      },
      query: {
        type: 'string',
        description:
          'Exact keyword or phrase to search in this Agent\'s saved conversation history. Required for mode="search".',
      },
      limit: {
        type: 'number',
        description:
          'Maximum number of search matches to return. Defaults to 10 and is capped at 50.',
      },
      offset: {
        type: 'number',
        description:
          'Search result offset for pagination. Defaults to 0. When hasMore=true, call again with nextOffset.',
      },
      startAt: {
        type: 'string',
        description:
          'Optional inclusive time lower bound for search/timeline. Use ISO timestamp with timezone when possible, or YYYY-MM-DD for local-day start.',
      },
      endAt: {
        type: 'string',
        description:
          'Optional exclusive time upper bound for search/timeline. Use ISO timestamp with timezone when possible, or YYYY-MM-DD for the next local-day boundary.',
      },
      order: {
        type: 'string',
        description:
          'Timeline order: "desc" (newest first, default) or "asc" (oldest first). Used for mode="timeline".',
        enum: ['desc', 'asc'],
      },
      role: {
        type: 'string',
...
function normalizeOffset(value: unknown): number {
  const raw = typeof value === 'number' ? value : typeof value === 'string' ? Number(value) : 0;
...
function formatSearchResults(
  ...
  const items = messages.map((message, index) =>
    translate('tools.conversationSearch.resultItem', {
      index: index + 1,
...
function formatTimelineResults(
  ...
  const header = translate('tools.conversationSearch.timelineHeader', {
    count: messages.length,
    order: translate(
      order === 'asc' ? 'tools.conversationSearch.orderAsc' : 'tools.conversationSearch.orderDesc'
    ),
    role,
  ...
  const items = messages.map((message, index) =>
    translate('tools.conversationSearch.timelineItem', {
      index: index + 1,
      time: formatTime(message.createdAt),
      role: formatRole(message.role),
      id: message.id,
...
function formatFullMessages(messages: BackendMessageItem[]): string {
  ...
  const items = messages.map((message, index) =>
    translate('tools.conversationSearch.getItem', {
      index: index + 1,
      time: formatTime(message.createdAt),
      role: formatRole(message.role),
      id: message.id,
...
class ConversationSearchTool implements Tool {
  ...
  private async executeGet(
    ...
    try {
      context.onProgress?.(
        translate('tools.conversationSearch.getting', {
          count: ids.length,
        })
      );
