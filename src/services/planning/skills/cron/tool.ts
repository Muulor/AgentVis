...
import { describeCronExpression, isValidCronExpression } from '@services/cron/cronExpression';
...
const SCHEMA: ToolSchema = {
  name: 'cron',
  description:
    'Manage scheduled tasks. Supports creating, listing, updating, and deleting cron jobs so the Agent can set up recurring automation. agentId is provided by the system automatically.',
  parameters: {
    type: 'object',
    properties: {
      action: {
      ...
      cronExpression: {
        type: 'string',
        description:
          'Five-field cron expression: minute hour day-of-month month day-of-week. Example: "0 9 * * *" means 9:00 every day.',
      },
      prompt: {
        type: 'string',
        description:
          'Prompt sent to the Agent when the schedule triggers, used by create and update.',
      },
      jobId: {
        type: 'string',
 ...
 */
async function handleCreate(params: Record<string, unknown>): Promise<ToolResult> {
 ...
 */
async function handleList(params: Record<string, unknown>): Promise<ToolResult> {
  const agentId = params.agentId as string;
 ...
 */
async function handleUpdate(params: Record<string, unknown>): Promise<ToolResult> {
