...
const SCHEMA: ToolSchema = {
    ...
    parameters: {
        ...
        properties: {
            platform: {
                type: 'string',
                description: 'Optional IM platform. Use feishu for Feishu/Lark or slack for Slack. Usually omit it in IM-triggered tasks because the current bot context is injected automatically.',
                enum: ['feishu', 'slack'],
            },
            action: {
                type: 'string',
            ...
            text: {
                type: 'string',
                description: 'Text content for send_text, or an optional caption/summary for send_image/send_file attachments.',
            },
            caption: {
                type: 'string',
            ...
            botId: {
                type: 'string',
                description: 'Optional AgentVis IM bot ID. Usually omit it; IM and cron contexts inject the correct bot automatically.',
            },
        },
        required: ['action'],
    },
...
function formatBotList(bots: BotConfig[]): string {
    return bots
        .map(bot => `- ${bot.displayName} (${bot.platform}, ${bot.botId})`)
        .join('\n');
}
...
function inferPlatformFromTargetParams(params: Record<string, unknown>): SupportedImSendPlatform | undefined {
...
function inferPlatformFromConfiguredBots(context: ToolExecutionContext): { platform?: SupportedImSendPlatform; error?: string } {
    const { botConfigs } = useImChannelStore.getState();
    const usableBots = botConfigs.filter(bot =>
        bot.enabled
        && bot.hasCredentials
        && isSupportedPlatform(bot.platform)
    );
