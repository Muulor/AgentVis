...
const SCHEMA: ToolSchema = {
  name: 'exec',
  description:
    'Execute a shell command. Supports working directory, timeout, and background execution. Sensitive operations require user approval.',
  parameters: {
    type: 'object',
    properties: {
      command: {
      ...
      timeout: {
        type: 'number',
        description:
          'Timeout in seconds. Defaults to 120. Maximum 1800. Use 300-600 for checks/builds, 600-1200 for dependency installs or large builds, and 1200-1800 for model downloads, video renders, or other known long-running work.',
      },
      background: {
        type: 'boolean',
...
function collectDeclaredAttachments(
  ...
  const context = isRecord(root) ? (root.agentvis_context ?? root.agentvisContext) : undefined;
...
export function shouldExtractPlainTextImageAttachmentPaths(command: string): boolean {
  ...
  const hasAutomationTool =
    /\b(?:agent-browser|browser-command(?:\.bat)?|desktop-control|desktop_control(?:\.py)?)\b/.test(
      normalizedCommand
    );
  ...
  return /\b(?:screenshot|snapshot|observe|find_text|click_text|save_image|move_downloaded|capture)\b/.test(
    normalizedCommand
  );
}
...
async function loadAutoImageAttachmentsFromOutput(
  ...
  const candidates = extractExternalImageAttachmentCandidates(output, {
    allowPlainTextFallback: command ? shouldExtractPlainTextImageAttachmentPaths(command) : false,
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      warnings.push(
        translate('tools.exec.imageAttachmentLoadFailed', {
          path: candidate.path,
          error: message,
        })
      );
...
export function normalizePowerShellCommandQuotes(command: string): string {
  ...
  const match = command.match(/^((?:powershell|pwsh)(?:\.exe)?\s+.*?-[Cc]ommand\s+)'(.*)'(\s*)$/s);
...
export function normalizeSmartQuotesForWindowsCommand(command: string): string {
  ...
  const normalizedSingleDelimiters =
    normalizeSmartSingleQuotesOutsideDoubleQuotes(normalizedDoubleQuotes);
...
function isForFCommandSubstitutionContext(command: string, quoteStart: number): boolean {
  return /(?:^|\s)@?for\s+\/f\b[\s\S]*\bin\s*\(\s*$/i.test(
    currentCommandSegmentPrefix(command, quoteStart)
  );
}

function isInlineEvalCodeArgumentContext(command: string, quoteStart: number): boolean {
  return /\b(?:node|python3?(?:\.exe)?)\b[\s\S]*\s-(?:e|c)\s*$/i.test(
    currentCommandSegmentPrefix(command, quoteStart)
  );
}

function shouldReplaceSingleQuotedSegment(
  command: string,
  quoteStart: number,
  inner: string
): boolean {
    ...
    if (char === "'" && !inDoubleQuotes) {
      ...
      if (closingIndex > index) {
        ...
        if (shouldReplaceSingleQuotedSegment(command, index, inner)) {
          result += quoteForWindowsCmdArg(inner, isInlineEvalCodeArgumentContext(command, index));
        } else {
 ...
 */
export function normalizePythonCommand(command: string, venvPythonPath: string): string {
  ...
  // (?:"?)? 匹配可选的后置引号
  // (\s+.*|$) 匹配后续参数或命令结束
  const pythonMatch = trimmed.match(/^("?)(?:python3?(?:\.exe)?)("?)(\s+.*|$)$/i);
...
export function normalizeWindowsCommandLineBreaks(command: string): string {
  ...
  for (let index = 0; index < command.length; index += 1) {
    ...
    const escapedQuote =
      isCaretEscaped(command, index) || (char === '"' && isBackslashEscaped(command, index));
...
export function normalizeFindstrSyntax(command: string): string {
  ...
  return command.replace(/"([^"]*\\\|[^"]*)"/g, (_match, inner: string) => {
...
const MOJIBAKE_PATTERN = /(?:鈥|檙|â€™|â€œ|â€�|â€“|â€”|Ã.|锟�|ï»¿)/;

const EXEC_TIMEOUT_PATTERN =
  /(?:timed?\s*out|timeout|deadline exceeded|execution exceeded|超时|逾时)/i;

const CARGO_TEST_COMMAND_PATTERN = /\bcargo(?:\.exe)?(?:\s+\+\S+)?\s+test\b/i;

const CMD_SET_VARIABLE_PATTERN = /\bset(?:\s+\/a)?\s+"?([A-Za-z_][A-Za-z0-9_]*)=/i;

const CMD_DELAYED_EXPANSION_PATTERN = /\bsetlocal\s+enabledelayedexpansion\b/i;
...
export function generateCmdVariableExpansionHint(command: string): string | null {
  ...
  if (setMatch?.[1]) {
    ...
    if (/[&\r\n]/.test(afterSet) && variableExpansionPattern.test(afterSet)) {
...
const LOCAL_WEB_SERVER_URL_PATTERN =
...
export function generateLocalWebServerVerificationHint(
  stdout: string,
  stderr: string
): string | null {
...
export function collectExecObservationHints(
  ...
  appendExecObservationHint(
    hints,
    generateSilentNonZeroExitHint(command, exitCode, stdout, stderr)
  );
...
function detectSandboxHintReasonKey(command: string, output: string): TranslationKey | undefined {
...
function formatSandboxGuardError(errorMessage: string): string | null {
  ...
  if (reasonCode === 'network_upload_confirmation_required') {
    return translate('tools.sandboxGuard.networkUploadConfirmationRequired', {
      error: errorMessage,
    });
  }
  if (reasonCode === 'network_sensitive_egress_confirmation_required') {
    return translate('tools.sandboxGuard.networkSensitiveEgressConfirmationRequired', {
      error: errorMessage,
    });
  }
  if (reasonCode === 'network_remote_destructive_confirmation_required') {
    return translate('tools.sandboxGuard.networkRemoteDestructiveConfirmationRequired', {
      error: errorMessage,
    });
  }
  ...
  if (reasonCode === 'network_direct_private_session_scope_blocked') {
    return translate('tools.sandboxGuard.networkDirectPrivateSessionBlocked', {
      error: errorMessage,
    });
  }
  return null;
...
function extractBrokerUnusedReasonClass(event: SandboxAuditEvent): string | null {
  return (
    extractAuditDetailValue(event, 'reasonClass') ??
...
function formatDetailedAuditEvent(event: SandboxAuditEvent): string {
  ...
  const detail = [
    auditEventRiskDetail(event),
    event.matchedPattern ? auditValue(event.matchedPattern) : null,
  ]
    .filter(Boolean)
    .join('; ');
...
export function buildExecSandboxAuditSummary(events: SandboxAuditEvent[]): string | null {
  ...
  const brokerBlocks = brokerEvents.filter(
    (event) => event.reason === 'broker_network_block' || event.decision === 'block'
  );
  const wfpCanaryEvents = events.filter(
    (event) => event.backend === 'wfpEnhanced' && event.reason.startsWith('wfp_canary_')
  );
  const wfpCanarySummaryEvents = wfpCanaryEvents.filter(
    (event) => event.reason !== 'wfp_canary_readiness' || event.guardMode === 'wouldBlock'
  );
  ...
  if (brokerRequests.length > 0 || brokerBlocks.length > 0) {
    lines.push(
      translate('tools.exec.auditSummaryBrokerLine', {
        requestCount: brokerRequests.length,
        blockedCount: brokerBlocks.length,
        hosts: uniqueLimited(
          brokerEvents.map((event) => event.targetHost),
          AUDIT_SUMMARY_HOST_LIMIT
        ),
      })
    );
  }
  ...
  if (wfpCanarySummaryEvents.length > 0) {
    lines.push(
      translate('tools.exec.auditSummaryWfpCanaryLine', {
        count: wfpCanarySummaryEvents.length,
        guardModes: uniqueLimited(
          wfpCanarySummaryEvents.map((event) => event.guardMode),
          4
        ),
        reasons: uniqueLimited(
          wfpCanarySummaryEvents.map((event) => event.reason),
          4
        ),
  ...
  const brokerUnusedEvents = events.filter(
    (event) => event.reason === 'broker_proxy_expected_but_unused'
  );
  if (brokerUnusedEvents.length > 0) {
    lines.push(
      translate('tools.exec.auditSummaryBrokerUnusedHint', {
...
async function loadExecSandboxAuditEvents(
  query: ExecAuditQueryContext | null
...
async function buildExecSandboxAuditSummaryForQuery(
  query: ExecAuditQueryContext | null
): Promise<string | null> {
...
export function buildGenericNetworkTargetsCommand(command: string): string | null {
  ...
  const preservedOptions = GENERIC_NETWORK_TARGETS_PASSTHROUGH_OPTIONS.flatMap((option) => {
...
export function resolveGuideSkillEntrypointNetworkDeclaration(
  ...
  for (const skill of skills) {
    ...
    for (const candidate of candidates) {
      ...
      const entry = normalizeEntrypointPath(
        candidate.slice(packagePath.length).replace(/^\/+/, '')
      );
      const mode = Object.entries(skill.agentvisNetworkEntrypoints).find(
        ([declaredEntry]) => normalizeEntrypointPath(declaredEntry).toLowerCase() === entry
      )?.[1];
      if (!mode) continue;
      return {
...
export function resolveExternalSkillCommandReference(
  ...
  for (const skill of skills) {
    ...
    if (
      candidates.some(
        (candidate) => candidate === packagePath || candidate.startsWith(`${packagePath}/`)
      )
    ) {
      return {
        skillName: skill.name,
...
class ExecToolImpl implements Tool {
  ...
  private async resolveExternalSkillCommandReference(
    ...
    await skillLoader.loadAllSkills();
    return resolveExternalSkillCommandReference(command, workdir, skillLoader.getAllSync());
  }
  ...
  private async resolveNetworkDirectTargetsForAuthorization(
    ...
    const { processLifecycle: _preflightProcessLifecycle, ...preflightSandboxOptions } =
      sandboxOptions;
  ...
  async execute(
    ...
    if (sandboxWorkdirViolation) {
      return {
        success: false,
        content:
          sandboxWorkdirViolation.reason === 'missingWorkdir'
    ...
    try {
      ...
      const sandboxPreflightBlock = generateSandboxPreflightCommandBlock(
        command,
        context.sandboxMode
      );
      ...
      try {
        ...
        if (confirmedRiskReasonCode) {
          ...
          result = await invokeShell(
            [...sessionAllowances, ...retryGrant.allowances],
            retryGrant.targets
          );
        }
      } finally {
      ...
      if (stdoutBackendTruncatedBytes > 0) {
        stdout =
          translate('tools.exec.stdoutBackendTruncated', {
            bytes: stdoutBackendTruncatedBytes,
          }) + stdout;
      }
      if (stderrBackendTruncatedBytes > 0) {
        stderr =
          translate('tools.exec.stderrBackendTruncated', {
