...
const SAFE_WINDOWS_CONDITIONAL_PATTERNS: ReadonlyArray<RegExp> = [
  ...
  new RegExp(String.raw`^if\s+exist\s+${WINDOWS_PATH_ARGUMENT}\s+${WINDOWS_TYPE_ACTION}\s*$`, 'i'),
  new RegExp(
...
export function isExecCommandSafe(command: string): boolean {
  ...
  return (
    SAFE_EXEC_PATTERNS.some((pattern) => pattern.test(trimmed)) || isSafeWindowsConditional(trimmed)
  );
}
...
const BLOCKED_EXEC_PATTERNS: ReadonlyArray<{ pattern: RegExp; reasonKey: TranslationKey }> = [
  ...
  {
    pattern: /\bwmic\b.*\b(delete|create|\bset\b|\bcall\b)/i,
    reasonKey: 'tools.execSafety.wmicWrite',
  },
  // 注册表删除
  ...
  {
    pattern: /\[(?:System\.)?Environment\]::SetEnvironmentVariable/i,
    reasonKey: 'tools.execSafety.envSet',
  },
  ...
  {
    pattern:
      /\b[ic]*acls\b.*(\/grant|\/deny|\/remove|\/setowner|\/reset|\/inheritance:[re]).*\b(system32|windows|syswow64)\b/i,
    reasonKey: 'tools.execSafety.aclSystem',
  },
  {
    pattern:
      /\b[ic]*acls\b.*\b(system32|windows|syswow64)\b.*(\/grant|\/deny|\/remove|\/setowner|\/reset|\/inheritance:[re])/i,
    reasonKey: 'tools.execSafety.aclSystem',
  },
  // cacls 特有参数（/G grant, /R revoke, /P replace, /D deny）
  {
    pattern: /\bcacls\b.*\b(system32|windows|syswow64)\b.*\/[grpd]\s/i,
    reasonKey: 'tools.execSafety.caclsSystem',
  },
  {
    pattern: /\bcacls\b.*\/[grpd]\s.*\b(system32|windows|syswow64)\b/i,
    reasonKey: 'tools.execSafety.caclsSystem',
  },
  // PowerShell Set-Acl + 系统路径
  {
    pattern: /\bset-acl\b.*\b(system32|windows|syswow64)\b/i,
    reasonKey: 'tools.execSafety.setAclSystem',
  },
  {
    pattern: /\bset-acl\b.*\b(\$env:systemroot|\$env:windir)\b/i,
    reasonKey: 'tools.execSafety.setAclSystem',
  },
];
