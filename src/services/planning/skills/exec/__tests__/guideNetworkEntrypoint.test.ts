...
const GUIDE_SKILL: SkillDefinition = {
  ...
  mode: 'guide',
  packagePath:
    'C:/Users/Muulo/AppData/Roaming/com.agentvis.app/skills/external/packages/custom-db-guide',
...
describe('Guide Skill network entrypoints for exec', () => {
  it('应从 exec 命令中的绝对脚本路径匹配 Guide frontmatter 入口声明', () => {
    ...
    expect(
      resolveGuideSkillEntrypointNetworkDeclaration(command, undefined, [GUIDE_SKILL])
    ).toEqual({
  ...
  it('should identify installed external skill commands even without network entrypoint metadata', () => {
    ...
    expect(
      resolveExternalSkillCommandReference(command, undefined, [skillWithoutNetworkMetadata])
    ).toEqual({
      skillName: 'custom-db-guide',
      packagePath: GUIDE_SKILL.packagePath,
      mode: 'guide',
