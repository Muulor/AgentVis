    ...
    it('surfaces broker unused and upload confirmation diagnostics', () => {
        const summary = buildExecSandboxAuditSummary([
            auditEvent({
                ...
                guardMode: 'auditOnly',
                matchedPattern: 'reasonCode=broker_proxy_expected_but_unused; reasonClass=potential_direct_egress',
            }),
            auditEvent({
                id: 'upload-1',
                backend: 'broker',
    ...
    it('surfaces structured network risk details before legacy matched pattern text', () => {
        const summary = buildExecSandboxAuditSummary([
            auditEvent({
                ...
                guardMode: 'auditOnly',
                matchedPattern: 'riskClass=sensitiveEgress; riskKind=curlSensitiveBody; pattern=command:curl-sensitive-body',
    ...
    it('surfaces direct-audit DNS risk details for aggregation', () => {
        const summary = buildExecSandboxAuditSummary([
            auditEvent({
                ...
                targetPort: 5432,
                matchedPattern: 'nonHttpOrRawSocket=postgres; target=postgres://127.0.0.1.sslip.io:5432; resolvedRisk=private; reason=hostnameEncodedPrivateOrLocalIp; ips=127.0.0.1',
            }),
        ]);
    ...
    it('surfaces broker encoded-hostname risk details for aggregation', () => {
        const summary = buildExecSandboxAuditSummary([
            auditEvent({
                ...
                targetScheme: 'http',
                matchedPattern: 'targetHost=169-254-169-254.sslip.io; resolvedRisk=metadata; resolvedRiskReason=hostnameEncodedMetadataIp; resolvedIpSamples=169.254.169.254',
