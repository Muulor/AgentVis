...
describe('extractExternalImageAttachmentCandidates', () => {
  ...
  it('should only enable plaintext fallback for image automation commands', () => {
    ...
    expect(
      shouldExtractPlainTextImageAttachmentPaths('browser-command.bat screenshot --annotate')
    ).toBe(true);
    expect(
      shouldExtractPlainTextImageAttachmentPaths('dir /s /b "%APPDATA%\\com.agentvis.app\\*"')
    ).toBe(false);

    const candidates = extractExternalImageAttachmentCandidates(output, {
      allowPlainTextFallback: shouldExtractPlainTextImageAttachmentPaths(
        'browser-command.bat screenshot'
      ),
    });
