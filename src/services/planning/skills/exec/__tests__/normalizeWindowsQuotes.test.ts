...
describe('normalizeWindowsQuotes — PowerShell 场景', () => {
    it('【真实 log 复现】应修正 Get-ChildItem 桌面搜索命令', () => {
        ...
        const input = "powershell -Command 'Get-ChildItem -Path $env:USERPROFILE\\Desktop | Where-Object { $_.Name -like '*网易云*' }'";
    ...
    it('【真实 log 复现】应修正 Program Files 递归搜索命令', () => {
        ...
        const input = "powershell -Command 'Get-ChildItem -Path 'C:\\Program Files', 'C:\\Program Files (x86)' -Recurse -Filter '*cloudmusic*.exe' -ErrorAction SilentlyContinue'";
        const result = normalizeWindowsQuotes(input);
...
describe('normalizeWindowsQuotes — cmd.exe 场景', () => {
    ...
    it('不应破坏管道后 python -c 中的 JSON 字段单引号', () => {
        const input = 'curl -s https://api.github.com/repos/openai/openai-python | python -c “import sys,json; d=json.load(sys.stdin); print(’stars:’, d.get(’stargazers_count’, ’N/A’))”';
        const result = normalizeWindowsQuotes(input);
        expect(result).toBe('curl -s https://api.github.com/repos/openai/openai-python | python -c "import sys,json; d=json.load(sys.stdin); print(\'stars:\', d.get(\'stargazers_count\', \'N/A\'))"');
    });
...
describe('normalizeInlineEvalCommandQuotes', () => {
    ...
    it('不应修改普通脚本路径和带空格参数', () => {
        const input = 'python "C:/Users/Muulo/AppData/Roaming/com.agentvis.app/skills/external/packages/arxiv-search/scripts/arxiv_search.py" search "LLM agent" --limit 15 --sort date';
        expect(normalizeInlineEvalCommandQuotes(input)).toBe(input);
    });
