  ...
  parameters: {
    ...
    properties: {
      ...
      text: {
        type: 'string',
        description:
          'Text content for send_text, or an optional initial comment for send_image/send_file attachments.',
      },
      caption: {
        type: 'string',
      ...
      botId: {
        type: 'string',
        description:
          'Optional AgentVis Slack bot ID. Usually omit it; IM and cron contexts inject the correct bot automatically.',
      },
    },
    required: ['action'],
  },
...
function resolveBotConfig(
  ...
  if (requestedBotId || contextBotId) {
    ...
    if (!bot.hasCredentials) {
      return {
        error: translate('tools.slackSend.botMissingCredentials', { name: bot.displayName }),
      };
    }
    return { bot };
  }

  const enabledSlackBots = botConfigs.filter(
    (bot) => bot.enabled && bot.platform === 'slack' && bot.hasCredentials
  );
...
function buildFileControlBlocks(
  fileId: string,
  fileName: string,
  caption: string
  ...
  async execute(
    ...
    if (!targetResolution.target) {
      return {
        ...
        content:
          ...
          translate('tools.slackSend.noTarget', { name: botResolution.bot.displayName }),
      };
    }
    ...
    try {
      ...
      if (action === 'send_text') {
        ...
        context.onProgress?.(
          translate('tools.slackSend.sendingText', {
      ...
      context.onProgress?.(
        translate(
          action === 'send_image'
            ? 'tools.slackSend.uploadingImage'
            : 'tools.slackSend.uploadingFile',
          {
      ...
      return {
        success: true,
        content: `${translate(
          action === 'send_image' ? 'tools.slackSend.imageSuccess' : 'tools.slackSend.fileSuccess',
          {
            bot: bot.displayName,
            fileName: localFile.fileName,
            target: formatTarget(target),
