...
export function findUniqueVisualSiblingPath(inputPath: string, siblingNames: string[]): string | null {
    ...
    const matches = siblingNames.filter((name) => {
        return (!shouldExcludeSameName || name !== parts.fileName) &&
...
class ReadToolImpl implements Tool {
    ...
    async execute(
        ...
        try {
            ...
            const EFFECTIVE_LIMIT = PLANNING_CONSTANTS.SUB_AGENT_COMPRESS_THRESHOLD_L1 - META_RESERVE_TOKENS;
    ...
    private isPathNotFoundError(errorMessage: string): boolean {
        ...
        return errorMessage.includes('\u4e0d\u5b58\u5728') ||
    ...
    private async readTextContentWithVisualPathFallback(path: string): Promise<ReadContentResolution> {
        try {
            return {
     ...
     */
    private async readImageRaw(
        path: string,
        context: ToolExecutionContext
    ): Promise<ToolResult> {
        const loaded = await loadImageAttachmentFromPath(path, context.workdir);

        logger.trace(`[ReadTool] 📷 直接读取图片(无压缩): ${loaded.path}, MIME: ${loaded.image.mimeType}`);
