 ...
 */
const PYTHON_LOCAL_HELPER_MODULES = new Set(['script_core']);
...
export function analyzeDependencies(
  ...
  const basePackagesLower = new Set(Array.from(basePackageNames).map((p) => p.toLowerCase()));
...
export function extractRequiresComments(text: string): string[] {
  ...
  for (const rawLine of lines) {
    ...
    const requiresMatch = line.match(/^#\s*requires:\s*pip\s+install\s+(.+)/i);
    if (requiresMatch?.[1]) {
      // 支持空格分隔和逗号分隔
 ...
 * `and` / `retry` 当作包名。
 */
function extractPipPackagesFromCommand(commandText: string, packages: Set<string>): void {
 ...
 * 因此不会误匹配自然语言描述。
 */
function extractNpmPackagesFromCommand(commandText: string, packages: Set<string>): void {
 ...
 * 提取第一个非标志参数作为包名。
 */
function extractNpxPackageFromCommand(commandText: string, packages: Set<string>): void {
 ...
 * 提取第一个非标志参数作为包名。
 */
function extractCargoPackageFromLine(line: string, packages: Set<string>): void {
 ...
 */
function extractGoPackageFromLine(line: string, packages: Set<string>): void {
...
export function extractSystemToolHints(markdown: string): SystemToolInfo[] {
  ...
  for (const block of extractMarkdownCodeBlocks(markdown)) {
    ...
    for (const blockLine of blockLines) {
      ...
      if (
        SYSTEM_TOOL_REGISTRY[cmdName] &&
        !foundPackages.has(SYSTEM_TOOL_REGISTRY[cmdName].packageName)
      ) {
  ...
  for (const block of extractMarkdownCodeBlocks(markdown)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        ...
        const cleanedCmd = lineWithoutComment.split(/\s+&&\s+|\s+\|\|\s+|\s*;\s*/)[0]?.trim() ?? '';
