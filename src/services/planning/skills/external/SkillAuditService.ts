...
function isWithinPackagePath(targetPath: string, packagePath: string): boolean {
  ...
  return normalizedTarget.startsWith(packagePrefix) || normalizedTarget === normalizedPackage;
}
...
function resolveAuditReadPath(inputPath: string, packagePath: string): string {
  ...
  const normalizedInput = inputPath.replace(/\\/g, sep).replace(/^\.?[\\/]+/, '');
 ...
 */
function normalizeAuditResult(raw: Record<string, unknown>, language: Language): SkillAuditResult {
  const auditResult = normalizeVerdict((raw.audit_result ?? raw.auditResult) as string | undefined);

  const riskScore = clampNumber(raw.risk_score ?? raw.riskScore, 1, 10, 5);
  const confidence = normalizeConfidence(raw.confidence as string | undefined);
...
export function buildAuditOutputLanguageInstruction(language: Language): string {
  const readableLanguage = language === 'zh-CN' ? 'Simplified Chinese (zh-CN)' : 'English (en-US)';
...
export function buildAuditTaskDescription(packagePath: string, fileList: string[]): string {
  ...
  const scriptFiles = fileList.filter((f) =>
    /\.(py|sh|js|ts|tsx|jsx|mjs|cjs|bat|cmd|ps1|rb|go|java|php|lua|pl|r)$/i.test(f)
  );
  const configFiles = fileList.filter((f) =>
    /\.(json|yaml|yml|toml|ini|env|xml|cfg|conf|properties|csv)$/i.test(f)
  );
  const docFiles = fileList.filter(
    (f) => /\.(md|txt|rst|html|htm)$/i.test(f) && !rootSkillFiles.includes(f)
  );
  const assetFiles = fileList.filter((f) => {
    ...
    return (
      isAssetPath && !scriptFiles.includes(f) && !configFiles.includes(f) && !docFiles.includes(f)
    );
  });
  ...
  if (scriptFiles.length > 0) {
    lines.push(
      `${step}. First use the read tool to inspect all script/code files (high priority): ${scriptFiles.join(', ')}`
    );
    step++;
  }
  ...
  if (assetFiles.length > 0) {
    lines.push(
      `${step}. Inspect assets by path/name and read only text-like or security-relevant assets when needed: ${assetFiles.join(', ')}`
    );
    step++;
  }
  ...
  if (otherFiles.length > 0) {
    lines.push(
      `${step}. Inspect remaining scoped files when security-relevant: ${otherFiles.join(', ')}`
    );
    step++;
  }
 ...
 */
const noopCheckpoint: CheckpointCallback = () =>
  Promise.resolve({
...
export async function auditSkillPackage(
  ...
  try {
    ...
    logger.debug(`[SkillAuditService] 使用模型: ${config.providerId}/${config.modelId}`);
    ...
    const toolExecutor = async (toolCall: {
      ...
      if (toolCall.name !== 'read') {
        logger.warn(`[SkillAuditService] 拒绝非 read 工具调用: ${toolCall.name}`);
        return {
          success: false,
      ...
      if (!isWithinPackagePath(targetPath, packagePath)) {
        logger.warn(`[SkillAuditService] 路径越权被拦截: ${targetPath} (包范围: ${packagePath})`);
        return {
          success: false,
      ...
      if (!relativeTargetPath || !allowedAuditFiles.has(normalizeAuditPath(relativeTargetPath))) {
        logger.warn(`[SkillAuditService] Path outside audit allowlist, refused: ${targetPath}`);
        return {
          success: false,
    ...
    const auditSpec: SubAgentSpec = {
      ...
      terminationCondition:
        'After reviewing all files, output a JSON audit verdict that follows the Output Schema',
    };
    ...
    return {
      ...
      riskScore: 5,
      confidence: 'LOW',
      summary: translate('settings.skills.auditExecutionFailed', { error: errorMsg }, language),
      intentMismatch: false,
      detectedCapabilities: [],
      findings: [],
    };
...
async function scanDirRecursive(
  ...
  const sep = basePath.includes('\\') ? '\\' : '/';
  const currentDir = relativePath ? `${basePath}${sep}${relativePath}` : basePath;
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const entryRelative = relativePath ? `${relativePath}${sep}${entry}` : entry;
