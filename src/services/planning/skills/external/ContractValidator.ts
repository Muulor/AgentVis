 ...
 * 参数验证结果
 */
export type ArgValidationResult = { valid: true } | { valid: false; errors: string[] };
 ...
 */
export function validateContract(frontmatter: ExternalSkillFrontmatter): ValidationResult {
  const errors: string[] = [];
  ...
  if (rawPermissions !== undefined) {
    if (
      ...
      if (networkPermission !== undefined && typeof networkPermission !== 'boolean') {
      ...
      if (longRunningPermission !== undefined && typeof longRunningPermission !== 'boolean') {
      ...
      if (desktopLaunchPermission !== undefined && typeof desktopLaunchPermission !== 'boolean') {
      ...
      if (desktopControlPermission !== undefined && typeof desktopControlPermission !== 'boolean') {
  ...
  if (exec.timeout !== undefined) {
    const maxTimeout = longRunning ? LONG_RUNNING_MAX_TIMEOUT_SECONDS : DEFAULT_MAX_TIMEOUT_SECONDS;
...
function validateArgContractMetadata(arg: Partial<ContractArg>, prefix: string): string[] {
  const errors: string[] = [];
  const type = arg.type;
...
function validateArgValueConstraints(key: string, value: unknown, argDef: ContractArg): string[] {
  const errors: string[] = [];
  ...
  if (
    ...
    errors.push(`Argument ${key} must be one of: ${formatAllowedValues(argDef.allowedValues)}`);
  }
...
function validateFilesystemGrants(grants: unknown[], argsSchema: unknown): string[] {
  const errors: string[] = [];
  ...
  grants.forEach((rawGrant, index) => {
    ...
    if (typeof access !== 'string' || !SUPPORTED_FILESYSTEM_ACCESS.includes(access)) {
...
function validateBrokerCredentials(credentials: unknown[]): string[] {
  ...
  for (let i = 0; i < credentials.length; i++) {
    ...
    if (
      ...
      !isSafeCredentialIdentifier(credential.id)
    ) {
      errors.push(
        `${prefix}: id must contain only letters, numbers, dots, underscores, or hyphens`
      );
    ...
    if (
      ...
      !isSafeCredentialIdentifier(credential.provider)
    ) {
      errors.push(
        `${prefix}: provider must contain only letters, numbers, dots, underscores, or hyphens`
      );
    }
    ...
    if (
      ...
      !SUPPORTED_CREDENTIAL_MODES.includes(credential.mode)
    ) {
    ...
    if (!Array.isArray(credential.hosts) || credential.hosts.length === 0) {
      ...
      for (const host of credential.hosts) {
        if (typeof host !== 'string' || !isExactPublicHost(host)) {
          errors.push(
            `${prefix}: hosts must be exact host names without scheme, port, wildcard, or path`
          );
          break;
        }
      }
    ...
    if (
      ...
      !isSafeHeaderName(credential.headerName)
    ) {
      errors.push(`${prefix}: headerName must be a safe HTTP header name`);
    }

    if (
      typeof credential.headerValuePrefix !== 'string' ||
      hasHeaderControlChars(credential.headerValuePrefix)
    ) {
