...
describe('bundled Script Skills', () => {
    it('应加载带凭据策略的 bundled brokerOnly Script Skill contract', async () => {
        ...
        expect(names).toEqual(expect.arrayContaining(['github-lookup', 'context7-docs', 'agnes-video', 'agnes-image']));
        for (const skill of scriptSkills.filter(skill =>
    ...
    it('应按真实 contract 校验 bundled Script Skill 的参数', async () => {
        ...
        expect(validateArgs({
            ...
            query: 'hermes agent',
            limit: 3,
        }, githubLookup!.contract!)).toEqual({ valid: true });
        ...
        expect(validateArgs({
            ...
            outputFormat: 'text',
            limit: 3,
        }, context7Docs!.contract!)).toEqual({ valid: true });
        ...
        expect(validateArgs({
            ...
            frameRate: 24,
            outputFormat: 'json',
        }, agnesVideo!.contract!)).toEqual({ valid: true });
        expect(validateArgs({
            action: 'status',
            taskId: 'task_123456',
            skipDownload: true,
        ...
        expect(validateArgs({
            ...
            size: '1024x1024',
            outputFormat: 'json',
        }, agnesImage!.contract!)).toEqual({ valid: true });
        expect(validateArgs({
            action: 'generate',
