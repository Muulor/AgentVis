...
describe('ExternalExecutor.buildCommand', () => {
  ...
  it('Bash runtime 应该使用 bash 解释器', () => {
    ...
    const cmd = executor.buildCommand(bashContract, {}, '/packages/my-tool');
  ...
  it('Node runtime 应该使用 node 解释器', () => {
    ...
    const cmd = executor.buildCommand(nodeContract, {}, '/packages/my-tool');
...
describe('ExternalExecutor.execute', () => {
  ...
  it('shell 沙箱阻断应该返回明确的 sandbox 错误', async () => {
    const shell = vi
      .fn()
      .mockRejectedValue(new Error('Sandbox block: network API was detected in script.'));
    const executor = new ExternalExecutor(shell);
  ...
  it('应该将正确的 timeout 传递给 Shell', async () => {
    ...
    await executor.execute(customContract, { file_path: '/test.csv' }, '/packages/csv-analyzer');

    expect(shell).toHaveBeenCalledWith(expect.objectContaining({ timeout: 120 }));
  });
  ...
  it('默认应该以外部 Skill 网络审计沙箱执行', async () => {
    ...
    await executor.execute(BASE_CONTRACT, { file_path: '/test.csv' }, '/packages/csv-analyzer');
  ...
  it('remote destructive checkpoint should request confirmation and retry once', async () => {
    ...
    const shell = vi
      ...
      .mockRejectedValueOnce(
        new Error(
          'Sandbox block [network_remote_destructive_confirmation_required]: delete detected.'
        )
      )
      .mockResolvedValueOnce({
        exitCode: 0,
  ...
  it('legacyNonHttp entrypoint should preflight targets, request authorization, and retry', async () => {
    vi.mocked(invoke)
      ...
      .mockResolvedValueOnce([{ protocol: 'imap', host: 'imap.qq.com', port: 993 }]);
    ...
    const shell = vi
      ...
      .mockResolvedValueOnce({
        ...
        stdout: JSON.stringify({
          targets: [{ protocol: 'imap', host: 'imap.qq.com', port: 993 }],
        }),
        stderr: '',
      })
      .mockResolvedValueOnce({
    ...
    expect(shell).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        networkDirectTargets: [{ protocol: 'imap', host: 'imap.qq.com', port: 993 }],
      })
    );
  });
});
