...
function createMockEmbeddingService(
    overrides?: Partial<EmbeddingServiceDep>
): EmbeddingServiceDep {
    return {
...
describe('SkillRetriever', () => {
    ...
    describe('register - 异常处理', () => {
        it('embedding 服务失败时应该降级为「仅关键词」模式（L1 仍可用）', async () => {
            const service = createMockEmbeddingService({
                encodeBatch: vi.fn().mockRejectedValue(
            ...
            await retriever.register([
                createGuideSkill('elite-coding', 'Always use this skill when user requirements involve coding'),
            ]);
    ...
    describe('retrieve - L1 关键词触发', () => {
        ...
        it('hyphenated skill names should match space-separated user text without triggers', async () => {
            ...
            await retriever.register([
                createGuideSkill('Marketing-Ideas', 'Marketing ideation guide'),
            ]);

            const results = await retriever.retrieve('我已经安装了Marketing Ideas这个skill，看看怎么使用');
    ...
    describe('retrieve - L2 Multi-Fragment 向量匹配', () => {
        ...
        it('过短的 fragment 应该被过滤', async () => {
            ...
            await retriever.register([
                createGuideSkill('test-skill', 'some description'),
            ]);
    ...
    describe('retrieve - 两层合并 + 复合任务', () => {
        ...
        it('L1 和 L2 同时命中同一技能时应取 max score', async () => {
            ...
            await retriever.register([
                createGuideSkill('pptx', 'PPTX 处理', { triggers: ['PPT'] }),
            ]);
    ...
    describe('clear - 索引清空', () => {
        ...
        it('clear() 后重新 register 应该正常工作', async () => {
            ...
            retriever.clear();
            await retriever.register([
                createGuideSkill('docx', 'Word'),
                createGuideSkill('csv', 'CSV'),
            ]);
