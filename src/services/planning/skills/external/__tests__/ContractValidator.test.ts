...
describe('validateContract', () => {
    ...
    it('permissions.networkMode 非支持值时应该失败', () => {
        ...
        expect(result.valid).toBe(false);
        if (!result.valid) {
            expect(result.errors).toContain('execution.permissions.networkMode must be direct or brokerOnly');
        }
    });
    ...
    it('permissions.network=false 与 brokerOnly 同时声明时应该失败', () => {
        ...
        expect(result.valid).toBe(false);
        if (!result.valid) {
            expect(result.errors).toContain('execution.permissions.network=false conflicts with networkMode=brokerOnly');
        }
    });
