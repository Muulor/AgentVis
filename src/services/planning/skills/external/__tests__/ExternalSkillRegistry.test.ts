    ...
    describe('loadAll - 正常加载', () => {
        it('应该正确加载 Guide 和 Script 模式技能', async () => {
            ...
            expect(csv!.contract!.permissions).toEqual({
                ...
                desktopControl: false,
                filesystem: [
                    { fromArg: 'file_path', access: 'readOnly' },
                ],
            });
        });
...
`;
            const files = new Map([
                [REGISTRY_PATH, registryOnlyDisabled],
            ]);
...
`;
            const files = new Map([
                [REGISTRY_PATH, registryConflict],
            ]);
...
`;
            const files = new Map([
                [REGISTRY_PATH, registryWrongVersion],
            ]);
            const dirs = new Set<string>();
...
`;
        ...
        it('listFiles 异常时 scriptFiles 应为 undefined', async () => {
            ...
            const listFilesError: ListFilesFn = vi.fn().mockRejectedValue(
                new Error('Permission denied')
            );
            const loader = new ExternalSkillRegistryLoader(
                PACKAGES_DIR, readFile, dirExists, listFilesError
            );
