...
async function reconcileVenvStateOnce(): Promise<void> {
  try {
    ...
    if (hermeticity.status === 'nonHermetic') {
      ...
      logger.warn('[ExternalSkillBootstrap] venv 存在但未通过 runtime 健康检查，已标记为需要重建');
    }
...
function toInstalledSkillInfo(
  ...
  const installableExplicitPackages = filterRuntimeBasePackages(explicitPackages, basePackageNames);
...
export function summarizeFailedDependencyImpact(
  ...
  const failedIdentities = new Set(
    (failedPackages ?? []).map(getPythonPackageIdentity).filter(Boolean)
  );
  ...
  return {
    ...
    packageSpecs:
      packageSpecs.size > 0 ? Array.from(packageSpecs.values()) : (failedPackages ?? []),
  };
}
...
export function filterRuntimeBasePackages(
  ...
  const basePackagesLower = new Set(Array.from(basePackageNames).map((pkg) => pkg.toLowerCase()));
 ...
 * @returns 本地模块名集合
 */
function buildLocalModuleNames(scriptFiles: string[], packagePath: string): Set<string> {
...
async function scanAndRegister(): Promise<LoadedExternalSkill[]> {
  ...
  if (!packagesDirExists) {
    logger.debug('[ExternalSkillBootstrap] packages 目录不存在:', packagesDir);
    return [];
  }

  // 扫描
  const loader = new ExternalSkillRegistryLoader(packagesDir, readFile, dirExists, listFiles);

  const result = await loader.scanAll();

  // 输出警告
  if (result.warnings.length > 0) {
    logger.warn('[ExternalSkillBootstrap] 加载警告:', result.warnings);
  }
  ...
  for (const skill of result.skills) {
    if (skill.dependencies?.packages && skill.dependencies.packages.length > 0) {
      ...
      if (skill.fullContent) {
        ...
        const localModuleNames = buildLocalModuleNames(skill.scriptFiles ?? [], skill.packagePath);
...
async function installPendingDependenciesIfReady(): Promise<SkillImportResult | null> {
  ...
  logger.debug(`[ExternalSkillBootstrap] 触发增量依赖安装:`, Array.from(allPackages).join(', '));
  ...
  try {
    ...
    if (result.status === 'ready') {
      ...
      const failedImpact = summarizeFailedDependencyImpact(
        pendingDependencies,
    ...
    const {
      setEnvStatus,
      setInstallProgress,
      pendingDependencies: failedDeps,
    } = useRuntimeStore.getState();
    setEnvStatus('ready');
    setInstallProgress(null);
...
function launchBackgroundInstall(): void {
  ...
  const timeoutPromise = new Promise<null>((resolve) =>
    setTimeout(() => {
      ...
      const {
        envStatus,
        pendingDependencies: timedOutDeps,
        setEnvStatus,
        setInstallProgress,
      } = useRuntimeStore.getState();
  ...
  Promise.race([installPendingDependenciesIfReady(), timeoutPromise]).catch((error: unknown) => {
...
export function bootstrapExternalSkills(): Promise<void> {
  ...
  bootstrapPromise = (async () => {
    try {
      ...
      logger.debug(
        `[ExternalSkillBootstrap] 外部技能初始化完成:` + ` ${skills.length} 个技能已注册`
      );
    } catch (error) {
...
export async function rescanExternalSkills(): Promise<void> {
  try {
    ...
    const skills = await scanAndRegister();
    logger.debug(`[ExternalSkillBootstrap] rescan 完成: ${skills.length} 个技能`);
