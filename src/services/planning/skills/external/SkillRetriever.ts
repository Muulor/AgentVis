...
export class SkillRetriever {
    ...
    async register(skills: LoadedExternalSkill[]): Promise<void> {
        // 仅索引 Guide 模式且已启用的技能
        const guideSkills = skills.filter(
            (s) => s.mode === 'guide' && s.enabled
        );
        ...
        const indexTexts = guideSkills.map(
    ...
    private keywordMatch(query: string): SkillRetrievalResult[] {
        ...
        for (const entry of this.entries) {
            ...
            const isRawHit = entry.normalizedTriggers.some(
                (trigger) => queryLower.includes(trigger)
            );
            const isCanonicalHit = entry.canonicalTriggers.some(
                (trigger) => containsCanonicalTrigger(canonicalQuery, trigger)
            );
            const isCompactHit = entry.compactTriggers.some(
                (trigger) => compactQuery.includes(trigger)
            );
    ...
    private async multiFragmentVectorMatch(
        ...
        const fragmentEmbeddings = fragments.length === 1
     ...
     */
    private buildTriggerIndex(skill: LoadedExternalSkill): Pick<
        SkillEmbeddingEntry,
        'normalizedTriggers' | 'canonicalTriggers' | 'compactTriggers'
    > {
