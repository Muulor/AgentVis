...
export class ExternalExecutor {
    ...
    async execute(
        ...
        try {
            ...
            if (this.requiresDesktopCapability(contract) && options.sandboxMode && options.sandboxMode !== 'LocalAudit') {
                return {
                    exitCode: -1,
                    stdout: '',
            ...
            const appContainerFilesystemGrants = this.resolveAppContainerFilesystemGrants(
                contract,
                args
            );
            ...
            logger.trace('[ExternalExecutor] Script Skill shell execution prepared', {
                ...
                filesystemGrantArgs: contract.permissions?.filesystem?.map(grant => grant.fromArg).sort() ?? [],
    ...
    private async executeWithNetworkDirectAuthorization(
        ...
        const sessionAllowances = options.enableNetworkDirectAuthorization
            ...
            : [];
        const firstAttemptParams = sessionAllowances.length > 0
            ? {
                ...shellParams,
    ...
    private requiresDesktopCapability(contract: ExecutionContract): boolean {
        return contract.permissions?.desktopLaunch === true ||
            contract.permissions?.desktopControl === true;
    }
    ...
    private resolveSandboxOptions(
        ...
        contract: ExecutionContract
    ): Pick<Parameters<ShellExecuteFn>[0], 'sandboxMode' | 'sandboxLevel' | 'sandboxNetwork' | 'networkScope'> {
        switch (sandboxMode ?? 'LocalAudit') {
            ...
            case 'ControlledNetwork':
                ...
                return {
                    ...
                    networkScope: this.resolveSandboxNetwork(contract) === 'blocked' ? 'blocked' : 'internetAudit',
                };
            case 'LocalAudit':
            default:
                return {
    ...
    private formatExecutionError(errorMessage: string): string {
        if (
            ...
            if (reasonCode === 'proxy_bypass_signal_blocked') {
                formatted = translate('tools.sandboxGuard.proxyBypassSignalBlocked', { error: errorMessage });
            } else if (reasonCode === 'broker_proxy_required_unavailable') {
                formatted = translate('tools.sandboxGuard.brokerProxyRequiredUnavailable', { error: errorMessage });
            } else if (reasonCode === 'network_upload_confirmation_required') {
                formatted = translate('tools.sandboxGuard.networkUploadConfirmationRequired', { error: errorMessage });
            } else if (reasonCode === 'network_sensitive_egress_confirmation_required') {
                formatted = translate('tools.sandboxGuard.networkSensitiveEgressConfirmationRequired', { error: errorMessage });
            } else if (reasonCode === 'network_remote_destructive_confirmation_required') {
                formatted = translate('tools.sandboxGuard.networkRemoteDestructiveConfirmationRequired', { error: errorMessage });
            } else if (reasonCode === 'network_direct_metadata_target_blocked') {
                formatted = translate('tools.sandboxGuard.networkDirectMetadataBlocked', { error: errorMessage });
            } else if (reasonCode === 'network_direct_private_session_scope_blocked') {
                formatted = translate('tools.sandboxGuard.networkDirectPrivateSessionBlocked', { error: errorMessage });
            } else {
