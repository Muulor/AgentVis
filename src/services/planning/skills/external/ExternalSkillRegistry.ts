...
export class ExternalSkillRegistryLoader {
    ...
    private static readonly RESOURCE_EXTENSIONS = new Set([
        '.md', '.pdf', '.txt', '.json', '.yaml', '.yml', '.csv', '.html', '.css',
    ]);
    ...
    async scanAll(): Promise<RegistryLoadResult> {
            ...
            try {
                ...
                if (skill) {
                    ...
                    if (registeredNames.has(skill.name)) {
                        warnings.push(
                            `Duplicate skill name "${skill.name}" (directory ${dirName}); skipped`
                        );
                        continue;
                    }
                    ...
                    skills.push(skill);
                    logger.trace(
                        `[ExternalSkillRegistry] 已加载技能: ${skill.name} (${skill.mode} 模式)`
                    );
                }
            } catch (error) {
                const message = error instanceof Error ? error.message : String(error);
        ...
        logger.trace(
            `[ExternalSkillRegistry] 扫描完成: ${skills.length} 个技能,` +
            ` ${warnings.length} 条警告`
        );
    ...
    async loadAll(registryPath: string): Promise<RegistryLoadResult> {
        ...
        logger.trace(
            `[ExternalSkillRegistry] 加载完成: ${skills.length} 个技能,` +
            ` ${warnings.length} 条警告`
        );
    ...
    private normalizeAgentVisNetworkEntrypoints(value: unknown): SkillAgentVisNetworkEntrypoints | undefined {
        if (value === undefined || value === null || value === '') {
            return undefined;
        }
    ...
    private normalizeAgentVisNetworkEntrypointMode(value: string): SkillAgentVisNetworkEntrypointMode {
        const normalized = value.trim();
    ...
    private async collectResourceFiles(packagePath: string): Promise<string[]> {
        try {
            ...
            return result.filter(filePath => {
                ...
                return !ExternalSkillRegistryLoader.RESOURCE_EXCLUDE_FILES.has(lowerName)
    ...
    private parseSkillMd(
        content: string
    ): { frontmatter: ExternalSkillFrontmatter; body: string } {
        ...
        const frontmatter = this.parseFrontmatterYaml(
            ...
            if (indent >= 4 && level1Key) {
                ...
                if (trimmed.startsWith('- ')) {
                    ...
                    if (level3Key) {
                        if (
                            level3CurrentArrayItem &&
                            Object.keys(level3CurrentArrayItem).length > 0
                        ) {
