...
export class ExternalToolProvider implements ToolProvider {
  ...
  registerSkill(skill: LoadedExternalSkill): void {
    ...
    const tool = new ExternalTool(skill, this.executor, this.venvPath);
...
class ExternalTool implements Tool {
  ...
  constructor(skill: LoadedExternalSkill, executor: ExternalExecutor, venvPath?: string) {
    if (!skill.contract) {
  ...
  async execute(
    ...
    // Step 2: 报告进度
    context.onProgress?.(`Executing external tool: ${this.skill.name}`);
    ...
    const result = await this.executor.execute(
      ...
      this.venvPath,
      {
        ...
        networkFallback: resolveSkillEntrypointNetworkFallback(networkEntrypointMode, this.skill),
