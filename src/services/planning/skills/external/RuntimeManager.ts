 ...
 */
const WINDOWS_PYTHON_CANDIDATES = ['python', 'python3', 'py -3'] as const;
...
export class RuntimeManager {
   ...
   */
  constructor(runtimeDir: string, shellExecute: ShellExecuteFn, pythonBinaryPath?: string) {
    ...
    try {
      ...
      if (allExtraPackages.length > 0) {
        ...
        if (packagesToInstall.length > 0) {
          extraPackageFailures = await this.installExtraPackagesWithFallback(
            packagesToInstall,
            onProgress
          );
        } else {
      ...
      if (extraPackageFailures.length > 0) {
        ...
        return {
          ...
          pythonVersion,
          error: this.formatExtraPackageFailureMessage(
            extraPackageFailures,
            allExtraPackages.length
          ),
  ...
  async detectPython(): Promise<string | null> {
    ...
    if (this.pythonBinaryPath) {
      ...
      logger.warn('[RuntimeManager] 注入的 Python 路径无效:', this.pythonBinaryPath);
    }
  ...
  private async createVenvWithRollback(): Promise<void> {
    ...
    const isEmbedded = this.embeddedPythonExe !== null && pythonCommand === this.embeddedPythonExe;
    ...
    try {
      ...
      logger.trace(`[RuntimeManager] 创建 venv 命令: ${createCmd.slice(0, 120)}`);
      ...
      if (result.exitCode !== 0) {
        throw new Error(`Failed to create venv (exit ${result.exitCode}): ${result.stderr}`);
      }
  ...
  private async bootstrapEmbeddedPipAndVirtualenv(embeddedPythonExe: string): Promise<void> {
    ...
    if (pipCheck.exitCode !== 0) {
      ...
      if (!this.embeddedGetPipPath) {
        throw new Error(
          'get-pip.py path is not configured, so pip cannot be bootstrapped for the embedded Python runtime'
        );
      }
  ...
  private async ensureVenvSslAvailable(): Promise<void> {
    ...
    if (result.exitCode !== 0 || this.containsPythonSslFailure(finalOutput)) {
      this.status = 'base_incomplete';
      throw new Error(
        translate('runtime.errors.pythonSslUnavailable', {
  ...
  private isAppManagedEmbeddedRuntimeDir(path: string): boolean {
    ...
    const normalizedRuntimeRoot = runtimeRoot
      .replace(/^["']|["']$/g, '')
      .replace(/\\/g, '/')
      .toLowerCase();
  ...
  private async validatePrebuiltBasePackages(packages: string[]): Promise<void> {
    ...
    const moduleList = modules
      .map((moduleName) => this.toPythonSingleQuotedString(moduleName))
      .join(',');
    const pythonBin = this.getPythonBin();
    const script = [
    ...
    const output = this.combineCommandOutput(result.stderr, result.stdout);
    throw new Error(
      translate('runtime.errors.prebuiltRuntimeMissingBasePackages', {
  ...
  private getBasePackageImportNames(packages: string[]): string[] {
    ...
    for (const packageSpec of packages) {
      ...
      modules.add(
        BASE_PACKAGE_IMPORT_NAME_OVERRIDES.get(packageName) ?? packageName.replace(/-/g, '_')
      );
    }
  ...
  async installBasePackages(packages: string[], onProgress?: ProgressCallback): Promise<void> {
    ...
    for (const batch of batches) {
      ...
      const percent =
        PROGRESS_START +
        Math.round((processedCount / packages.length) * (PROGRESS_END - PROGRESS_START));

      // 提取包名（去掉版本号）用于 UI 显示
      const batchNames = batch.map(
        (pkg) => (pkg.split('==')[0] ?? pkg).split('>=')[0]?.split('<=')[0]?.split('~=')[0] ?? pkg
      );
      onProgress?.({
  ...
  private async installPackageBatch(
    ...
    logger.trace(
      `[RuntimeManager] [${batchLabel}] 批量安装: ${batch.join(', ')} (超时: ${batchTimeout}s)`
    );
  ...
  private async probeExtraPackageSatisfaction(
    ...
    if (result.exitCode !== 0) {
      logger.debug(
        ...
        this.sanitizeFailureOutput(this.combineCommandOutput(result.stderr, result.stdout)) ||
  ...
  private formatBasePackageFailureMessage(
    ...
    if (failures.length > details.length) {
      details.push(
        translate('runtime.errors.packageInstallFailureMore', {
  ...
  private sanitizeFailureOutput(output: string): string {
    const cleaned = output.replace(ANSI_ESCAPE_SEQUENCE_RE, '').replace(/\s+/g, ' ').trim();
  ...
  private containsPythonSslFailure(output: string): boolean {
    return /no module named ['"]?_ssl['"]?|ssl module.*not available|can't connect to https url because the ssl module is not available/i.test(
      output
    );
  }
  ...
  private async removeVenvDirectory(): Promise<void> {
    try {
      ...
      const command = isWindows ? `rmdir /s /q "${this.venvPath}"` : `rm -rf "${this.venvPath}"`;
   ...
   */
  private aggregateExtraDependencies(dependencies: SkillDependencies[]): string[] {
