...
const SCHEMA: ToolSchema = {
  ...
  description:
    'Execute an installed external Script Skill by exact skillName using its declared Execution Contract.',
  parameters: {
    type: 'object',
    properties: {
      skillName: {
...
function findExternalSkillByName(name: string): SkillDefinition | undefined {
  return skillLoader
    .getAllSync()
    .find((skill) => skill.source === 'external' && skill.name === name);
}
...
class ExternalScriptSkillExecuteToolImpl implements Tool {
  ...
  async execute(
    ...
    const networkEntrypointMode = resolveSkillEntrypointNetworkMode(skill, skill.contract.entry);
    const result = await this.executor.execute(
      ...
      resolveVenvRoot(context.venvPythonPath),
      {
        ...
        networkFallback: resolveSkillEntrypointNetworkFallback(networkEntrypointMode, skill),
