...
import {
    buildOutboundMessageCard,
} from '@services/im-channel/cardTemplates';
...
const IMAGE_EXTENSIONS = new Set(['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp', '.ico', '.tif', '.tiff', '.heic']);
...
const SCHEMA: ToolSchema = {
    ...
    parameters: {
        ...
        properties: {
            ...
            text: {
                type: 'string',
                description: 'Text content for send_text, or an optional caption/summary sent before send_image/send_file attachments.',
            },
            caption: {
                type: 'string',
                description: 'Optional caption or status summary to send before an image or file attachment.',
            },
            filePath: {
                type: 'string',
            ...
            receiveIdType: {
                type: 'string',
                description: 'Optional Feishu receiver ID type. Supported: chat_id, open_id, user_id, union_id, email. Defaults to the bot setting or chat_id.',
                enum: ['chat_id', 'open_id', 'user_id', 'union_id', 'email'],
            },
            receiveId: {
                type: 'string',
                description: 'Optional Feishu receiver ID. Examples: chat_id starting with oc_, open_id, user_id, union_id, or email.',
            },
            botId: {
                type: 'string',
                description: 'Optional AgentVis Feishu bot ID. Usually omit it; IM and cron contexts inject the correct bot automatically.',
            },
        },
        required: ['action'],
    },
...
function resolveBotConfig(
    ...
    if (requestedBotId || contextBotId) {
        ...
        if (!bot.hasCredentials) {
            return { error: translate('tools.feishuSend.botMissingCredentials', { name: bot.displayName }) };
        }
        return { bot };
    }

    const enabledFeishuBots = botConfigs.filter(bot =>
        bot.enabled
        && bot.platform === 'feishu'
        && bot.hasCredentials
    );
...
async function resolveTarget(
    ...
    if (directReceiveId) {
        const explicitType = normalizeReceiveIdType(getStringParam(params, 'receiveIdType', 'receive_id_type'));
        const receiveIdType = explicitType ?? (getStringParam(params, 'chatId', 'chat_id') ? 'chat_id' : null);
        if (!receiveIdType) {
...
async function sendOptionalAttachmentText(
    ...
    context.onProgress?.(translate('tools.feishuSend.sendingAttachmentText', {
...
function shouldAttachTargetHint(error: string): boolean {
    ...
    return normalized.includes('failed to send feishu message')
...
class FeishuSendToolImpl implements Tool {
    ...
    async execute(
        ...
        if (!targetResolution.target) {
            return {
                success: false,
                content: targetResolution.error ?? translate('tools.feishuSend.noTarget', { name: botResolution.bot.displayName }),
            };
        }
        ...
        try {
            ...
            if (action === 'send_text') {
                ...
                context.onProgress?.(translate('tools.feishuSend.sendingText', {
                    target: formatTarget(target),
                }));
            ...
            if (action === 'send_image') {
                ...
                context.onProgress?.(translate('tools.feishuSend.uploadingImage', {
            ...
            context.onProgress?.(translate(isVideoMessage ? 'tools.feishuSend.uploadingVideo' : 'tools.feishuSend.uploadingFile', {
                fileName: localFile.fileName,
                target: formatTarget(target),
            }));
            ...
            const successKey = textMessageId
                ? isVideoMessage ? 'tools.feishuSend.videoSuccessWithText' : 'tools.feishuSend.fileSuccessWithText'
                : isVideoMessage ? 'tools.feishuSend.videoSuccess' : 'tools.feishuSend.fileSuccess';
            const content = translate(
                successKey,
                {
                    bot: bot.displayName,
