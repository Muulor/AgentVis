...
export function getSandboxPathViolation(
    ...
    if (normalizedRoots.some((root) => normalizedPath === root || normalizedPath.startsWith(`${root}/`))) {
        return null;
    }
...
function normalizePathForCompare(path: string): string {
    const unified = path.trim().replace(/^["']|["']$/g, '').replace(/\\/g, '/');
    ...
    const rest = hasDrive ? unified.slice(3) : hasUnc ? unified.slice(2) : unified.replace(/^\/+/, '');
    const parts: string[] = [];
...
function getSandboxRoots(context: ToolExecutionContext): string[] {
    const roots = context.sandboxRoots?.length ? context.sandboxRoots : context.workdir ? [context.workdir] : [];
