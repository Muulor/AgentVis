...
export function getToolNamesForSchemaFilter(toolNames: string[]): string[] {
  return Array.from(
    new Set(toolNames.flatMap((toolName) => [toolName, getCanonicalToolName(toolName)]))
  );
}

export function isAllowedToolName(toolName: string, allowedTools: string[]): boolean {
  return allowedTools.includes(toolName) || allowedTools.includes(getCanonicalToolName(toolName));
}
...
export function normalizeToolCallForExecution(toolCall: ToolCall): ToolCall {
  ...
  if (!legacyPlatform) {
    return canonicalName === toolCall.name ? toolCall : { ...toolCall, name: canonicalName };
  }
