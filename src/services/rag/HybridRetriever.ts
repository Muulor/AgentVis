...
export class HybridRetriever {
    ...
    async retrieve(
        ...
        if (finalResults.length < factFilteredCount) {
            logger.trace(
                `[HybridRetriever] 排除记忆事实: ${factFilteredCount} → ${finalResults.length}`
            );
        }
        ...
        finalResults = finalResults.filter(r => {
            ...
            if (
                ...
                && rerankScore >= config.strongRerankScoreThreshold
            ) {
                return true;
            }
    ...
    private searchBm25WithFragments(
        ...
        const combined = new Map<string, { docId: string; score: number; hitCount: number; bestRank: number }>();

        const mergeResults = (results: Array<{ docId: string; score: number }>, weight: number): void => {
    ...
    private getBm25RrfContributionWeight(
        ...
        return this.isBroadOverviewSourceCandidate(chunk)
            ? 1
            : BROAD_QUERY_BM25_ONLY_RRF_WEIGHT;
    }

    private getEmbeddingRrfContributionWeight(
        chunk: Chunk,
     ...
     */
    private aggregateByParent(fusedResults: FusedResult[], isFocusedQuery: boolean): AggregatedResult[] {
        ...
        for (const result of fusedResults) {
            ...
            if (existing) {
                ...
                existing.hasRerankScore = (existing.hasRerankScore ?? false)
    ...
    private selectBalancedParentResults(
        ...
        const primaryQuota = preferSourceDiversity
            ...
            : Math.min(
                maxPerParent,
                targetCount,
                Math.max(1, Math.ceil(targetCount / 2))
            );
    ...
    private expandFocusedParentResults(
        ...
        for (const agg of aggregated) {
            const children = agg.scoredChunks
                .slice(0, maxPerParent)
                .sort((a, b) => {
    ...
    private getContentDedupeKey(chunk: Chunk): string {
        const normalizedPrefix = chunk.content
            .replace(/\s+/g, ' ')
            .trim()
            .slice(0, 160);
    ...
    private getFinalRelevanceDecision(
        ...
        return {
            keep: false,
            reason: fused?.bm25Score !== undefined
    ...
    private buildFinalRelevanceTerms(preprocessedQuery: RagQueryPreprocessResult): RelevanceTerm[] {
        ...
        for (const match of preprocessedQuery.originalQuery.matchAll(FINAL_RELEVANCE_FILE_TERM_PATTERN)) {
            addTerm(match[0]);
            addTerm(match[0].replace(/\.[A-Za-z0-9]{1,8}$/, ''));
        }

        for (const match of preprocessedQuery.originalQuery.matchAll(FINAL_RELEVANCE_ENGLISH_TERM_PATTERN)) {
            addTerm(match[0]);
        }
    ...
    private prioritizeSameSource(
        ...
        for (const [source, results] of bySource) {
            const scores = results.map(result => this.getSourcePriorityScore(result, isBroadOverviewQuery));
        ...
        for (const source of sortedSources) {
            const results = [...(bySource.get(source) ?? [])].sort(
                (a, b) => this.getSourcePriorityScore(b, isBroadOverviewQuery)
    ...
    private hasRerankScaleScores(results: AggregatedResult[]): boolean {
        return results.some(result =>
            result.hasRerankScore === true
            || result.bestScore >= RERANK_SOURCE_SCORE_SCALE_THRESHOLD
        );
    }
