...
const FOCUSED_QUERY_PATTERN = /\u300a[^\u300b]{1,80}\u300b|translator(?:'s)? note|translator.{0,40}(?:reflection|review|afterword|postscript|note)|afterword|postscript|preface|foreword|book review|reading reflection|reader response|appendix|reading guide|chapter\s+\d+|section\s+\d+|\u8bd1\u8005|\u8bd1\u540e|\u540e\u8bb0|\u5e8f\u8a00|\u524d\u8a00|\u8bfb\u540e\u611f|\u4e66\u8bc4|\u8bc4\u8bba|\u611f\u60f3|\u8d4f\u6790|\u7ae0\u8282|\u9644\u5f55|\u5bfc\u8bfb|\u7b2c[\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u767e\u5343\u4e07\d]+[\u7ae0\u8282\u7bc7\u56de]/i;
const BROAD_OVERVIEW_QUERY_PATTERN = /(\u6709\u4ec0\u4e48|\u90fd\u6709\u54ea\u4e9b|\u54ea\u4e9b|\u4ecb\u7ecd|\u6982\u8ff0|\u603b\u7ed3|\u8bf4\u8bf4).{0,20}(\u7279\u6027|\u7279\u70b9|\u529f\u80fd|\u80fd\u529b|\u4eae\u70b9)|(\u7279\u6027|\u7279\u70b9|\u529f\u80fd|\u80fd\u529b|\u4eae\u70b9).{0,20}(\u6709\u4ec0\u4e48|\u90fd\u6709\u54ea\u4e9b|\u54ea\u4e9b|\u4ecb\u7ecd|\u6982\u8ff0|\u603b\u7ed3)|what (features|capabilities)|what can .{0,40} do|(show|tell|introduce|summarize|describe).{0,40}(features|capabilities|overview|introduction)|(features|capabilities).{0,20}(of|for)/i;
...
function addReadingAliasTerms(target: Set<string>, query: string): void {
    ...
    if (
        hasChineseTranslatorTerm && /(\u8bfb\u540e\u611f|\u8bfb\u540e|\u611f\u60f3|\u4e66\u8bc4|\u8bc4\u8bba|\u8d4f\u6790|\u540e\u8bb0)/.test(query) ||
        /translator/i.test(query) && /(review|reflection|commentary|afterword|postscript|note)/i.test(query)
    ) {
        target.add('\u8bd1\u8005\u540e\u8bb0');
        target.add('\u8bd1\u540e\u8bb0');
        target.add('\u540e\u8bb0');
