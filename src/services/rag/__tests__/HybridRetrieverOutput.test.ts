...
describe('HybridRetriever output allocation', () => {
    ...
    it('keeps full BM25 weight for broad overview source candidates', () => {
        ...
        const regressionScore = typedFused.find(result => result.chunkId === 'regression-command')?.rrfScore;
    ...
    it('泛 query 下优先保留主 Parent 细节，再穿插辅助来源，并跳过重复内容', () => {
        ...
        expect(results.map((result: { chunk: Chunk }) => result.chunk.id)).not.toContain('feature-duplicate');
        expect(results.slice(3).map((result: { chunk: Chunk }) => result.chunk.metadata.fileName)).toEqual([
    ...
    it('final relevance gate treats broad overview cue terms as useful grounding', () => {
        ...
        expect(filtered.map((result: { chunk: Chunk }) => result.chunk.id)).toEqual(['features-overview']);
    });
    ...
    it('final relevance gate keeps cross-language reranked semantic hits without lexical grounding', () => {
        ...
        const filtered = retriever.applyFinalRelevanceGate(
            ...
            preprocessRagQuery('\u8fd9\u4efd\u6d4b\u8bd5\u62a5\u544a\u80fd\u89e3\u91ca\u4e00\u4e0b\u5417'),
            retriever.getConfig()
        );

        expect(filtered.map((result: { chunk: Chunk }) => result.chunk.id)).toEqual(['robustness-report']);
    });
    ...
    it('rerank-scale source prioritization does not rely solely on an explicit rerank marker', () => {
        ...
        const sandboxChunks = ['dns', 'browser', 'goals'].map((name, index) => makeChunk({
