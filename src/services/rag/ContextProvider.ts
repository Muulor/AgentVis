...
export class ContextProvider {
  ...
  formatMarkdown(results: SearchResult[]): string {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      // 标题行
      const source = chunk.metadata.fileName ? ` (Source: \`${chunk.metadata.fileName}\`)` : '';
 ...
 */
export function createContextProvider(options?: Partial<FormatOptions>): ContextProvider {
