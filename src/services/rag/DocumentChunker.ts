...
export class DocumentChunker {
    ...
    private splitIntoChildren(
        ...
        if (content.length <= this.CHILD_SIZE * 1.5) {
            if (content.trim().length >= this.config.minChunkSize) {
                children.push(this.createChunk(
                    content.trim(),
                    agentId,
                    documentId,
                    0,
            ...
            if (currentChunk.length + paragraph.length > this.config.chunkSize) {
                if (currentChunk.length >= this.config.minChunkSize) {
                    chunks.push(this.createChunk(
                        ...
                        documentId,
                        chunkIndex++,
                        { ...baseMetadata, documentType: 'text' }
                    ));
                }
    ...
    private chunkCode(
        ...
        const functionPatterns: Record<string, RegExp> = {
            typescript: /^(export\s+)?(async\s+)?function\s+\w+|^(export\s+)?(const|let)\s+\w+\s*=|^(export\s+)?class\s+\w+|^(export\s+)?interface\s+\w+/gm,
            ...
            for (let i = 0; i < matches.length; i++) {
                ...
                if (chunkContent.length >= this.config.minChunkSize) {
                    chunks.push(this.createChunk(
                        ...
                        documentId,
                        chunkIndex++,
                        { ...baseMetadata, documentType: 'code', codeLanguage: language }
                    ));
                }
            }
        } else {
    ...
    private detectCodeLanguage(content: string): string {
        ...
        if (/^def\s+\w+\s*\(/m.test(content) || /^from\s+\w+\s+import\s/m.test(content)) return 'python';
