...
import { HybridRetriever, createHybridRetriever, type HybridRetrieverConfig } from './HybridRetriever';
...
export class RagService {
    ...
    async indexDocument(
        ...
        const result = this.documentChunker.chunkWithHierarchy(
            content,
            agentId,
            documentId,
            metadata
        ...
        const chunks: Chunk[] = overviewChunk
            ? [overviewChunk, ...childChunks]
            : childChunks;
        ...
        // 第二步：批量向量化
        const texts = chunks.map(chunk => buildEmbeddingIndexText({
            fileName: chunk.metadata.fileName,
            filePath: chunk.metadata.filePath,
            sectionPath: chunk.metadata.sectionPath,
            heading: chunk.metadata.heading,
    ...
    async retrieve(
        ...
        if (this.enableHybridSearch) {
            ...
            logger.trace('[RagService] Vector 检索结果数量:', results.length, '阈值:', options?.threshold ?? 0.4);
        }
