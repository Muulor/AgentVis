...
import type { ModificationApplyResult, DiffResult, FullFileDiffLine } from '../types';
...
function createMockModification(params: {
  ...
  const { id, startLine, endLine, removeLines, addLines, status = 'pending' } = params;
...
describe('FullFileDiffBuilder', () => {
  ...
  describe('多修改场景（核心用例：行号偏移累积）', () => {
    it('两个 REPLACE：第二个修改的行号应考虑第一个修改的行数变化', () => {
      ...
      const mod2Remove = getModLines(result.lines, 'mod-2').find((l) => l.type === 'remove');
      expect(mod2Remove?.oldLineNumber).toBe(10);

      // mod2 的 add 行：newLineNumber 应为 10 + 2（前面 mod1 的偏移）= 12
      const mod2Add = getModLines(result.lines, 'mod-2').find((l) => l.type === 'add');
    ...
    it('两个 REPLACE + 行数减少：行号偏移应正确累积', () => {
      ...
      // mod2 的 remove 行
      const mod2Removes = getModLines(result.lines, 'mod-2').filter((l) => l.type === 'remove');
      ...
      const mod2Adds = getModLines(result.lines, 'mod-2').filter((l) => l.type === 'add');
    ...
    it('多修改 + hunk context：右列行号不应因重复行而偏大', () => {
      ...
      const mod1: ModificationApplyResult = {
        ...
        matchResult: {
          ...
          startLine: 5,
          endLine: 5,
          matchedContent: 'line 5',
        },
        diff: {
          oldContent: '',
          newContent: '',
          hasChanges: true,
          hunks: [{ oldStart: 2, oldLines: 7, newStart: 2, newLines: 8, lines: mod1Hunk }],
        },
        status: 'pending',
      };
      ...
      const mod2: ModificationApplyResult = {
        ...
        matchResult: {
          ...
          startLine: 20,
          endLine: 20,
          matchedContent: 'line 20',
        },
        diff: {
          oldContent: '',
          newContent: '',
          hasChanges: true,
          hunks: [{ oldStart: 17, oldLines: 7, newStart: 17, newLines: 7, lines: mod2Hunk }],
        },
        status: 'pending',
      };
      ...
      for (let i = 1; i <= 30; i++) {
        ...
        const occurrences = result.lines.filter(
          (l) => l.content === `line ${i}` && l.type === 'context'
        );
  ...
  describe('INSERT→REPLACE 锚点型修改（核心 bug 场景）', () => {
    ...
    it('纯插入（INSERT→REPLACE 锚点）：总行数应精确等于原始+新增', () => {
      ...
      const anchor4 = result.lines.find(
        (l) => l.content === 'line 4' && l.modificationId === 'mod-insert-1'
      );
    ...
    it('1095 行文件 + 5 个修改（3 个 INSERT→REPLACE）：总行数应精确', () => {
      ...
      const mod4: ModificationApplyResult = {
        ...
        matchResult: {
          ...
          startLine: 1089,
          endLine: 1091,
          matchedContent: 'line 1089\nline 1090\nline 1091',
        },
        diff: { oldContent: '', newContent: '', hasChanges: true, hunks: [] },
        status: 'pending',
      };

      const builder = new FullFileDiffBuilder(content, [mod0, mod1, mod2, mod3, mod4], 'test.html');
      const result = builder.build();
      ...
      for (let i = 1; i <= 1095; i++) {
        ...
        const isModified = [
          424, 425, 542, 543, 544, 560, 561, 562, 584, 585, 1089, 1090, 1091,
        ].includes(i);
