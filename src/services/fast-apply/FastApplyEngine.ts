    ...
    for (let i = 0; i < modifications.length; i++) {
      ...
      try {
        ...
        if (matchResult.success) {
          ...
          logger.trace(
            `[preview] mod[${i}] ${modification.operation}: matched ${matchResult.matchLevel} L${matchResult.startLine}-${matchResult.endLine}, searchLen=${modification.search.length}, diff: +${addCount} -${removeCount}, hunks=${diff.hunks.length}`
          );
          status = 'pending';
          pendingCount++;
        } else {
          // 匹配失败
          logger.trace(
            `[preview] mod[${i}] ${modification.operation}: MATCH FAILED, searchLen=${modification.search.length}`
          );
          diff = {
            oldContent: content,
            newContent: content,
  ...
  async applyFromXml(
    ...
    for (const result of previewResult.results) {
      if (result.status === 'pending' && result.matchResult.success) {
        try {
          // 重新匹配（因为内容可能已变化）
          const matchResult = await this.matcher.match(currentContent, result.modification.search);
