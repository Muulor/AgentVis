...
export class ModificationExecutor {
   ...
   * @returns 修改后的内容
   */
  applyModification(content: string, modification: Modification, matchResult: MatchResult): string {
   ...
   * 执行替换操作
   */
  private executeReplace(content: string, matchResult: MatchResult, replacement: string): string {
    // 如果有精确的字符偏移，使用它
    if (matchResult.startOffset !== undefined && matchResult.matchLength !== undefined) {
      return (
    ...
    // 否则使用行号
    return this.replaceByLines(content, matchResult.startLine, matchResult.endLine, replacement);
  }

  /**
   * 执行在匹配内容后插入
   */
  private executeInsertAfter(content: string, matchResult: MatchResult, insertion: string): string {
  ...
  private executeDelete(content: string, matchResult: MatchResult): string {
    ...
    // 否则删除指定行
    return this.replaceByLines(content, matchResult.startLine, matchResult.endLine, '');
  }
