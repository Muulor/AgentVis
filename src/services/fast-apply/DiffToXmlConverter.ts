...
export function diffToXml(diff: DiffResult, originalContent?: string): string {
    ...
    for (let hunkIdx = 0; hunkIdx < diff.hunks.length; hunkIdx++) {
        ...
        logger.trace(`[diffToXml] hunk[${hunkIdx}]: ${hunk.lines.length} lines → ${blocks.length} blocks: [${blockSummary.join(', ')}]`);
    ...
    logger.trace(`[diffToXml] 输出: ${diff.hunks.length} hunks → ${modifications.length} modifications`);
 ...
 * @returns XML 修改协议字符串
 */
export function generateWholeFileReplaceXml(
    ...
    logger.trace(`[generateWholeFileReplaceXml] 生成单一 REPLACE: searchLen=${originalContent.length}, replaceLen=${newContent.length}`);
    return xml;
}
