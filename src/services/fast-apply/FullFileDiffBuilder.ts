...
export class FullFileDiffBuilder {
    ...
    for (let originalLineNum = 1; originalLineNum <= originalLines.length; originalLineNum++) {
      ...
      if (mod && !processedMods.has(mod.modificationId)) {
        ...
        if (mod.status === 'rejected') {
          for (
            let lineNum = mod.matchResult.startLine;
            lineNum <= mod.matchResult.endLine;
            lineNum++
          ) {
            result.push({
              type: 'context',
  ...
  private mergeOverlappingMods(
    ...
    // 使用较宽 mod 的 ID 和元数据
    const wider =
      a.matchResult.endLine - a.matchResult.startLine >=
      b.matchResult.endLine - b.matchResult.startLine
        ? a
        : b;
