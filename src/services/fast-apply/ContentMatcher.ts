...
import type { MatchResult, MatchCandidate, ContentMatcherConfig } from './types';
...
export class ContentMatcher {
  ...
  fuzzyMatch(content: string, search: string): MatchResult | null {
    ...
    if (search.length > MAX_FUZZY_SEARCH_CHARS) {
      logger.trace(
        `[ContentMatcher] fuzzyMatch 跳过: search 长度 ${search.length} 超过阈值 ${MAX_FUZZY_SEARCH_CHARS}`
      );
      return null;
    }
    const contentLines = splitLines(content);
   ...
   * 返回多个可能的匹配候选，供用户选择
   */
  getFuzzyCandidates(content: string, search: string, maxCandidates: number = 5): MatchCandidate[] {
    const contentLines = splitLines(content);
    const searchLines = splitLines(search);
    const searchLength = searchLines.length;
