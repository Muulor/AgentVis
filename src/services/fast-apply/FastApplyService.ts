...
export class FastApplyService {
   ...
   * @returns 批量预览结果
   */
  async preview(documentId: string, content: string, xml: string): Promise<BatchApplyResult> {
   ...
   * @returns XML 格式的修改指令
   */
  generateEditInstructions(originalContent: string, newContent: string): string {
    ...
    for (const hunk of diff.hunks) {
      ...
      if (removedLines.length > 0 && addedLines.length > 0) {
        // REPLACE 模式: 有删除也有添加
        modifications.push(
          this.buildReplaceModification(removedLines.join('\n'), addedLines.join('\n'))
        );
      } else if (addedLines.length > 0) {
        // INSERT_AFTER 模式: 仅添加
        // 找到插入位置的前一行作为锚点
        const anchorLine = this.findAnchorLineForInsert(hunk, originalLines, contextBefore);
        modifications.push(this.buildInsertAfterModification(anchorLine, addedLines.join('\n')));
      } else if (removedLines.length > 0) {
        // DELETE 模式: 仅删除
        modifications.push(this.buildDeleteModification(removedLines.join('\n')));
      }
    }
   ...
   * @returns Diff 结果
   */
  async compareSnapshots(snapshotId1: string, snapshotId2: string): Promise<DiffResult | null> {
    try {
