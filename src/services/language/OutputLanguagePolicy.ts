...
const LANGUAGE_DEFINITIONS: readonly LanguageDefinition[] = [
    {
        ...
        aliases: [
            '繁体中文', '繁體中文', '正體中文', '繁体字', '繁體字',
            '繁体中国語', '繁體中國語', '번체 중국어', 'traditional chinese',
        ],
    },
    {
        tag: 'zh-CN',
        ...
        aliases: [
            '简体中文', '簡體中文', '简体中国語', '簡体中国語', '簡体字', '简体字',
            '간체 중국어', 'simplified chinese',
        ],
    },
    {
        tag: 'zh',
        label: 'Chinese',
        aliases: [
            '中文', '汉语', '漢語', '华语', '華語', '中国語', '中國語', '중국어',
            'chinese',
        ],
    },
    { tag: 'en', label: 'English', aliases: ['英语', '英語', '英文', '영어', 'english'] },
    { tag: 'ja', label: 'Japanese', aliases: ['日语', '日語', '日文', '日本語', '일본어', 'japanese'] },
    { tag: 'ko', label: 'Korean', aliases: ['韩语', '韓語', '韩文', '韓文', '韓国語', '한국어', 'korean'] },
    { tag: 'fr', label: 'French', aliases: ['法语', '法語', '法文', 'フランス語', '프랑스어', 'français', 'french'] },
    { tag: 'de', label: 'German', aliases: ['德语', '德語', '德文', 'ドイツ語', '독일어', 'deutsch', 'german'] },
    { tag: 'es', label: 'Spanish', aliases: ['西班牙语', '西班牙語', '西班牙文', 'スペイン語', '스페인어', 'español', 'spanish'] },
    { tag: 'pt', label: 'Portuguese', aliases: ['葡萄牙语', '葡萄牙語', '葡文', 'ポルトガル語', '포르투갈어', 'português', 'portuguese'] },
    { tag: 'ru', label: 'Russian', aliases: ['俄语', '俄語', '俄文', 'ロシア語', '러시아어', 'русский', 'russian'] },
    { tag: 'ar', label: 'Arabic', aliases: ['阿拉伯语', '阿拉伯語', 'アラビア語', '아랍어', 'العربية', 'arabic'] },
    { tag: 'hi', label: 'Hindi', aliases: ['印地语', '印地語', 'ヒンディー語', '힌디어', 'हिन्दी', 'हिंदी', 'hindi'] },
    { tag: 'th', label: 'Thai', aliases: ['泰语', '泰語', 'タイ語', '태국어', 'ภาษาไทย', 'thai'] },
    { tag: 'vi', label: 'Vietnamese', aliases: ['越南语', '越南語', 'ベトナム語', '베트남어', 'tiếng việt', 'vietnamese'] },
    { tag: 'id', label: 'Indonesian', aliases: ['印度尼西亚语', '印尼语', 'インドネシア語', '인도네시아어', 'bahasa indonesia', 'indonesian'] },
    { tag: 'it', label: 'Italian', aliases: ['意大利语', '意大利語', 'イタリア語', '이탈리아어', 'italiano', 'italian'] },
    { tag: 'nl', label: 'Dutch', aliases: ['荷兰语', '荷蘭語', 'オランダ語', '네덜란드어', 'nederlands', 'dutch'] },
    { tag: 'pl', label: 'Polish', aliases: ['波兰语', '波蘭語', 'ポーランド語', '폴란드어', 'polski', 'polish'] },
    { tag: 'tr', label: 'Turkish', aliases: ['土耳其语', '土耳其語', 'トルコ語', '터키어', 'türkçe', 'turkish'] },
    { tag: 'he', label: 'Hebrew', aliases: ['希伯来语', '希伯來語', 'ヘブライ語', '히브리어', 'עברית', 'hebrew'] },
];
...
const LATIN_LANGUAGE_PROFILES: readonly LatinLanguageProfile[] = [
    ...
    },
    {
        ...
        `),
        distinctiveCharacters: /[ăâđêôơưàáảãạằắẳẵặầấẩẫậèéẻẽẹềếểễệìíỉĩịòóỏõọồốổỗộờớởỡợùúủũụừứửữựỳýỷỹỵ]/giu,
    },
    {
        tag: 'id',
        label: 'Indonesian',
...
const CODE_CONTENT_PATTERNS: readonly RegExp[] = [
    /```[\s\S]*?```/gu,
    /`[^`\r\n]*`/gu,
];
...
function stripCodeContent(input: string): string {
    return CODE_CONTENT_PATTERNS.reduce(
        (current, pattern) => current.replace(pattern, ' '),
        input
    );
}
...
function findLanguageAliasOccurrences(input: string): AliasOccurrence[] {
    ...
    for (const occurrence of [...allOccurrences].sort((left, right) =>
        (right.end - right.start) - (left.end - left.start)
    )) {
        const contained = nonOverlapping.some(existing =>
...
function isNegatedOutputTarget(before: string, after: string): boolean {
    ...
    const negatedBefore = [
        new RegExp(`(?:do\\s+not|don't|never|must\\s+not|should\\s+not|avoid|except|excluding|other\\s+than|without|not(?!\\s+only\\b))\\b[\\s\\S]{0,100}${quoteBoundary}$`, 'iu'),
        new RegExp(`(?:不要|別|别|禁止|請勿|请勿|避免|不可|不能|不應|不应)[\\s\\S]{0,80}${quoteBoundary}$`, 'u'),
...
function isPositiveOutputTarget(before: string, after: string): boolean {
    ...
    const targetBeforeLanguage = [
        new RegExp(`(?:translate|rewrite|respond|reply|answer|write|output|render|provide|present|return)[\\s\\S]{0,100}(?:into|to|in|as)${trailingBoundary}$`, 'iu'),
        new RegExp(`(?:output|target|response)\\s+language\\s*[:：]?${trailingBoundary}$`, 'iu'),
        new RegExp(`(?:翻译|翻譯|译|譯|转换|轉換|改写|改寫|回答|回复|回覆|输出|輸出|撰写|撰寫|写成|寫成|生成)[\\s\\S]{0,100}(?:为|為|成|用|以|：|:)${trailingBoundary}$`, 'u'),
        new RegExp(`(?:目标语言|目標語言|输出语言|輸出語言|回答语言|回答語言)\\s*[:：]?${trailingBoundary}$`, 'u'),
...
function detectExplicitLanguageDirectives(input: string): ExplicitLanguageDirectives {
    ...
    const classified = findLanguageAliasOccurrences(input)
        .map(occurrence => {
            const containingSpan = spans.find(span =>
...
function buildHint(
    ...
    const defaultGuidance = source === 'explicit_target'
...
function countDistinctPatternMatches(input: string, pattern: RegExp): number {
    return new Set(
        (input.match(pattern) ?? []).map(match => match.toLocaleLowerCase())
    ).size;
}
...
function inferChineseVariant(
    ...
    const simplifiedHits = [...characters]
        .filter(character => SIMPLIFIED_CHINESE_EVIDENCE.has(character))
        .length;
    const traditionalHits = [...characters]
        .filter(character => TRADITIONAL_CHINESE_EVIDENCE.has(character))
        .length;
...
function refineGenericChineseTarget(latestUserRequest: string): OutputLanguageHint {
    ...
    const kanaCount = countPatternMatches(
        instructionShell,
        /[\u3040-\u30ff\u31f0-\u31ff]/gu
    );
...
function scoreLatinLanguages(detectionText: string): {
    ...
    const latinTokens = normalizedText.match(
    ...
    const scores = LATIN_LANGUAGE_PROFILES.map(profile => {
        ...
        const distinctiveCharacterHits = profile.distinctiveCharacters && lexicalHits > 0
            ? Math.min(
                2,
                countDistinctPatternMatches(normalizedText, profile.distinctiveCharacters)
            )
            : 0;
    ...
    const hasEnoughIndependentEvidence = winner.strongHits >= 2
    ...
    const hasClearLead = !runnerUp
        || runnerUp.score === 0
        || (
            winner.score - runnerUp.score >= 2
            && winner.score >= runnerUp.score * 1.25
        );
...
function inferLanguageFromText(
    ...
    const hangulCount = countPatternMatches(detectionText, /[\u1100-\u11ff\u3130-\u318f\uac00-\ud7af]/gu);
    const hanCount = countPatternMatches(detectionText, /[\u3400-\u9fff\uf900-\ufaff]/gu);
    const cyrillicCount = countPatternMatches(detectionText, /[\u0400-\u04ff]/gu);
    const arabicCount = countPatternMatches(detectionText, /[\u0600-\u06ff\u0750-\u077f\u08a0-\u08ff]/gu);
    ...
    const japaneseSignal = source === 'request_language'
        ? kanaCount >= 2
    ...
    if (latinResult.winner) {
        return buildHint(
            latinResult.winner.profile.tag,
            latinResult.winner.profile.label,
            source
        );
    }
...
function hintFromLocale(tag: string): OutputLanguageHint | undefined {
    ...
    const label = LOCALE_LANGUAGE_NAMES[languageCode]
        ?? `the language identified by locale ${normalizedTag}`;
...
function isLanguageExcluded(
    tag: string,
    exclusions: readonly OutputLanguageExclusion[]
): boolean {
    ...
    return exclusions.some(exclusion => {
        ...
        return getBaseLanguageTag(exclusion.tag) === baseTag
            && baseTag !== 'zh';
    });
}
...
export function resolveOutputLanguage(
    ...
    if (directives.target) {
        const resolvedTarget = directives.target.tag === 'zh'
    ...
    return buildHint(
        'und',
        'the language used by the latest request',
        'unknown'
    );
}
...
export function buildSourceLanguagePreservationContract(sourceText: string): string {
    ...
    const signalRule = sourceHint.source === 'unknown'
        ? `- ${sourceHint.guidance}`
