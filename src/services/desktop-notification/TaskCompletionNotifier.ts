...
export function resolveTaskCompletionNotificationSource(value: unknown): TaskCompletionNotificationSource {
 ...
 * 发布任务完成桌面通知。
 */
export async function notifyTaskCompleted(payload: TaskCompletionNotificationPayload): Promise<void> {
    if (!hasTauriRuntime()) {
        return;
    }
