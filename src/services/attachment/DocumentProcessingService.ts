...
import type { DocumentProcessingResult, ProcessorContext } from './types';
...
export class DocumentProcessingService {
  ...
  async processDocument(
    ...
    if (!rawContent.trim()) {
      throw new DocumentProcessingError('EMPTY_DOCUMENT', DOCUMENT_ERROR_MESSAGES.EMPTY_DOCUMENT());
    }
    ...
    // 6. 执行处理
    logger.trace(
      `[DocumentProcessingService] 开始处理: ${fileName} (${ext}, ${(fileSize / 1024).toFixed(1)} KB)`
    );

    const result = await processor.process(context, rawContent);

    logger.trace(
      `[DocumentProcessingService] 处理完成: ${fileName}`,
  ...
  private async readRawContent(filePath: string, extension: DocumentExtension): Promise<string> {
    ...
    throw new DocumentProcessingError('UNSUPPORTED_FORMAT', `Unsupported format: ${extension}`);
  }
}
