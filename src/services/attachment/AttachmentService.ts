...
export class AttachmentService {
  ...
  async addAttachment(
    ...
    if (validation.type === 'image') {
      ...
      try {
        ...
        logger.trace(
          `[AttachmentService] 图片压缩完成: ${originalFileName} → ${compressionResult.fileName}`,
        ...
        throw new Error(
          translate('chat.imageProcessingFailedDescription', {
    ...
    try {
      ...
      // 记录处理结果
      logger.trace(
        `[AttachmentService] 文档解析成功: ${fileName}`,
    ...
    logger.trace(
      `[AttachmentService] 附件已添加: ${attachment.fileName} (${validation.type ?? 'unknown'})`
    );
    return attachment;
  }
  ...
  async indexToKnowledge(attachment: AttachmentInfo, agentId: string): Promise<void> {
    ...
    if (agentCheck?.autoIndexDeliverables === false) {
      logger.trace(
        `[AttachmentService] autoIndexDeliverables 已关闭，跳过索引: ${attachment.fileName}`
      );
      return;
    }
    ...
    try {
      ...
      logger.trace(
        `[AttachmentService] 已索引到知识库: ${attachment.fileName} (${chunkCount} chunks)`
      );
  ...
  buildAttachmentContext(
    ...
    const documentAttachments = attachments.filter((a) => a.type === 'document' && a.parsedContent);
    ...
    const sections = [manifest, ...parts].filter((part): part is string => Boolean(part.trim()));
  ...
  buildAttachmentManifest(
    ...
    const items = attachments
      ...
      .map((attachment) =>
        translate('chat.attachmentManifestItem', {
          fileName: attachment.fileName,
