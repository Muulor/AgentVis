    ...
    async compressImage(
        ...
        if (originalMimeType === 'image/heif' || originalMimeType === 'image/heic') {
            ...
            try {
                ...
                logger.trace(`[ImageCompression] HEIF→JPEG 转换完成: ${(originalBlob.size / 1024).toFixed(1)}KB`);
            } catch (error) {
        ...
        logger.trace(`[ImageCompression] 原始图片: ${width}x${height}, 长边=${longEdge}px, 大小=${(originalSize / 1024).toFixed(1)}KB`);
        ...
        try {
            ...
            logger.trace(`[ImageCompression] 压缩完成: ${(compressedSize / 1024).toFixed(1)}KB (节省 ${compressionRatio}%)`);
