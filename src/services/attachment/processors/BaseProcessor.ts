...
export abstract class BaseProcessor implements IDocumentProcessor {
    ...
    async process(context: ProcessorContext, rawContent: string): Promise<DocumentProcessingResult> {
        ...
        if (ragOnly) {
            warnings.push('Document is large; using knowledge base retrieval is recommended instead of direct context injection');
        }
    ...
    protected abstract processInternal(
        ...
        rawContent: string,
        warnings: string[]
    ): { content: string; metadata: DocumentMetadata } | Promise<{ content: string; metadata: DocumentMetadata }>;
     ...
     * 保留头部截断
     */
    private truncateHead(content: string, maxTokens: number): { content: string; tokens: number; preservedParts?: string[] } {
     ...
     * 保留尾部截断
     */
    private truncateTail(content: string, maxTokens: number): { content: string; tokens: number; preservedParts?: string[] } {
        const lines = content.split('\n');
        const result: string[] = [];
        let currentTokens = 0;
     ...
     */
    private truncateHeadTail(content: string, maxTokens: number): { content: string; tokens: number; preservedParts?: string[] } {
    ...
    protected shouldBeRagOnly(fileSize: number, estimatedTokens: number): boolean {
        ...
        return fileSize > RAG_ONLY_THRESHOLDS.FILE_SIZE || estimatedTokens > RAG_ONLY_THRESHOLDS.TOKEN_COUNT;
    }
