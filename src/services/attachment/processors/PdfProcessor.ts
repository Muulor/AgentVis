...
import type { DocumentMetadata, ProcessorContext, TocEntry, PdfProcessorConfig } from '../types';
...
export class PdfProcessor extends BaseProcessor {
  ...
  private applyPageStrategy(
    ...
    warnings.push(
      `PDF is large (${totalPages} pages); using knowledge base retrieval is recommended instead of direct reading`
    );
  ...
  private extractToc(content: string): TocEntry[] {
    ...
    for (let i = 0; i < Math.min(lines.length, 200); i++) {
      ...
      if (inToc) {
        ...
        if (
          line.length > 100 ||
          /^(\u7b2c[\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\d]+\u7ae0|Chapter|Section|Part|Appendix)/i.test(
            line
          )
        ) {
          break;
  ...
  private inferTocLevel(title: string): number {
    // 第一章、Chapter 1 等为一级
    if (
      /^(\u7b2c[\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\d]+\u7ae0|Chapter\s+\d+|Part\s+\d+|Appendix\s+[A-Z\d]+)/i.test(
        title
      )
    ) {
      return 1;
