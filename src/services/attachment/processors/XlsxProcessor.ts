...
import type { DocumentMetadata, ProcessorContext, XlsxProcessorConfig } from '../types';
...
export class XlsxProcessor extends BaseProcessor {
  ...
  private processSheet(sheet: Sheet, warnings: string[]): string {
    ...
    if (needsSampling) {
      ...
      warnings.push(
        `Worksheet "${name}" is large (${dataRows.length} rows); sampled preview is shown`
      );
    } else {
  ...
  private extractSchema(rows: string[][]): { columns: string[]; types: string[] } {
    ...
    for (let colIndex = 0; colIndex < columns.length; colIndex++) {
      const colValues = sampleRows.map((row) => row[colIndex] ?? '').filter((val) => val !== '');
