...
import { StabilityVerifier, createStabilityVerifier, CANDIDATE_POOL_OVERFLOW_THRESHOLD } from './StabilityVerifier';
import { MemoryTriggerManager, createMemoryTriggerManager } from './MemoryTriggerManager';
import { CategoryConsolidationTracker, createCategoryConsolidationTracker } from './CategoryConsolidationTracker';
...
export class MemoryService {
    ...
    async addInteraction(userMessage: Message, assistantMessage: Message): Promise<void> {
        ...
        try {
            ...
            logger.trace(`[MemoryService] 触发判断: shouldTrigger=${decision.shouldTrigger}, reason=${decision.reason}`);
            ...
            if (decision.shouldTrigger) {
                ...
                if (lastProcessedId) {
                    // 有历史处理记录，增量加载
                    messages = await this.loadRecentMessagesAfter(lastProcessedId, DEFAULT_INCREMENTAL_LOAD_LIMIT);
                    logger.trace(`[MemoryService] Fast 增量加载: 从 ${lastProcessedId} 之后加载了 ${messages.length} 条消息`);
                } else {
                    // 首次处理，加载最近的消息
     ...
     * @returns 消息列表
     */
    private async loadRecentMessagesAfter(afterMessageId: string, limit: number = 20): Promise<Message[]> {
        try {
            const messages = await invoke<Array<{
                id: string;
                agentId: string;
        ...
        try {
            for (;;) {
                try {
                    ...
                    const shortTermRecords = await invoke<Array<{
                        id: string;
                        agentId: string;
                        layer: string;
                        content: string;
                    ...
                    const userMessageCount = shortTermRecords
                        .filter(r => r.content.startsWith('User:'))
                        .length;
                    const usageRatio = userMessageCount / config.windowSize;

                    logger.trace(`[MemoryService] 水位线检查: ${userMessageCount}/${config.windowSize} = ${(usageRatio * 100).toFixed(0)}%, 阈值 ${(config.watermarkThreshold * 100).toFixed(0)}%`);
                    ...
                    if (shortTermRecords.length === previousRecordCount) {
                        logger.warn(`[MemoryService] 水位线循环消费中止：LLM 持续失败，short_term 记录数未变化 (${shortTermRecords.length})`);
                        break;
                    }
                    previousRecordCount = shortTermRecords.length;

                    logger.trace(`[MemoryService] 水位线触发！开始转换... (记录数: ${shortTermRecords.length})`);
     ...
     * @returns 消息列表
     */
    private parseRecordsToMessages(records: Array<{
        id: string;
        content: string;
        sourceMessageIds: string | null;
        createdAt: number;
    ...
    async processCandidates(messages: Message[]): Promise<void> {
        ...
        for (const [category, candidates] of categoryCounts) {
            if (candidates.length >= CANDIDATE_POOL_OVERFLOW_THRESHOLD) {
                logger.trace(`[MemoryService]  类别 ${category} 保留池溢出 (${candidates.length} 条)，触发批量晋升`);
        ...
        if (retainedAfterPromotion.length > 0) {
            ...
            retainedAfterPromotion.forEach((c, i) => {
                logger.trace(`  ${i + 1}. [${c.category}] ${c.content.substring(0, 60)}... (score: ${c.score})`);
            });
        }
        ...
        if (passed.length > 0) {
            const { savedCount, failedCandidateIds } = await this.factExtractor.extractAndSaveFromVerified(passed);
            ...
            if (failedCandidateIds.length > 0) {
                ...
                logger.warn(`[MemoryService] ${failedCandidateIds.length} 个候选因 API 失败已写回候选池等待重试`);
            }
        ...
        try {
            ...
            if (passed.length > 0) {
                ...
                if (failedCandidateIds.length > 0) {
                    const failedCandidates = passed.filter(c => failedIdSet.has(c.id));
                    await this.saveCandidatePool(failedCandidates);
                    logger.warn(`[MemoryService] 重试仍有 ${failedCandidateIds.length} 个候选失败，已写回候选池`);
                }
        ...
        for (const category of pending) {
            try {
                ...
                if (facts.length > 0) {
                    ...
                    if (result.wrote) {
                logger.trace(`[MemoryService]  类别 ${category} 汇总完成: ${result.consolidatedFact ?? ''}`);
                    } else {
                logger.trace(`[MemoryService]  类别 ${category} 汇总跳过: ${result.reason ?? 'unknown'}`);
                    }
    ...
    private async processConversionBatch(batch: Message[]): Promise<void> {
        ...
        if (summaryResult.summary) {
            ...
            logger.trace(`[MemoryService] 摘要已保存（Chain-of-State）: ${summaryResult.summary.substring(0, 50)}...`);
        }
     ...
     */
    private async loadLatestSummaryState(): Promise<import('./SummaryManager').PriorSummaryState | null> {
        try {
    ...
    async onSessionEnd(): Promise<void> {
        ...
        try {
            ...
            const recentMessages = await invoke<Array<{
                id: string;
                agentId: string;
                role: string;
                content: string;
            ...
            if (decision.shouldTrigger) {
                ...
                if (lastProcessedId) {
                    // 有历史处理记录，增量加载
                    messages = await this.loadRecentMessagesAfter(lastProcessedId, DEFAULT_INCREMENTAL_LOAD_LIMIT);
                    logger.trace(`[MemoryService] 增量加载: 从 ${lastProcessedId} 之后加载了 ${messages.length} 条消息`);
                } else {
                    // 首次处理，加载最近的消息
                    messages = await this.loadRecentMessagesFromDB(DEFAULT_INITIAL_LOAD_COUNT);
                ...
                if (pendingCandidates.length > 0) {
                    logger.trace(`[MemoryService] 候选池有 ${pendingCandidates.length} 个待重试候选，触发重试`);
    ...
    async updateBufferConfig(config: Partial<ShortTermBufferConfig>): Promise<void> {
        ...
        if (pendingBatch.length > 0) {
            ...
            try {
                ...
                // 抛出错误供调用方处理（显示 Toast）
                throw new Error(`Configuration update failed: LLM call error - ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    ...
    private async reconcileUnindexedSummaries(): Promise<void> {
        try {
            ...
            const unindexed = summaries.filter(
                s => !indexedSet.has(`${SUMMARY_DOC_PREFIX}${s.id}`)
            );
 ...
 */
export function getOrCreateMemoryService(
    agentId: string,
