...
export function buildSummaryPrompt(
    ...
    return SUMMARY_PROMPT_TEMPLATE
        .replace('{prior_state}', priorStateText)
...
export class SummaryManager {
    ...
    async generateSummary(
        ...
        logger.trace('[SummaryManager] 前置状态:', priorState
        ...
        const conversations = input
            .map((msg, idx) => `[#${idx + 1} ${msg.role === 'user' ? 'User' : 'Assistant'}] ${msg.content}`)
            .join('\n');
        ...
        logger.trace(`[SummaryManager] ===== 完整 Prompt 开始 =====\n${prompt}\n[SummaryManager] ===== 完整 Prompt 结束 =====`);
    ...
    async retryPendingIndexes(): Promise<number> {
        ...
        logger.trace(`[SummaryManager] 补索引完成: 成功 ${successCount}, 仍待重试 ${stillPending.length}`);
        return successCount;
    }
     ...
     */
    async getSummaries(): Promise<Array<{
        id: string;
        content: string;
        createdAt: number;
    ...
    private parseSummaryResponse(response: string): SummaryResult {
        ...
        if (result.success && result.data) {
            ...
            return {
                ...
                confirmedDecisions: Array.isArray(parsed.confirmedDecisions) ? parsed.confirmedDecisions : undefined,
                openQuestions: openQuestions && openQuestions.length > 0 ? openQuestions : undefined,
                invalidatedPoints: Array.isArray(parsed.invalidatedPoints) ? parsed.invalidatedPoints : undefined,
            };
        }
