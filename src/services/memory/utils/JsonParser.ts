  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    if (char === '"') {
      if (!inString) {
        inString = true;
        currentStringRole =
          findPreviousNonSpaceInText(jsonStr, i - 1) === ':' ? 'objectValue' : 'other';
      } else {
        // 检查这是否是字符串的结束
        ...
        if (
          nextNonSpace === '"' &&
          shouldEscapeQuoteBeforeAdjacentClosingQuote(
            chars,
            i,
            nextNonSpaceIndex,
            currentStringRole
          ...
          (nextNonSpace === ',' &&
            thisQuoteCanEndBeforeComma(chars, nextNonSpaceIndex, currentStringRole)) ||
          nextNonSpace === '}' ||
          nextNonSpace === ']'
        ) {
          // 高置信度：值结束后直接跟分隔符或闭括号
...
function isAtLineStartAfterIndentInChars(chars: string[], index: number): boolean {
  ...
  while (
    linePrefixIndex >= 0 &&
    (chars[linePrefixIndex] === ' ' || chars[linePrefixIndex] === '\t')
  ) {
    linePrefixIndex--;
  }
 ...
 * 6. 截断补全后解析
 */
export function parseWithFallback<T>(text: string, options: ParseOptions = {}): ParseResult<T> {
  ...
  if (extracted) {
    ...
    if (balancedRootPrefix) {
      try {
        ...
        repairSucceeded(
          'JSON repair succeeded: ignored trailing structural noise after the root value'
        );
        return {
          success: true,
          data: parsed,
        ...
        try {
          ...
          repairSucceeded(
            'JSON repair succeeded: ignored trailing structural noise after a sanitized root value'
          );
          return {
            success: true,
            data: parsed,
          ...
          noteParseFailure(
            `Strategy 2b failed after balanced root prefix repair: ${String(error)}`
          );
        }
      }
    }
    ...
    if (propertyRepaired !== extracted) {
      try {
        ...
        repairSucceeded(
          'JSON repair succeeded: repaired a missing value terminator before a following property key'
        );
        return {
          success: true,
          data: parsed,
        ...
        noteParseFailure(
          `Strategy 4 failed after repairing missing value terminator: ${String(error)}`
        );
      }
    }
    ...
    try {
      ...
      repairSucceeded(
        'JSON repair succeeded with aggressive sanitization; result may be incomplete or semantically modified'
      );
    ...
    if (repaired !== extracted) {
      try {
        ...
        repairSucceeded(
          'JSON repair succeeded after truncated input repair; some fields may be incomplete'
        );
...
function repairMissingValueTerminatorBeforeProperty(jsonStr: string): string {
  ...
  for (let i = 0; i < jsonStr.length; i++) {
    ...
    if (char === '"') {
      inString = true;
      currentStringRole =
        findPreviousNonSpaceInText(jsonStr, i - 1) === ':' ? 'objectValue' : 'key';
    }
...
function findUnescapedQuoteWithinLimit(
  text: string,
  startIndex: number,
  maxLength: number
): number {
