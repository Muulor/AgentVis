...
export class MemoryVectorIndex {
    ...
    async searchRelevant(
        ...
        try {
            ...
            const documentIdPrefix = options.memoryType
                ? prefixMap[options.memoryType]
                : undefined;
    ...
    private filterAndTransform(
        ...
        if (skippedNonMemory > 0 || skippedTypeMismatch > 0) {
            logger.trace(`[MemoryVectorIndex] 过滤详情: 跳过非记忆=${skippedNonMemory}, 类型不匹配=${skippedTypeMismatch}, 保留=${transformed.length}`);
        }
