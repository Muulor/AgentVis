...
const IDENTITY_ROLE_DICT: IntentEntry = {
    keywords: [
        // 中文 - 职业/身份
        '我是', '我叫', '我的职业', '我从事', '我在做', '我的工作',
        '作为一个', '作为一名', '身为', '职业是', '身份是',
        '工程师', '开发者', '设计师', '产品经理', '项目经理',
        '学生', '研究员', '教师', '老师', '讲师', '教授',
        '程序员', '码农', '前端', '后端', '全栈', '运维',
        '创业者', '自由职业', '博主', '写手', '作者',
        '经理', '总监', '主管', '负责人', '老板', '领导',
        ...
        '互联网', '金融', '医疗', '教育', '游戏', '电商',
        // 英文
        'I am a', 'I\'m a', 'my job', 'my role', 'I work as',
        'my profession', 'my title', 'I work in', 'I work at', 'I work for',
        'engineer', 'developer', 'designer', 'manager', 'student', 'teacher',
        'professor', 'researcher', 'analyst', 'consultant', 'founder',
        'writer', 'author', 'frontend', 'backend', 'full-stack', 'DevOps',
        'product manager', 'project manager', 'data scientist', 'marketer',
        'operator',
    ],
    patterns: [
...
const PREFERENCE_STYLE_DICT: IntentEntry = {
    keywords: [
        // 中文 - 喜欢/偏好
        '喜欢', '偏好', '偏爱', '更喜欢', '更爱', '最爱', '超爱',
        '迷上', '着迷', '痴迷', '沉迷', '热衷', '钟爱', '中意',
        '倾向', '习惯', '爱好', '兴趣',
        // 中文 - 不喜欢
        '不喜欢', '讨厌', '反感', '不爱', '不想', '不要',
        '烦', '嫌', '受不了', '不习惯', '不太喜欢',
        ...
        'I\'m into', 'I\'m fond of', 'I\'m a fan of',
        'please be', 'please use', 'I want', 'I\'d like',
        'please keep', 'please avoid', 'I prefer that you', 'I would rather',
        'I can\'t stand', 'I lean toward', 'favorite', 'favourite',
        'concise', 'detailed', 'brief', 'short', 'long', 'formal', 'casual',
        'friendly', 'structured', 'step-by-step', 'bullet points', 'shorter',
        'longer',
    ],
    patterns: [
...
const LONG_TERM_GOAL_DICT: IntentEntry = {
    keywords: [
        ...
        '三年', '五年', '十年',
        // 英文
        'my goal', 'my plan', 'I want to', 'I\'m going to',
        'I aim to', 'I intend to', 'I\'m preparing',
        'I\'m working toward', 'I plan on', 'my objective', 'career goal',
        'long-term', 'this year', 'next year', 'in the future',
        'learn', 'study', 'develop', 'build', 'achieve', 'launch',
        'ship', 'publish', 'graduate', 'switch careers', 'start a company',
        'startup', 'fundraise',
    ],
    patterns: [
...
const KNOWLEDGE_LEVEL_DICT: IntentEntry = {
    keywords: [
        ...
        '这个我懂', '这我知道', '我清楚',
        // 英文
        'I know', 'I understand', 'I\'m familiar with',
        'I\'m experienced in', 'I\'m proficient in',
        'I already know', 'no need to explain', 'skip the basics',
        'I\'m skilled in', 'I\'m comfortable with',
        'I don\'t know', 'I\'m new to', 'I\'m a beginner',
        'I\'m not familiar with', 'I have not used',
    ],
    patterns: [
...
const INTERACTION_SIGNALS_DICT: IntentEntry = {
    keywords: [
        ...
        '时区', '在北京', '在上海', '在美国', '在国外',
        '远程', '在家', '办公室',
        '用Mac', '用Windows', '用Linux',
        '用VS Code', '用Vim', '用Cursor',
        '通常', '一般', '平时', '每天', '每周',
        '习惯', '日常', '固定', '总是',
        'I use', 'I speak', 'in Chinese', 'in English',
        'my timezone', 'time zone', 'based in', 'working from', 'remote',
        'office', 'home office', 'I use VS Code', 'I use Cursor',
        'I use macOS', 'I use Windows', 'I use Linux',
        'usually', 'always', 'every day', 'most days', 'typically',
        ...
        '一直在意', '始终', '每次所以',
        '从来都', '每次话到', '沿用',
        'every time', 'always been', 'consistently', 'repeatedly',
        'I keep running into', 'I keep coming back to', 'I struggle with',
        ...
        '却总是', '却还是', '只是我',
        '其实我', '其实这', '才是我真正',
        'but I still', 'but actually', 'however I', 'yet I',
        'the truth is', 'what I really want', 'what I actually need',

        // 四、重复强调类（多次说过的大原则）
        '之前说过', '我反复说', '我多次提到',
        '我一再强调', '这点很重要', '不知道为什么总是',
        '我对这个反感', '我对这个怪', '总是会',
        'I keep saying', "I've mentioned", 'I always end up', 'somehow I always',
        'as I said before', 'this matters to me', 'important to me',
        ...
        '还在初定', '一直在纠结', '径渭不明',
        '一方面...一方面', '到底该', '起迟不决',
        'still undecided', 'torn between', 'on the fence', 'not sure whether',
        'haven\'t decided', 'weighing whether', 'choosing between',
    ],
    patterns: [
        // 语言/地区环境
...
export function matchIntent(
    text: string,
    targetCategory?: LongTermFactCategory
): MatchResult {
 ...
 * 获取词典统计信息
 */
export function getDictionaryStats(): Record<LongTermFactCategory, {
    keywordCount: number;
    patternCount: number;
}> {
...
export const MEMORY_COMMAND_KEYWORDS = [
    ...
    '要记住', '你要记住', '得记住', '可要记住',
    '一定要记住', '务必记住', '千万要记住',
    'remember this', 'please remember', 'keep in mind', "don't forget",
    'make a note of this', 'save this memory',

    // === 口语化存储请求 ===
    '帮我记下', '存一下', '录入一下', '留个底', '备注一下', '做个笔记',
    '记在心里', '别丢了这段',
    'save this', 'note down', 'take a note', 'log this', 'jot this down', 'mark this',
    'write this down', 'store this', 'record this',

    // === 时效与未来参考类（暗示记忆） ===
    '以后你要记得', '下次我问你时', '以后就按这个来', '从现在起', '长期记住',
    '这对我以后很重要', '以后都',
    'for future reference', 'from now on', 'next time I ask', 'always remember',
    'keep this for later', 'remember this later', 'use this going forward',

    // === 强调重要性类（侧面暗示需要记忆） ===
    '这是重点', '划重点', '这个很重要', '你要注意这点', '关键点是',
    'this is important', 'key point', 'pay attention to this', 'crucial information',
...
export const CONFIRMATION_KEYWORDS = [
    '对的', '对', '是的', '没错', '正确', '就是这样',
    'yes', 'right', 'correct', 'exactly', 'that is right', 'that\'s right',
    'yep', 'yeah',
];
