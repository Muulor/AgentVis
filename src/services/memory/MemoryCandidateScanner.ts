...
import type { Message, MemoryCandidate } from './types';
import {
  matchAllIntents,
...
export class MemoryCandidateScanner {
  ...
  scan(messages: Message[]): ScanResult {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (userCandidates.length > 0) {
        logger.trace(
          '[CandidateScanner] 用户消息匹配到',
          userCandidates.length,
          '个候选, userConfirmed:',
          hasUserConfirmation
        );
        userCandidates.forEach((c) =>
          logger.trace('  -', c.category, ':', c.content.substring(0, 50))
        );
      }
  ...
  private scanMessage(
    ...
    for (const match of intentMatches) {
      ...
      const baseScore =
        certaintyScore + Math.floor(match.confidence * 2) - fuzzyCount - emotionalCount;
