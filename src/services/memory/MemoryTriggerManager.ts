...
export class MemoryTriggerManager {
  ...
  async recordInteractionAndDecide(
    ...
    if (semanticDelta > 0) {
      ...
      logger.trace(
        `[MemoryTrigger] 累加语义分数: +${semanticDelta.toFixed(1)}, 当前: ${state.candidateSignalScore.toFixed(1)}`
      );
    }
  ...
  async triggerOnLifecycleEvent(latestMessageId?: string): Promise<TriggerDecision> {
    ...
    const hasAccumulatedSignals = state.candidateSignalScore > 0 || state.turnsSinceLastExtract > 0;

    // 只要有新内容就强制触发（生命周期触发的设计意图）
    if (hasNewContent) {
      logger.trace(
        `[MemoryTrigger] 生命周期强制触发: 新内容=${hasNewContent}, 累积信号=${hasAccumulatedSignals}`
      );
      return {
        shouldTrigger: true,
        reason: 'lifecycle',
  ...
  private makeDecision(state: MemoryTriggerState, isLifecycleEvent: boolean): TriggerDecision {
    ...
    if (state.candidateSignalScore >= SEMANTIC_SCORE_THRESHOLD) {
      logger.trace(
        `[MemoryTrigger]  语义触发: 分数 ${state.candidateSignalScore.toFixed(1)} >= ${SEMANTIC_SCORE_THRESHOLD}`
      );
      return {
        shouldTrigger: true,
        reason: 'semantic',
    ...
    if (
      ...
      logger.trace(
        `[MemoryTrigger]  兜底触发: 轮次 ${state.turnsSinceLastExtract} >= ${MAX_TURNS_BEFORE_FALLBACK}, 分数 ${state.candidateSignalScore.toFixed(1)}`
      );
      return {
        shouldTrigger: true,
        reason: 'fallback',
