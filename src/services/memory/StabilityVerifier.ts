...
import type { MemoryCandidate, StabilityVerificationResult, LongTermFactCategory } from './types';
...
export class StabilityVerifier {
  ...
  verify(candidate: MemoryCandidate): StabilityVerificationResult {
    ...
    const hasFuzzyWords = FUZZY_KEYWORDS.some((kw) => content.includes(kw.toLowerCase()));
    if (!hasFuzzyWords) {
    ...
    const hasWeakTemporal = WEAK_TEMPORAL_KEYWORDS.some((kw) => content.includes(kw.toLowerCase()));
  ...
  async verifyBatchAsync(candidates: MemoryCandidate[]): Promise<{
    ...
    // 同步快速过滤
    const grayZoneCandidates: Array<{
      candidate: MemoryCandidate;
      result: StabilityVerificationResult;
      index: number;
    }> = [];
    ...
    if (grayZoneCandidates.length > 0) {
      logger.trace(
        `[StabilityVerifier] 灰度地带候选数量: ${grayZoneCandidates.length}，触发语义增强`
      );
      ...
      for (let i = 0; i < grayZoneCandidates.length; i++) {
        ...
        if (enhancedResult.score >= MEMORY_WRITE_THRESHOLD) {
          // 语义增强后晋升
          logger.trace(
            `[StabilityVerifier]  候选晋升: "${candidate.content.substring(0, 30)}..." (${result.score} -> ${enhancedResult.score})`
          );
          passed.push(candidate);
        } else {
          // 仍在灰度地带，保留
  ...
  private applySemanticAmplification(
    ...
    const reasons = baseResult.reason
      ? baseResult.reason.split(REASON_SPLIT_PATTERN).filter(Boolean)
      : [];
    ...
    if (hints.certaintyBoost > 0 && enhancedBreakdown.certainty > 0) {
      const amplifiedCertainty = Math.round(
        enhancedBreakdown.certainty * SCORE_WEIGHTS.SEMANTIC_AMPLIFIER
      );
    ...
    if (hints.certaintyBoost > 0 && enhancedBreakdown.decisionImpact > 0) {
      const amplifiedImpact = Math.round(
        enhancedBreakdown.decisionImpact * SCORE_WEIGHTS.SEMANTIC_AMPLIFIER
      );
    ...
    if (hints.uncertaintyPenalty > 0 && enhancedBreakdown.temporaryPenalty < 0) {
      const amplifiedPenalty = Math.round(
        enhancedBreakdown.temporaryPenalty * SCORE_WEIGHTS.SEMANTIC_AMPLIFIER
      );
  ...
  mergeCandidates(
    ...
    for (const newCandidate of newCandidates) {
      // 查找相似的已有候选
      const existingIndex = result.findIndex((existing) => this.isSimilar(existing, newCandidate));
  ...
  async mergeCandidatesAsync(
    ...
    for (const newCandidate of newCandidates) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        // 使用 Embedding 语义匹配
        try {
          const isSimilar = await embeddingService.isSemanticallySimilar(
            existing.content,
      ...
      if (existingIndex >= 0) {
        ...
        if (existing) {
          ...
          existing.score = Math.max(existing.score, newCandidate.score);
          logger.trace(
            `[StabilityVerifier]  语义合并: "${newCandidate.content.substring(0, 30)}..." -> occurrenceCount=${existing.occurrenceCount}`
          );
        }
      } else {
        // 添加为新候选
   ...
   * 使用简单的文本相似度判断
   */
  private isSimilar(a: MemoryCandidate, b: Omit<MemoryCandidate, 'id'>): boolean {
    // 类别必须相同
