        ...
        for (let attempt = 1; attempt <= MEMORY_LLM_MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (!response.content || response.content.trim().length === 0) {
                    logger.warn(`[LLMAdapter]  收到空响应 (尝试 ${attempt}/${EMPTY_RESPONSE_MAX_RETRIES + 1})`);
                    ...
                    // 最后一次尝试仍然失败
                    throw new Error('LLM API returned an empty response. The service may be rate-limited or temporarily unavailable.');
                }
                ...
                if (
                    ...
                    attempt < MEMORY_LLM_MAX_ATTEMPTS
                ) {
    ...
    async generateWithSystem(
        ...
        for (let attempt = 1; attempt <= MEMORY_LLM_MAX_ATTEMPTS; attempt++) {
            try {
                ...
                // 检测空响应
                if (!response.content || response.content.trim().length === 0) {
                    logger.warn(`[LLMAdapter]  generateWithSystem 收到空响应 (尝试 ${attempt}/${EMPTY_RESPONSE_MAX_RETRIES + 1})`);
                ...
                logger.error(`[LLMAdapter] generateWithSystem 调用失败 (${retryClassification.reason}):`, error);
                throw lastError;
            }
        }
