...
import {
  getMemorySafeMessageContent,
  stripMemoryVisualCodeBlocks,
} from './utils/SafeMessageContent';
...
export class MemoryContextProvider {
  ...
  async getMemoryContext(
    ...
    try {
      // 获取全量事实和摘要
      const result = await invoke<MemoryContextResponse>('memory_get_context', { agentId });
      ...
      if (options?.userQuery && summaries.length > 0) {
        ...
        logger.trace(
          `[MemoryContextProvider] 摘要混合召回: ${result.summaries.length} → ${summaries.length}${isDegraded ? ' (降级)' : ''}`
        );
      }
      ...
      if (!isDegraded) {
        ...
        if (options?.includeOriginal && summaries.length > 0) {
          ...
          const loadedCount = summaries.filter(
            (s) => s.originalMessages && s.originalMessages.length > 0
          ).length;
      ...
      if (summaries.length > 0) {
        ...
        for (const [i, s] of summaries.entries()) {
          ...
          logger.trace(
            `  │ 内容: ${s.content.substring(0, 120)}${s.content.length > 120 ? '...' : ''}`
          );
          ...
          if (s.originalMessages && s.originalMessages.length > 0) {
            ...
            s.originalMessages.forEach((msg, idx) => {
              ...
              logger.trace(
                `  │    ${idx + 1}. [${roleTag}] "${msg.content.substring(0, 80)}${msg.content.length > 80 ? '...' : ''}"`
              );
            });
          } else {
          ...
          const questionSliceCount =
            s.openQuestions?.reduce((sum, q) => sum + (q.evidenceSlices?.length ?? 0), 0) ?? 0;
          if (questionSliceCount > 0) {
            ...
            s.openQuestions?.forEach((q) => {
              q.evidenceSlices?.forEach((slice, idx) => {
                ...
                logger.trace(
                  `  │    [${q.question.substring(0, 25)}...] ${idx + 1}. [Turn ${slice.turnId} - ${speaker}] "${slice.content.substring(0, 60)}${slice.content.length > 60 ? '...' : ''}"`
                );
              });
            });
  ...
  private async loadOriginalMessages(
    ...
    try {
      ...
      messagesMap = new Map(
        messages.map((m) => [m.id, { role: m.role, content: getMemorySafeMessageContent(m) }])
      );
    } catch (error) {
   ...
   */
  private async loadEvidenceSlices(
    summaries: MemoryItem[],
    userQuery?: string
  ): Promise<MemoryItem[]> {
    ...
    logger.trace(
      `[MemoryContextProvider]  开始 Evidence 回溯，${summariesWithQuestions.length} 条摘要有待决问题`
    );
    ...
    const updatedSummaries = await Promise.all(
      summaries.map(async (summary) => {
        ...
        if (totalSliceCount > 0) {
          logger.trace(
            `[MemoryContextProvider]  摘要 ${summary.id.substring(0, 8)} 加载了 ${totalSliceCount} 个 Evidence Slices:`
          );
          updatedQuestions.forEach((q) => {
            q.evidenceSlices?.forEach((slice, idx) => {
              logger.trace(
                `    [${q.question.substring(0, 30)}...] ${idx + 1}. [Turn ${slice.turnId} - ${slice.speaker}] "${slice.content.substring(0, 60)}..."`
              );
            });
          });
        }
  ...
  buildBindingFactsPrompt(facts: MemoryItem[]): string | null {
    ...
    const priorityNote =
      "> **Note**: The information above is summarized from historical interactions with the user. If the current user request conflicts with historical preferences, follow the user's **currently explicit intent**.";
  ...
  buildContextFactsPrompt(facts: MemoryItem[]): string | null {
    ...
    return (
      '# Factual Background From User Interactions (for reference only)\n\n' + parts.join('\n\n')
    );
  }
        ...
        if (groupedByEvidence.size > 0) {
          ...
          for (const group of groupedByEvidence.values()) {
            for (const sl of group.slices) {
              ...
              lines.push(
                `    [Turn ${sl.turnId} - ${speaker}] "${stripMemoryVisualCodeBlocks(sl.content)}"`
              );
            }
          }
        }
