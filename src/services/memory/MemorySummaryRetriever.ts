...
export class MemorySummaryRetriever {
    async retrieve<TSummary extends MemorySummaryItem>(
        ...
        try {
            ...
            logger.warn(`[MemorySummaryRetriever] embedding 摘要召回失败，尝试 BM25 强锚点兜底: ${errorMessage}`);
        }
    ...
    private searchBm25(
        ...
        return bm25.search(SUMMARY_AGENT_SCOPE, query, topK)
            .map((result, index) => ({
    ...
    private fuseResults<TSummary extends MemorySummaryItem>(
        ...
        const bm25Weight = anchors.length > 0
...
function getRecentSummaries<TSummary extends MemorySummaryItem>(summaries: TSummary[], topK: number): TSummary[] {
    return [...summaries]
