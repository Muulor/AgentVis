...
describe('memory prompt invariants', () => {
  ...
  it('keeps memory context fact prompts in English while preserving fact content', () => {
    ...
    const facts: MemoryItem[] = [
      ...
      createMemoryItem({
        content: 'The user is preparing for system design interviews.',
        category: 'long_term_goal',
        updatedAt: now - 2 * 60 * 60 * 1000,
      }),
      createMemoryItem({
        content: 'The user is comfortable with TypeScript.',
        category: 'knowledge_level',
        updatedAt: now - 24 * 60 * 60 * 1000,
      }),
      createMemoryItem({
        content: 'The user often weighs speed against implementation quality.',
        category: 'interaction_signals',
        updatedAt: now - 7 * 24 * 60 * 60 * 1000,
      }),
      createMemoryItem({
        content: 'On Windows, prefer findstr over grep.',
