...
describe('MemoryContextProvider evidence budget', () => {
  beforeEach(() => {
    ...
    summaryRetrieveMock.mockImplementation(
      async (_agentId: string, _query: string, allSummaries: MemoryItem[]) => ({
  ...
  it('expands evidence only for the top recalled summary and defaults to one turn', async () => {
    ...
    expect(
      topSummary?.openQuestions?.[0]?.evidenceSlices?.map(
        (slice) => `${slice.turnId}:${slice.speaker}`
      )
...
> = {
  ...
  'top-a2': {
    ...
    role: 'assistant',
    content:
      '宁德时代核心分析摘要，包含公司地位、财务数据、核心竞争力、近期事件、机构观点和风险提示。',
    createdAt: 4,
  },
  'other-u1': {
    id: 'other-u1',
