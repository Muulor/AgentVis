...
const installImmediateRetryTimers = (): ReturnType<typeof vi.spyOn> => {
    ...
    setTimeoutSpy.mockImplementation((
        ...
        if (typeof timeout === 'number' && retryDelaysMs.has(timeout) && typeof handler === 'function') {
...
const createAdapter = (): LLMAdapter => new LLMAdapter({
...
describe('LLMAdapter retry policy', () => {
    ...
    it('retries transient memory LLM errors before returning generated content', async () => {
        ...
        vi.mocked(invoke)
            .mockRejectedValueOnce(new Error('API returned an error (524 <unknown status code>): error code: 524'))
    ...
    it('applies the same retry policy to generateWithSystem', async () => {
        ...
        await expect(createAdapter().generateWithSystem('system', 'extract fact')).resolves.toBe('fact ok');
    ...
    it('keeps the existing lightweight empty-response retry separate from API retries', async () => {
        ...
        await expect(createAdapter().generate('summarize empty first')).resolves.toBe('summary after empty retry');
