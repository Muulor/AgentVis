...
export function buildMemoryExtractorPrompt(candidate: MemoryCandidate): string {
  ...
  return MEMORY_EXTRACTOR_PROMPT.replace('{category}', candidate.category)
...
export class FactExtractor {
   ...
   * @returns 提取结果
   */
  async extractFromVerifiedCandidate(candidate: MemoryCandidate): Promise<MemoryExtractorResult> {
  ...
  async extractAndSaveFromVerified(candidates: MemoryCandidate[]): Promise<{
    ...
    for (const candidate of candidates) {
      ...
      if (result.extract && factContent && result.category) {
        ...
        // LLM 正常拒绝提取，不需要重试
        logger.trace(
          `[FactExtractor]  跳过候选: ${candidate.content} (${result.reason ?? 'unknown'})`
        );
      }
    }
   ...
   * 获取指定类别的事实 (V2 格式)
   */
  async getFactsByCategoryV2(category: LongTermFactCategory): Promise<
    Array<{
      id: string;
      content: string;
      confidence: number;
   ...
   * 获取指定类别的事实
   */
  async getFactsByCategory(category: LongTermFactCategory): Promise<
    Array<{
      id: string;
      content: string;
      importance: number;
