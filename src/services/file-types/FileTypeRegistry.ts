...
export type PreviewRenderer = 'audio' | 'binaryDoc' | 'code' | 'image' | 'markdown' | 'plainText' | 'video';
...
export const PREVIEW_IMAGE_EXTENSIONS = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'bmp', 'ico'] as const;
...
export function getFileTypeInfo(filePathOrName: string): FileTypeInfo {
    ...
    const mimeType = IMAGE_MIME_BY_EXTENSION[ext]
        ?? VIDEO_MIME_BY_EXTENSION[ext]
        ?? AUDIO_MIME_BY_EXTENSION[ext];
