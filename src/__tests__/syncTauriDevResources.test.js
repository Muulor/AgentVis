...
describe('syncTauriDevResources', () => {
  it('syncs the skills bundle from a Unicode project path and replaces stale output', () => {
    ...
    expect(existsSync(join(repoRoot, 'src-tauri', 'target', 'debug', 'native-scripts', 'stale.txt'))).toBe(false)
    expect(existsSync(join(repoRoot, 'src-tauri', 'target', 'debug', 'python-runtime', 'stale.txt'))).toBe(false)
    expect(result.resources.map(resource => resource.name)).toEqual(['skills-bundle', 'native-scripts', 'python-runtime'])
  })
})
