 ...
 */
export const AttachmentCard = memo(function AttachmentCard({ attachments }: AttachmentCardProps) {
  const { t } = useI18n();
  ...
  return (
    <div className={styles.container}>
      {attachments.map((attachment) => (
        ...
        >
          <span className={styles.icon}>{getAttachmentIcon(attachment.type)}</span>
          <span className={styles.fileName}>{truncateFileName(attachment.fileName)}</span>
          {attachment.indexed && (
