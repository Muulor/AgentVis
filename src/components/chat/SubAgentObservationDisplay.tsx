...
export const SubAgentObservationDisplay = memo(function SubAgentObservationDisplay({
  ...
  const steps = useMemo(() => groupObservationsIntoSteps(data), [data]);
      ...
      {showContent && (
        <div className={styles.contentContainer}>
          {steps.map((step, index) => (
            <StaticStepItem key={`step-static-${index}-${step.timestamp}`} step={step} />
          ))}
        </div>
      )}
    </div>
...
function parseResultText(raw: string): string {
  // Step 1: 移除终止信号
  const cleaned = raw.replace(/TASK_COMPLETE/g, '').trim();
...
function StaticStepItem({ step }: StaticStepItemProps) {
  ...
  return (
    <>
      {step.reasoningTrace && <StaticReasoningTrace trace={step.reasoningTrace} />}
      {/* LLM 思考文字：每步只显示一次 */}
