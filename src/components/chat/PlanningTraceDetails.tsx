...
import {
  ...
  type ThinkingChainData,
} from './ThinkingChainDisplay';
...
function hasThinkingContent(
  ...
  return [thinkingChain?.analyzing, thinkingChain?.planning, thinkingChain?.decided].some(
...
function StaticReasoningTrace({ trace }: { trace: PersistedReasoningTrace }) {
  ...
  return (
    <div className={styles.reasoningTrace}>
      ...
      {isExpanded && <div className={styles.reasoningTraceBody}>{content}</div>}
    </div>
  );
}
