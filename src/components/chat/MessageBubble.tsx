...
export const MessageBubble = memo(function MessageBubble({
  ...
  const visualEnhancementJob = useChatStore((state) =>
    state.visualEnhancementJobsByMessage.get(message.id)
  );
  ...
  const originalContent = useMemo(() => getMessageOriginalDisplayContent(message), [message]);
  const canSwitchVisualVersion =
    role === 'assistant' &&
  ...
  const content =
    resolvedVisualView === 'original' && originalContent ? originalContent : enhancedContent;
  ...
  const displayContent = useMemo(() => {
    ...
    return typeof metadataDisplayContent === 'string' ? metadataDisplayContent : content;
  ...
  const handleAction = useCallback(
    (action: 'copy' | 'quote' | 'delete' | 'revoke' | 'multiselect') => {
      onAction?.(message.id, action, action === 'copy' ? { contentOverride: content } : undefined);
    },
  ...
  const handleCodePreview = useCallback(
    (code: string, language: string) => {
      ...
      const previewCode = language === 'svg' ? wrapSvgInHtml(code) : code;
  ...
  const hasRichContent = useMemo(() => {
    ...
    const hasVisualBlock = extractFencedCodeBlocks(content).some(
      (block) =>
  ...
  const renderThinkingBlock = (): React.ReactNode => {
    ...
    return <ChatReasoningTrace content={reasoning} defaultExpanded={false} />;
  };
    ...
    return (
      <span className={styles.inlineMessageContent}>
        {displayParts.map((part, index) => {
          ...
          const Icon =
            token.type === 'skill' ? Toolbox : token.type === 'folder' ? Folder : FileText;
  ...
  return (
    <div
      className={cx(
        ...
        multiSelectMode && styles.multiSelectBubble,
    ...
    >
      {/* 多选模式：统一左侧显示选择圆圈 */}
      {multiSelectMode && (
        <SelectCheckbox checked={selected} onChange={() => onToggleSelect?.(message.id)} />
      )}
      {/* Agent 消息显示头像 */}
      {!isUser && (
      ...
      <div className={styles.contentWrapper}>
        ...
        <div className={styles.header}>
          {!isUser && <span className={styles.senderName}>{agentName}</span>}
        ...
        {!isUser && message.metadata?.mode === 'planning' && (
          <PlanningTraceDetails
            reasoningTrace={
              message.metadata.reasoningTrace as
                | { content: string; isCompleted?: boolean }
                | undefined
            }
            thinkingChain={
              message.metadata.thinkingChain as
                | { analyzing: string; planning: string; decided: string }
                | undefined
            }
            thinkingSteps={
              message.metadata.thinkingSteps as
                | Array<{
                    stepNumber: number;
                    analyzing: string;
                    planning: string;
            ...
            subAgentObservations={
              message.metadata.subAgentObservations as
                | import('@/services/planning/agent-loop/types').SubAgentObservationEvent[]
                | undefined
            }
        ...
        {hasWidgetChoices && !isUser && contextId && (
          <BubbleReplyBar messageId={message.id} contextId={contextId} agentId={message.agentId} />
        )}

        {/* standalone 决策树已处理态操作栏 */}
        {hasStandaloneTreeWidget && !isUser && contextId && (
          <StandaloneTreeReplyBar messageId={message.id} contextId={contextId} content={content} />
        )}
        ...
        {visualEnhancementJob && !multiSelectMode ? (
          <div className={styles.visualEnhancementFooter}>
            <div className={styles.visualEnhancementStatus} role="status">
              ...
              <span>
                {t(
                  visualEnhancementJob.status === 'queued'
        ...
        {isHovered && !multiSelectMode && (
          <MessageActions isUser={isUser} onAction={handleAction} />
        )}
      </div>
    </div>
  );
      ...
      {quotes.map((quote, index) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={cx(
                styles.quotedFromContent,
                isExpanded && styles.quotedFromContentExpanded
              )}
            >
              {displayContent}
            </span>
