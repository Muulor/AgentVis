...
export function FSMVisualizationPanel({ className, contextId }: FSMVisualizationPanelProps) {
  ...
  const contextState = useFSMVisualizationStore((s) =>
    contextId ? s.contextStates[contextId] : undefined
  );
  ...
  const { currentDecision, isThinking, thinkingSteps, subAgentObservations, isSubAgentRunning } =
    contextState;
  ...
  const hasReasoningTrace =
    Boolean(maybeReasoningTrace?.content ?? '') || (maybeReasoningTrace?.isStreaming ?? false);
