...
function getDecisionLabel(decisionType: MasterBrainDecision['decision'], t: ReturnType<typeof useI18n>['t']) {
    switch (decisionType) {
            ...
            {decision.decision === 'SPAWN_SUB_AGENT' && decision.nextStep && (
                <div className={styles.details ?? ''}>
                    ...
                    <span className={styles.detailValue ?? ''}>
                        {((decision.nextStep as Record<string, unknown>).task as string | undefined) ?? t('chat.decisionTaskFallback')}
                    </span>
                </div>
            )}
