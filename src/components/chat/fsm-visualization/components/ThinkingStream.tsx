...
function getNextChunkSize(remainingChars: number, maxCharsPerFrame: number): number {
    return Math.max(
        1,
        Math.min(maxCharsPerFrame, Math.ceil(remainingChars / CATCH_UP_FRAME_COUNT))
    );
}
...
export function ThinkingStream({
    ...
    const maxCharsPerFrame = typeSpeed && typeSpeed > 0
        ? Math.max(1, Math.round(typeSpeed))
        : DEFAULT_MAX_CHARS_PER_FRAME;
