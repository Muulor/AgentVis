...
function cloneDisplayParts(parts?: InputDisplayPart[]): InputDisplayPart[] | undefined {
  return parts?.map((part) =>
    part.type === 'text' ? { ...part } : { type: 'token', token: { ...part.token } }
  );
}
...
function getTokenFromElement(element: HTMLElement): InputContextToken {
  ...
  const tokenType: InputContextTokenType =
    rawTokenType === 'skill' || rawTokenType === 'folder' ? rawTokenType : 'file';
...
function isInlineTokenElement(node: Node): node is HTMLElement {
  return (
    ...
    (node.classList.contains(INLINE_TOKEN_CLASS) || node.classList.contains(FILE_TOKEN_CLASS))
  );
}

function findClosestInlineTokenElement(
  ...
  return tokenElement instanceof HTMLElement && root.contains(tokenElement) ? tokenElement : null;
}
...
function buildRichNodesFromClipboardHtml(html: string): Node[] | null {
  if (
    !html.includes(INLINE_TOKEN_CLASS) &&
    !html.includes(FILE_TOKEN_CLASS) &&
    !html.includes(SKILL_TOKEN_CLASS)
  ) {
    return null;
  }
  ...
  if (
    !template.content.querySelector(
      `.${INLINE_TOKEN_CLASS}, .${FILE_TOKEN_CLASS}, .${SKILL_TOKEN_CLASS}`
    )
  ) {
    return null;
  }
...
export const ChatInput = memo(function ChatInput({
  ...
  const [contextTokens, setContextTokensState] = useState<InputContextToken[]>(
    initialDraft.contextTokens
  );
  ...
  const setContextTokens = useCallback((nextTokens: SetStateAction<InputContextToken[]>) => {
    setContextTokensState((previousTokens) => {
      const resolvedTokens =
        typeof nextTokens === 'function' ? nextTokens(previousTokens) : nextTokens;
  ...
  const handleSend = useCallback(() => {
    ...
    const fileReferenceLines = richSnapshot?.fileTokens
      ...
      .map((token) =>
        t(token.type === 'folder' ? 'chat.folderReferenceLine' : 'chat.fileReferenceLine', {
          name: token.label,
          path: token.path ?? '',
        })
      )
      ...
      richSnapshot
        ? {
            displayContent: displayParts
              ? buildDisplayContent(displayParts)
              : [contextPrefix, trimmed].filter(Boolean).join(' '),
            displayParts,
  ...
  const updateSkillSlashState = useCallback(
  ...
  const updateFileMentionState = useCallback(
    (text: string, cursorPosition: number) => {
      if (enableMention) {
        closeFileDropdown();
        return;
  ...
  const selectSkill = useCallback(
    (skill: SkillSlashOption) => {
      ...
      const token: InputContextToken = {
        ...
        badge: skill.mode === 'guide' ? t('chat.skillModeGuide') : t('chat.skillModeScript'),
  ...
  // 键盘事件 - 快捷发送
  const handleRichEditorMouseDown = useCallback(
  ...
  const replaceTextareaRange = useCallback(
    (start: number, end: number, insertText: string) => {
      const textarea = textareaRef.current;
      if (!textarea) return;
  ...
  const handleInputContextMenuAction = useCallback(
    async (action: InputContextMenuAction) => {
      const textarea = textareaRef.current;
      if (!textarea) return;
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (event: MouseEvent) => {
      if (
        inputContextMenuRef.current &&
        !inputContextMenuRef.current.contains(event.target as Node)
      ) {
        closeInputContextMenu();
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (event: MouseEvent) => {
      const target = event.target as Node;
      if (fileDropdownRef.current?.contains(target) || richEditorRef.current?.contains(target)) {
        return;
      }
      closeFileDropdown();
    };
  ...
  const handleAttachmentReorder = useCallback(
    (reorderedAttachments: AttachmentInfo[]) => {
  ...
  return (
    <div className={styles.inputArea}>
      ...
      {/* 引用预览 */}
      {pendingQuotes.length > 0 && <QuotePreview quotes={pendingQuotes} onRemove={onRemoveQuote} />}
          ...
          <div className={styles.skillChipList}>
            {contextTokens.map((token) => (
              <span className={styles.skillChip} key={token.id}>
                ...
                {token.badge && <span className={styles.skillChipBadge}>{token.badge}</span>}
                <button
                  type="button"
      ...
      <div className={styles.composer}>
        ...
        >
          ...
          <div className={styles.textareaWrapper} onPaste={handlePaste} data-custom-context-menu>
            {enableMention ? (
            ...
            {!enableMention && showFileDropdown && (
              <div className={styles.skillDropdown} ref={fileDropdownRef}>
                <div className={styles.skillDropdownHeader}>{t('chat.fileMentionHeader')}</div>
            ...
            {inputContextMenu && (
              ...
              >
                <button
                  ...
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => {
                    void handleInputContextMenuAction('cut');
                  }}
                ...
                <button
                  ...
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => {
                    void handleInputContextMenuAction('copy');
                  }}
                ...
                <button
                  ...
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => {
                    void handleInputContextMenuAction('paste');
                  }}
                  disabled={disabled}
                >
                ...
                <button
                  ...
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => {
                    void handleInputContextMenuAction('selectAll');
                  }}
        ...
        <div className={styles.toolbar}>
          <div className={styles.toolbarLeft}>
            ...
            />
            {enableAttachment && (
              <AttachmentButton onFileSelect={handleAttachmentSelect} disabled={disabled} />
            )}
            {/* 项目路径按钮：仅 Agent 窗口显示（agentId 存在时） */}
            {agentId && (
              <ProjectPathButton agentId={agentId} projectPath={projectPath} disabled={disabled} />
            )}
          </div>
