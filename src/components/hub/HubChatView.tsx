...
export function HubChatView({ setupChecklistState }: HubChatViewProps) {
    ...
    const currentStreamingState = useMemo(() => {
        ...
        return streamingByContext.get(currentHubId) ?? { content: '', isStreaming: false, agentName: undefined };
    ...
    useEffect(() => {
        const unsubscribe = useWidgetStore.subscribe((state) => {
            ...
            if (action?.contextId !== currentHubId || isSendingRef.current || isStreamingRef.current) return;
            ...
            let targetAgent = action.agentId
    ...
    const handleSend = useCallback(async (content: string) => {
        ...
        const createUserMessageOnly = async () => {
            ...
            addHubMessage(currentHubId, {
                ...
                quotedFrom: pendingQuotes.length > 0
                    ? serializeQuotesForMessage(pendingQuotes)
                    : undefined,
    ...
    }, [currentHubId, isSending, agents, mode, pendingQuotes, pendingAttachments, addHubMessage, clearQuotes, toast, getAttachmentsCopy, clearAttachments, sendHubMessage, executePlanningTask, t]);
            ...
            case 'quote': {
                ...
                for (const msg of selectedMsgs) {
                    ...
                    addQuote(currentHubId, {
                        ...
                        hubId: currentHubId,
                        agentName: msg.role === 'user' ? t('chat.userLabel') : (msg.metadata?.agentName ?? 'Hub'),
                    });
                }
                exitMultiSelect(currentHubId);
                break;
