...
import { buildFullFileDiff, toggleRegionExpanded } from '../../services/fast-apply/FullFileDiffBuilder';
...
function getEstimatedWrapChars(containerWidth: number): number {
    ...
    return Math.max(
        MIN_ESTIMATED_WRAP_CHARS,
        Math.floor(textWidth / ESTIMATED_MONO_CHAR_WIDTH)
    );
}
...
function estimateWrappedLineHeight(content: string | undefined, wrapChars: number): number {
    const lineCount = Math.max(
        1,
        Math.min(
            MAX_ESTIMATED_LINE_WRAP,
            Math.ceil((content?.length ?? 0) / wrapChars)
        )
    );
...
function clampVisibleRange(
    ...
    const endOffset = scrollTop + Math.max(viewportHeight, DEFAULT_VIEWPORT_HEIGHT) + VIRTUAL_OVERSCAN_PX;
...
function isMostlyWholeFileReplace(
    ...
    const coversMatchedRange = mod.matchResult.startLine <= 1
        && mod.matchResult.endLine >= Math.max(1, originalLines - 1);
    const coversMostContent = mod.modification.search.length >= originalContent.length * 0.8
        && searchLines >= Math.max(1, Math.floor(originalLines * 0.8));

    return coversMatchedRange || coversMostContent;
}

function countChangedDiffLines(mod: ModificationApplyResult): { addedLines: number; removedLines: number } {
    let addedLines = 0;
    let removedLines = 0;
        ...
        <div className={styles.largeDiffSummary}>
            ...
            <div className={styles.largeDiffBody}>
                ...
                <div className={styles.largeDiffMeta}>
                    ...
                    <span>{t('diff.largeDiffChangedLines', { count: summary.addedLines + summary.removedLines })}</span>
                </div>
            </div>
        </div>
    );
        ...
        switch (item.type) {
            ...
            case 'collapsed':
            {
                ...
                return (
                    <CollapsedLines
                        lineCount={lineCount}
                        onExpand={() => onToggleRegion(regionIndex)}
                    />
                );
            }
