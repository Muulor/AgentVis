  ...
  return (
    <div className={cx(styles.item, isCurrentVersion && styles.currentItem)}>
      <div className={styles.itemHeader}>
        <span className={styles.itemIndicator}>{isCurrentVersion ? '●' : '○'}</span>
        <span className={styles.itemTitle}>{currentVersionLabel}</span>
        ...
        <Tooltip content={formatRelativeTime(snapshot.timestamp, t)}>
          <span className={styles.itemTime}>{formatExactTime(snapshot.timestamp, language)}</span>
        </Tooltip>
      </div>
      ...
      {!isCurrentVersion && (
        <div className={styles.itemActions}>
          <button className={styles.previewBtn} onClick={() => onPreview(snapshot)}>
            {t('diff.preview')}
          </button>
          <Tooltip content={t('diff.rollback')}>
            <button className={styles.rollbackBtn} onClick={() => onRollback(snapshot)}>
              {t('diff.rollback')}
            </button>
          </Tooltip>
          {onDelete && (
            <Tooltip content={t('diff.deleteVersionTitle')}>
              <button className={styles.deleteBtn} onClick={() => onDelete(snapshot)}>
                {t('common.delete')}
              </button>
            </Tooltip>
          )}
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.list}>
        ...
        {snapshots.length === 0 && <div className={styles.empty}>{t('diff.emptyHistory')}</div>}
      </div>
