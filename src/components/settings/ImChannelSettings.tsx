...
import type {
  ...
  FeishuChannelConfig,
  SlackChannelConfig,
} from '@services/im-channel/types';
...
export function ImChannelSettings() {
   ...
   * 4. 清除旧版哨兵字段
   */
  async function performLegacyMigration(hubId: string | null, agentId: string): Promise<void> {
  ...
  const getFormState = useCallback(
    (botId: string): BotFormState => formStates[botId] ?? createEmptyFormState(),
    [formStates]
  );

  /** 更新指定 Bot 的部分表单状态 */
  const updateFormState = useCallback((botId: string, updates: Partial<BotFormState>) => {
    setFormStates((prev) => ({
      ...prev,
  ...
  useEffect(() => {
    for (const config of botConfigs) {
      ...
      const shouldClearAgent = shouldClearAgentAfterHubResolve(config.hubId, resolvedHubId);

      updateBotConfig(
        config.botId,
        shouldClearAgent ? { hubId: resolvedHubId, agentId: null } : { hubId: resolvedHubId }
      );
  ...
  const handleToggleExpand = useCallback(
    async (botId: string) => {
      ...
      // 展开时加载凭据（如果尚未加载）
      const hasLoadedCredentials =
        botConfig.platform === 'slack' ? Boolean(current.botToken) : Boolean(current.appId);
  ...
  useEffect(() => {
    for (const config of botConfigs) {
      ...
      const agentsNotLoaded = form.lastLoadedHubId !== config.hubId && !form.isLoadingAgents;
      ...
      if (missingAgentAction === 'reload' && config.agentId) {
        updateFormState(config.botId, {
          lastMissingAgentReloadKey: getMissingAgentReloadKey(config.hubId, config.agentId),
        });
      }
  ...
  const renderPlatformSection = (platform: 'feishu' | 'slack') => {
    ...
    const title = platform === 'slack' ? t('settings.im.slackTitle') : t('settings.im.feishuTitle');
    const description =
      platform === 'slack' ? t('settings.im.slackDescription') : t('settings.im.feishuDescription');
    const addLabel =
      platform === 'slack' ? t('settings.im.addSlackBot') : t('settings.im.addFeishuBot');
    const openAppTitle =
      platform === 'slack'
            ...
            platformBotConfigs.map((botConfig) => (
              <BotCard
                ...
                onDisplayNameChange={(name) =>
                  updateBotConfig(botConfig.botId, { displayName: name })
                }
                onOutboundReceiveIdTypeChange={(outboundReceiveIdType) =>
                  updateBotConfig(botConfig.botId, { outboundReceiveIdType })
                }
                onOutboundReceiveIdChange={(outboundReceiveId) =>
                  updateBotConfig(botConfig.botId, { outboundReceiveId })
                }
                onSlackDefaultChannelIdChange={(slackDefaultChannelId) =>
                  updateBotConfig(botConfig.botId, { slackDefaultChannelId })
                }
...
interface BotCardProps {
  ...
  onOutboundReceiveIdTypeChange: (
    receiveIdType: NonNullable<BotConfig['outboundReceiveIdType']>
  ) => void;
...
function BotCard({
  ...
  const statusText = isConnected
    ? t('settings.im.online')
    : isConnecting
      ? t('settings.im.connecting')
      : t('settings.im.offline');
  ...
  return (
    <div className={botCardClass}>
      ...
      <div className={styles.botCardHeader} onClick={onToggleExpand}>
        ...
        {/* 阻止按钮点击冒泡到折叠逻辑 */}
        <div className={styles.botHeaderActions} onClick={(e) => e.stopPropagation()}>
          {!isConnected ? (
            <button
      ...
      {isExpanded && (
        <div className={styles.botConfigPanel}>
          ...
          <div className={styles.fieldGroup}>
            ...
            />
            <span className={styles.hint}>{t('settings.im.targetAgentHint')}</span>
          </div>
          ...
          {botConfig.platform === 'slack' ? (
            <div className={styles.fieldGroup}>
              ...
              />
              <span className={styles.hint}>{t('settings.im.slackDefaultChannelHint')}</span>
            </div>
          ) : (
            <div className={styles.fieldGroup}>
              ...
              <span className={styles.hint}>{t('settings.im.defaultOutboundTargetHint')}</span>
            </div>
          )}
