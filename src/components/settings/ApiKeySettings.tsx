...
export function ApiKeySettings() {
  ...
  const handleTest = async (provider: string) => {
    ...
    try {
      ...
      toast({
        title: success
          ? t('settings.apiKeys.connectionSuccess')
          : t('settings.apiKeys.connectionFailed'),
  ...
  const handleDelete = async (provider: string) => {
    try {
      ...
      toast({
        title: t('settings.apiKeys.deleteFailed'),
  ...
  return (
    <div className={styles.container}>
      <p className={styles.description}>{t('settings.apiKeys.description')}</p>
            ...
            {provider.apiKeyUrl && (
              <Tooltip content={t('settings.apiKeys.getApiKeyTitle')}>
                <button
                  ...
                  aria-label={t('settings.apiKeys.getProviderApiKeyAria', {
          ...
          <div className={styles.inputRow}>
            <div className={styles.inputWrapper}>
              <input
                ...
                placeholder={
                  apiKeyStatus[provider.id] ? '••••••••••••' : provider.apiKeyPlaceholder
                }
            ...
            {apiKeys[provider.id]?.trim() && (
              <button className={styles.saveButton} onClick={() => handleSave(provider.id)}>
                {t('common.save')}
              </button>
            )}
            ...
            {provider.id !== 'local' && (
              <button
                className={cx(
                  styles.testButton,
                  testResult[provider.id] === 'success' && styles.testSuccess,
                  testResult[provider.id] === 'error' && styles.testError
                )}
            ...
            <div className={styles.localUrlSection}>
              ...
              <div className={styles.inputRow}>
                ...
                {localUrlInput !== localApiUrl && localUrlInput.trim() && (
                  <button className={styles.saveButton} onClick={handleSaveLocalUrl}>
                    {t('common.save')}
                  </button>
                )}
              </div>
