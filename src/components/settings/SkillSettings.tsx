...
import { copySkillPackageToPackagesDir, removeSkillPackage, rescanExternalSkills, reconcileVenvState, uninstallSkill } from '@services/planning/skills/external/ExternalSkillBootstrap';
import { auditSkillPackage, collectPackageFiles } from '@services/planning/skills/external/SkillAuditService';
import { SkillAuditModal } from './SkillAuditModal';
import type { AuditUserDecision } from './SkillAuditModal';
import { installNpmPackage, installSystemTool, installCargoPackage, installGoPackage, isCommandAvailableWithFreshPath, isNpmPackageInstalled, isCargoPackageInstalled, isGoPackageInstalled, isNetworkRelatedError, isChromeForTestingInstallFailure } from '@services/planning/skills/external/DependencyInstaller';
...
function SkillAuditStatusBanner() {
    ...
    return (
        <div className={cx(styles.auditBanner, toneClass)}>
            <div className={styles.auditBannerIcon}>
                {getAuditBannerIcon(auditStatus)}
            </div>
            <div className={styles.auditBannerText}>
                <div className={styles.auditBannerTitleRow}>
                    <span className={styles.auditBannerTitle}>
                        {getAuditBannerTitle(auditStatus, t)}
                    </span>
                    ...
                    {line.split(URL_SPLIT).map((part, partIdx) =>
                        URL_TEST.test(part) ? (
                            <a
                                key={partIdx}
                                href="#"
                                onClick={(e) => { e.preventDefault(); void handleLinkClick(part); }}
                                style={{ color: 'var(--accent-color, #4fc3f7)', textDecoration: 'underline', cursor: 'pointer' }}
                            >
                                {part}
                            </a>
                        ) : (
...
function useInstallHandler() {
    const { t } = useI18n();
    const { setToolInstallStatus, toolInstallStatuses, markPostInstallCommandCompleted } = useRuntimeStore();

    const installNpm = useCallback(async (packageName: string) => {
    ...
    const installSysTool = useCallback(async (
    ...
    const installCargo = useCallback(async (packageName: string) => {
    ...
    const installGo = useCallback(async (modulePath: string) => {
    ...
    const runPostInstallCmd = useCallback(async (command: string) => {
    ...
    const getStatus = useCallback((key: string): ToolInstallEntry | undefined => {
...
function SkillListSection() {
    ...
    const { installNpm, installSysTool, installCargo, installGo, runPostInstallCmd, getStatus } = useInstallHandler();
    const { setToolInstallStatus, toggleSkillEnabled, completedPostInstallCommands } = useRuntimeStore();
    ...
    // 删除状态管理
    const [pendingDelete, setPendingDelete] = useState<{ name: string; packagePath: string } | null>(null);
    ...
    const handleOpenSkillFolder = useCallback(async (packagePath: string) => {
        try {
            ...
            try {
                ...
                for (const skill of installedSkills) {
                    ...
                    for (const postInstall of skill.npmPostInstallCommands) {
                        npmPostInstallCommands.set(
                            postInstallCommandKey(postInstall.command),
                            postInstall
                        );
                    }
                ...
                // 并行检测系统工具
                const sysChecks = Array.from(sysToolsMap.entries()).map(
                    async ([, tool]) => {
                        ...
                        const available = await isCommandAvailableWithFreshPath(checkCmd, shellExec, tool.windowsExePaths);
                        setToolInstallStatus(key, {
                ...
                const postInstallChecks = Array.from(npmPostInstallCommands.entries()).map(
                    async ([key, postInstall]) => {
                        ...
                        const npmStatus = useRuntimeStore.getState()
                            .toolInstallStatuses[`npm:${postInstall.npmPackage}`];
                        const npmAvailable = npmStatus?.status === 'installed'
        ...
        <section className={styles.section}>
            ...
            {installedSkills.length === 0 ? (
                <div className={styles.emptyState}>
                    {t('settings.skills.emptyInstalled')}
                </div>
            ) : (
                <div className={styles.skillList}>
                    {installedSkills.map((skill) => {
                        ...
                        return (
                            <div key={skill.name} className={cx(styles.skillCard, !skill.enabled && styles.skillCardDisabled)}>
                                ...
                                >
                                    {/* 状态图标 */}
                                    <div
                                        className={cx(styles.skillStatusIcon, skill.dependencyStatus === 'satisfied'
                                            ? styles.statusReady
                                    ...
                                    <div className={styles.skillExpandedContent}>
                                        {skill.missingDependencies.length > 0 && (
                                            <p className={styles.skillMissing}>
                                                {t('settings.skills.missing', { items: skill.missingDependencies.join(', ') })}
                                            </p>
                                        )}
                                                ...
                                                {skill.systemDependencies.map((tool) => {
                                                    ...
                                                    return (
                                                        <div key={tool.command} className={styles.manualDepItem}>
                                                            <div className={styles.depToolInfo}>
                                                                <span className={styles.depToolName}>
                                                                    {tool.packageName}
                                                                </span>
            ...
            <ConfirmDialog
                ...
                description={t('settings.skills.deleteSkillDescription', { name: pendingDelete?.name ?? '' })}
...
function InstallSection() {
    ...
    const handleImportFolder = useCallback(async () => {
        try {
            ...
            setResultMessage({
                type: 'error',
                text: translateSkillInstallError(getErrorMessage(error, t('settings.skills.importFailed')), t),
            });
    ...
    // 处理用户对审查结果的决策
    const handleAuditDecision = useCallback(async (decision: AuditUserDecision) => {
        ...
        try {
            ...
            switch (decision) {
                case 'proceed': {
                    ...
                    const skillName = pending?.skillName ?? packagePath?.split(/[\\/]/).pop() ?? t('settings.skills.skillPackageFallback');
    ...
    const handleInstallFromGithub = useCallback(async () => {
        ...
        try {
            ...
            // 获取 packages 目录
            const { getPackagesDir } = await import(
                '@services/planning/skills/external/ExternalSkillBootstrap'
            );
    ...
    const isProcessing =
        ...
        auditStatus !== 'idle';
    const visibleResultMessage = resultMessage ?? (githubInstallError
        ? { type: 'error' as const, text: githubInstallError }
        : null);
        ...
        <section className={styles.section}>
            ...
            <div className={styles.installActions}>
                ...
                <p className={styles.hint}>
                    {t('settings.skills.githubHint')}
                </p>

                {/* 安装结果提示 */}
                {visibleResultMessage && (
                    <div
                        className={cx(styles.resultMessage, visibleResultMessage.type === 'success'
...
function EnvironmentSection() {
    ...
    const handleSetup = useCallback(async () => {
        try {
            ...
            const msg = error instanceof Error ? error.message
                : typeof error === 'string' ? error : t('settings.skills.setupFailed');
    ...
    const executeRebuild = useCallback(async () => {
        ...
        try {
            ...
            const { performEnvironmentRebuild } = await import(
                '@services/planning/skills/external/requirementsProvider'
            );
            await performEnvironmentRebuild();
        } catch (error) {
            const msg = error instanceof Error ? error.message
                : typeof error === 'string' ? error : t('settings.skills.rebuildFailed');
            useRuntimeStore.getState().setError(msg);
        } finally {
