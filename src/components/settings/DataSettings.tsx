...
export function DataSettings() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (cleanDropdownRef.current && !cleanDropdownRef.current.contains(e.target as Node)) {
  ...
  const handleImportClick = async () => {
    try {
      ...
      toast({
        type: 'error',
        title: t('settings.data.selectFileFailed', { error: String(error) }),
      });
    }
  };
    ...
    try {
      ...
      const stats = [
        ...
        result.importedMessages > 0
          ? `${result.importedMessages} ${t('settings.data.messages')}`
          : null,
        result.importedMemories > 0
          ? `${result.importedMemories} ${t('settings.data.memories')}`
          : null,
        result.importedVectors > 0
          ? `${result.importedVectors} ${t('settings.data.vectorChunks')}`
          : null,
      ]
        .filter(Boolean)
        .join(', ');
      ...
      const hasImportedData =
        result.importedHubs > 0 || result.importedAgents > 0 || result.importedMessages > 0;
    ...
    return (
      <div className={styles.container}>
        <div className={styles.webWarning}>
          ...
          <p className={styles.webWarningHint}>{t('settings.data.desktopOnlyHint')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.hint}>{t('settings.data.dataLocationHint')}</p>
      </section>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.pathRow}>
          <span className={styles.pathText}>{backupStats?.dirPath ?? t('common.loading')}</span>
          ...
          {showCleanDropdown && (
            <div className={styles.cleanDropdownPanel}>
              <div className={styles.cleanOption}>
                <span className={styles.cleanOptionLabel}>
                  {t('settings.data.deleteOlder7Days')}
                </span>
                <button
              ...
              <div className={cx(styles.cleanOption, styles.cleanOptionDanger)}>
                <span className={styles.cleanOptionLabel}>
                  {t('settings.data.deleteAllBackups')}
                </span>
                <button
        ...
        <p className={styles.hint}>{t('settings.data.backupHint')}</p>
      </section>
      ...
      {showCleanConfirm && pendingCleanPolicy && (
        <div className={styles.dialogOverlay}>
          <div className={styles.dialog}>
            ...
            <p className={styles.dialogText}>{getCleanConfirmDescription(pendingCleanPolicy)}</p>
      ...
      {showResetConfirm && (
        <div className={styles.dialogOverlay}>
          <div className={styles.dialog}>
            ...
            <p className={styles.dialogText}>{t('settings.data.resetWillDelete')}</p>
            <ul className={styles.dialogList}>
              <li>{t('settings.data.hubItems', { count: stats?.hubCount ?? 0 })}</li>
              <li>{t('settings.data.agentItems', { count: stats?.agentCount ?? 0 })}</li>
              <li>
                {stats?.messageCount ?? 0} {t('settings.data.messages')}
              </li>
              <li>
                {stats?.memoryCount ?? 0} {t('settings.data.memories')}
              </li>
              <li>
                {stats?.vectorChunkCount ?? 0} {t('settings.data.vectorChunks')}
              </li>
            </ul>
            <p className={styles.dialogWarning}>
              {t('settings.data.resetPhrasePrompt')}{' '}
              <strong>"{t('settings.data.resetPhrase')}"</strong>{' '}
              {t('settings.data.resetPhraseSuffix')}
            </p>
            <input
              type="text"
      ...
      {showImportModeDialog && (
        <div className={styles.dialogOverlay}>
          <div className={styles.dialog}>
            ...
            <p className={styles.dialogText}>{t('settings.data.chooseImportModeDesc')}</p>
            <div className={styles.importModeOptions}>
              <button className={styles.importModeButton} onClick={() => handleImport('merge')}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
      ...
      {showClearVectorConfirm && (
        <div className={styles.dialogOverlay}>
          <div className={styles.dialog}>
            ...
            <p className={styles.dialogText}>{t('settings.data.clearVectorCacheDesc')}</p>
            ...
            <div className={styles.dialogActions}>
              ...
              <button className={styles.dialogDangerButton} onClick={handleClearVectors}>
