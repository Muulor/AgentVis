...
function buildAuditSignals(
...
): Array<{ label: TranslationKey; value: string }> {
  ...
  const resolvedRiskReason =
    extractAuditDetailValue(event, 'resolvedRiskReason') ??
    (extractAuditDetailValue(event, 'resolvedRisk')
      ? extractAuditDetailValue(event, 'reason')
      : null);
...
function eventMatchesFilters(event: SandboxAuditEvent, filters: AuditFilters): boolean {
  ...
  if (targetHost && !event.targetHost?.toLowerCase().includes(targetHost)) {
    return false;
  }
...
export function SandboxAuditSettings() {
  ...
  const fetchEvents = useCallback(
    async (offset: number) =>
      invoke<SandboxAuditEvent[]>('sandbox_audit_events', {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setEvents((current) => {
        ...
        return [...current, ...loaded.filter((event) => !seen.has(event.id))];
      });
  ...
  useEffect(() => {
    ...
    void import('@tauri-apps/api/event')
      .then(({ listen }) =>
        listen<SandboxAuditEvent>(AUDIT_EVENT_NAME, (event) => {
          const incoming = event.payload;
          if (!eventMatchesFilters(incoming, filters)) {
            return;
          }
          setEvents((current) =>
            [incoming, ...current.filter((item) => item.id !== incoming.id)].slice(
              0,
              Math.max(nextOffset, AUDIT_EVENT_PAGE_SIZE)
            )
          );
        })
      )
    ...
    () => [
      {
        ...
        count: visibleEvents.filter((event) => event.reason === 'proxy_bypass_signal_blocked')
          .length,
      },
      {
        key: 'upload',
      ...
      },
      {
        ...
        count: visibleEvents.filter((event) => event.reason === 'broker_proxy_expected_but_unused')
          .length,
      },
      {
        key: 'dnsOrWfp',
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.filterPanel}>
        ...
        <div className={styles.filterSecondaryRow}>
          <label className={styles.filterField}>
            ...
            <Tooltip
              content={reasonFilter === 'all' ? t('settings.audit.filters.all') : reasonFilter}
            >
          ...
          <label className={styles.filterField}>
            ...
            <Tooltip
              content={
                guardModeFilter === 'all' ? t('settings.audit.filters.all') : guardModeFilter
              }
            >
              <span className={styles.tooltipSelectWrap}>
                <Select
      ...
      {error && (
        <div className={styles.errorState}>{t('settings.audit.loadFailed', { error })}</div>
      )}
      ...
      {!error && events.length > 0 && visibleEvents.length === 0 && (
        <div className={styles.emptyState}>{t('settings.audit.empty')}</div>
      )}
                ...
                <div className={styles.eventBody}>
                  <div className={styles.eventTitle}>
                    <Tooltip content={eventTarget(event)}>
                      <span className={styles.eventTarget}>{eventTarget(event)}</span>
                    </Tooltip>
                  </div>
                  <Tooltip content={event.blockedReason ?? event.reason}>
                    <div className={styles.eventReason}>{event.blockedReason ?? event.reason}</div>
                  </Tooltip>
                  {(() => {
                      ...
                      <div className={styles.eventSignals}>
                        {auditSignals.map((signal) => (
                          <Tooltip
                            ...
                            content={`${t(signal.label)}: ${signal.value}`}
                          >
                  ...
                  <div className={styles.detailGrid}>
                    <span>
                      {t('settings.audit.source')}:{' '}
                      <span className={styles.detailValue}>
                        {t(SOURCE_LABEL_KEYS[event.source])}
                      </span>
                    </span>
                    <span>
                      {t('settings.audit.backend')}:{' '}
                      <span className={styles.detailValue}>
                        {t(BACKEND_LABEL_KEYS[event.backend])}
                      </span>
                    </span>
                    <span>
                      {t('settings.audit.mode')}:{' '}
                      <span className={styles.detailValue}>{event.sandboxMode}</span>
                    </span>
                    <span>
                      {t('settings.audit.status')}:{' '}
                      <span className={styles.detailValue}>{event.statusCode ?? '-'}</span>
                    </span>
                    {event.targetHost && (
                      <span className={styles.detailWide}>
                        {t('settings.audit.targetHost')}:{' '}
                        <span className={styles.detailValue}>{event.targetHost}</span>
                      </span>
                    )}
                    {event.networkProtocol && (
                      <span>
                        {t('settings.audit.networkProtocol')}:{' '}
                        <span className={styles.detailValue}>{event.networkProtocol}</span>
                      </span>
                    )}
                    {event.targetPort && (
                      <span>
                        {t('settings.audit.targetPort')}:{' '}
                        <span className={styles.detailValue}>{event.targetPort}</span>
                      </span>
                    )}
                    {event.guardMode && (
                      <span>
                        {t('settings.audit.guardMode')}:{' '}
                        <span className={styles.detailValue}>{event.guardMode}</span>
                      </span>
                    )}
                    {event.subjectId && (
                      <span className={styles.detailWide}>
                        {t('settings.audit.subjectId')}:{' '}
                        <span className={styles.detailValue}>{event.subjectId}</span>
                      </span>
                    )}
                  </div>
                </div>
