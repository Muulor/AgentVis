...
import { Search, ShieldCheck, ShieldX, AlertTriangle, AlertCircle, Minimize2 } from 'lucide-react';
...
function getStatusTitle(status: string, t: TFunction): string {
  switch (status) {
    case 'preparing':
      return t('settings.skills.auditStatusPreparing');
    case 'auditing':
      return t('settings.skills.auditStatusAuditing');
    case 'approved':
      return t('settings.skills.auditStatusApproved');
    case 'rejected':
      return t('settings.skills.auditStatusRejected');
    case 'manual_review':
      return t('settings.skills.auditStatusManualReview');
    case 'error':
      return t('settings.skills.auditStatusError');
    default:
...
function getStatusClass(status: string): string {
  switch (status) {
    ...
    case 'manual_review':
      return styles.manualReview ?? '';
    case 'error':
      return styles.error ?? '';
    case 'preparing':
      return styles.auditing ?? '';
    default:
      return styles.auditing ?? '';
  }
}
...
function PreparingState(): React.ReactElement {
  ...
  return (
    <div className={styles.body}>
      <div className={styles.summary}>{t('settings.skills.auditPreparingPackage')}</div>
    </div>
  );
}
  ...
  return (
    <div className={styles.body}>
      <div className={styles.summary}>
        {progress
          ? t('settings.skills.auditScannedFiles', {
  ...
  return (
    <div className={cx(styles.findingItem, styles[level])}>
      <div className={styles.findingMeta}>
        <span className={cx(styles.findingBadge, styles[level])}>{finding.riskLevel}</span>
...
function ErrorContent({ error }: { error: string | null }): React.ReactElement {
  ...
  return (
    <div className={styles.body}>
      <div className={styles.summary}>{error ?? t('settings.skills.auditDefaultError')}</div>
    </div>
  );
}
...
function ApprovedFooter({
  ...
  onDecision: (d: AuditUserDecision) => void;
  isProcessing?: boolean;
}): React.ReactElement {
  const { t } = useI18n();
  return (
...
function RejectedFooter({
  ...
  return (
    <div className={styles.footer}>
      ...
      >
        {isProcessing
          ? t('settings.skills.auditProcessing')
          : t('settings.skills.riskProceedInstall')}
      </button>
    </div>
  );
}
  ...
  return (
    <div className={styles.footer}>
      ...
      >
        {isProcessing
          ? t('settings.skills.auditProcessing')
          : t('settings.skills.skipAuditInstall')}
      </button>
    </div>
  );
}
 ...
 */
export function SkillAuditModal({
  onDecision,
  isProcessing,
}: SkillAuditModalProps): React.ReactElement | null {
  const { t } = useI18n();
      ...
      <div className={styles.modal} role="dialog" aria-modal="true">
        ...
        <div className={styles.header}>
          ...
          <div className={styles.headerText}>
            <h3 className={styles.title}>{getStatusTitle(auditStatus, t)}</h3>
        ...
        {auditStatus === 'approved' && (
          <ApprovedFooter onDecision={onDecision} isProcessing={isProcessing} />
        )}
        {auditStatus === 'rejected' && (
          <RejectedFooter onDecision={onDecision} isProcessing={isProcessing} />
        )}
        {auditStatus === 'manual_review' && (
          <ManualReviewFooter onDecision={onDecision} isProcessing={isProcessing} />
        )}
        {auditStatus === 'error' && (
          <ErrorFooter onDecision={onDecision} isProcessing={isProcessing} />
        )}
      </div>
    </div>,
    document.body
