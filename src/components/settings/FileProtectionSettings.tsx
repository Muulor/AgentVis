...
export function FileProtectionSettings() {
    ...
    const restorableEntries = useMemo(
        () => trashEntries.filter(canRestoreEntry),
        [trashEntries]
    );
    ...
    const allEntriesSelected = trashEntries.length > 0
        && trashEntries.every((entry) => selectedIds.has(entry.id));

    const formatDeletedAt = useCallback((deletedAt: string) => {
    ...
    const showRestoreToast = useCallback((result: TrashRestoreResult) => {
    ...
    const showDeleteToast = useCallback((result: TrashDeleteResult) => {
    ...
    const restoreEntries = useCallback(async (ids: string[], restoreKey: string) => {
        if (ids.length === 0) {
        ...
        try {
            ...
            toast({ type: 'error', title: t('settings.fileProtection.toastRestoreFailed', { error: String(error) }) });
        } finally {
    ...
    const cleanEntries = useCallback(async (ids: string[], deleteKey: string) => {
        if (ids.length === 0) {
            toast({ type: 'warning', title: t('settings.fileProtection.toastSelectEntriesToClean') });
            return;
        }

        const confirmed = window.confirm(t('settings.fileProtection.confirmCleanEntries', { count: ids.length }));
        if (!confirmed) return;
        ...
        try {
            ...
            toast({ type: 'error', title: t('settings.fileProtection.toastCleanFailed', { error: String(error) }) });
        } finally {
            setDeletingKey(null);
        }
    ...
    const handleAddPath = useCallback(async () => {
        try {
            ...
            try {
                ...
                toast({ type: 'error', title: t('settings.fileProtection.toastSaveFailed', { error: String(error) }) });
            } finally {
    ...
    const handleRemovePath = useCallback(async (index: number) => {
        ...
        try {
            ...
            toast({ type: 'error', title: t('settings.fileProtection.toastRemoveFailed', { error: String(error) }) });
        } finally {
            setIsSavingPaths(false);
        }
    }, [protectedPaths, toast, t]);
                ...
                <div className={styles.trashToolbar}>
                    ...
                    <div className={styles.toolbarActions}>
                        ...
                        <button
                            ...
                            disabled={selectedRestorableCount === 0 || restoringKey !== null || deletingKey !== null}
                        >
