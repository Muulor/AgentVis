...
import { ChevronDown, Download, Pencil, Plus, RotateCcw, Save, Trash2, Upload, X } from 'lucide-react';
...
export function ModelSettings() {
    ...
    const [modelForm, setModelForm] = useState<CustomModelFormState>(() => createEmptyModelForm(defaultProvider || 'local'));
    ...
    const resetModelForm = useCallback((providerId = currentProvider) => {
    ...
    const handleModelFormSubmit = useCallback(async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!Number.isFinite(contextWindow) || contextWindow <= 0 || !Number.isInteger(contextWindow)) {
        ...
        try {
            ...
            toast({
                title: isEditingModel ? t('settings.model.updateModelFailed') : t('settings.model.addModelFailed'),
    ...
    const handleDownloadTemplate = useCallback(async () => {
        try {
            ...
            toast({ title: t('settings.model.templateSaveFailed'), description: error instanceof Error ? error.message : String(error), type: 'error' });
        }
    }, [toast, t]);
    ...
    const handleImport = useCallback(async () => {
        try {
            ...
            toast({ title: t('settings.model.importFailed'), description: error instanceof Error ? error.message : String(error), type: 'error' });
        }
    }, [toast, t]);
    ...
    const handleExport = useCallback(async () => {
        try {
            ...
            toast({ title: t('settings.model.exportFailed'), description: error instanceof Error ? error.message : String(error), type: 'error' });
        }
    }, [toast, t]);
    ...
    const handleResetConfirmed = useCallback(async () => {
        ...
        try {
            ...
            toast({ title: t('settings.model.resetFailed'), description: error instanceof Error ? error.message : String(error), type: 'error' });
        }
    }, [toast, t]);

    // 删除单个用户自定义模型
    const handleRemoveUserModel = useCallback(async (modelId: string, providerId: string) => {
    ...
    return (
        <div className={styles.container}>
            ...
            <section className={styles.section}>
                ...
                <div className={styles.providerSelector}>
                    {providers.map((provider) => (
                        <button
                            key={provider.id}
                            className={cx(styles.providerOption, currentProvider === provider.id && styles.providerOptionActive)}
            ...
            <section className={styles.section}>
                ...
                <p className={styles.description}>
                    {t('settings.model.configDescription')}
                </p>

                <form className={styles.modelForm} onSubmit={handleModelFormSubmit}>
                    <div className={styles.modelFormHeader}>
                        <span>{isEditingModel ? t('settings.model.editCustomModel') : t('settings.model.addCustomModel')}</span>
                    </div>
                    <div className={styles.modelFormGrid}>
                        ...
                        <label className={styles.field}>
                            ...
                            <input
                                ...
                                onChange={(event) => setModelForm(form => ({ ...form, modelName: event.target.value }))}
                        ...
                        <label className={styles.field}>
                            ...
                            <input
                                ...
                                onChange={(event) => setModelForm(form => ({ ...form, contextWindow: event.target.value }))}
                    ...
                    <div className={styles.modelFormFooter}>
                        <label className={styles.toggleLabel}>
                            <input
                                ...
                                onChange={(event) => setModelForm(form => ({ ...form, supportsVision: event.target.checked }))}
                            />
                        ...
                        <div className={styles.formActions}>
                            {isEditingModel && (
                                <button type="button" className={styles.actionButton} onClick={() => resetModelForm()}>
                                    <X size={14} />
                                    {t('common.cancel')}
                                </button>
                            )}
                            <button type="submit" className={cx(styles.actionButton, styles.actionButtonPrimary)}>
                                ...
                                {isEditingModel ? t('settings.model.saveCustomModel') : t('settings.model.addCustomModel')}
                            </button>
                        </div>
                    </div>
                </form>
                ...
                {userModels.length > 0 && (
                    <div className={styles.userModelList}>
                        ...
                        >
                            ...
                            <ChevronDown size={12} className={cx(styles.toggleChevron, userModelsExpanded && styles.toggleChevronOpen)} />
                        </button>
                        {userModelsExpanded && (
                                    ...
                                    <div key={`${model.id}-${model.providerId}`} className={styles.userModelItem}>
                                        <div className={styles.userModelInfo}>
                                            ...
                                            <span className={styles.userModelMeta}>
                                                ...
                                                {model.supportsVision === true ? ` - ${t('settings.model.visionSupported')}` : ''}
                                            </span>
                                        </div>
            ...
            <section className={styles.infoSection}>
                <div className={styles.infoCard}>
                    ...
                    <ul className={styles.infoList}>
                        <li><strong>OpenAI</strong> - {t('settings.model.providerOpenAI')}</li>
                        <li><strong>Anthropic</strong> - {t('settings.model.providerAnthropic')}</li>
                        <li><strong>Google AI</strong> - {t('settings.model.providerGoogle')}</li>
                        <li><strong>ZhipuAI</strong> - {t('settings.model.providerZhipu')}</li>
                        <li><strong>DeepSeek</strong> - {t('settings.model.providerDeepSeek')}</li>
                        <li><strong>Agnes AI</strong> - {t('settings.model.providerAgnes')}</li>
                        <li><strong>StepFun</strong> - {t('settings.model.providerStepFun')}</li>
                        <li><strong>Xiaomi MiMo</strong> - {t('settings.model.providerXiaomi')}</li>
                        <li><strong>MiniMax</strong> - {t('settings.model.providerMiniMax')}</li>
                        <li><strong>{t('settings.model.providerVolcengineName')}</strong> - {t('settings.model.providerVolcengine')}</li>
                        <li><strong>OpenRouter</strong> - {t('settings.model.providerOpenRouter')}</li>
                        <li><strong>Local</strong> - {t('settings.model.providerLocal')}</li>
                    </ul>
                </div>
            </section>
