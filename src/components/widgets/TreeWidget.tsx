...
export const TreeWidget = memo(function TreeWidget({
  ...
  if (!parsed) {
    return <div className={styles.errorFallback}>{t('widgets.treeInvalid')}</div>;
  }
  ...
  return (
    <div className={styles.container}>
        ...
        <div className={styles.breadcrumb}>
          {breadcrumbs.map((crumb, i) => (
            <span key={i} className={styles.breadcrumbItem}>
              ...
              <span
                className={i === breadcrumbs.length - 1 ? styles.breadcrumbItemActive : undefined}
              >
                {crumb}
              </span>
            </span>
      ...
      {resolvedLevels.map(({ level, selectedIndex, depth }) => {
        ...
        return (
          <div key={depth} className={cx(styles.levelContainer, isAnimated && styles.levelEnter)}>
            {level.question && <p className={styles.levelQuestion}>{level.question}</p>}
