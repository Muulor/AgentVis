...
export function extractCodeLanguage(className?: string): string {
  ...
  const languageClass = className.split(/\s+/).find((part) => part.startsWith('language-'));
...
export function containsChoicesWidgetBlock(markdown: string): boolean {
  return extractFencedCodeBlocks(markdown).some(
    (block) => resolveWidgetBlockType(block) === 'choices'
  );
}

export function containsTreeWidgetBlock(markdown: string): boolean {
  return extractFencedCodeBlocks(markdown).some(
    (block) => resolveWidgetBlockType(block) === 'tree'
  );
}

export function shouldDeferTreeWidgetSubmit(markdown: string): boolean {
  const widgetTypes = extractFencedCodeBlocks(markdown).map(resolveWidgetBlockType).filter(Boolean);
