...
function getSupportedEditable(target: EventTarget | null): EditableElement | null {
    ...
    if (
        ...
        && EDITABLE_INPUT_TYPES.has(editable.type)
    ) {
        return editable;
    }
...
function getSelectedTextWithin(root: HTMLElement | null): string {
    ...
    const commonElement = commonAncestor instanceof Element
        ? commonAncestor
...
export function useTextContextMenu() {
    ...
    const openEditableMenu = useCallback((event: ReactMouseEvent) => {
    ...
    const handleMenuAction = useCallback(async (action: TextContextMenuAction) => {
        if (!menu) return;
