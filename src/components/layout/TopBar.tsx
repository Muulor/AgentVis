...
export function TopBar() {
  ...
  const leftPanelLabel = isLeftPanelCollapsed
    ? t('layout.expandSidebar')
    : t('layout.collapseSidebar');
  const rightPanelLabel = isRightPanelVisible
    ? t('layout.hideFilePanel')
    : t('layout.showFilePanel');
  ...
  useEffect(() => {
    const handleOpenSettings = (event: Event) => {
      ...
      setSettingsInitialTab(tab && ONBOARDING_SETTINGS_TABS.includes(tab) ? tab : 'general');
      setIsSettingsOpen(true);
    };
      ...
      <header className={styles.topBar}>
        ...
        <div className={styles.actions}>
          ...
          <Tooltip content={themeLabel}>
            <button className={styles.iconButton} onClick={toggleTheme} aria-label={themeLabel}>
              {theme === 'light' ? (
