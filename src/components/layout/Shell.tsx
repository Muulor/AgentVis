...
export function Shell() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('agentvis:image-generation-error', handleImageGenerationError);
  }, []);
  ...
  const handleLeftResize = (delta: number) => {
    ...
    const newWidth = Math.max(
      160,
      Math.min(400, leftStartWidth.current + accumulatedDelta.current)
    );
  ...
  const handleRightResize = (delta: number) => {
    ...
    const newWidth = Math.max(
      200,
      Math.min(rightPanelMaxWidth, rightStartWidth.current - accumulatedDelta.current)
    );
