...
export function RightPanel() {
    ...
    <>
      ...
      {completedDiffFiles.length === 1 && (
        <div className={styles.historyAccessBar}>
          <span className={styles.historyAccessHint}>{t('layout.historyHint')}</span>
      ...
      {currentAttachments.length > 0 && (
        <div className={styles.attachmentListContainer}>
          <div className={styles.attachmentListHeader}>
            <span className={styles.attachmentListTitle}>
              {t('layout.attachmentsTitle', { count: currentAttachments.length })}
            </span>
              ...
              <Tooltip key={att.id} content={att.fileName}>
                <div
                  className={cx(
                    ...
                    selectedFile?.id === att.id && styles.attachmentItemSelected
                  )}
                  onClick={() => {
      ...
      {/* 交付物列表 */}
      <div className={styles.fileListContainer} style={{ height: fileListHeight }}>
      ...
      {/* 预览区 */}
      <div
        className={cx(styles.previewContainer, isFilePreviewFullscreen && styles.previewFullscreen)}
      >
        {/* 预览头部：文件名 + 全屏/关闭按钮 */}
          ...
          <div className={styles.previewHeader}>
            ...
            <div className={styles.previewHeaderActions}>
              ...
              <Tooltip
                content={
                  isFilePreviewFullscreen
                    ? t('layout.fullscreenExit')
                    : t('layout.fullscreenPreview')
                }
              >
                <button
   ...
   */
  const handleFileTabWheel = useCallback(
    (e: globalThis.WheelEvent) => {
      ...
      const nextIndex =
        delta > 0 ? Math.min(currentIndex + 1, fileList.length - 1) : Math.max(currentIndex - 1, 0);
    ...
    <div className={styles.diffContainer}>
      ...
      <div className={styles.diffToolbar}>
        ...
        <Tooltip content={t('layout.viewHistory')}>
          <button className={styles.historyBtn} onClick={toggleSnapshotPanel}>
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
      ...
      {fileList.length > 1 && (
        <div ref={fileTabBarRef} className={styles.fileTabBar}>
          {fileList.map((file) => (
            <Tooltip key={file.documentId} content={file.documentId}>
              <button
                ...
                className={cx(
                  styles.fileTab,
                  file.documentId === diffState.activeFileId && styles.fileTabActive
                )}
  ...
  return (
    <div
      className={cx(
        styles.rightPanel,
        isDiffModePanel && styles.diffModePanel,
    ...
    >
      ...
      <header className={styles.header}>
        <h2 className={styles.title}>
          {isPreviewActive
            ? t('layout.panelPreview')
            : mode === 'diff'
              ? t('layout.diffPreview')
              : t('layout.files')}
        </h2>
