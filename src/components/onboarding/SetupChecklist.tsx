...
export function SetupChecklist({ state }: SetupChecklistProps) {
  const { t } = useI18n();
  const primaryStep = state.steps.find((step) => step.status === 'pending' && !step.disabled);
  ...
  return (
    <section className={styles.panel} aria-label={t('onboarding.setupTitle')}>
      ...
      {primaryStep && (
        <button className={styles.primaryButton} type="button" onClick={primaryStep.onAction}>
