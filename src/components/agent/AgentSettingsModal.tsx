...
const SANDBOX_MODE_OPTIONS: AgentSandboxMode[] = [
  'LocalAudit',
  'ControlledNetwork',
  'OfflineIsolated',
];
...
export function AgentSettingsModal({ isOpen, agentId, onClose }: AgentSettingsModalProps) {
  ...
  const [subAgentSafetyFooterText, setSubAgentSafetyFooterText] = useState(
    DEFAULT_SAFETY_FOOTER_BODY_TEXT
  );
  ...
  useEffect(() => {
    ...
    if (isOpen && agent) {
      ...
      if (!agent.chatRules) {
        const legacyRulesPaths = [
          ...new Set([agent.mbRulesFilePath, agent.saRulesFilePath].filter(Boolean) as string[]),
        ];
  ...
  const handleSave = useCallback(async () => {
    ...
    if (trimmedName.toLowerCase() !== agent.name.toLowerCase()) {
      const sameHubAgents = useAgentStore
        .getState()
        .agents.filter((a) => a.hubId === agent.hubId && a.id !== agentId);
  ...
  const handleConfirmBatchDelete = useCallback(async () => {
    ...
    try {
      ...
      logger.trace(
        '[Knowledge:BatchDelete] 各路径 fileIndexStatus:',
      ...
      logger.trace(
        `[Knowledge:BatchDelete] ========== 批量删除完成: ${pathsToDelete.length} 个文件 ==========`
      );
    ...
    <div className={styles.overlay}>
      ...
      >
        ...
        <div className={styles.header}>
          <h2 id="agent-settings-title" className={styles.title}>
        ...
        <div className={styles.content}>
          ...
          {activeTab === 'basic' && (
            <div className={styles.formGroup}>
                ...
                {showCropper ? (
                  <AvatarCropper
                    ...
                    onCrop={async (base64) => {
                      ...
                      if (agentId && cropperImageUrl) {
                        try {
                          const { appDataDir: getAppDataDir, join } =
                            await import('@tauri-apps/api/path');
                          const { mkdir, exists, writeFile } =
                          ...
                          if (originalBase64) {
                            const binaryData = Uint8Array.from(atob(originalBase64), (c) =>
                              c.charCodeAt(0)
                            );
                  ...
                  <>
                    ...
                    <div className={styles.avatarActions}>
                      ...
                      {avatar && (
                        <button className={styles.browseBtn} onClick={() => setAvatar(null)}>
                          {t('common.remove')}
                        </button>
                      )}
                    </div>
              ...
              {/* 名称 */}
              <label htmlFor="agent-name" className={styles.label} style={{ marginTop: 16 }}>
                {t('agent.settings.name')}
              </label>
              <input
                id="agent-name"
                type="text"
              ...
              <div
                className={styles.sandboxModeGroup}
                role="radiogroup"
                aria-label={t('agent.settings.sandboxMode')}
              >
                ...
                <div className={styles.toggleRow}>
                  <div className={styles.toggleInfo}>
                    <span className={styles.toggleLabel}>
                      {t('agent.settings.subAgentSafetyFooter')}
                    </span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ color: 'var(--color-text-tertiary)', fontSize: 13 }}>
                  {t('agent.settings.loopUnit')}
                </span>
                {planningLoopBudget !== null && (
                  <Tooltip
                    content={t('agent.settings.resetLoopTitle', { max: defaultDecisionRounds })}
                  >
                    <button
                      className={styles.browseBtn}
              ...
              <div className={styles.toggleRow} style={{ marginTop: 16 }}>
                <div className={styles.toggleInfo}>
                  <span className={styles.toggleLabel}>
                    {t('agent.settings.visualEnhancement')}
                  </span>
                  <p className={styles.toggleHint}>{t('agent.settings.visualEnhancementHint')}</p>
                </div>
                <label className={styles.toggleSwitch}>
                  <input
                    type="checkbox"
          ...
          {activeTab === 'knowledge' && (
            <div className={styles.formGroup}>
              ...
              <div className={styles.toggleRow}>
                <div className={styles.toggleInfo}>
                  <span className={styles.toggleLabel}>
                    {t('agent.settings.autoIndexDeliverables')}
                  </span>
                  <p className={styles.toggleHint}>{t('agent.settings.autoIndexHint')}</p>
                  <p className={styles.toggleTip}>{t('agent.settings.autoIndexTip')}</p>
                </div>
                <label className={styles.toggleSwitch}>
                  <input
                    type="checkbox"
              ...
              {/* 知识库文件管理 */}
              <label className={styles.label} style={{ marginTop: 16 }}>
                {t('agent.settings.knowledgeFiles')}
              </label>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      const selected = await open({
                        multiple: true,
                        filters: [{ name: 'Documents', extensions: ['md', 'txt', 'json', 'pdf'] }],
                      });
                      if (selected) {
              ...
              {isSelectMode && knowledgePaths.length > 0 && (
                <div className={styles.batchBar}>
                  ...
                  >
                    {selectedPaths.size === knowledgePaths.length
                      ? t('agent.settings.cancelSelectAll')
                      : t('common.selectAll')}
                  </button>
                  <span className={styles.batchCount}>
                    {t('agent.settings.selectedFiles', {
                      selected: selectedPaths.size,
                      total: knowledgePaths.length,
                    })}
                  </span>
                  <button
              ...
              {indexingProgress && (
                <div className={styles.indexingProgress}>
                  ...
                  <span className={styles.progressText}>
                    {t('agent.settings.indexingProgress', {
            ...
            <div className={styles.formGroup}>
              ...
              <div className={styles.toggleRow}>
                <div className={styles.toggleInfo}>
                  ...
                  <p className={styles.toggleTip}>{t('agent.settings.pinnedTip')}</p>
                </div>
                <label className={styles.toggleSwitch}>
                  <input
                    type="checkbox"
                  ...
                  {installedSkills.length > 0 ? (
                    <>
                      ...
                      {installedSkills.map((skill) => {
                        ...
                        const isDisabled =
                          !isChecked && pinnedSkillNames.length >= pinnedSkillsMaxCount;
                        return (
                          <label
                            key={skill.name}
                            className={cx(
                              styles.pinnedSkillItem,
                              isDisabled && styles.pinnedSkillDisabled
                            )}
                          >
                            <input
                              type="checkbox"
          ...
          {/* 记忆标签页 */}
          {activeTab === 'memory' && agentId && <MemoryPanel agentId={agentId} />}

          {/* 定时任务标签页 */}
          {activeTab === 'cron' && agentId && <CronSettingsTab agentId={agentId} />}
        </div>
        ...
        {activeTab !== 'cron' && activeTab !== 'memory' && (
          <div className={styles.footer}>
            ...
            <button className={styles.saveBtn} onClick={handleSave} disabled={isSaving}>
              {indexingProgress
                ? t('agent.settings.indexingProgressSaving', {
                    current: indexingProgress.current,
                    total: indexingProgress.total,
                  })
                : isSaving
      ...
      <ConfirmDialog
        ...
        description={t('agent.settings.deleteKnowledgeDescription', {
          name: pendingDeletePath?.split(/[/\\]/).pop() ?? '',
        })}
      ...
      <ConfirmDialog
        ...
        description={t('agent.settings.batchDeleteKnowledgeDescription', {
