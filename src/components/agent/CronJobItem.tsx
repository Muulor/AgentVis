...
export function CronJobItem({ job, onToggle, onEdit, onDelete }: CronJobItemProps) {
    ...
    // 状态指示器颜色
    const statusClass = job.lastRunStatus === 'success' ? styles.statusSuccess
        : job.lastRunStatus === 'failed' ? styles.statusFailed
            : job.lastRunStatus === 'running' ? styles.statusRunning
                : '';
        ...
        <div className={cx(styles.card, !job.enabled && styles.disabled)}>
            <div className={styles.header}>
                <div className={styles.nameRow}>
                    ...
                    {job.lastRunStatus && (
                        <span className={cx(styles.statusDot, statusClass)} title={t('agent.cron.lastStatus', { status: job.lastRunStatus })} />
                    )}
                </div>
