...
export function CronSettingsTab({ agentId }: CronSettingsTabProps) {
  ...
  const { jobs, isLoading, loadJobsByAgent, createJob, updateJob, deleteJob, toggleJob } =
    useCronStore();
  const {
  ...
  return (
    <div className={styles.container} data-custom-context-menu onContextMenu={openEditableMenu}>
      {/* 头部：标题 + 新增按钮 */}
          ...
          {!form.isAdvancedMode ? (
            <div className={styles.formField}>
              <label className={styles.fieldLabel}>
                ...
                {cronPreview && <span className={styles.cronPreview}>{cronPreview}</span>}
              </label>
              ...
              {form.schedule.frequency === 'every_n_minutes' && (
                <div className={styles.scheduleRow}>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateSchedule({ intervalMinutes: parseInt(value, 10) })
                    }
                  ...
                  {form.schedule.frequency === 'weekly' && (
                    <>
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateSchedule({ dayOfWeek: parseInt(value, 10) })
                        }
                  ...
                  {form.schedule.frequency === 'specific' && (
                    <>
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateSchedule({ dayOfMonth: parseInt(value, 10) })
                        }
                  ...
                  {form.schedule.frequency === 'specific' && (
                    <label className={styles.toggleLabel}>
                      <div
                        className={cx(
                          styles.toggleSwitch,
                          form.schedule.autoDisable && styles.toggleOn
                        )}
            ...
            <div className={styles.formField}>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, rawCronExpression: e.target.value }))
                }
                placeholder={t('agent.cron.cronPlaceholder')}
              />
              <span className={styles.fieldHint}>{t('agent.cron.cronHint')}</span>
              <button
                className={styles.advancedToggle}
                onClick={handleToggleAdvanced}
                type="button"
