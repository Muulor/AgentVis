...
import { ChatHistory, ChatInput, ChatSearchBar, type ChatInputRestoreDraft, type ChatInputSendOptions } from '@components/chat';
...
function buildRevokeRestoreDraft(message: UIMessage): ChatInputRestoreDraft {
    ...
    const value = typeof metadataDisplayContent === 'string'
...
export function AgentChatView() {
    ...
    const isSearchOpen = currentAgentId ? (searchByContext.get(currentAgentId)?.isOpen ?? false) : false;
            ...
            if (boundAgentId) {
                void (async () => {
                    try {
                        ...
                        logger.trace('[AgentChatView]  生命周期事件: onSessionEnd 已触发, agentId:', boundAgentId);
                    } catch (error) {
    ...
    useEffect(() => {
        ...
        const setupListener = async () => {
            try {
                ...
                unlisten = await listen<{ messageId: string; agentId: string }>('chat:jump_to_message', (event) => {
        ...
        const setupCronPlanningListener = async () => {
            try {
                ...
                const unlistenFn = await listen<{
                    ...
                    const { agentId, prompt, cronJobId, cronJobName, source, imTaskId, imPlatform, botId } = event.payload;
                    ...
                    logger.info(`[AgentChatView] 收到${source === 'im' ? ' IM' : '定时任务'} Planning 触发: ${cronJobName}`);

                    if (source === 'im' && !shouldStartImTask(botId, imTaskId)) {
                        logger.trace('[AgentChatView] IM 任务已取消或结束，跳过 Planning 启动', { botId, imTaskId });
                        return;
                    }

                    // 构建提示词：IM 消息为 Agent/DB 保留来源前缀，UI 渲染层会剥离此前缀
                    const imPromptPrefixKey = imPlatform === 'slack'
                        ? 'im.bridge.userPromptPrefixSlack'
                        : imPlatform === 'feishu'
                            ? 'im.bridge.userPromptPrefixFeishu'
                            : 'im.bridge.userPromptPrefix';
                    const taskPrompt = source === 'im'
                        ? `${t(imPromptPrefixKey)}\n${prompt}`
                    ...
                    executePlanningTaskRef.current(taskPrompt, {
                        userMessageMeta: {
                            source: source ?? 'cron',
                            cronJobId,
                            cronJobName,
    ...
    useEffect(() => {
        const unsubscribe = useWidgetStore.subscribe((state) => {
            ...
            if (action?.contextId === currentAgentId && !isSendingRef.current && !isStreamingRef.current) {
                // 立即消费事件（防止重复触发）
                ...
                const widgetMeta: Record<string, unknown> = {
                    ...
                    ...(action.widgetExtraText !== undefined ? { widgetExtraText: action.widgetExtraText } : {}),
                };
                if (mode === 'planning') {
            ...
            if (undo?.contextId === currentAgentId) {
                ...
                if (widgetUserMsg) {
                    ...
                    for (const msg of allMessages) {
                        if (
                            msg.role === 'assistant' &&
                            msg.createdAt > widgetCreatedAt
                        ) {
                            if (
                                !nearestFollowUpAssistant ||
                                msg.createdAt < nearestFollowUpAssistant.createdAt
                            ) {
    ...
    // 发送消息（流式响应）
    const handleSend = useCallback(async (content: string, inputOptions?: ChatInputSendOptions) => {
        if (!currentAgentId || !currentAgent) return;
    ...
    }, [currentAgentId, currentAgent, mode, executePlanningTask, getAttachmentsCopy, clearAttachments, pendingQuotes, clearQuotes, sendChatMessage]);
    ...
    const handleMultiSelectAction = useCallback((action: 'copy' | 'quote' | 'delete' | 'cancel') => {
        if (!currentAgentId) return;
        switch (action) {
            case 'copy': {
                // 按时间顺序合并选中消息文本
    ...
    }, [currentAgentId, currentAgent, agentMessages, selectedMessageIds, exitMultiSelect, handleMessageAction, t]);

    // 保存生成图片到交付物目录（deliverables/{hubName}/{agentName}/）
    const handleImageSave = useCallback(async (dataUrl: string, fileName: string) => {
        if (!currentAgentId) return;
        try {
    ...
    const handleLoadMore = useCallback(async () => {
        ...
        try {
            ...
            const olderMessagesFromDb = await invoke<Array<{
                id: string;
                agentId: string;
                role: string;
                content: string;
        ...
        <>
            <div className={styles.chatView}>
                ...
                <ChatHistory
                    ...
                    emptyHint={mode === 'planning'
                        ? t('agent.chat.planningEmptyHint')
