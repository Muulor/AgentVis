...
export function FactsView({ agentId }: FactsViewProps) {
    ...
    useEffect(() => {
        ...
        const setupListener = async () => {
            try {
                ...
                unlisten = await listen<{ agentId: string; count: number }>('memory:facts_updated', (event) => {
                    if (event.payload.agentId === agentId) {
                        logger.trace(`[FactsView] 收到事实更新事件，刷新列表 (新增 ${event.payload.count} 条)`);
                        void loadData();
                    }
                });
            } catch {
    ...
    // 保存编辑
    const handleSave = useCallback(async (id: string | null, content: string, category: LongTermFactCategory) => {
        try {
            if (!id) {
            ...
            setFacts((prev) => prev.map((f) =>
                f.id === id ? { ...f, content, category, updatedAt: Date.now() } : f
            ));
