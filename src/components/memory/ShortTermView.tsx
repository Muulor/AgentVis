  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const items: ShortTermMessageItem[] = sortedResult.map((msg) => {
        ...
        const isCronUserMessage = role === 'user' && /✉.*✉/.test(displayContent);

        return {
          id: msg.id,
          // 解析原始消息 ID（用于跳转到对话历史）
          // cron user 消息在 UI 中已隐藏，清空 sourceMessageId 以禁用跳转
          sourceMessageId: isCronUserMessage ? undefined : (msg.sourceMessageIds ?? undefined),
          role,
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      try {
        ...
        // 监听水位线触发事件
        unlistenTriggered = await listen<{ agentId: string }>(
          'memory:watermark_triggered',
          (event) => {
        ...
        // 监听水位线完成事件
        unlistenCompleted = await listen<{ agentId: string }>(
          'memory:watermark_completed',
          (event) => {
            if (event.payload.agentId === agentId) {
        ...
        unlistenChanged = await listen<{ agentId: string }>(
    ...
    <div className={styles.container}>
      ...
      <div className={styles.messageList}>
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            <div key={msg.id} className={styles.messageItem} data-role={msg.role}>
              <span className={styles.turnNumber}>#{msg.turnNumber}</span>
              <span className={styles.role}>{msg.role === 'user' ? 'User' : 'Agent'}:</span>
              ...
              {onJumpToMessage && (
                <button
                  ...
                  title={
                    msg.sourceMessageId
                      ? t('memory.jumpOriginalMessage')
                      : t('memory.jumpUnavailable')
                  }
                >
                  {t('memory.jump')}
                </button>
