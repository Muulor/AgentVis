...
} from '@services/memory/utils/SafeMessageContent';
...
function parseMetadataJson(
  json: string | null
): Pick<SummaryItem, 'confirmedDecisions' | 'openQuestions' | 'invalidatedPoints'> {
  if (!json) return {};
  ...
  try {
    ...
    return {
      ...
      openQuestions: metadata.openQuestions?.filter(
        (q): q is { question: string; scope?: string } => !!q.question
      ),
...
export function SummaryView({ agentId }: SummaryViewProps) {
    ...
    try {
      ...
      const items: SummaryItem[] = result.map((mem, index) => {
        ...
        const importance: 'high' | 'medium' | 'low' =
          mem.content.length > 200 ? 'high' : mem.content.length > 100 ? 'medium' : 'low';
  ...
  const toggleExpanded = useCallback(
    async (id: string, sourceMessageIds: string | null) => {
      // 如果已展开，则收起
  ...
  const getImportanceClass = (importance: string) => {
    switch (importance) {
      case 'high':
        return styles.importanceHigh;
      case 'medium':
        return styles.importanceMedium;
      default:
        return styles.importanceLow;
    }
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <span className={styles.title}>
          {t('memory.summariesTitle', { count: summaries.length })}
        </span>
            ...
            <div key={summary.id} className={styles.summaryCard}>
              ...
              <div className={styles.cardHeader}>
                ...
                <span className={styles.turnRange}>
                  {t('memory.createdAt', {
                    time:
                      (summary as SummaryItem & { createdAtStr?: string }).createdAtStr ??
                      t('memory.unknown'),
                  })}
                </span>
              </div>
              ...
              {/* 摘要内容 */}
              <div className={styles.summaryContent}>{summary.content}</div>
                  ...
                  {summary.confirmedDecisions && summary.confirmedDecisions.length > 0 && (
                    <div className={styles.statusSection}>
                      <div className={cx(styles.statusSectionTitle, styles.confirmed)}>
                        {t('memory.confirmedDecisions', {
                          count: summary.confirmedDecisions.length,
                        })}
                      </div>
                ...
                <div className={styles.cardActions}>
                  {hasStatusData(summary) && (
                    <button
                      className={cx(
                        styles.actionBtn,
                        statusExpandedIds.has(summary.id) && styles.activeStatus
                      )}
                      onClick={() => toggleStatusExpanded(summary.id)}
                    >
                      {statusExpandedIds.has(summary.id)
                        ? t('memory.collapseDetails')
                        : t('memory.statusDetails')}
                    </button>
                  )}
                  <button
                  ...
                  >
                    {expandedIds.has(summary.id)
                      ? t('memory.collapseOriginal')
                      : t('memory.expandOriginal')}
                  </button>
                  <button
