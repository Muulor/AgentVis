...
describe('buildSafeEChartsOption', () => {
  it('为朴素柱状图注入安全视觉样式', () => {
    ...
    const itemStyle = (series as Record<string, unknown> | undefined)?.itemStyle as
  ...
  it('裁剪未注册或高风险字段，保留可渲染 series', () => {
    ...
    expect(option.series).toEqual([expect.objectContaining({ type: 'line', smooth: true })]);
  });
  ...
  it('雷达图单 series 多 data 时为每个对比项分配不同颜色', () => {
    ...
    const series = Array.isArray(option.series) ? option.series[0] : undefined;
    const data = (series as Record<string, unknown> | undefined)?.data as
      | Record<string, unknown>[]
      | undefined;
