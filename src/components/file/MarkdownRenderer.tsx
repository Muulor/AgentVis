 ...
 */
function extractBase64Images(content: string): {
...
function isPassthroughImageSrc(src: string): boolean {
  return (
    /^(?:https?:|data:|blob:|about:)/i.test(src) || src.startsWith('//') || src.startsWith('#')
  );
}
...
function MarkdownLocalImage({ src, alt, markdownFilePath }: MarkdownLocalImageProps) {
    ...
    async function loadLocalImage() {
      try {
        ...
        const shouldDownscale =
          fileSize > MARKDOWN_LOCAL_IMAGE_DOWNSCALE_BYTES && canDownscaleLocalImage(filePath);
  ...
  if (imageSrc) {
    return <img src={imageSrc} alt={imageAlt} className={styles.image} loading="lazy" />;
  }
  ...
  const imgComponent = useMemo<Partial<Components>>(
    () => ({
      ...
      img({ src, alt }) {
        ...
        return (
          <MarkdownLocalImage src={actualSrc} alt={imageAlt} markdownFilePath={markdownFilePath} />
        );
      },
    }),
