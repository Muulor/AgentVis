...
function AudioPreview({ filePath, fileName }: { filePath: string; fileName: string }) {
  ...
  return (
    <div className={styles.audioContainer}>
      ...
      <audio controls className={styles.audioPlayer} preload="metadata" src={audioUrl} />
    </div>
  );
}
    ...
    <div className={styles.binaryDocWrapper}>
      ...
      <div className={styles.binaryDocCard}>
        ...
        <Tooltip content={t('file.renderHtmlTitle')}>
          <button className={styles.openSystemBtn} onClick={onPreview}>
            <Play size={14} />
        ...
        {textPreviewDecision && textPreviewDecision.mode !== 'rich' && filePath ? (
          ...
          content.length > LARGE_HTML_THRESHOLD_CHARS ? (
            <LargeHtmlCard
              ...
              content={content}
              onPreview={() => {
                void handleHtmlPreview(content);
              }}
            />
          ) : (
            <CodeHighlight
