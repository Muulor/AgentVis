...
export function buildSafeEChartsOption(
    ...
    option.color = Array.isArray(option.color) && option.color.length > 0 ? option.color : theme.colors;
...
function stripRiskyFields(value: unknown, parentKey?: string): void {
    ...
    for (const key of Object.keys(value)) {
        if (RISKY_OPTION_KEYS.has(key) || key === 'renderItem' || (parentKey === 'dataset' && key === 'transform')) {
...
function applyBasicDefaults(option: AnyRecord, theme: ChartVisualTheme): void {
    ...
    const hasCartesianSeries = ['bar', 'line', 'scatter', 'heatmap'].some((type) => seriesTypes.has(type));
    ...
    option.tooltip = mergeDefaults(asRecord(option.tooltip), {
        ...
        extraCssText: 'border-radius: 8px; box-shadow: 0 4px 24px rgba(0,0,0,0.22), 0 1px 4px rgba(0,0,0,0.12);',
    });
...
function applyLegendDefaults(option: AnyRecord, theme: ChartVisualTheme, seriesTypes: Set<string>): void {
...
function applyAxisDefaults(option: AnyRecord, key: 'xAxis' | 'yAxis', theme: ChartVisualTheme): void {
    ...
    for (const item of axes) {
        ...
        const isValueAxis = item.type === 'value' || item.type === 'log' || (!item.type && key === 'yAxis');
...
function applyRadarSeriesRecipe(series: AnyRecord, theme: ChartVisualTheme, index: number): void {
    ...
    series.areaStyle = mergeDefaults(asRecord(series.areaStyle), { opacity: theme.radarAreaOpacity, color });
...
function applyRadarOptionRecipe(option: AnyRecord, theme: ChartVisualTheme): void {
    ...
    for (const radar of radars) {
        ...
        radar.axisName = mergeDefaults(asRecord(radar.axisName), { color: theme.axisLabel, fontSize: 12 });
        radar.axisLine = mergeDefaults(asRecord(radar.axisLine), { lineStyle: { color: theme.axisLine } });
        radar.splitLine = mergeDefaults(asRecord(radar.splitLine), { lineStyle: { color: theme.splitLine } });
