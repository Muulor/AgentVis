...
describe('FileListPathRecovery', () => {
    ...
    it('does not treat unrelated errors as missing path errors', () => {
        expect(isPathMissingError(new Error('Invalid path: access outside the deliverables root is not allowed'))).toBe(false);
    });

    it('recovers non-root missing paths to their parent directory', () => {
        const error = new Error('Path does not exist: os error 2');

        expect(getMissingDirectoryRecoveryPath('scraped_content/images', error)).toBe('scraped_content');
        expect(getMissingDirectoryRecoveryPath('scraped_content', error)).toBe('');
    });

    it('does not recover root or non-missing errors', () => {
        expect(getMissingDirectoryRecoveryPath('', new Error('Path does not exist: os error 2'))).toBeNull();
        expect(getMissingDirectoryRecoveryPath('scraped_content', new Error('Permission denied'))).toBeNull();
    });
});
