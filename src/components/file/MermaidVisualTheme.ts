...
export function buildMermaidVisualConfig(dark: boolean, diagramType: MermaidDiagramType): MermaidVisualConfig {
...
export function applyMermaidVisualDirectives(code: string, diagramType: MermaidDiagramType, dark: boolean): string {
...
function buildThemeVariables(palette: MermaidPalette, diagramType: MermaidDiagramType): Record<string, string> {
    const common = {
...
function applyTextColor(node: Element, color: string): void {
    ...
    for (const label of Array.from(node.querySelectorAll('foreignObject, foreignObject *, .nodeLabel'))) {
