...
describe('fixFlowchartUnsafeSubgraphTitles', () => {
    ...
    Alice->>Bob: subgraph 中文标题`;
        ...
        expect(fixFlowchartUnsafeSubgraphTitles(code)).toContain('subgraph subgraph_auto_2["中文标题"]');
...
describe('quoteFlowchartNodeLabelsForFallback', () => {
    ...
    F --> G[\`where agent-browser\` 命令无法定位可执行文件]`;
        ...
        expect(fixed).toContain('D["无法执行关键验证步骤： <br/>1. agent-browser 命令运行<br/>2. Chrome 运行时启动"]');
        expect(fixed).toContain('F["agent-browser 未在常见安装路径中找到<br/>（全局 node_modules、AgentVis 外部包目录）"]');
...
describe('sanitizeFlowchartFallbackLabel', () => {
    it('keeps quoted labels parse-friendly without changing ordinary prose', () => {
        expect(sanitizeFlowchartFallbackLabel('命令 `where agent-browser` <br> "失败"'))
            .toBe('命令 \'where agent-browser\' <br/> \'失败\'');
    });
});
