...
export function fixFlowchartReservedNodeIds(code: string): string {
  ...
  return lines.map((line) => replaceReservedNodeIds(line, renames)).join('\n');
}
...
export function quoteFlowchartNodeLabelsForFallback(code: string): string {
  ...
  return wrapSquare.replace(
    ...
    (_match, nodeId: string, label: string) =>
      `${nodeId}{"${sanitizeFlowchartFallbackLabel(label)}"}`
  );
}
...
function makeUniqueFlowchartNodeId(
  ...
  return makeUniqueFlowchartGeneratedId(usedIds, FLOWCHART_GENERATED_NODE_ID_PREFIX, startIndex);
}
...
function collectReservedNodeRenames(lines: string[]): Map<string, string> {
  ...
  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith('%%')) continue;
    if (isStyleDirective(trimmed) || isClassDirective(trimmed) || isClickDirective(trimmed))
      continue;
...
function readCompletePipeLabelOperatorAt(
  line: string,
  index: number
): FlowchartOperatorMatch | null {
...
function replaceDirectiveTarget(
  line: string,
  directive: 'style' | 'click',
