...
echarts.use([
    ...
    GaugeChart, FunnelChart, HeatmapChart,
...
export function sanitizeEChartsJson(json: string): string {
    ...
    const noStrayQuotes = noEmojiColors.replace(
        /:\s*(-?\d+(?:\.\d+)?)"(\s*[,}\]])/g,
        ': $1$2'
    );
    ...
    while (i < json.length) {
        ...
        if (expectingArrayValue && stack[stack.length - 1] === 'array') {
            ...
            if (!trimmedValue || isJsonBareLiteral(trimmedValue)) {
                ...
                const trailingWhitespace = rawValue.slice(rawValue.lastIndexOf(trimmedValue) + trimmedValue.length);
...
function extractFiniteDataValue(item: unknown): number | null {
    ...
    if (typeof item === 'object' && item !== null) {
        ...
        if (Array.isArray(value)) {
            ...
            const firstNumber = values.find((entry) => typeof entry === 'number' && Number.isFinite(entry));
            return typeof firstNumber === 'number' ? firstNumber : null;
        }
    }
    ...
    try {
        ...
        if (json.length > 20) {
            ...
            console.warn('[EChartsBlock] JSON 解析失败（两次尝试均失败）', {
                ...
                sanitizedError: sanitizedError instanceof Error ? sanitizedError.message : String(sanitizedError),
            });
        }
    }
...
export const EChartsBlock = memo(function EChartsBlock({ code }: EChartsBlockProps) {
    ...
    // 初始化或更新图表
    const renderChart = useCallback((option: EChartsOption) => {
        ...
        <div className={styles.wrapper}>
            {tableFallback ? (
                <div className={styles.tableFallback} style={tableRowStyle}>
                    {tableFallback.title && (
                        <div className={styles.tableTitle}>{tableFallback.title}</div>
                    )}
