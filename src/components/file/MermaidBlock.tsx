  ...
  return lines
    .map((line) => {
      for (const sgId of collisions) {
        const sgDefPattern = new RegExp('^(\\s*subgraph\\s+)' + sgId + '(\\s*\\[|\\s*$)');
...
function fixGanttDateFormat(code: string): string {
  ...
  if (dateFormatIdx !== -1) {
    ...
    if (hasYearDates && !declaredFormat.includes('YYYY')) {
      ...
      if (fmtLine) {
        lines[dateFormatIdx] = fmtLine.replace(/dateFormat\s+.+/, 'dateFormat YYYY-MM-DD');
      }
    }
  }
  ...
  // 检测以日期结尾的任务行，自动补 `, 1d`
  const taskDateOnly =
    /^(\s*\S.*:\s*(?:done|active|crit|milestone)?\s*,?\s*(?:\d{4}-)?\d{2}-\d{2})\s*$/;
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (!line) continue;
    // 跳过 dateFormat / title / section 等指令行
    if (
      /^\s*(gantt|title|dateFormat|axisFormat|tickInterval|excludes|includes|todayMarker|section)\b/.test(
        line
      )
    )
      continue;
...
function commentOutFlowchartDescriptions(code: string): string {
  ...
  for (const line of lines) {
    ...
    // 跳过指令行
    if (/^\s*(flowchart|graph|subgraph|end|classDef|class|style|click|linkStyle)\b/.test(trimmed))
      continue;
