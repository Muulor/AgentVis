...
const PROJECT_PREVIEWABLE_LANGUAGES = new Set([
  'jsx',
  'tsx',
  'javascript',
  'typescript',
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <span className={styles.language}>
          {normalizedLang}
          {shouldCollapse && <span className={styles.lineCountBadge}>{lineCount} lines</span>}
        </span>
        {canPreview && (
        ...
        <Tooltip content={copied ? t('file.copied') : t('file.copyCode')}>
          <button className={styles.copyBtn} onClick={handleCopy} aria-label={t('file.copyCode')}>
            {copied ? <Check size={14} className={styles.copyIconSuccess} /> : <Copy size={14} />}
          </button>
        </Tooltip>
      </div>
      ...
      >
        <Highlight theme={themes.vsDark} code={code.trim()} language={normalizedLang}>
          {({ className: highlightClassName, style, tokens, getLineProps, getTokenProps }) => (
            <pre className={cx(styles.pre, highlightClassName, className)} style={style}>
              <code className={styles.code}>
                {tokens.map((line, lineIndex) => {
                  // 解构 key 避免重复传递到 props 中
                  const lineProps = getLineProps({ line });
                  const { key: _lineKey, ...lineRestProps } = lineProps as {
                    key?: React.Key;
                    ...
                    >
                      ...
                      <span className={styles.lineContent}>
                        {line.map((token, tokenIndex) => {
                          ...
                          const { key: _tokenKey, ...tokenRestProps } = tokenProps as {
                            key?: React.Key;
                          } & Record<string, unknown>;
      ...
      {/* 展开/收起按钮 */}
      {shouldCollapse && (
        <button className={styles.collapseToggle} onClick={() => setIsExpanded((prev) => !prev)}>
          {isCollapsed ? (
            <>
              <ChevronDown size={14} />
