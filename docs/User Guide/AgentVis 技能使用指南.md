...
进入「Agent 设置 -> 技能」，开启「绑定技能模式」，选择这个 Agent 常用的 1-5 个技能，然后保存。
...
「绑定技能模式」用于给某个 Agent 固定绑定少量技能。开启后，你选择的技能会直接进入这个 Agent 的任务上下文。
...
1. 技能是否在「设置 -> 技能」中启用。
2. 当前 Agent 是否开启了「绑定技能模式」。
3. 如果开启了绑定技能模式，目标技能是否已被选中。
