...
### 10.6 RAG 模型连接的网络与凭据边界

「设置 -> 云端服务 -> RAG 模型连接」可以使用 SiliconFlow 推荐配置、自定义 OpenAI 兼容 Embedding 端点、原生 Google Gemini Embeddings 协议，以及单独配置的可选 Reranker。配置前应确认以下边界：

- Embedding 端点会收到知识库片段、记忆、技能描述及其他待语义比较文本；Reranker 会收到查询和知识库候选片段。只连接你信任且隐私政策可接受的服务。
- OpenAI 兼容 Embedding、原生 Gemini Embeddings 与 Reranker 的 API Key 使用不同的用途级系统凭据槽，不会回显到界面；SiliconFlow 推荐配置另有独立凭据槽。自定义 OpenAI 兼容 Embedding 与 Reranker 凭据还会绑定保存时的端点 origin（协议、主机与端口）：同一 origin 更换模型或接口路径可以复用 Key，origin 变化后必须重新输入并保存，Rust 后端也会拒绝把旧 Key 发给新端点。供应商显示名不参与绑定，因为它是可编辑标签，不是安全边界。旧版未绑定端点的自定义凭据必须重新保存一次才能使用。
- API Key 在点击输入框旁的“保存”时立即写入系统凭据库；底部“保存并应用”只持久化非敏感连接配置并启用路由，不会保存输入框中的 Key。端点 URL、协议、供应商显示名、模型 ID 和输出维度属于非敏感设置，会随应用设置持久化。
- 不要把 API Key、token 或其他 secret 写入端点 URL 或查询参数。需要 `api-version` 等非敏感查询参数时可以保留；凭据应只填写在 API Key 输入框。
