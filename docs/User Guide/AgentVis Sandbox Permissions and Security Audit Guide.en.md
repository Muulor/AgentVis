...
- Provider error-response bodies and complete request URLs are not written to the UI or logs. The internal error path retains only a stable category / HTTP status. A multi-item Embedding request that receives HTTP 400 may be bisected automatically, but logs contain only item count, total UTF-8 bytes, and maximum item bytes, never text, endpoints, or credentials. The UI shows localized troubleshooting guidance, preventing URL query parameters, query text, or candidate content from being exposed again through errors.
- During Knowledge Base or memory indexing, including index rebuilding, a final document-purpose Embedding input larger than 6 KiB UTF-8 is sent as multiple overlapping windows and locally aggregated back into one logical vector. Stored source content is not truncated and no extra logical rows are created, but boundary context may be sent twice in adjacent windows and increase API usage; continue to use only a data-processing service you trust.
