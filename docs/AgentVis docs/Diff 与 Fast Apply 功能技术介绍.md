...
- 对可识别的中小型代码文件，Diff 会在首屏纯文本完成后利用浏览器空闲时段对原始版本和新版本各做一次 Prism token 化；JS/TS/JSX/TSX 会额外对变量、参数、成员属性和对象键做保守的词法补充，再按新旧行号复用 token；虚拟滚动区域只挂载可见行的高亮节点
- 语法高亮受 300,000 总字符和单侧 6,000 行预算保护；未知语言、超出预算或行号/内容校验不一致时自动保持纯文本，不影响 Diff 生成、审批和回滚链路
