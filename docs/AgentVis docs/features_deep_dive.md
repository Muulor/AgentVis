...
宿主 renderer 自身采用两级错误隔离：Markdown 中按需加载的 Mermaid/ECharts 失败时只降级对应图表并允许重试；其他未捕获 React 异常由入口级 boundary 接管，保留“重新加载 / 关闭应用”和错误详情。窗口关闭确认使用 Tauri 标准 `onCloseRequested` listener，因此 renderer 根树卸载并移除 listener 后会恢复系统默认关闭，不会留下无法退出的黑屏窗口。Windows debug 可执行文件启动前还会有界探测 `127.0.0.1:1420`；若开发服务未运行，会显示双语说明并安全退出，避免依赖 WebView 缓存进入残缺界面。
