...
对于无法解析的复杂命令（多层管道链、复合条件语句等），系统回退为正常执行流程，不拦截。
...
完整恢复信息仍然保存在 `trash_manifest.json` 和内部日志中，面向用户或后续安全 UI 展示；Agent 默认不应拿到这些路径。
...
应用启动时自动扫描 manifest，**超过 30 天**的条目物理删除并从 manifest 移除。短期内被误删的文件在此期间可通过手动移回原路径的方式恢复。
...
### Trash Bin 回退策略
对于命令解析器无法识别的复杂格式（如嵌套多级管道、带条件判断的脚本片段），Trash Bin 选择回退而不是阻断，使命令继续进入 OS 执行。这样既不破坏 Agent 的正常工作流，也保持了兜底机制的可靠性——回退场景本身通常是 Agent 执行更复杂脚本的场景，此时 `validate_script_content()` 的脚本内容扫描层仍然生效。
