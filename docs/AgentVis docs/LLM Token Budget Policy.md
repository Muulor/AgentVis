...
可选档位必须由实际的 `provider + model + protocol/route` 能力共同决定。未知的、自定义的以及未经验证的、与聚合器兼容的路由仅会显示“推荐”选项，同时会忽略那些未经验证的出站路由控制机制（即用于确保路由安全性的规则）。经过验证的聚合器路由可能只会显示与该特定提供商/模型组合相关的控制机制。后端系统还需要将那些不被支持的输入数据转换为该路由所允许的、经过验证的、较为保守的值；切勿将前端菜单视为判断数据可靠性的依据。
