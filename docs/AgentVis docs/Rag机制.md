...
原始 Query ───────────────→ Embedding Top 30 ──┐
                                               ├── RRF 融合 → 可选 Rerank → Parent 聚合 → 上下文分配 → Final Relevance Gate → Parent Context Restore → 注入 Prompt
Query Preprocess → 增强 BM25 Query/Fragments ─→ BM25 Top 30 ─────┘
```

推荐配置使用 SiliconFlow `BAAI/bge-m3` 与 `BAAI/bge-reranker-v2-m3`；用户也可以分别配置其他 Embedding 与 Reranker 服务，或关闭 Reranker。初召回仍依赖 Query 预处理、BM25 元数据增强、Document Overview 合成块和 RRF 融合保证召回率；Reranker 负责对 RRF 候选池做二阶段语义重排与低分过滤。Rerank 调用失败、超时、关闭或返回异常时会降级到原有 RRF + Gate 链路。
...
├── EmbeddingService.ts        # profile-aware 向量化服务；SiliconFlow、自定义 OpenAI 兼容或原生 Gemini
...
├── RagConnectionConfig.ts     # RAG 路由、验证与 Embedding profile 定义
├── RagConnectionService.ts    # 连接测试、配置启用与索引迁移编排
├── RagIndexRebuildService.ts  # 按 Agent 重建 Embedding 索引
├── RerankService.ts           # 推荐或自定义 Reranker 二阶段重排序服务
...
src-tauri/src/commands/cloud_embedding.rs # SiliconFlow、自定义 OpenAI / Reranker、原生 Gemini 与凭据命令
...
Embedding 路由由“RAG 模型连接”设置决定：

1. **推荐模式（默认）**：固定使用 `https://api.siliconflow.cn/v1/embeddings` 与 `BAAI/bge-m3`。
2. **自定义 OpenAI 模式**：使用用户填写的完整 OpenAI Embeddings 兼容端点、模型 ID 与认证方式。
3. **原生 Google Gemini 模式**：网络目标固定为 Gemini Developer API `v1beta` 端点。首版只处理文本，支持稳定版 `gemini-embedding-2` 与 `gemini-embedding-001`，输出维度可选 768、1536 或 3072；不包含 Vertex AI、OAuth、自定义 Gemini 代理、多模态输入或异步 Batch job。

Gitee AI fallback 已移除；系统不会在不同 Embedding 模型之间自动切换，避免混用不同语义空间的向量。网络或 Embedding 服务不可用时，各调用方按自身语义降级：知识库仍保留本地 BM25 召回，记忆与语义比较链路会跳过对应向量能力。
...
Gemini 的 purpose 处理按模型区分：`gemini-embedding-001` 会把 document、query/test 与 generic similarity 分别映射为 `RETRIEVAL_DOCUMENT`、`RETRIEVAL_QUERY` 与 `SEMANTIC_SIMILARITY`；`gemini-embedding-2` 不接受 `taskType`，因此 AgentVis 会按 Google 文档添加 query、document 或 sentence-similarity 文本前缀。purpose 不进入持久化 profile，否则 query 与 document 向量将无法比较；profile 只记录任务策略版本。两个模型的向量空间不兼容，互相切换必定触发索引重建。

Google 说明，未付费 Gemini API 服务提交的内容可能用于改进产品并可能经过人工审核。该路由会收到知识库片段、记忆、技能描述及其他待语义比较文本，因此用户不应通过未付费层级提交敏感、机密或个人数据。当前定价、支持地区、限额和条款属于外部服务属性，AgentVis 不作永久承诺。

当前代码注释标注 `bge-m3` 为 1024 维。`EmbeddingService` 还提供：

- LRU 缓存：最多 1000 条 embedding
- 批量调用：每批最多 25 条
- Embedding 调度：按 profile 串行；Gemini 起点至少间隔 1 秒，OpenAI-compatible 遇到限流后自适应退避
- 超时分层：Rust 网络硬超时 15 秒；renderer IPC 兜底 18 秒，避免网络断开时长期阻塞
- 余弦相似度计算
...
| `rag_list_vector_agent_ids`         | 列出含持久化向量的 Agent，用于 profile 迁移发现   |
| `rag_batch_update_chunk_embeddings` | 在单个 Agent 事务内批量更新向量、维度与 profile   |
...
Step 5: 可选 Rerank              RRF Top20 进入已配置 Reranker；关闭或失败自动降级
...
| `bm25FragmentTopK`              | 10                                    |
| `enableRerank`                  | true（自定义连接可独立关闭 Reranker） |
| `rerankTopK`                    | 20                                    |
| `rerankMinScore`                | 0.08                                  |
| `strongRerankScoreThreshold`    | 0.20                                  |

Rerank 位于 RRF 融合之后、Parent 聚合之前。它只处理候选池，不参与全库扫描。推荐 SiliconFlow 路由使用经 BGE 校准的原始 rerank score 作为后续排序/聚合主分数，并丢弃低于 `rerankMinScore` 的候选；自定义路由则把返回顺序转换成稳定的 rank score，不让原始分值幅度或 BGE 阈值影响聚合。两种路径都不会回填被丢弃的候选。Final Relevance Gate 位于上下文分配之后、Parent Context Restore 之前，用来避免无关 query 被硬塞满 `finalTopK`。它不会在丢弃候选后回填，因此 `finalTopK` / `topK` 是输出上限，不是保证数量。

`embeddingThreshold`、`finalEmbeddingThreshold`、`strongFinalEmbeddingThreshold`、`rerankMinScore` 与 `strongRerankScoreThreshold` 只针对推荐的 BGE 组合校准。自定义 Embedding / Reranker 的分数尺度没有统一含义，因此只用于候选排序：自定义 Embedding 检索不套用 BGE cosine 阈值，自定义 Reranker 不套用 BGE rerank 阈值，最终候选必须有有效词汇依据；即使调用方关闭普通 Final Relevance Filter，这条自定义路由的词汇依据 gate 仍保持启用。
...
5. **可选 Rerank 提升精排质量**：Hybrid Search 负责召回覆盖，推荐模型或自定义 Jina/Cohere、Voyage 兼容 Reranker 对候选池做二阶段语义重排；关闭或失败时降级到原 RRF + Gate 链路。
