...
- `buildTaskExperiencePrompt()`：任务经验，供 Planning/SA 参考
...
Planning 模式中，`AgentService.processMessage()` 会在创建 `AgentLoop` 前调用 `loadRuntimeContext()`：
...
普通 Chat 发送链路也会在 Agent 模式下执行类似流程：
