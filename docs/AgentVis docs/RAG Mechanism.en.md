...
Raw Query ---------------------> Embedding Top 30 --+
                                                    +--> RRF fusion -> SiliconFlow Rerank -> Parent aggregation -> context allocation -> Final Relevance Gate -> Parent Context Restore -> Prompt injection
Query Preprocess -> enhanced BM25 query/fragments -> BM25 Top 30 ------+
```

The current version uses the SiliconFlow `BAAI/bge-reranker-v2-m3` reranker by default. Initial recall still relies on query preprocessing, BM25 metadata enhancement, Document Overview synthetic chunks, and RRF fusion to preserve recall. The reranker performs second-stage semantic reranking and low-score filtering over the RRF candidate pool. If the rerank call fails, times out, or returns abnormal data, the system silently degrades to the original RRF + Gate pipeline.
...
+-- EmbeddingService.ts        # Embedding service; default SiliconFlow BAAI/bge-m3, fallback Gitee AI bge-m3
...
+-- RerankService.ts           # SiliconFlow bge-reranker-v2-m3 second-stage reranking service
...
The current default Embedding path is:

1. Prefer the `siliconflow` provider with model `BAAI/bge-m3`.
2. If the primary provider fails and a Gitee AI key is configured, fall back to the `giteeai` provider with model `bge-m3`.
3. The Rust backend still supports the `zhipu` provider and `Embedding-3-pro`, but the TS default RAG service does not prefer it.
...
- Single-batch timeout: 15 seconds, preventing long blocking when the network is disconnected.
...
Step 5: SiliconFlow Rerank           RRF Top20 enters bge-reranker-v2-m3; failure degrades automatically
...
Rerank sits after RRF fusion and before Parent aggregation. It only processes the candidate pool and does not scan the entire database. After success, rerank score becomes the main score for later sorting / aggregation. Candidates below `rerankMinScore` are discarded and not backfilled. Final Relevance Gate sits after context allocation and before Parent Context Restore, preventing unrelated queries from force-filling `finalTopK`. It does not backfill after discarding candidates, so `finalTopK` / `topK` is an output upper bound, not a guaranteed count.
...
| Default Embedding model | `BAAI/bge-m3` | SiliconFlow provider. |
...
| Embedding timeout | 15 seconds | Single-batch API call. |
...
5. **SiliconFlow Rerank improves precision**: Hybrid Search provides recall coverage, while `BAAI/bge-reranker-v2-m3` performs second-stage semantic reranking over the candidate pool. On failure, it silently degrades to the original RRF + Gate pipeline.
