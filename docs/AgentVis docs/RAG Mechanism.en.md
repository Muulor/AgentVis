...
- **Markdown** uses Parent-Child chunking: H1~H6 headings are parsed into sections, section content becomes Parent chunks, and section internals are further grouped at natural boundaries into Child chunks.
...
Every Child or flat chunk content satisfies one common hard limit. The chunker prefers line, punctuation, or whitespace boundaries, falls back to a Unicode-safe forced split when no natural boundary exists, and keeps up to 50 characters of overlap. A large class/function, a very long single-line JSON value, or an unpunctuated sentence can no longer bypass `chunkSize`. Markdown Parents remain complete because they are used only for hierarchical context restoration and are not written directly to the vector store. A separate final-input planner runs after metadata enrichment, so long titles or section paths cannot reintroduce an oversized remote request.
...
| `CHILD_SIZE`                           | 500 characters | Maximum size for Markdown Child chunks.                          |
| `CHILD_OVERLAP`                        | 50 characters  | Overlap window between Child chunks.                             |
| `DEFAULT_CHUNKING_CONFIG.chunkSize`    | 500 characters | Hard limit for every Child or flat chunk content.                |
...
Normal requests for the same Embedding profile are serialized within one `EmbeddingService`, while a rate-limit cooldown does not block another profile. Gemini request starts remain spaced by at least one second. OpenAI-compatible routes have no hard-coded baseline rate and instead adapt after 429 responses, retryable 5xx responses, and transient network timeouts. Both protocols use exponential backoff with jitter; OpenAI-compatible routes make at most six retries to span a common one-minute limit window and prefer a parsed, bounded `Retry-After` hint when present. HTTP 400 remains a non-backoff client error. When a multi-item request returns 400, `EmbeddingService` recursively bisects that batch to isolate an undocumented aggregate constraint or one rejected item; a single-item rejection is surfaced immediately. Split diagnostics contain only item counts and UTF-8 byte metrics, never text, endpoints, response bodies, or credentials. Connection tests remain single-attempt so they report the current state promptly. RPM, TPM, and daily limits are properties of the user's provider and account; total credits are not treated as remaining per-minute capacity, and an exhausted daily allowance still requires waiting for quota recovery. Rebuilds persist an atomic checkpoint after every successful 25 original chunks, so a later failure retries only unfinished chunks. Agent Knowledge Base settings persist only file paths whose indexing completed and reconcile historical paths against local document IDs; failed files remain visibly retryable instead of being shown as successfully indexed.

For every document-purpose index write, AgentVis plans the final Embedding text after metadata enrichment. This covers new Knowledge Base Overview/Child chunks, new memory facts and summaries, and existing rows processed by index rebuilding. A final input larger than 6 KiB UTF-8 is split into Unicode-safe windows of at most 6 KiB with 512 B overlap. Each window vector is L2-normalized, averaged using the number of newly covered non-overlap UTF-8 bytes as its weight, and normalized again. The result is still persisted as one logical vector under the original chunk or memory ID: complete stored content is not truncated, the number of SQLite/BM25 records does not grow, and overlap is not counted twice. Metadata records `embeddingAggregationVersion` and `embeddingSegmentCount`, so an oversized existing vector with stale or missing aggregation metadata is migrated by Check and rebuild index. Ordinary single-window inputs retain the provider vector unchanged, with no additional normalization. Adjacent windows may send boundary context more than once and can therefore increase provider usage.
...
| ------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Markdown Child Chunk hard limit | 500 characters                        | `DocumentChunker.CHILD_SIZE`                                                                                                                                 |
...
| Default Child/flat content limit | 500 characters                       | `DEFAULT_CHUNKING_CONFIG.chunkSize`                                                                                                                          |
...
| Final index input window / overlap | 6 KiB / 512 B                      | Used for oversized Knowledge Base, memory, and rebuild document-purpose inputs.                                                                               |
