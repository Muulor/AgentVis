...
For evolving public APIs or external services, a Script Skill should not emit only prose errors. Prefer a stable internal observation contract on stdout that distinguishes complete success, partial success, retryable failure, and failures requiring user or maintainer action. Preserve at least `status`, `errorKind`, `reasonCode`, `retryable`, optional `retryAfter`, requested versus measured values, and `nextStep`. Missing upstream fields must not be replaced with zero, and optional endpoint failures must not be swallowed; return usable data with warnings instead. JSON output must be bounded by the Skill itself at row or item boundaries and mark `truncated` rather than relying on `ExternalExecutor` byte truncation, whose appended marker would make JSON invalid. Network diagnostics must still omit full URL queries, Cookie/Set-Cookie, Authorization, crumb values, broker tokens, and other credentials.
