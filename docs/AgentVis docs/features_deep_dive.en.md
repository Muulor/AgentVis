...
The host renderer itself uses two error-isolation layers. A failed lazy Mermaid/ECharts module degrades only that diagram and remains retryable. Any other uncaught React error is contained by an entry-level boundary that preserves Reload, Close App, and diagnostic details; the crash page's Close App action has a separate full-process exit fallback. Rust owns the normal window lifecycle: the title-bar X and Alt+F4 publish a retained hidden state for Renderer acknowledgement and then hide the main window, while tray `Open AgentVis`, tray left/double click, a second launch, and task-notification activation all share one restore-and-focus helper. The single-instance callback ignores the second launch's arguments and working directory and only restores the existing process. Every explicit tray `Exit` receives a fresh `requestId` and supersedes the older request. The Renderer peeks the retained request, synchronously hands it to the handler, and then ACKs it; a request-owned latest-attempt queue subsequently performs active-task confirmation, Preview cleanup, and native exit only with that same still-valid, acknowledged `requestId`. X cancels an unfinished Exit, and a new Exit issued during stale cleanup continues automatically after the older request fails. If the current attempt is not ACKed within five seconds, a Rust watchdog terminates the process. If the Renderer becomes unresponsive only after an ACK, another explicit `Exit` creates a fresh unacknowledged attempt and re-arms the watchdog. Before a Windows debug executable starts, it also performs a bounded probe of `127.0.0.1:1420`; if the development server is absent, a bilingual explanation is shown and the process exits safely instead of relying on a cached, incomplete WebView page.

Windows release acceptance must hide the main window for at least ten minutes while a real Chat/Planning tool loop and stream run, then verify renderer heartbeat/stale logs, the final notification, and restoration to the correct Hub/Agent through that notification. Automated tests cannot prove that work continues after a long WebView2 background suspension; this real-task manual run is a release gate, and a failure means tray-hidden background execution must not be described as reliable.
